...
function AzureCli(name, parent) {
  ...
  if (!parent) {
    ...
    var loader = new CmdLoader(this);
    if (loader.cmdMetadataExists()) {
      loader.harvestPlugins(true);//TODO: see whether we can avoid loading them.
...
_.extend(AzureCli.prototype, {
  ...
  getErrorFile: function () {
    return path.join(utils.azureDir(), 'azure.err');
  },
  ...
  normalizeAuthorizationError: function (msg) {
    ...
    if (msg.match(regex)) {
      msg = 'Certificate based Authentication is not supported in current mode: \'' + this.getMode() + 
  ...
  */
        ...
        if (!commandOption && arga && command && command.commands) {
          for (var j in command.commands) {
            if (command.commands[j].name === arga) {
              for (var m = 0; m < command.commands[j].options.length; ++m) {
                if (command.commands[j].options[m].short === arg || command.commands[j].options[m].long === arg) {
                  break;
                }
              }
              commandOption = command.commands[j].options[m];
    ...
    command.on('*', function () {
      ...
      if (cat && cat.stub) {
        for (var i = 2; i <= args.length - 1; i++) {
          var temp = cat.categories[args[i]];
          if (temp) {
            cat = temp;
            continue;
          } else {
            for (var j = 0; j < cat.commands.length; j++) {
              if (cat.commands[j].name === args[i]) {
                this.executingCmd = true;
                cmdLoaded = true;
                var cmd = require(cat.commands[j].filePath); //TODO, find the matched one
                cmd.init.apply(this, [this]); //TODO: figure out why we can't use cmd.init(this) ?
        ...
        //for (var j = 0; j < command.commands.length; j++) {
        //  if (command.commands[j].name === category) {
        //    this.executingCmd = true;
        //    cmdLoaded = true;
        //    var cmd = require(command.commands[j].filePath); //TODO, find the matched one
        //    cmd.init.apply(this, [this]); //TODO: figure out why we can't use cmd.init(this) ?
        //    cat = command.commands[j];
        //    break;
        //  }
        //}
      }

      if (!cat || ( cat.stub && !cmdLoaded)) {
  ...
  isAsmMode: function () {
    return utils.ignoreCaseEquals(this.getMode(), 'asm');
  },
