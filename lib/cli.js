...
_.extend(AzureCli.prototype, {
  ...
  execute: function (fn) {
    ...
    if (self.isCurrentMode(self._mode)) {
      // Add command to category
      self.parent.commands.push(self);
        ...
        try {
          ...
          for(var j = 0; j < self.args.length; j++) {
            ...
            if (options[self.args[j].name] &&
              typeof options[self.args[j].name] === 'string' &&
  ...
  */
  ...
  enableNestedCommands: function(command) {
    ...
    command.on('*', function () {
      ...
      if (!cat || !cat.hasCurrentModeCommands()) {
  ...
  harvestPlugins: function() {
    ...
    function scan(scanPath) {
      var results = utils.getFiles(scanPath);
