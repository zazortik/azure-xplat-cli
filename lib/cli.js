  ...
  */
  ...
  enableNestedCommands: function (command) {
    ...
    command.on('*', function () {
      ...
      var targetCmd;
      var i, j;
      for (i = 2; cat && (i < args.length); i++) {
        for (j = 0; j < cat.commands.length; j++) {
          if (cat.commands[j].name === args[i]) {
            targetCmd = cat.commands[j];
            break;
          }
        }
        if (targetCmd) {
      ...
      if (targetCmd) {
        ...
        targetCmd = require(targetCmd.filePath); //TODO, find the matched one
        targetCmd.init.apply(this, [this]); //TODO: figure out why we can't use cmd.init(this) ?
