...
//bring in extension methods such as 'bold', 'green', etc
  ...
  setupCommandOutput: function (raw) {
    ...
    for (var i = 0, len = raw.length; i < len; ++i) {
      if (hasOption('--json') && raw[i] === '--json') {
        ++json;
      } else if (hasOption('--terse') && raw[i] === '--terse') {
        ++terse; 
    ...
    var opts = { json: false };
    //2 TC failed if break to individual conditions: 
    //  cli vm Create: with custom data:
    //  cli vm static ip operations: Removing the static ip address:
    if (json || verbose) {
      if (json) {
        opts.json = true;
        opts.level = 'data';
