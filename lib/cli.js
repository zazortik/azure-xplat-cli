...
function AzureCli(name, parent, gen, mode) {
  this.parent = parent;
  this.output = log;
...
_.extend(AzureCli.prototype, {
  ...
  */
    ...
    command.on('*', function () {
      ...
      if (targetCmd) {
        ...
        if (path.basename(targetCmd.filePath).indexOf('site.') !== -1) {
          ...
          var siteCmdFiles = ['site._js', 'site.appsetting._js', 'site.config._js', 
            'site.connectionstring._js', 'site.defaultdocument._js', 
            'site.deployment._js', 'site.domain._js', 'site.handler._js',
            'site.jobs._js', 'site.log._js', 'site.repository._js', 'site.scale._js'
          ];
          var sitePlugins = siteCmdFiles.map(function (siteCmdFile) {
            return require(path.join(siteCmdDir, siteCmdFile));
          });
  ...
  getMode: function () {
    if (!this._mode) {
      var config = utilsCore.readConfig();
      if (config.mode) {
        ...
        if (fs.existsSync(modeDirectory)) {
          ...
          delete config.mode;
          utilsCore.writeConfig(config);
        }
      }
      else {
        this._mode = Constants.API_VERSIONS.ASM;
      }
    }
  ...
  isAsmMode: function () {
    return utilsCore.ignoreCaseEquals(this.getMode(), 'asm');
  },
