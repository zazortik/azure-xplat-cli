  ...
  mobilePreview.command('list [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          if (log.format().json) {
            ...
            if(result && (result.enabled && result.enabled.length > 0) || (result.available && result.available.length > 0)) {
              var features = __.union(result.enabled, result.available);
              log.table(features, function (row, f) {
                row.cell('Preview feature', f);
                row.cell('Enabled', __.indexOf(result.enabled, f) !== -1 ? 'Yes'.green : 'No'.red);
              });
