...
exports.init = function (cli) {
  ...
  */
  ...
  function promptKeyFields(options, servicename, type, _) {
    options.servicename = promptServiceNameIfNotGiven(options, servicename, _);    
    options.type = interaction.chooseIfNotGiven(cli, 'Key type: ', 'Retrieving choices', type, 
      function(cb) {
        cb(null, keyTypes);
      }, _);

    if (__.indexOf(keyTypes, options.type) == -1) {
  ...
  mobileKey.command('regenerate [servicename] [type]')
        ...
        .execute(function (servicename, type, options, _) {
          ...
          var result,
              progress = cli.progress('Regenerating key');
          ...
          if (log.format().json) {
            ...
            log.info('New ' + options.type + ' key is ' + result[options.type + 'Key']);
          }
        });

  mobileKey.command('set [servicename] [type] [value]')
        .description('Specify the key to use for your Mobile Service')
        .option('-s, --subscription <id>', 'use the subscription id')
        .execute(function (servicename, type, value, options, _) {
          promptKeyFields(options, servicename, type, _);          

          options.value = interaction.promptIfNotGiven(cli, 'Key value: ', value, _);

          var result,
              progress = cli.progress('Setting key');              
          try {
