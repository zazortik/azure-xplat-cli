  ...
  mobileConfig.command('list [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          function displayConfiguration (results) {
            ...
            interaction.formatOutput(cli, settings, function (output) {
              log.table(output, function (row, item) {
                row.cell('Setting', item);
                if(output[item] === 'Not configured') {
                  row.cell('Value', output[item].blue);
                } else if (output[item] === 'Unable to obtain the value of this setting') {
                  row.cell('Value', output[item].red);
                } else {
                  row.cell('Value', output[item].green);
                }       
              });
            });
          }
          ...
          interaction.formatOutput(cli, value, function (output) {
            if (output[key]) {
              ...
              if (typeof options.file === 'string') {
                fs.writeFileSync(options.file, output[key], key == 'apnsCertificate' ? 'base64' : 'utf8');
