...
exports.init = function(cli) {
          ...
          deleteRoleOrDeployment(channel, found.svc, found.deploy, name, function(error) {
            ...
            if (!error) {
              ...
              var done = function () {
                progress.end();
                if (errorCount) {
                  cmdCallback(util.format($('While VM was deleted successfully, deletion of %s of its %s disk(s) failed'), errorCount, allCount));
                }
  ...
  function shutdownVM(name, options, cmdCallback) {
    ...
    var vmClient = new VMClient(cli, cli.category('account').lookupSubscriptionId(options.subscription));
    vmClient.getDeployments(options, function(error, deployments) {
      if (error) { return cmdCallback(errors[0]); }
      else {
        var found = null;
