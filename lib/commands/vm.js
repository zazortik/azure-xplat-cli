...
var ODataUtil = require('../odata-util');
...
exports.init = function(cli) {
  ...
  .execute(function(vmName, endpoints, options, callback) {
    ...
    var inputEndpoints = [];
    j = 0;
    endpointsAsList.forEach(function(endpointInfoStr) {
      j++;
      if (!endpointInfoStr) {
          callback(message);
      }
      ...
      endpointInfoAsList.forEach(function(item) {
        ...
        if (keys[i] === 'lb_port' || keys[i] === 'vm_port') {
          ...
          if ((item != portAsInt) || (portAsInt > 65535)) {
            callback('lb-port or vm-port must be an integer less than or equal to 65535 (endpoint ' + j + ')');
          }
          ...
          if (endpointInfoAsList.length < 5) {
            callback('probe-port is required to enable probing (endpoint ' + j + ')');
          }
          ...
          if (item === 'tcp' || item == 'http') {
            ...
            callback('Possible values for probe-protocol are tcp and http (endpoint ' + j + ')');
          }
        } else if (keys[i] === 'prob_port') {
          probeportAsInt = parseInt(item, 10);
          if ((item != probeportAsInt) || (probeportAsInt > 65535)) {
            callback('probe-port must be an integer less than or equal to 65535 (endpoint ' + j + ')');
          }
          ...
          inputEndPoint.LoadBalancerProbe.Port = probeportAsInt;
            ...
            if (inputEndPoint.LoadBalancerProbe.Protocol == 'http') {
              ...
              logger.warn('probe-path option will be ignored when probe-protocol is tcp (endpoint ' + j + ')');
            }
         }
         i++;
      });
  ...
  function createVM(options, cmdCallback) {
    ...
    if (options.role) {
      ...
      if (options.community) {
        ...
        var odataclient = new ODataUtil();
        odataclient.resolveUid(options.imageName, function(error, response) {
          progress.end();
          if (!error) {
