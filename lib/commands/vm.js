      ...
      endpointInfoAsList.forEach(function(item) {
        ...
        if (keys[i] === 'lb_port' || keys[i] === 'vm_port') {
          ...
          if ((item != portAsInt) || (portAsInt > 65535)) {
            callback('lb-port or vm-port must be an integer less than or equal to 65535 (endpoint ' + (j + 1) + ')');
          }
          ...
          if (endpointInfoAsList.length < 5) {
            callback('probe-port is required to enable probing (endpoint ' + (j + 1) + ')');
          }
          ...
          if (item === 'tcp' || item == 'http') {
            ...
            callback('Possible values for probe-protocol are tcp and http (endpoint ' + (j + 1) + ')');
          }
        } else if (keys[i] === 'prob_port') {
          probeportAsInt = parseInt(item, 10);
          if ((item != probeportAsInt) || (probeportAsInt > 65535)) {
            callback('probe-port must be an integer less than or equal to 65535 (endpoint ' + (j + 1) + ')');
          }
