  ...
  function getOrCreateAffinityGroupIfRequired(options, callback) {
    ...
    if (options.affinityGroup) {
      _getAffinityGroups(function(error, groups) {
        ...
        return callback(null, { agName: affinityGroup.name, location: affinityGroup.location, newAg: false });
      });
    } else {
        ...
        if (options.createNewAffinityGroup) {
          ...
          managementClient.affinityGroups.create(affinityGroupOptions, function (error) {
            ...
            return callback(null, { agName: agName, location: location.name, newAg: true });
          });
        } else {
          return callback(null, { agName: null, location: location.name, newAg: false });
        }
      });
    }
  }
