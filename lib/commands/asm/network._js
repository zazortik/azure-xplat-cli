...
exports.init = function(cli) {
  ...
  vnet.command('create [vnet]')
    ...
    .option('-c, --create-new-affinity-group', $('creates a new affinity group in the location specified in --location'))
  ...
  function getOrCreateAffinityGroupIfRequired(options, callback) {
    ...
    if (options.affinityGroup) {
      _getAffinityGroups(function(error, groups) {
        ...
        return callback(null, { AGName: affinityGroup.name, Location: affinityGroup.location, New: false });
      });
    } else {
        ...
        if (options.createNewAffinityGroup) {
          ...
          managementClient.affinityGroups.create(affinityGroupOptions, function (error) {
            ...
            return callback(null, { AGName: agName, Location: location.name, New: true });
          });
        } else {
          return callback(null, { AGName: null, Location: location.name, New: false });
        }
      });
    }
  }
