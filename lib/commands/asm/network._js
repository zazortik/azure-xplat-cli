...
exports.init = function (cli) {
  ...
  var routeTable = subnet.category('route-table')
    .description($('Commands to manage route tables'));

  routeTable.command('add [vnet-name] [subnet-name] [route-table-name]')
  ...
  routeTable.command('delete [vnet-name] [subnet-name] [route-table-name]')
    .usage('[options] <vnet-name> <subnet-name> <route-table-name>')
  ...
  routeTable.command('get [vnet-name] [subnet-name]')
    ...
    .execute(function (vnetName, subnetName, routeTableName, options, _) {
      ...
      var networkClient = new NetworkClient(cli, options.subscription);
      networkClient.getRouteTable(vnetName, subnetName, options, _);
    });
  ...
  vpnConnection.command('list [vnet-name]')
    ...
    .execute(function (vnetName, options, _) {
      ...
      var networkClient = new NetworkClient(cli, options.subscription);
      networkClient.listConnections(vnetName, options, _);
    });
