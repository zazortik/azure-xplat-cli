...
exports.init = function (cli) {
  ...
  */
  function deleteFile(share, path, options, _) {
    var fileService = getFileServiceClient(options);
    share = StorageUtil.promptIfNotGiven($('Share name: '), share, _);
    path = StorageUtil.promptIfNotGiven($('Path to the file: '), path, _);
    ...
    if (force !== true) {
      force = StorageUtil.confirm(util.format($('Do you want to delete file %s?'), path), _);
      if (force !== true) {
        return;
      }
    }
  ...
  */
  function uploadFile(source, share, path, options, _) {
    var fileService = getFileServiceClient(options);
    source = StorageUtil.promptIfNotGiven($('File to be uploaded: '), source, _);
    share = StorageUtil.promptIfNotGiven($('Share name: '), share, _);
    var storageOptions = getStorageFileOperationDefaultOption();
    ...
    try {
      if (path) {
        ...
        if (file !== '' && !pathIsDirectory && performStorageOperation(
    ...
    if (remoteFileExists === true) {
      if (force !== true) {
        force = StorageUtil.confirm(util.format($('Do you want to overwrite remote file %s?'), pathUtil.join(directory, file)), _);
        if (force !== true) {
          return;
        }
      }
  ...
  function downloadFile(share, path, destination, options, _) {
    var fileService = getFileServiceClient(options);
    share = StorageUtil.promptIfNotGiven($('Share name: '), share, _);
    path = StorageUtil.promptIfNotGiven($('Path of the file to be downloaded: '), path, _);
    var storageOptions = getStorageFileOperationDefaultOption();
    var force = !!options.quiet;
    ...
    if (utils.fileExists(destination, _)) {
      if (force !== true) {
        force = StorageUtil.confirm(util.format($('Do you want to overwrite file %s?'), destination), _);
        if (force !== true) {
          return;
        }
      }
    ...
    try {
      ...
      if (StorageUtil.isNotFoundException(e)) {
        throw new Error(util.format($('File \'%s\' in share %s does not exists'), path, share));
      } else {
        throw e;
      }
    }
  ...
  function createDirectory(share, path, options, _) {
    var fileService = getFileServiceClient(options);
    share = StorageUtil.promptIfNotGiven($('Share name: '), share, _);
    path = StorageUtil.promptIfNotGiven($('Path to the directory to be created: '), path, _);
    path = normalizePath(path);
  ...
  function deleteDirectory(share, path, options, _) {
    ...
    if (force !== true) {
      force = StorageUtil.confirm(util.format($('Do you want to delete directory %s in share %s?'), path, share), _);
      if (force !== true) {
        return;
      }
    }
