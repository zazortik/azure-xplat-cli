...
exports.init = function (cli) {
    ...
    function getFileServiceClient(options) {
        ...
        if (isConnectionStringDefined && isAccountDefined) {
            ...
            if (isConnectionStringDefined) {
                serviceClient = StorageUtil.getFileService(options.connectionString);
                ...
                if (isNameDefined && isKeyDefined) {
                    ...
                    serviceClient = StorageUtil.getFileService(connString);
                } else {
                    throw new Error($('Please set both --account-name and --account-key'));
                }
            } else {
                //Use environment variable
                serviceClient = StorageUtil.getFileService();
            }
    ...
    */
    function createShare(share, options, _) {
        ...
        logger.verbose(util.format($('Share %s created successfully'), share));
    ...
    */
    function deleteShare(share, options, _) {
        ...
        if (force !== true) {
            force = confirm(util.format($('Do you want to remove share %s?'), share), _);
            if (force !== true) {
                return;
            }
        }
        ...
        logger.info(util.format($('Share %s deleted successfully'), share));
    }
