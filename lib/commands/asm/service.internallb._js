...
exports.init = function (cli) {
  ...
  internallb.command('list [serviceName]')
    .description($('List internal load balancers for a clod service deployment'))
    ...
    .execute(function (serviceName, options, _) {
      ...
      var computeClient = utils._createComputeClient(profile.current.getSubscription(options.subscription), log);
  ...
  internallb.command('add [serviceName] [internalLBName]')
    ...
    .description($('Add an internal load balancer for a clod service deployment'))
    .option('-r, --serviceName <serviceName>', $('the cloud service name'))
  ...
  internallb.command('delete [serviceName] [internalLBName]')
    ...
    .execute(function (serviceName, internalLBName, options, _) {
      ...
      if (loadBalancer) {
        ...
        try {
          computeClient.loadBalancers.delete(serviceName, deployment.name, loadBalancer.name, _);
        } finally {
          progress.end();
        }
      } else {
  ...
  internallb.command('set [serviceName] [internalLBName]')
    ...
    .execute(function (serviceName, internalLBName, options, _) {
      ...
      if (loadBalancer) {
        ...
        try {
          computeClient.loadBalancers.update(serviceName, deployment.name, loadBalancer, _);
        } finally {
          progress.end();
        }
      } else {
