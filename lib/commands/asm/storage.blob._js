...
exports.init = function (cli) {
  ...
  */
  function showAzureContainer(container, options, _) {
    var blobService = getBlobServiceClient(options);
    container = StorageUtil.promptIfNotGiven($('Container name: '),container,_);
  ...
  */
  function deleteAzureContainer(container, options, _) {
    ...
    if (force !== true) {
      force = StorageUtil.confirm(util.format($('Do you want to remove the storage container %s?'),container),_);
      if (force !== true) {
        return;
      }
    }
  ...
  */
  function createBlobSAS(container,blob,permissions,expiry,options,_) {
    ...
    blob = StorageUtil.promptIfNotGiven($('Blob name: '),blob,_);

    if (!options.policy) {
      permissions = StorageUtil.promptIfNotGiven($('Permissions: '),permissions,_);
      if (isOnContainer) {
      ...
      expiry = StorageUtil.promptIfNotGiven($('Expiry: '),expiry,_);
      expiry = StorageUtil.parseDateTime(expiry);
    }

    var start;
    if (options.start) {
      start = StorageUtil.parseDateTime(options.start);
    }
  ...
  */
  function listAzureBlob(container, blobName, options, _) {
    var blobService = getBlobServiceClient(options);
    var specifiedContainerName = StorageUtil.promptIfNotGiven($('Container name:'),container,_);
  ...
  */
  function getAzureBlobProperties(container, blobName, options, _) {
    var blobService = getBlobServiceClient(options);
    var specifiedContainerName = StorageUtil.promptIfNotGiven($('Container name:'),container,_);
    var specifiedBlobName = StorageUtil.promptIfNotGiven($('Blob name:'),blobName,_);
    var storageOptions = getStorageBlobOperationDefaultOption();
    var blob = {};
  ...
  */
  function uploadAzureBlob(file, container, blobName, options, _) {
    ...
    var specifiedContainerName = StorageUtil.promptIfNotGiven($('Container name:'),container,_);
    var specifiedFileName = StorageUtil.promptIfNotGiven($('File name:'),file,_);
    ...
    if (force !== true) {
      ...
      if (blobProperties !== null) {
        if (blobProperties.blobType !== specifiedBlobType) {
          ...
          if (!StorageUtil.confirm(util.format($('Do you want to remove the blob %s in container %s?'),
  ...
  */
  function downloadAzureBlob(container, blobName, destination, options, _) {
    ...
    if (force !== true && utils.fileExists(fullName, _)) {
      if (!StorageUtil.confirm(util.format($('Do you want to overwrite %s?'),fullName),_)) {
        return;
      }
    }
  ...
  function getDestAccountOptions(options) {
    ...
    if (isConnectionStringDefined && isAccountDefined) {
      ...
      if (destOptions.connectionString) {
        destOptions.accountName = undefined;
        destOptions.accountKey = undefined;
      }
    }
  ...
  */
  function getSourceUri(options,_) {
    ...
    if (!options.sourceContainer && !options.sourceBlob) {
      source = promptIfNotGiven($('Source URI:'),source,_);
    } else {
      if (!options.sourceContainer) {
        options.sourceContainer = promptIfNotGiven($('Source container:'),options.sourceContainer,_);
      } else if (!options.sourceBlob) {
        options.sourceBlob = promptIfNotGiven($('Source blob:'),options.sourceBlob,_);
      }
  ...
  */
  function startBlobCopy(sourceUri, destContainer, options, _) {
    ...
    var destContainerName = StorageUtil.promptIfNotGiven($('Destination container name:'),destContainer,_);
    ...
    while (retry === true || retry === undefined) {
      try {
        ...
        if (StorageUtil.isBlobExistsException(e)) {
          retry = StorageUtil.confirm(util.format($('Do you want to overwrite the existing blob %s in container %s? (Y/N)'),destBlobName,destContainerName),_);
        }
        if (!retry) {
          throw e;
        }
  ...
  */
  function showBlobCopy(container,blob,options,_) {
    container = StorageUtil.promptIfNotGiven($('Destination container:'),container,_);
    blob = StorageUtil.promptIfNotGiven($('Destination blob:'),blob,_);
  ...
  */
  function stopBlobCopy(container,blob,copyid,options,_) {
    container = StorageUtil.promptIfNotGiven($('Destination container:'),container,_);
    blob = StorageUtil.promptIfNotGiven($('Destination blob:'),blob,_);
    copyid = StorageUtil.promptIfNotGiven($('Copy ID:'),copyid,_);
