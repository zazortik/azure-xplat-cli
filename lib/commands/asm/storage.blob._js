  ...
  function startBlobCopy(sourceUri, destContainer, options, _) {
    ...
    if (!sourceUri) {
      if (!options.sourceContainer && !options.sourceBlob) {
        source = StorageUtil.promptIfNotGiven($('Source URI:'),sourceUri,_);
      } else {
        if (!options.sourceContainer) {
          sourceContainer = StorageUtil.promptIfNotGiven($('Source container:'),sourceContainer,_);
        } else if (!options.sourceBlob) {
          sourceBlob = StorageUtil.promptIfNotGiven($('Source blob:'),sourceBlob,_);
        }
