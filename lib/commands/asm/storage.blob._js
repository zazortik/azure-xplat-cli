  ...
  function showContainerPolicy(container, name, options, _) {
    ...
    cli.interaction.formatOutput(policy, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the container %s.'), showPolicySettings.policyName, showPolicySettings.resourceName));
      }
    });
  }
  ...
  function setContainerPolicy(container, name, options, _) {
    ...
    cli.interaction.formatOutput(policies, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the container %s.'), setPolicySettings.policyName, setPolicySettings.resourceName)); 
      }
    });
  }
