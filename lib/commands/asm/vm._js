...
exports.init = function(cli) {
  ...
  docker.command('create <dns-name> <image> <user-name> [password]')
    ...
    .description($('Create a VM'))
    .option('-p, --docker-port [port]', $('Port to use for docker [2376]'))
  ...
  endpoint.command('set <vm-name> <endpoint-name>')
    ...
    .option('-l, --load-balanced-set-name <load-balanced-set-name>', $('the load balanced set name'))
    .option('-b, --public-port <public-port>', $('the public port'))
    .option('-c, --local-port <local-port>', $('the local port'))
    .option('-l, --load-balancer-set-name <load-balancer-set-name>', $('the load balancer set name'))
    .option('-o, --protocol <protocol>', $('the transport layer protocol for the endpoint (tcp or udp)'))
  ...
  endpoint.command('show <vm-name> <endpoint-name>')
    .description($('Get a endpoint in a VM'))
    .option('-d, --dns-name <name>', $('filter VMs for the specified DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
