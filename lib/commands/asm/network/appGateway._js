...
__.extend(AppGateway.prototype, {
  ...
  deleteFrontendIp: function (gatewayName, frontendIpName, options, _) {
    ...
    if (index === null) {
      throw new Error(util.format($('A frontend ip with name "%s" not found for an application gateway "%s'), frontendIpName, gatewayName));
    }
  ...
  addHttpListener: function (gatewayName, httpListenerName, frontendPortName, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (options.frontendIpName) {
      if (!utils.findFirstCaseIgnore(config.frontendIPConfigurations, {name: options.frontendIpName})) {
        throw new Error(util.format($('Frontend ip with name "%s" not found for an application gateway "%s'), options.frontendIpName, gatewayName));
      }
