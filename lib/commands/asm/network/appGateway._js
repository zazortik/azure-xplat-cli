...
__.extend(AppGateway.prototype, {
  ...
  addHttpSettings: function (appGatewayName, settingsName, options, _) {
    ...
    if (config) {
      ...
      if (settings) {
        throw new Error(util.format($('A http settings with name "%s" already exists for an application gateway "%s"'), settingsName, appGatewayName));
      } else {
  ...
  removeHttpSettings: function (appGatewayName, settingsName, options, _) {
    var config = this.getConfig(appGatewayName, _);
    if (config) {
      ...
      if (index !== null) {
        ...
        throw new Error(util.format($('A http settings with name "%s" not found for an application gateway "%s'), settingsName, appGatewayName));
      }
    } else {
      throw new Error(util.format($('Application gateway "%s" not found'), appGatewayName));
    }
  ...
  deleteFrontendIp: function (gatewayName, settingsName, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }

    var index = utils.indexOfCaseIgnore(config.frontendIPConfigurations, {name: settingsName});
    if (index === null) {
      throw new Error(util.format($('A frontend ip with name "%s" not found for an application gateway "%s'), settingsName, gatewayName));
    }

    if (!options.quiet && !this.cli.interaction.confirm(util.format($('Delete a frontend ip "%s?" [y/n] '), settingsName), _)) {
      return;
    }
  ...
  addFrontendPort: function (gatewayName, frontendPortName, port, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (frontendPort) {
      throw new Error(util.format($('A frontend port with name "%s" already exists for an application gateway "%s"'), frontendPortName, gatewayName));
    }
    frontendPort = {
    ...
    this.setConfig(gatewayName, config, _);
    this.show(gatewayName, options, _);
  },

  deleteFrontendPort: function (gatewayName, frontendPortName, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (index === null) {
      throw new Error(util.format($('Frontend port with name "%s" not found for an application gateway "%s'), frontendPortName, gatewayName));
    }
  ...
  addHttpListener: function (gatewayName, httpListenerName, frontendPort, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (!config.httpListeners || !config.httpListeners.length) {
      ...
      httpListener = utils.findFirstCaseIgnore(config.httpListeners, {name: httpListenerName});
    }

    if (httpListener) {
      throw new Error(util.format($('An http listener with name "%s" already exists for an application gateway "%s"'), httpListenerName, gatewayName));
    }

    if (!utils.findFirstCaseIgnore(config.frontendPorts, {name: frontendPort})) {
      throw new Error(util.format($('Frontend port with name "%s" not found for an application gateway "%s'), options.frontendPort, gatewayName));
    }
    ...
    if (options.frontendIp) {
      if (!utils.findFirstCaseIgnore(config.frontendIPConfigurations, {name: options.frontendIp})) {
        throw new Error(util.format($('Frontend ip with name "%s" not found for an application gateway "%s'), options.frontendIp, gatewayName));
      } else {
        httpListener.frontendIP = options.frontendIP;
      }
    }

    if (options.protocol) {
      if (options.protocol.toLowerCase() === 'http' && !options.sslCert) {
        throw new Error($('--ssl-cert parameter is required when "--protocol Https" parameter is specified'));
      } else {
        httpListener.protocol = options.protocol;
  ...
  deleteHttpListener: function (gatewayName, httpListenerName, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (index === null) {
      throw new Error(util.format($('Http listener with name "%s" not found for an application gateway "%s'), httpListenerName, gatewayName));
    }
  ...
  addLoadBalancingRule: function (gatewayName, ruleName, httpSettings, httpListener, addressPool, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (!config.httpLoadBalancingRules || !config.httpLoadBalancingRules.length) {
      ...
      lbRule = utils.findFirstCaseIgnore(config.httpLoadBalancingRules, {name: ruleName});
    }

    if (lbRule) {
      throw new Error(util.format($('An http load balancing rule with name "%s" already exists for an application gateway "%s"'), ruleName, gatewayName));
    }

    if (!utils.findFirstCaseIgnore(config.backendHttpSettingsList, {name: httpSettings})) {
      throw new Error(util.format($('Http settings with name "%s" not found for an application gateway "%s'), httpSettings, gatewayName));
    }

    if (!utils.findFirstCaseIgnore(config.httpListeners, {name: httpListener})) {
      throw new Error(util.format($('Http listener with name "%s" not found for an application gateway "%s'), httpListener, gatewayName));
    }

    if (!utils.findFirstCaseIgnore(config.backendAddressPools, {name: addressPool})) {
      throw new Error(util.format($('Address pool with name "%s" not found for an application gateway "%s'), addressPool, gatewayName));
    }
    ...
    lbRule = {
      ...
      backendAddressPool: addressPool
    };
  ...
  deleteLoadBalancingRule: function (gatewayName, ruleName, options, _) {
    var config = this.getConfig(gatewayName, _);
    if (!config) {
      throw new Error(util.format($('Application gateway "%s" not found'), gatewayName));
    }
    ...
    if (index === null) {
      throw new Error(util.format($('An http load balancing rule with name "%s" not found for an application gateway "%s'), ruleName, gatewayName));
    }
