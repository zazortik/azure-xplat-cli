...
__.extend(AppGateway.prototype, {
  create: function (appGatewayName, options, _) {
    var self = this;

    var appGateway = self.get(appGatewayName, _);
    if (appGateway) {
      throw new Error(util.format($('Application Gateway with name "%s" already exists'), appGatewayName));
    }

    appGateway = {};
    self._parseAppGateway(appGatewayName, appGateway, options, true);

    var progress = self.interaction.progress(util.format($('Creating an Application Gateway "%s"'), appGatewayName));
    try {
      self.networkManagementClient.applicationGateways.create(appGateway, _);
    } finally {
      progress.end();
    }
  },

  set: function (appGatewayName, options, _) {
    var self = this;

    var appGateway = self.get(appGatewayName, _);
    if (!appGateway) {
      throw new Error(util.format($('Application Gateway with name "%s" not found'), appGatewayName));
    }

    self._parseAppGateway(appGatewayName, appGateway, options, false);

    var progress = self.interaction.progress(util.format($('Updating an Application Gateway "%s"'), appGatewayName));
    try {
  ...
  addSsl: function (appGatewayName, certName, options, _) {
    ...
    try {
      self.networkManagementClient.applicationGateways.addCertificate(appGatewayName, certName, certificateObject, _);
    } finally {
      progress.end();
    }
  },
