...
__.extend(VpnGateway.prototype, {
  ...
  create: function (networkName, options, _) {
    ...
    var progress = this.cli.interaction.progress(util.format($('Creating VPN Gateway for the virtual network "%s"'), networkName));
    try {
  ...
  show: function (networkName, options, _) {
    ...
    if (!gateway) {
      if (output.format().json) {
        ...
        output.warn(util.format($('VPN Gateway not found for the virtual network "%s"'), networkName));
      }
      return;
    }
  ...
  delete: function (networkName, options, _) {
    ...
    if (!options.quiet && !this.cli.interaction.confirm(util.format($('Delete VPN Gateway for the virtual network %s? [y/n] '), networkName), _)) {
      return;
    }

    var progress = this.cli.interaction.progress(util.format($('Deleting VPN Gateway for the virtual network "%s"'), networkName));
    try {
  ...
  resize: function (networkName, sku, options, _) {
    ...
    options.sku = sku;
    var parameters = this._parseGateway(options);
    var progress = this.cli.interaction.progress(util.format($('Resizing VPN Gateway for the virtual network "%s"'), networkName));
    try {
  ...
  reset: function (networkName, options, _) {
    ...
    var progress = this.cli.interaction.progress(util.format($('Resetting VPN Gateway for the virtual network "%s"'), networkName));
    try {
  ...
  get: function (networkName, _) {
    var gateway = null;
    var progress = this.cli.interaction.progress(util.format($('Looking up network VPN Gateway in virtual network "%s"'), networkName));
    try {
  ...
  getDiagnosticsSession: function(vnetName, options, _) {
    ...
    this.cli.interaction.formatOutput(session, function (session) {
      output.nameValue('Diagnostics URL', session.diagnosticsUrl);
    });
  },

  _validateDefaultSite: function(vnetName, options, _) {
    if(!options.siteName) {
      var gateway = this.get(vnetName, _);
      if (!gateway) {
