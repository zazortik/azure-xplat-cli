...
__.extend(DnsServer.prototype, {
  ...
  register: function (dnsIp, options, _) {
    ...
    if (utils.findFirstCaseIgnore(vnetConfiguration.Dns.DnsServers, {Name: dnsServer.Name})) {
      throw new Error(util.format($('A DNS Server with name identifier "%s" already exists'), dnsServer.Name));
    }

    if (utils.findFirstCaseIgnore(vnetConfiguration.Dns.DnsServers, {IPAddress: dnsServer.IPAddress})) {
      throw new Error(util.format($('A DNS Server with ip address "%s" already exists'), dnsServer.IPAddress));
    }

    vnetConfiguration.Dns.DnsServers.push(dnsServer);
  ...
  unregister: function (dnsIp, options, _) {
    ...
    var dnsName = vnetConfiguration.Dns.DnsServers[dnsEntryIndex].Name.toLowerCase();
    ...
    for (var j = 0; j < vnetConfiguration.VirtualNetworkSites.length; j++) {
      ...
      if (site.DnsServersRef) {
        for (var k = 0; k < site.DnsServersRef.length; k++) {
          if (site.DnsServersRef[k].Name.toLowerCase() === dnsName) {
    ...
    if (!options.quiet && !this.cli.interaction.confirm(util.format($('Delete the DNS server "%s" (%s)? [y/n] '), dnsName, dnsIPAddress), _)) {
      return;
    }
