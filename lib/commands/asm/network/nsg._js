...
__.extend(Nsg.prototype, {
  ...
  _validateAddressPrefix: function (ipInCidrFormat, addressPrefixType) {
    ...
    if (ipInCidrFormat === '*' || ipInCidrFormat === '"*"' || ipInCidrFormat === 'Internet' || ipInCidrFormat === 'VirtualNetwork' || ipInCidrFormat === 'AzureLoadBalancer') {
    ...
    if (ipValidationResult.error || ipValidationResult.cidr === null) {
      throw new Error(util.format($('IPv4 %s address prefix must be in CIDR format. Asterisk can be used also.'), addressPrefixType));
    }
  ...
  _validatePortRange: function (port, portType) {
    ...
    if (isNaN(port) || port < portRange[0] || port > portRange[1]) {
      throw new Error(util.format($('%s port parameter must be an integer between %s and %s. Asterisk can be used also.'), portType, portRange[0], portRange[1]));
    }
  ...
  _parseSecurityRule: function (params, useDefaults) {
    ...
    if (params.protocol) {
      ...
      if (params.protocol === '*' || params.protocol === '"*"') {
        ruleProfile.protocol = params.protocol;
      } else {
