  ...
  function showTablePolicy(table, name, options, _) {
    ...
    cli.interaction.formatOutput(policy, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the table %s.'), showPolicySettings.policyName, showPolicySettings.resourceName));
      }
    });
  }
  ...
  function setTablePolicy(table, name, options, _) {
    ...
    cli.interaction.formatOutput(policies, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the table %s.'), setPolicySettings.policyName, setPolicySettings.resourceName)); 
      }
    });
  }
