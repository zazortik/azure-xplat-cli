...
__.extend(WebsitesClient.prototype, {
  ...
  createWebsiteManagementService: function(callback) {
    return utils._createWebsiteClient(profile.current.getSubscription(this.subscription), this.cli.output, callback);
  },
  ...
  createRepository: function (options, callback) {
    ...
    var service = self.createWebsiteManagementService(options.subscription, callback);
  ...
  syncRepository: function(options, callback) {
    ...
    var oldService = utils.createWebsiteManagementService(profile.current.getSubscription(self.subscription), self.cli.output, callback);
