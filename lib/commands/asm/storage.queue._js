  ...
  function showQueuePolicy(queue, name, options, _) {
    ...
    cli.interaction.formatOutput(policy, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the queue %s.'), showPolicySettings.policyName, showPolicySettings.resourceName));
      }
    });
  }
  ...
  function setQueuePolicy(queue, name, options, _) {
    ...
    cli.interaction.formatOutput(policies, function(outputData) {
      if (outputData) {
        ...
        logger.info(util.format($('The stored access policy %s doesn\'t exist on the queue %s.'), setPolicySettings.policyName, setPolicySettings.resourceName)); 
      }
    });
  }
