...
_.extend(VMClient.prototype, {
            ...
            if (!error) {
              ...
              if (image) {
                ...
                if (response.vmImage) {
                  ...
                  return callback(new Error(util.format($('There is another image of type "%s" exits with the same name, CLI will stop here to avoid duplicates and potential conflicts. Please use another name for the image'), imageType)));
                }
              }
          ...
          var captureVMIntern = function() {
            if (!options.osState) {
              ...
              if (found.roleInstance.instanceStatus === 'ReadyRole') {
                logger.warn($('VM image capture operation is trigged while VM is still running. As this may cause data corruption while creating VMs from this image, we recommend using "azure vm shutdown" to stop the VM before capturing..'));
              }
        ...
        if (result.error) {
          ...
          for (var l = 0; l < newEndPoints.length; l++) {
            ...
            if (newEndPoints[l].loadBalancerName) {
              var r = checkInternalLoadBalancerExists(result.deployment.deploy.loadBalancers, newEndPoints[l].loadBalancerName);
              if (r.error) {
        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function (error, response) {
            ...
            if (error) {
              ...
              publicIPIndex = _.indexOf(_.map(networkConfiguration.publicIPs, function (p) { return p.name.toLowerCase(); }), publicipName.toLowerCase());
  ...
  diskAttachDetach: function (options, callback) {
    ...
    if (options.hostCaching) {
      var i = _.indexOf(_.map(supportedHostCaching, function (s) { return s.toLowerCase(); }), options.hostCaching.toLowerCase());
      if (i === -1) {
...
function checkInternalLoadBalancerExists(loadBalancers, loadBalancerName) {
  ...
  if (loadBalancerIndex == -1) {
    return {
      error: util.format($('An internal load balancer (ILB) with name %s not found for this deployment, refer \'azure service internal-load-balancer\' commands to manage ILB'), loadBalancerName)
    };
  }
...
function findConfigurationSet(configurationSets, configurationSetName) {
  return _.indexOf(_.map(configurationSets, function (p) { return p.configurationSetType; }), configurationSetName);
}
...
function validateVMCaptureParams(vmImageTypes, osState, deleteAfterCapture) {
  ...
  if (osState) {
    var i = _.indexOf(_.map(vmImageTypes, function (s) { return s.toLowerCase(); }), osState.toLowerCase());
    if (i === -1) {
