        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function (error, response) {
            ...
            if (error) {
              ...
              var networkConfiguration = persistentVMRole.configurationSets[networkConfigIndex];
              var endpointIndex = findEndpoint(networkConfiguration.inputEndpoints, endpointName);
              if (endpointIndex == -1) {
                return callback(new Error(util.format($('Endpoint %s not found in the network configuration'), endpointName)));
              }

              var epToAddAclRule = networkConfiguration.inputEndpoints[endpointIndex];
              ...
              networkConfiguration.inputEndpoints[endpointIndex] = epToAddAclRule;
              progress = self.cli.interaction.progress($('Updating network configuration'));
              computeManagementClient.virtualMachines.update(result.deployment.svc, result.deployment.deploy.name, vmName, persistentVMRole, function (error) {
                progress.end();
                return callback(error);
    ...
    self.getDeployments(options, function (error, deployments) {
      if (error) {
        ...
        if (result.vmRole) {
          ...
          var networkConfiguration = persistentVMRole.configurationSets[networkConfigIndex];
          var endpoints = networkConfiguration.inputEndpoints;
          var endpointIndex = findEndpoint(endpoints, endpointName);
          if (endpointIndex == -1) {
            return callback(new Error(util.format($('Endpoint %s not found in the network configuration'), endpointName)));
          }

          var getAclRules = function () {
            return !endpoints[endpointIndex].endpointAcl || !endpoints[endpointIndex].endpointAcl.rules ? [] : endpoints[endpointIndex].endpointAcl.rules;
          };
        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function (error, response) {
            ...
            if (error) {
              ...
              var epToRemoveAclRule = endpoints[endpointIndex];
              ...
              var removeEnpointACLRuleInternal = function () {
                ...
                networkConfiguration.inputEndpoints[endpointIndex] = epToRemoveAclRule;
                progress = self.cli.interaction.progress($('Updating network configuration'));
                computeManagementClient.virtualMachines.update(result.deployment.svc, result.deployment.deploy.name, vmName, persistentVMRole, function (error) {
                  progress.end();
                  return callback(error);
        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function (error, response) {
            ...
            if (error) {
              ...
              var epToUpdateAclRule = endpoints[endpointIndex];
