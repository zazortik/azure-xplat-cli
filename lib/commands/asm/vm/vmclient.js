      ...
      if (error) {
        ...
        if (found) {
          // confirm deleting if required
            var shouldDelete = options.quiet || cli.interaction.confirm(util.format($('Delete the VM %s ? [y/n] '), vmName), _);
            if (!shouldDelete) {
                return;
            }
...
function createDockerVM(dnsName, options, logger, callback) {
	...
	function checkAndGenerateCertificatesIfNeeded(cb) {
		utils.fileExists(options.dockerCertDir, function(certDirError, exists) {
			...
			if (!exists) {
				...
				logger.verbose($('Found docker certs folder.'));
	...
	function generateDockerCertificates(cb) {
		...
		openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.caKey }, function(err) {
			if(err) {
				logger.verbose(err); 
			}
			
			openssl.exec('req', { new: true, x509: true, days: 365, passin: 'pass:' + password, subj: '/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\\*', key: options.dockerCerts.caKey, out: options.dockerCerts.ca }, function(err) {
				if(err) {
					logger.verbose(err); 
				}
				
				openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.serverKey }, function(err) {
					if(err) {
						logger.verbose(err); 
					}
				
					openssl.exec('req', { new: true, passin: 'pass:' + password, subj: '/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\\*', key: options.dockerCerts.serverKey, out: options.dockerCerts.server }, function(err) {
						if(err) {
							logger.verbose(err); 
						}
						
						openssl.exec('x509', { req: true, days: 365, in: options.dockerCerts.server,  passin: 'pass:' + password, set_serial: 01,
										CA: options.dockerCerts.ca, CAkey: options.dockerCerts.caKey, out: options.dockerCerts.serverCert }, function(err) {
							if(err) {
									logger.verbose(err.toString()); 
								}
							
							openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.clientKey }, function(err) {
								if(err) {
									logger.verbose(err); 
								}
								
								openssl.exec('req', { new: true, passin: 'pass:' + password, subj: '/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\\*', key: options.dockerCerts.clientKey, out: options.dockerCerts.client }, function(err) {
									if(err) {
										logger.verbose(err.toString()); 
									}
									...
									fs.writeFile(configPath, 'extendedKeyUsage = clientAuth', function(err) {
										...
										openssl.exec('x509', { req: true, days: 365, in: options.dockerCerts.client,  passin: 'pass:' + password,  set_serial: 01, extfile: configPath, CA: options.dockerCerts.ca, CAkey: options.dockerCerts.caKey, out: options.dockerCerts.clientCert }, function(err) {
											if(err) {
												logger.verbose(err.toString()); 
											}
					
											openssl.exec('rsa', { passin: 'pass:' + password, in: options.dockerCerts.serverKey, passout: 'pass:' + password, out: options.dockerCerts.serverKey }, function(err) {
												if(err) {
													logger.verbose(err.toString()); 
												}
											
												openssl.exec('rsa', { passin: 'pass:' + password, in: options.dockerCerts.clientKey, passout: 'pass:' + password, out: options.dockerCerts.clientKey }, function(err) {
													if(err) {
														logger.verbose(err.toString()); 
													}
