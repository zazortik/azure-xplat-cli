...
_.extend(VMClient.prototype, {
  ...
  createDockerVM: function(dnsName, imageName, userName, password, options, callback, logger) {
    ...
    if ((options.dockerPort && typeof options.dockerPort === 'boolean') || !options.dockerPort) {
      options.dockerPort = constants.DEFAULT_DOCKER_PORT;
    }
...
function setDockerVMExtension(role, version, options, logger, callback) {
  ...
  setVMExtension(role, 'DockerExtension', 'Microsoft.Azure.Extensions', version, 'DockerExtension', 'enable', privateConfig, publicConfig, function (err) {
    return callback(err);
  });
...
function verifyCertFingerPrint(osName, options, logger, cb) {
  ...
  if (!options.ssh || !options.sshEndpoint) {
    return cb();
  }
...
function generateSSHKeysAndCertIfNeeded(options, logger, cb) {
  var homePath = process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'];
  var azureDir = path.join(homePath, '.azure');
  var privateKeyPath = path.join(azureDir, options.dnsPrefix + '-privateKey.pem');
  var certPath = path.join(azureDir, options.dnsPrefix + '-cert.pem');

  utils.fileExists(privateKeyPath, function(error, exists) {
    if (error) {
      return cb(error);
    }

    if (exists) {
      utils.fileExists(certPath, function(error, exists) {
        if (error) {
          return cb(error);
        }

        if (exists) {
          return cb(null, certPath, privateKeyPath);
        } else {
          generateSSHKeyAndCert(privateKeyPath, certPath);
          return cb(null, certPath, privateKeyPath);
        }
      });
    } else {
...
function generateSSHKeyAndCert(privateKeyPath, certPath) {
  var keys = certUtils.generatePemKeyPair();
  var cert = certUtils.generateX509PemCert(keys.public, keys.private, '');
...
function createOrLoadSshCert(options, logger, cb) {
  ...
  if (!options.sshCert) {
    logger.info($('--ssh-cert option is not specified, using auto generated ssh keys'));
    generateSSHKeysAndCertIfNeeded(options, logger, function(error, sshCertPath, privateKeyPath) {
      if (error) {
        return cb(error);
      }

      logger.info(util.format($('Saved SSH private key to %s'), privateKeyPath));
      options.sshCert = sshCertPath;
