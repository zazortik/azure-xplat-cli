...
_.extend(VMClient.prototype, {
  ...
  createVM: function(dnsName, imageName, userName, password, options, callback, logger) {
    ...
    if (options.sshEndpoint && !options.ssh) {
      // there is no --no-ssh-endpoint parameter, i.e. user don't want to explicitly turn off the ssh endpoint
      if (password || options.sshCert) {
        return callback(new Error($('--ssh (SSH port, default to 22) is required when --ssh-cert or --ssh-password is specified')));
      }
    }
  ...
  createEP: function(vmName, publicPort, localPort, options, callback, logger) {
    ...
    if (options.loadBalancerSetName) {
      epInput.loadBalancerSetName = {
        'value': options.loadBalancerSetName,
        'argName': '--load-balancer-set-name'
      };
    }
  ...
  createMultipleEP: function(vmName, endpointsConfig, options, callback, logger) {
    var self = this;
    var message = 'each endpoint configuration in the endpoints configuration should be of the form \r\n       <public-port>:<local-port>:<protocol>:<idle-timeout>:<direct-server-return>:<probe-protocol>:<probe-port>:<probe-path>:<load-balanced-set-name>:<internal-load-balancer-name>:<load-balancer-distribution>';
    var endpointPropMap = {
      0: {
       propName: 'publicPort',
       argName: 'public-port'
    ...
    endpointsConfigAsList.forEach(function(endpointConfig) {
      ...
      if (endpointConfig) {
        ...
        if (endpointConfigAsList.length !== 11) {
...
function loadSshCert(options, logger, cb) {
  ...
  if (certUtils.isOpenSshRSAPub(pemSshCertStr)) {
    ...
    return cb($('Specified SSH certificate is not in PEM or SSH RSA format'));
  }
