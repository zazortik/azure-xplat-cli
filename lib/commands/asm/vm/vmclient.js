        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function(error, response) {
            ...
            if (error) {
              ...
              progress = self.cli.interaction.progress($('Updating network configuration'));
              updateDebugSettings(persistentVMRole, options);
              if(epToAddAclRule.loadBalancedEndpointSetName) {
                if (!networkConfiguration.loadBalancedEndpoints) {
                  networkConfiguration.loadBalancedEndpoints = [];
                }
                epToAddAclRule.rules = epToAddAclRule.endpointAcl.rules;
                networkConfiguration.loadBalancedEndpoints[networkEPResult.endpointIndex] = epToAddAclRule;
                computeManagementClient.virtualMachines.updateLoadBalancedEndpointSet(result.deployment.svc, result.deployment.deploy.name, persistentVMRole, function (error) {
                  progress.end();
                  return callback(error);
                });
              } else {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function(error, response) {
            ...
            if (error) {
              ...
              var removeEnpointACLRuleInternal = function() {
                ...
                epToRemoveAclRule.rules = epToRemoveAclRule.endpointAcl.rules;
                networkConfiguration.inputEndpoints[networkEPResult.endpointIndex] = epToRemoveAclRule;
                progress = self.cli.interaction.progress($('Updating network configuration'));
                updateDebugSettings(persistentVMRole, options);
                if(epToRemoveAclRule.loadBalancedEndpointSetName) {
                  computeManagementClient.virtualMachines.updateLoadBalancedEndpointSet(result.deployment.svc, result.deployment.deploy.name, persistentVMRole, function (error) {
                    progress.end();
                    return callback(error);
                  });
        ...
        if (result.error) {
          ...
          computeManagementClient.virtualMachines.get(result.deployment.svc, result.deployment.deploy.name, vmName, function(error, response) {
            ...
            if (error) {
              ...
              progress = self.cli.interaction.progress($('Updating network configuration'));
              updateDebugSettings(persistentVMRole, options);
              if(epToUpdateAclRule.loadBalancedEndpointSetName) {
                epToUpdateAclRule.rules = epToUpdateAclRule.endpointAcl.rules;
                computeManagementClient.virtualMachines.updateLoadBalancedEndpointSet(result.deployment.svc, result.deployment.deploy.name, persistentVMRole, function (error) {
                  progress.end();
                  return callback(error);
                });
...
function createVMView(role, deployment, logger) {
  ...
  try {
    ...
    result = {
      ...
      DebugSettings: !role.debugSettings ? {} : {
        BootDiagnosticsEnabled: (role.debugSettings.bootDiagnosticsEnabled ? role.debugSettings.bootDiagnosticsEnabled : {}),
        ConsoleScreenshotBlobUri: (role.debugSettings.consoleScreenshotBlobUri ? role.debugSettings.consoleScreenshotBlobUri : null),
        SerialOutputBlobUri: (role.debugSettings.serialOutputBlobUri ? role.debugSettings.serialOutputBlobUri : null)
      }
    };
  }
  catch (ex) {
...
function createRole(name, dnsPrefix, image, options, logger, cli, callback) {
  ...
  role.debugSettings = {
    bootDiagnosticsEnabled: (options.bootDiagnosticsEnabled === true ? true : false)
  };
...
function verifyUserNameAndPwd(osName, options, logger, cli, callback) {
  ...
  if (utils.ignoreCaseEquals(osName, 'windows')) {
    ...
    if (typeof options.password === 'undefined') {
      cli.interaction.password(promptMsg, '*', function(err, password) {
        if (err) {
          throw err;
        }
        process.stdin.pause();
...
function updateDebugSettings(role, options) {
  if (options.bootDiagnosticsEnabled === true || options.bootDiagnosticsEnabled === false) {
    role.debugSettings = {
      bootDiagnosticsEnabled: options.bootDiagnosticsEnabled
    };
  }
  else if (role && role.debugSettings) {
    role.debugSettings = null;
  }
}
