...
function validateNicSubnets(networkInfo, networkInterfaces) {
  ...
  for (var m = 0; m < networkInterfaces.length; m++) {
    ...
    if (nicIPConfig.address) {
      // validate NIC VNet Static IP Address
      var parsedIp = vnetUtil.parseIPv4(nicIPConfig.address);
      if (parsedIp.error) {
        return {
          error: util.format($('Invalid IP address "%s". "%s"'), nicIPConfig.address, parsedIp.error)
        };
      }
      ...
      if (!checkResult.inRange) {
        return {
          error: util.format($('The IP address "%s" does not belongs to the subnet "%s".'), nicIPConfig.address, nicIPConfig.subnetName)
        };
      }
    }
...
function createRole(name, dnsPrefix, image, options, logger, cli, callback) {
  ...
  function createRoleInternal() {
    ...
    if (inputEndPoints.length || options.subnetNames || options.staticIp || publicIPs.length) {
      ...
      nicSubnetResult = validateNicSubnets(options.networkInfo, options.networkInterfaces);
...
function parseNICParams(nicParams) {
  ...
  var nicConfigFmtMsg = 'the --nic-config must be comma seperated NIC definitions, format of each NIC definition must be <nicName>:<subnetName>:[vnetStaticIP]:[nsgName]:[ipForwarding]';
  if (nicParams && (nicParams = nicParams.trim())) {
    nicParams = nicParams.split(',');
    for (var i = 0; i < nicParams.length; i++) {
      ...
      if (nicParam) {
        ...
        arg = nic[2].trim();
        if (arg) {
          networkInterface.iPConfigurations[0].address = arg;
        }
        ...
        arg = nic[4].trim();
        if (arg) {
          networkInterface.iPForwarding = arg;
        }
      }
    }
  }
