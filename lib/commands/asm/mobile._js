    ...
    function checkMigrationStatus() {
      mobile.getMigrationState(options, function (error, body) {
        ...
        if (status === 'Completed') {
          ...
          setTimeout(checkMigrationStatus, 10000);
        }
      });
    }
    setTimeout(checkMigrationStatus, 10000);
...
{          progress.end();
  ...
  function aadTenantResultHandler(aad){
    if(typeof aad.tenants === 'undefined' || aad.tenants.length === 0){
      ...
      aad.tenants.forEach(function (tenant) { log.data($('Tenant'), setColorOrDefault(tenant)); });
    }
  }
  ...
  mobileAuthAadTenant.command('list [servicename]')
      ...
      .execute(function (servicename, options, _) {
        ...
        cli.interaction.formatOutput(aad.tenants, function() { return aadTenantResultHandler(aad); });
      });
  ...
  mobileAuthAadTenant.command('remove [servicename] [tenant]')
    .usage('[options] [servicename] [tenant]')
