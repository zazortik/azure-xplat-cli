      ...
      computeManagementClient.virtualMachineOSImages.list(function(error, response) {
        ...
        if (!error) {
          if (response.images.length > 0) {
            logger.table(response.images, function (row, item) {
              row.cell('Name', item.name);
              if (what === DISK) {
                row.cell('OS', item.operatingSystemType || '');
              } else {
...
exports.delete = function(what, cli) {
  return function(diskName, deleteOptions, callback) {
    ...
    var logger = cli.output;
    deleteImage.deleteImage(whatAPI[what], ['Disk', 'OS Image'][what], logger, computeManagementClient, diskName, deleteOptions, cli.interaction.progress, callback);
  };
};
...
exports.create = function(what, cli) {
  return function(name, sourcePath, options, callback) {
    ...
    // TODO: remove channel after blobUtils update
    ...
    function getBlobNameAndUpload() {
      //TODO: blobUtils uses hand-written Azure SDK version
    ...
    function createImage(finalBlobUrl) {
      ...
      if (what === DISK) {
        //utils.doServiceManagementOperation(channel, 'addDisk', name, normUrl, parameters.imageOptions, function(error) {
        computeManagementClient.virtualMachineDisks.createDisk(parameters.imageOptions, function(error) {
          callback(error);
        });
      } else {
        //utils.doServiceManagementOperation(channel, 'createOSImage', os, name, normUrl, parameters.imageOptions, function(error) {
...
function createComputeManagementClient (cli, options) {
  return utils._createComputeClient(profile.current.getSubscription(options.subscription), cli.output);
  // return utils._createComputeClient(cli.category('account').getCurrentSubscription(options.subscription), cli.output);
}

function createManagementClient (cli, options) {
  return utils._createManagementClient(profile.current.getSubscription(options.subscription), cli.output);
 // return utils._createManagementClient(cli.category('account').getCurrentSubscription(options.subscription), cli.output);
}
