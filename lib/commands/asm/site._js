        ...
        .execute(function (nameArg, options, _) {
          ...
          function promptForLocation(_) {
            ...
            if (!context.site.location && !context.site.webspace) {
              location = cli.interaction.chooseIfNotGiven($('Location: '), '', context.site.location,
                function(cb) {
                  cb(null, locations.map(function (location) { return location.name; }));
                }, _);
  ...
  site.command('delete [name]')
        ...
        .execute(function (name, options, _) {
          ...
          try {
            ...
            cacheUtils.deleteSpace(context, context.site.webspace, _);
          } finally {
            progress.end();
          }
