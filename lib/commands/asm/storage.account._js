...
exports.init = function (cli) {
  ...
  var connectionString = storageAccount.category('connection-string')
    .description($('Commnands to show your Storage connection string'));
  ...
  function wrapEndpoint(uri, type) {
    ...
    if (uri.indexOf('//') != -1
      && !utils.stringStartsWith(uri, 'http://')
      && !utils.stringStartsWith(uri, 'https://')) {
  ...
  connectionString.showCommand = function (name, options, _) {
    ...
    cli.interaction.formatOutput(connection, function (outputData) {
      log.data($('connection-string'), outputData.string);
    });
  }
  ...
  connectionString.command('show <name>')
    .description($('Generate a connection string for your account'))
