...
var ExecutionProcessor = function(cli) {
  ...
  this.createHDInsightClusterManagementClient = function (regionCloudServiceName) {
    return utils.getHDInsightClusterManagementClient(regionCloudServiceName, profile.current.getSubscription()._createCredentials());
  };

  this.createHDInsightCluster2ManagementClient = function (regionCloudServiceName) {
    return utils.getHDInsightCluster2ManagementClient(regionCloudServiceName, profile.current.getSubscription()._createCredentials());
  };
...
var hdInsightCommandLine = function(cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function (clusterName, osType, storageAccountName, storageAccountKey, storageContainer, dataNodeCount, headNodeSize, dataNodeSize, location, userName, password, sshUserName, sshPassword, _) {
    ...
    if(osType === 'windows')
    {
      ...
      cluster2Payload += ' "configurations": [';
        ...
        cluster2Payload += ' }';
      cluster2Payload += ' ],';
      cluster2Payload += ' "host_groups": [';
        cluster2Payload += ' {';
          cluster2Payload += ' "name": "headnode"';
        cluster2Payload += ' },';
        cluster2Payload += ' {';
          cluster2Payload += ' "name": "workernode"';
        cluster2Payload += ' }';
      cluster2Payload += ' ]';
      cluster2Payload += ' }</a:Value></a:KeyValueOfstringstring><a:KeyValueOfstringstring><a:Key>CsmDocument</a:Key><a:Value>{';
        cluster2Payload += ' "location": "' + location + '",';
        cluster2Payload += ' "resources": [';
            cluster2Payload += ' {';
               cluster2Payload += ' "name": "gateway",';
                cluster2Payload += ' "type": "VirtualMachineGroup",';
                cluster2Payload += ' "location": "' + location + '",';
                cluster2Payload += ' "properties": {';
                    cluster2Payload += ' "instanceCount": 1,';
                    cluster2Payload += ' "dnsName": "' + clusterName + '",';
                    cluster2Payload += ' "osProfile": {';
                        ...
                        cluster2Payload += ' }';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "hardwareProfile": {';
                        cluster2Payload += ' "vmSize": "Medium"';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "storageProfile": {';
                        cluster2Payload += ' "osDisk": {';
                            cluster2Payload += ' "name": "Disk1",';
                            cluster2Payload += ' "vhdUri": "",';
                            cluster2Payload += ' "sourceImageName": ""';
                        cluster2Payload += ' }';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "networkProfile": {';
                        cluster2Payload += ' "networkInterfaces": [';
                            cluster2Payload += ' {';
                                cluster2Payload += ' "name": "NIC1",';
                                cluster2Payload += ' "properties": {';
                                    cluster2Payload += ' "virtualNetwork": {';
                                        cluster2Payload += ' "id": ""';
                                    cluster2Payload += ' },';
                                    cluster2Payload += ' "ipConfigurations": [';
                                        cluster2Payload += ' {';
                                            cluster2Payload += ' "name": "IpConfigName1",';
                                            cluster2Payload += ' "subnet": {';
                                                cluster2Payload += ' "id": ""';
                                            cluster2Payload += ' }';
                                        cluster2Payload += ' }';
                                    cluster2Payload += ' ]';
                            ...
                            cluster2Payload += ' }';
                        cluster2Payload += ' ],';
                        cluster2Payload += ' "inputEndpoints": [';
                            cluster2Payload += ' {';
                                cluster2Payload += ' "name": "HTTPS",';
                                cluster2Payload += ' "protocol": "TCP",';
                                cluster2Payload += ' "localPort": 443,';
                                cluster2Payload += ' "publicPort": 443';
                            cluster2Payload += ' }';
                        cluster2Payload += ' ]';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "certificates": [';
                    cluster2Payload += ' ],';
                    cluster2Payload += ' "provisionGuestAgent": true';
                cluster2Payload += ' }';
            cluster2Payload += ' },';
            cluster2Payload += ' {';
                cluster2Payload += ' "name": "headnode",';
                ...
                cluster2Payload += ' "properties": {';
                    cluster2Payload += ' "instanceCount": 1,';
                    cluster2Payload += ' "dnsName": "' + clusterName + '-ssh",';
                    cluster2Payload += ' "osProfile": {';
                        cluster2Payload += ' "computerNamePattern": "headnode####",';
                        cluster2Payload += ' "linuxOperatingSystemProfile": {';
                            cluster2Payload += ' "DisableSshPasswordAuthentication": false,';
                            cluster2Payload += ' "username": "' + sshUserName + '",';
                            cluster2Payload += ' "userPassword": "' + sshPassword + '",';
                            cluster2Payload += ' "sshProfile": {';
                                cluster2Payload += ' "publicKeys": [],';
                                cluster2Payload += ' "sshKeyPairs": []';
                            cluster2Payload += ' },';
                            cluster2Payload += ' "customData": ""';
                        cluster2Payload += ' }';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "hardwareProfile": {';
                        cluster2Payload += ' "vmSize": "Large"';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "storageProfile": {';
                        cluster2Payload += ' "osDisk": {';
                            cluster2Payload += ' "name": "Disk1",';
                    ...
                    cluster2Payload += ' "networkProfile": {';
                        ...
                        cluster2Payload += ' "inputEndpoints": [';
                            cluster2Payload += ' {';
                                cluster2Payload += ' "name": "SSH",';
                                cluster2Payload += ' "protocol": "TCP",';
                                cluster2Payload += ' "localPort": 22,';
                                cluster2Payload += ' "publicPort": 22';
                            cluster2Payload += ' }';
                        cluster2Payload += ' ]';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "certificates": [],';
                    cluster2Payload += ' "vmExtensions": [';
                        cluster2Payload += ' {';
                            cluster2Payload += ' "referenceName": "OSPatchingForLinux",';
                            cluster2Payload += ' "publisher": "Microsoft.OSTCExtensions",';
                            cluster2Payload += ' "name": "OSPatchingForLinux",';
                            cluster2Payload += ' "version": "1.0",';
                            cluster2Payload += ' "resourceExtensionParameterValues": [';
                                cluster2Payload += ' { ';
                                    cluster2Payload += ' "Key" : "OSPatchingForLinuxPrivateConfigParameter",';
                                    cluster2Payload += ' "Value" : "{\\"startTime\\" : \\"03:00\\", \\"rebootAfterPatch\\" : \\"Required\\", \\"category\\": \\"Important\\"}",';
                                    cluster2Payload += ' "Type" : "Private"';
                                cluster2Payload += ' }';
                            cluster2Payload += ' ]';
                        cluster2Payload += ' }';
                    cluster2Payload += ' ],';
            ...
            cluster2Payload += ' {';
                ...
                cluster2Payload += ' "location": "' + location + '",';
                cluster2Payload += ' "properties": {';
                    cluster2Payload += ' "instanceCount": ' + dataNodeCount + ',';
                    cluster2Payload += ' "osProfile": {';
                        cluster2Payload += ' "computerNamePattern": "workernode####",';
                        cluster2Payload += ' "linuxOperatingSystemProfile": {';
                            cluster2Payload += ' "DisableSshPasswordAuthentication": false,';
                            cluster2Payload += ' "username": "' + sshUserName + '",';
                            cluster2Payload += ' "userPassword": "' + sshPassword + '",';
            ...
            cluster2Payload += ' },';
            cluster2Payload += ' {';
                cluster2Payload += ' "name": "",';
                cluster2Payload += ' "type": "VirtualNetwork",';
                cluster2Payload += ' "location": "' + location + '",';
                cluster2Payload += ' "properties": {';
                    cluster2Payload += ' "addressSpace": {';
                        cluster2Payload += ' "addressPrefixes": [';
                            cluster2Payload += ' "10.0.0.0/20"';
                        cluster2Payload += ' ]';
                    cluster2Payload += ' },';
                    cluster2Payload += ' "subnets": [';
                        cluster2Payload += ' {';
                            cluster2Payload += ' "name": "",';
                            cluster2Payload += ' "properties": {';
                                cluster2Payload += ' "addressPrefix": "10.0.0.0/20"';
                            cluster2Payload += ' }';
                        cluster2Payload += ' }';
                    cluster2Payload += ' ]';
                cluster2Payload += ' }';
  ...
  this.listClustersCommand = function (options, _) {
    ...
    var windows = self.processor.listClusters(options.subscription, _);
    var linux = self.processor.listClusters2(options.subscription, _);
    self.user.endProgress();
    var result = [];
    result = windows.cloudService.concat(linux.cloudService);
...
hdInsightCommandLine.init = function (cli) {
  ...
  var hdInsight = cli.category('hdinsight')
    .description($('PREVIEW: Commands to manage your HDInsight clusters and jobs'));
  ...
  cluster.command('create [clusterName] [osType] [storageAccountName] [storageAccountKey] [storageContainer] [dataNodeCount] [headNodeSize] [dataNodeSize] [location] [userName] [password] [sshUserName] [sshPassword]')
    ...
    .option('--storageAccountName <storageAccountName>', $('Storage account to use for default HDInsight storage'))
  ...
  cluster.command('show [clusterName] [osType]')
    .description($('Shows cluster details'))
  ...
  cluster.command('list')
    .description($('Lists the clusters'))
