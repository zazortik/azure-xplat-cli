...
function getAsmWarning(spacing, showHelp){
  if (!spacing) spacing = ' ';
  var warning = util.format('help: %sHDInsight commands in ASM mode are deprecated and will be removed in January 2017. ' + 
  'Please use ARM mode instead by entering:  azure config mode arm\n' + 
  ...
  if (showHelp === false) {
    warning = util.format('%sHDInsight commands in ASM mode are deprecated and will be removed in January 2017. ' + 
    'Please use ARM mode instead by entering:  azure config mode arm\n' + 
...
hdInsightCommandLine.init = function (cli) {
  ...
  cluster.command('list')
    .description($('List the clusters' + getAsmWarning('\t\t')))
