...
exports.init = function(cli) {
  ...
  */
  ...
  metrics.command('set')
    ...
    .option('--hourly', $('set hourly metrics properties'))
    .option('--hourly-off', $('turn off hourly metrics properties'))
  ...
  */
  function generateServiceMetricsProperties(serviceProperties, options) {
    if (!options.hourly && !options.minute && !options.hourlyOff && !options.minuteOff) {
      throw new Error($('Please define one of them: --hourly, --minute, --hourly-off or --minute-off'));
    } else if (options.hourly && options.minute) {
      throw new Error($('Only one of --hourly and --minute should be defined'));
    }

    if (options.hourly && options.hourlyOff) {
      throw new Error($('--hourly and --hourly-off cannot be both defined'));
    } else if (options.hourly) {
      setMetrics(serviceProperties.HourMetrics, options);
    } else if (options.hourlyOff) {
