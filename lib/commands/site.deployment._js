...
exports.init = function (cli) {
  ...
  user.command('set [username] [pass]')
    ...
    .execute(function (username, pass, options, _) {
      ...
      username = cli.interaction.promptIfNotGiven($('Git username: '), params.values.username, _);
      pass = cli.interaction.promptPasswordIfNotGiven($('Git password: '), params.values.pass, _);

      var progress = cli.interaction.progress($('Setting user credentials'));
      try {
        var websiteManagementService = createWebsiteManagementService(cli.category('account').getCurrentSubscription(options.subscription).Id);
  ...
  scm.doDeploymentsGet = function (context, _) {
    ...
    try {
      var deployments = ensureShortCommitId(service.listDeployments({ orderby: 'ReceivedTime desc', top: maxItems }, _));
  ...
  scm.doDeploymentGet = function (context, _) {
    ...
    try {
      return formatDeployment(service.getDeployment(context.id, _));
    } finally {
      progress.end();
    }
  };
  ...
  scm.doDeploymentPut = function (context, _) {
    ...
    try {
      return service.updateDeployment(context.id, _);
    } finally {
      progress.end();
    }
  };
  ...
  scm.doLogGet = function (context, _) {
    ...
    try {
      var logs = service.listLogs(context.id, _);
      ...
      try {
        results = async.map(logs, function (currentLog, _) {
          if (currentLog.hasDetails) {
            var service = utils.createScmManagementService(context.repositoryUri, context.repositoryAuth, log);
            var details = service.getLog(context.id, currentLog.id, _);
