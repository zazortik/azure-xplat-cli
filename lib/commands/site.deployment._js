...
exports.init = function (cli) {
  ...
  var user = scm.category('user')
    .description('Commands to manage your deployment users');

  user.command('set [username] [password]')
    .usage('[options] [username] [password]')
    .description('Sets the deployment credentials')
    .option('-s, --subscription <id>', 'use the subscription id')
    .option('--gitUsername <gitUsername>', 'The new git username')
    .option('--gitPassword <gitPassword>', 'The new git password')
    .execute(function (username, password, options, _) {
      var params = utils.normalizeParameters({
        username: [username, options.gitUsername],
        password: [password, options.gitPassword]
      });

      if (params.err) { throw params.err; }

      username = interaction.promptIfNotGiven(cli, 'Git username: ', params.values.username, _);
      password = interaction.promptPasswordIfNotGiven(cli, 'Git password: ', params.values.password, _);
