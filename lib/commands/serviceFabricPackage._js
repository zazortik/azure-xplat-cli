...
exports.init = function (cli) {
  ...
  applicationPackage.command('copy [applicationPackagePath] [imageStoreConnectionString] [applicationPackagePathInImageStore]')
    ...
    .option('-i --application-package-path-in-image-store <applicationPackagePathInImageStore>', $('the path of the application package in image store'))
      ...
      try {
        ...
        async.eachSeries(files, function (file, callback) {
          ...
          var updateProgress = function () {
            progress.write(function () {}, util.format('[%d%%] %d/%d files, %d/%d bytes', Math.floor((currentTotalSize + currentSize) / totalSize * 100), currentFilesCount, files.length, currentTotalSize + currentSize, totalSize));
          };
          if (file.path.endsWith('_.dir')) {
            ...
            var progressInterval = setInterval(function () {
              ...
              currentSize = writeStream.req.connection._bytesDispatched;
              updateProgress();
            }, 100);
        ...
        cli.interaction.formatOutput({}, function (data) {
          log.json(data);
        });
      } catch (e) {
        progress.end();
  ...
  applicationPackage.command('test [applicationPackagePath] [imageStoreConnectionString] [applicationPackagePathInImageStore]')
    ...
    .option('-p --application-package-path <applicationPackagePath>', $('the path of the application package'))
    .option('-a --application-parameter <applicationParameter>', $('the parameter of the application, json string "{"Key": "key1", "Value": "value1"}"'))
    .option('-c --image-store-connection-string <imageStoreConnectionString>', $('the string of the image store connection'))
    .execute(function (applicationPackagePath, applicationParameter, imageStoreConnectionString, options, _) {
      ...
      try {
        ...
        if (config.clientCertPath) {
          fingerprint = serviceFabricUtils.getCertificateFingerprint(config.clientCertPath, _);
        }
        
        var command = util.format('testapplicationpackage "%s" "%s" "%s" "%s" "%s"', config.tcpConnectionEndpoint, applicationPackagePath, applicationParameter ? applicationParameter : '', imageStoreConnectionString ? imageStoreConnectionString : '', fingerprint ? fingerprint : '');
  ...
  applicationPackage.command('delete [imageStoreConnectionString] [applicationPackagePathInImageStore]')
    .description($('Delete application package'))
    .option('-c --image-store-connection-string <imageStoreConnectionString>', $('the string of the image store connection'))
    .option('-i --application-package-path-in-image-store <applicationPackagePathInImageStore>', $('the path of the application package in image store'))
    .execute(function (imageStoreConnectionString, applicationPackagePathInImageStore, options, _) {
      ...
      var progress = cli.interaction.progress($('Delete application package'));
      ...
      try {
        ...
        var command = util.format('removeapplicationpackage "%s" "%s" "%s" "%s"', config.tcpConnectionEndpoint, imageStoreConnectionString, applicationPackagePathInImageStore, fingerprint ? fingerprint : '');
        var res = serviceFabricUtils.runChildProcess(command, _);
        // Check for error
        if (res[0]) {
          throw res;
  ...
  servicePackageDeployedHealth.command('send [nodeName] [applicationName] [serviceManifestName] [sourceId] [property] [healthState] [description] [timeToLiveInMilliSeconds] [sequenceNumber] [removeWhenExpired]')
    ...
    .execute(function (nodeName, applicationName, serviceManifestName, sourceId, property, healthState, description, timeToLiveInMilliseconds, sequenceNumber, removeWhenExpired, options, _) {
      ...
      try {
        ...
        var res = client.deployedServicePackageHealthReports.send(nodeName, applicationName, serviceManifestName, applicationHealthReport, options, _);
        serviceFabricUtils.setPackageEnumVal(res);
