...
exports.generateDeploymentScript = function (repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, noDotDeployment, logger, confirmFunc, _) {
    ...
    if (projectType === "WAP") {
        generateWapDeploymentScript(repositoryRoot, relativeProjectPath, relativeSolutionPath, scriptType, relativeSitePath, noDotDeployment, _);
    }
    else if (projectType === "WEBSITE") {
        generateWebSiteDeploymentScript(repositoryRoot, relativeSolutionPath, scriptType, relativeSitePath, noDotDeployment, _);
    }
    else if (projectType === "NODE") {
        generateNodeDeploymentScript(repositoryRoot, scriptType, relativeSitePath, noDotDeployment, _);
    }
...
function generateNodeDeploymentScript(repositoryRoot, scriptType, sitePath, noDotDeployment, _) {
    argNotNull(repositoryRoot, "repositoryRoot");

    createIisNodeWebConfigIfNeeded(repositoryRoot);
    utils.copyIisNodeWhenServerJsPresent(log, repositoryRoot, _);

    generateBasicDeploymentScript("node.template", scriptType, repositoryRoot, sitePath, noDotDeployment, _);
}
...
function generateWapDeploymentScript(repositoryRoot, projectPath, solutionPath, scriptType, sitePath, noDotDeployment, _) {
    argNotNull(repositoryRoot, "repositoryRoot");
    ...
    generateDotNetDeploymentScript("deploy.batch.aspnet.wap.template", msbuildArguments, repositoryRoot, sitePath, noDotDeployment, _);
}

function generateWebSiteDeploymentScript(repositoryRoot, solutionPath, scriptType, sitePath, noDotDeployment, _) {
    if (solutionPath) {
        ...
        generateDotNetDeploymentScript("deploy.batch.aspnet.website.template", msbuildArguments, repositoryRoot, sitePath, noDotDeployment, _);
    }
    else {
        // Basic website
        generateBasicDeploymentScript("basic.template", scriptType, repositoryRoot, sitePath, noDotDeployment, _);
    }
}

function generateBasicDeploymentScript(templateFileName, scriptType, repositoryRoot, sitePath, noDotDeployment, _) {
    ...
    writeDeploymentFiles(templateContent, scriptType, repositoryRoot, noDotDeployment, _);
}

function generateDotNetDeploymentScript(templateFileName, msbuildArguments, repositoryRoot, sitePath, noDotDeployment, _) {
    argNotNull(templateFileName, "templateFileName");
    ...
    writeDeploymentFiles(templateContent, "BATCH", repositoryRoot, noDotDeployment, _);
}
...
function writeDeploymentFiles(templateContent, scriptType, repositoryRoot, noDotDeployment, _) {
