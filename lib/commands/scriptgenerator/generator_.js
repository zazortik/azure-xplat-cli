...
ScriptGenerator.prototype.generateDeploymentScript = function (_) {
    if (this.projectType === "WAP") {
        this.generateWapDeploymentScript(_);
    }
    else if (this.projectType === "WEBSITE") {
        this.generateWebSiteDeploymentScript(_);
    }
    else if (this.projectType === "NODE") {
        this.generateNodeDeploymentScript(_);
    }
    else if (this.projectType === "BASIC") {
...
ScriptGenerator.prototype.generateNodeDeploymentScript = function(_) {
    createIisNodeWebConfigIfNeeded(this.repositoryRoot);
    utils.copyIisNodeWhenServerJsPresent(log, this.repositoryRoot, _);
...
ScriptGenerator.prototype.generateWapDeploymentScript = function(_) {
    ...
    if (this.solutionPath != null) {
        msbuildArguments += " /p:SolutionDir=\"%DEPLOYMENT_SOURCE%\\" + this.solutionPath + "\"";
    }
