...
exports.generateDeploymentScript = function (repositoryRoot, projectType, projectPath, solutionPath, scriptType, logger, confirmFunc, _) {
    ...
    if (projectType === "WAP") {
        generateWapDeploymentScript(repositoryRoot, projectPath, solutionPath, scriptType, _);
    }
    else if (projectType === "WEBSITE") {
        generateWebSiteDeploymentScript(repositoryRoot, solutionPath, scriptType, _);
    }
    else if (projectType === "NODE") {
        generateNodeDeploymentScript(repositoryRoot, scriptType, _);
    }
...
function generateNodeDeploymentScript(repositoryRoot, scriptType, _) {
    argNotNull(repositoryRoot, "repositoryRoot");

    createIisNodeWebConfigIfNeeded(repositoryRoot);

    generateBasicDeploymentScript("node.template", scriptType, _);
}
...
function generateWapDeploymentScript(repositoryRoot, projectPath, solutionPath, scriptType, _) {
    argNotNull(repositoryRoot, "repositoryRoot");
    ...
    generateDotNetDeploymentScript("deploy.batch.aspnet.wap.template", msbuildArguments, _);
}

function generateWebSiteDeploymentScript(repositoryRoot, solutionPath, scriptType, _) {
    if (solutionPath) {
        ...
        generateDotNetDeploymentScript("deploy.batch.aspnet.website.template", msbuildArguments, _);
    }
    else {
        // Basic website
        generateBasicDeploymentScript("basic.template", scriptType, _);
    }
}

function generateBasicDeploymentScript(templateFileName, scriptType, _) {
    ...
    writeDeploymentFiles(templateContent, scriptType, _);
}

function generateDotNetDeploymentScript(templateFileName, msbuildArguments, _) {
    argNotNull(templateFileName, "templateFileName");
    ...
    writeDeploymentFiles(templateContent, "BATCH", _);
}
...
function writeDeploymentFiles(templateContent, scriptType, _) {
    ...
    writeContentToFile(deployScriptFileName, templateContent, _);
