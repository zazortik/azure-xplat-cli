    ...
    .execute(function (resourceTypes, resourceGroup,  options, _) {
      ...
      cli.interaction.withProgress($('Getting Resource Providers'),
        function (log, _) {
          ...
          while (nextLink) {
            response = client.providers.listNext(nextLink, _);
            allProvgiders.concat(response.providers);
            nextLink = response.nextLink;
          }
        }, _);

      var allRerouces = new Array();
      var locationList;
      var currentLocation;
      ...
      cli.interaction.formatOutput(allRerouces, function (data) {
        if (data.length === 0) {
