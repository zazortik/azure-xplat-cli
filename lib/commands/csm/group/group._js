...
exports.init = function (cli) {
  ...
  grouplog.command('show [name]')
    ...
    .execute(function (name, options) {
      ...
      withProgress($('Getting group logs'),
        function (log, _) {
          if (options.all) {
            logs = groupUtils.getAllEvents(subscription, name, _);
          } else if (options.deployment) {
            logs = groupUtils.getDeploymentLog(subscription, name, options.deployment, _);
          } else {
            logs = groupUtils.getLastDeploymentLog(subscription, name, _);
          }
        }, _);
