...
// TODO: Add various "get log" functions here

function getDeploymentLogs(subscription, trackingId) {
  var output = through();
...
exports.getDeploymentLog = function (subscription, name, deploymentName) {
  ...
  client.deployments.get(name, deploymentName, function (err, result) {
    ...
    getDeploymentLogs(subscription, result.deployment.properties.trackingId).pipe(output);
  });
  return output;
};
...
exports.getLastDeploymentLog = function (subscription, name) {
  ...
  client.deployments.list(name, { top: 1}, function (err, response) {
    ...
    getDeploymentLogs(subscription, response.deployments[0].properties.trackingId).pipe(output);
  });
  return output;
}
