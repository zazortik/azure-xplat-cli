...
exports.validateTemplate = function (cli, resourceGroup, options, _) {
  ...
  var client = subscription.createResourceClient('createResourceManagementClient');
  var result;
  cli.interaction.withProgress($('Validating templates'),
    function (log, _) {
      var templateParameters = buildDeploymentObject(cli, subscription, resourceGroup, '', options, _);
      result = client.deployments.validate(resourceGroup, 'TODO', templateParameters, _);
    }, _);
  return result;
};
...
function buildDeploymentObject(cli, subscription, resourceGroup, name, options, _) {
  if (options.galleryTemplate && options.fileTemplate) {
    throw new Error($('Either --gallery-template or --file-template need to be specified. Not both.'));
  } else if (!options.galleryTemplate && !options.fileTemplate) {
    throw new Error($('Either --gallery-template or --file-template need to be specified.'));
  ...
  var deploymentObject;
  var templateParameters;
  cli.interaction.withProgress($('Initializaing templates and parameters'),
    function (log, _) {
