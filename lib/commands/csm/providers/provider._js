...
exports.init = function (cli) {
  ...
  provider.command('list')
    .description($('List currently registered providers in csm and RDFE'))
    .option('-s --subscription <subscription>', $('Subscription to list providers for'))
    .execute(withClient(function (client, options, _) {
      var providers = withProgress($('Getting CSM registered providers'),
        function (log, _) {
  ...
  provider.command('show [namespace]')
    ...
    .option('-n --namespace <namespace>', $('provider namespace to show'))
  ...
  provider.command('register [namespace]')
    ...
    .option('-n --namespace <namespace>', $('provider namespace to register'))
    ...
    .execute(withClient(function (client, namespace, options, _) {
      ...
      var response = withProgress(util.format($('Registering provider %s'), namespace),
        function (log, _) {
          client.providers.register(namespace, _);
        }, _);
