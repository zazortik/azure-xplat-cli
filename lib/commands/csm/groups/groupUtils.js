...
exports.normalizeDownloadFileName = function normalizeDownloadFileName(name, file, quiet, confirmer, callback) {
  name = name + '.json';
  var downloadFile = file || path.join('.', name);
  downloadFile = path.resolve(process.cwd(), downloadFile);

  function ensureDirExists(dirname) {
    console.log('ensuring directory', dirname);
    if (!dirname) {
      return;
    }

    if (utils.pathExistsSync(dirname)) {
      console.log('directory',dirname, 'exists, returning');
