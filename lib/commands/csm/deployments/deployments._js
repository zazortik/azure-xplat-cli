...
exports.init = function (cli) {
    ...
    .execute(function (resourceGroup, deploymentName, options, _) {
      ...
      try {
        ...
        if (!deploymentName){
          ...
          deploymentToStop = client.deployments.get(resourceGroup, deploymentName, _).name;
        }

        if (!options.quiet &&
            !cli.interaction.confirm(util.format($('Stop deployment %s? [y/n]: '), deploymentToStop.deploymentName), _)) {
            return;
          }
  ...
  deployment.command('create [templateLink] [resourceGroup] [mode] [deploymentName] [parametersFile]')
    ...
    .execute(function (templateLink, resourceGroup, mode, deploymentName, parametersFile, options, _) {
      ...
      try {
        client.deployments.createOrUpdate(resourceGroup, deploymentName,
          {
