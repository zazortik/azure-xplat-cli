...
exports.init = function (cli) {
  ...
  deployment.command('list [resourceGroup] [state]')
    ...
    .option('--state <state>', $('Optional. Filter the deployments by provisioning state.'
      + 'Valid values are Accepted, Running, Failed and Succeeded'))
    .option('--subscription <subscription>', $('Subscription containing deployments to list'))
    .execute(function (resourceGroup, state, options, _) {
      ...
      try {
        var allDeployments = retrieveDeployments(client, resourceGroup, state, _);
  ...
  deployment.command('show [resourceGroup] [deploymentName] ')
    .description($('get deployments'))
    .option('-g --resource-group <resourceGroup>', $('Name of the resource group.'))
    .option('-n --deploymentName <deploymentName>', $('Name of the deployment'))
	  .option('--subscription <subscription>', $('Subscription containing the deployment to display'))
        ...
        if (name) {
          ...
          if (allDeployments && allDeployments.length > 0) {
            ...
            allDeployments.sort( function (a, b) {
                return Date.parse(a.properties.timestamp) < parseFloat(b.properties.timestamp)
            });
  ...
  deployment.command('stop [resourceGroup]')
    ...
    .option('--subscription <subscription>', $('Subcription containing the deployment to stop'))
    .execute(function (resourceGroup, options, _) {
      ...
      try {
        ...
        if (allRunningDeployments && allRunningDeployments.length > 0) {
          //Each resource group can only have 1 running deployment, we just take the first one
          var runningDeployment = allRunningDeployments[0];
          if (!options.quiet &&
              !cli.interaction.confirm(util.format($('Stop deployment %s? [y/n]: '),
                  runningDeployment.deploymentName), _)) {
            return;
          }
          client.deployments.cancel(resourceGroup, runningDeployment.deploymentName, _);
        }
        else {
          log.data($('There is no running deployments to stop'));
        }
      }
      finally {
        progress.end();
...
function retrieveDeployments(client, resourceGroup, state, _) {
  //Wrap the value with single quot(needed for constructing RESTful query string)
  var stateFilter = state;
  if (stateFilter && !(utils.stringStartsWith(stateFilter, '\''))) {
    stateFilter = '\'' + stateFilter + '\'';
  }

  var response = client.deployments.listForResourceGroup(resourceGroup, { provisioningState: stateFilter }, _);
