...
exports.init = function (cli) {
  ...
  deployment.command('create <name> <resourceGroup> <mode> <templateLink> <parameterFile>')
    ...
    .option('-p --parameters-file <parameterFile>', $('file with parameters'))
    .option('--subscription <subscription>', $('Subcription containing group to delete'))
    .execute(function (name, resourceGroup, mode, templateLink, parameterFile, options, _) {
      ...
      var jsonFile = fs.readFileSync(parameterFile, 'utf8');
      var deploymentParameters = JSON.parse(stripBOM(jsonFile));

      try{
        client.deployments.create(resourceGroup, name, 
          {
