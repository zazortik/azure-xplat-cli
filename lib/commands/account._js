...
exports.init = function (cli) {
  ...
  account.command('logout [username]')
    .description($('Log out from Azure subscription using Active Directory'))
    .option('-u --username <username>', $('Required. User name used to log out from Aazure Active Directory.'))
    .execute(function (username, options, _) {
    if (!username){
      return cli.missingArgument('username');
    }
    if (profile.current.clearActiveDirectoryToken(username)) {
      profile.current.save();
      log.info(util.format($('You have been logged out.')));
    } else {
      log.info(util.format($('You are not appearing log in.')));
    }
  });
