...
exports.init = function (cli) {
  ...
  account.command('import <file>')
    ...
    .execute(function (file, options, callback) {
      ...
      if (!options.skipregister && publishSettings && publishSettings.PublishProfile.Subscription) {
        var progress = cli.interaction.progress($('Verifying account'));
      ...
      function processSettings(file, settings) {
        var accountClient = new AccountClient(cli, cli.category('account').lookupSubscriptionId(options.subscription));
        var publishSettings = accountClient.processSettings(settings, pemPath);
  ...
  function listLAG(what, options, callback) {
    ...
    var progress = cli.interaction.progress(util.format($('Enumerating %s'), textName));
