...
exports.init = function (cli) {
  ...
  account.command('show [subscriptionNameOrId]')
    ...
    .option('-s --subscriptionNameOrId <subscriptionNameOrId>', $('The subscription name to show'))
    .option('-d --details', $('Show extra information about the subscription'))
    .execute(function (subscriptionNameOrId, options, _) {
      var subscriptions = [];
      if (utils.isGuid(subscriptionNameOrId)) {
        subscriptions.push(profile.current.getSubscription(subscriptionNameOrId));
      }
      else {
        subscriptions = profile.current.getSubscriptionByName(subscriptionNameOrId);
      }
      var format = [
        [$('Name'), 'name'],
        [$('ID'), 'id'],
    ...
    .execute(function (subscriptionNameOrId, options, _) {
      ...
      if (subscriptions.length > 1) {
        log.warn(util.format($('We found multiple subscriptions matching the name "%s". ' + 
          ...
          subscriptionNameOrId, newSubscription.id))
      }
