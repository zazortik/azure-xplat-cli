...
exports.init = function (cli) {
  ...
  account.command('list')
    ...
    .execute(function () {
      var subscriptions = __.values(profile.subscriptions);
  ...
  account.command('import <file>')
    ...
    .execute(function (file, options, _) {
      profile.importPublishSettings(file);
      profile.save();
  ...
  account.command('clear')
    ...
    .execute(function (options, _) {
      __.keys(profile.subscriptions).forEach(function (subName) {
        profile.deleteSubscription(subName);
      });

      __.keys(profile.environments).forEach(function (envName) {
        profile.deleteEnvironment(envName);
      });
  ...
  account.getCurrentSubscription = function (subscription) {
    ...
    return profile.subscriptions[subscription];
  };
