  ...
  cli.command('login')
    ...
    .execute(function(options, _) {
      ...
      try {
        ...
        progress.end();
        log.info($('Please note that currently the user can login only via Microsoft organizational account.'));
        if(e.message.indexOf('ErrorCode: NONE : FaultMessage: NONE') > -1) {
          // forward link for xplat-cli msdn article 
          cli.interaction.launchBrowser('http://aka.ms/Dhf67j', _);
        }
        throw (e);
      }finally {
        progress.end();
