...
exports.init = function (cli) {
  ...
  cli.command('login [username]')
    ...
    .option('-u --username <username>', $('user name or service principal ID, will prompt if not given'))
    ...
    .execute(function(username, options, _) {
      ...
      username = cli.interaction.promptIfNotGiven('Username: ', username, _);
      ...
      try {
        adalAuth.removeCachedToken(username, tenant, _);
        var loginType = {
        ...
        try {
          result = account.load(username, password, tenant, loginType, _);
        } catch (ex) {
          //we only retry on US Cloud env
          if (ex[account.MFAEnabledErrFieldName] && environment.isDefaultEnv()) {
            loginType.mfa = true;
            result = account.load(username, password, tenant, loginType, _);
          } else {
            throw ex;
        ...
        if (newSubscriptions.length > 0) {
          ...
          if (loginType.mfa && result.tenantIds && result.tenantIds.length > 1) {
            log.info('You have been signed into the default tenant.');
            var extraTenantMessage = 'You can use command argument \'--tenant\' to login one of other tenants:';
            result.tenantIds.forEach(function (t) {
              if (t !== newSubscriptions[0].tenantId) {
                log.info('  ' + t);
              }
            });
          }
