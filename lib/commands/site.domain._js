...
exports.init = function (cli) {
  ...
  siteDomain.command('add [domain] [name]')
    .usage('[options] <dns> [name]')
    .description($('Add a site domain'))
    .option('-d, --dns <dns>', $('the new domain'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function (dns, name, options, _) {
      if (options.dns) {
        name = dns;
        dns = undefined;
      }
      ...
      dns = interaction.promptIfNotGiven(cli, $('Domain: '), params.values.dns, _);
      ...
      var siteConfigurations = site.doSiteGet(context, _);
      siteConfigurations.HostNames = getHostNames(siteConfigurations.HostNames);
      siteConfigurations.HostNames['a:string'].push(dns);
      site.doSitePUT(context,  {
        HostNames: siteConfigurations.HostNames
      }, _);
    });

  siteDomain.command('delete [dns] [name]')
    .usage('[options] <dns> [name]')
    ...
    .execute(function (dns, name, options, _) {
      ...
      dns = interaction.promptIfNotGiven(cli, $('Domain DNS: '), params.values.dns, _);

      if (!options.quiet && !interaction.confirm(cli, util.format($('Delete domain %s? [y/n] '), dns), _)) {
        return;
      }
      ...
      if (siteConfigurations.HostNames && siteConfigurations.HostNames['a:string']) {
        ...
        for (var i = 0; i < siteConfigurations.HostNames['a:string'].length; i++) {
          if (utils.ignoreCaseEquals(siteConfigurations.HostNames['a:string'][i], dns)) {
      ...
      if (!found) {
        throw new Error(util.format($('Domain "%s" does not exist'), dns));
      }
    });
