...
exports.init = function (cli) {
  ...
  siteJobs.command('upload [jobName] [jobType] [jobFile] [name]')
    ...
    .execute(function (jobName, jobType, jobFile, name, options, _) {
      ...
      if (options.singleton &&
        !utils.ignoreCaseEquals(params.values.jobType, 'continuous')) {
      ...
      if (options.singleton) {
        var progress = cli.interaction.progress($('Setting web job singleton'));
        try {
          // Give it some time before calling to make sure the repo is created
          setSingleton(service, params.values.jobName, _);
        } finally {
          progress.end();
        }
      }
  ...
  function setSingleton(service, jobName, callback) {
    var numberOfAttempts = 0;

    function trySetSingleton() {
      setTimeout(function () {
        service.webJobs.setSingleton(jobName, true, function (err) {
          if (err && numberOfAttempts < 3) {
            numberOfAttempts++;
            trySetSingleton();
          } else {
            callback(err);
