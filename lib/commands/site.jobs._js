...
exports.init = function (cli) {
  ...
  function checkIfValid (service, jobName, jobType, callback) {
    setTimeout(function () {
      var progress = cli.interaction.progress($('Getting web job'));
      if (utils.ignoreCaseEquals(jobType, 'continuous')) {
        service.webJobs.getContinuous(jobName, function (err) {
          progress.end();
        ...
        service.webJobs.getTriggered(jobName, function (err) {
          progress.end();
  ...
  function createWebsiteExtensionsClient(context, _) {
    ...
    var repositoryUri = websiteClient.getRepositoryUri(siteData);

    return utils._createWebSiteExtensionsClient(WebsitesClient.getSiteHostName(context.site.name, context.site.slot), authData.username, authData.password, repositoryUri, log);
  }
};