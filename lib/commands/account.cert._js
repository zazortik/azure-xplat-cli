...
function outputFile(options) {
  return options.file || util.format('%s.pem', options.subscription);
}
...
function exportPublishSettingsCertificate(options) {
  ...
  console.log('Outputting cert for subscription', subscription.id);
  writePemFile(outputFile(options), subscription.managementCertificate);
}

function exportLoadedSubscription(options) {
  var subscription = profile.current.getSubscription(options.subscription);
  console.log('looking for subscription', options.subscription, 'found subscription', subscription.id);
  subscription.exportManagementCertificate(outputFile(options));
}
