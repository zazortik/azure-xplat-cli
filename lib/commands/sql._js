...
exports.init = function (cli) {
  ...
  db.command('create [serverName] [databaseName] [administratorPassword] [collationName] [edition] [maxSizeInGB]')
    .description('Create a new database')
    .usage('<serverName> <databaseName> <administratorPassword> <collationName> <edition> <maxSizeInGB> [options]')
    .option('--serverName <serverName>', 'The SQL server name')
    ...
    .option('--location <location>', 'The location')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (serverName, databaseName, administratorPassword, collationName, edition, maxSizeInGB, options, _) {
      ...
      // Find out administrator login
      var progress = cli.progress('Getting SQL Server');
      var servers = sqlManagementService.listServers(_);
      progress.end();
      ...
      administratorPassword = interaction.promptPasswordOnceIfNotGiven(cli, "Administrator login password: ", params.values.administratorPassword, _);
      collationName = params.values.collationName;
      edition = params.values.edition;
      maxSizeInGB = params.values.maxSizeInGB;

      var sqlService = createSqlService(serverName, server.AdministratorLogin, administratorPassword);
  ...
  db.command('list [serverName] [administratorPassword]')
    .description('Get the list of databases')
    .usage('<serverName> <administratorPassword> [options]')
    .option('--serverName <serverName>', 'The SQL server name')
    .option('--administratorPassword <administratorPassword>', 'The administrator password')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (serverName, administratorPassword, options, _) {
      var sqlManagementService = createSqlManagementService(options.subscription);
      ...
      // if the server does exist, then we can get the password if it wasn't passed
      administratorPassword = interaction.promptPasswordOnceIfNotGiven(cli, "Administrator login password: ", params.values.administratorPassword, _);
      ...
      interaction.formatOutput(cli, databases, function(outputData) {
        if(outputData.length === 0) {
          log.info('No SQL Server Databases created');
        } else {
          log.table(outputData, function (row, item) {
            row.cell('Name', item.Name);
  ...
  db.command('show [serverName] [databaseName] [administratorPassword]')
    ...
    .option('--administratorPassword <administratorPassword>', 'The administrator password')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (serverName, databaseName, administratorPassword, options, _) {
      var sqlManagementService = createSqlManagementService(options.subscription);
      ...
      // if the server does exist, then we can get the database name and password if it wasn't passed
      databaseName = interaction.promptIfNotGiven(cli, "Database name: ", params.values.databaseName, _);
      administratorPassword = interaction.promptPasswordOnceIfNotGiven(cli, "Administrator login password: ", params.values.administratorPassword, _);
  ...
  db.command('delete [serverName] [databaseName] [administratorPassword]')
