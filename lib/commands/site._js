...
exports.init = function (cli) {
        ...
        .execute(function (name, options, _) {
          ...
          if (name) {
            sites = sites.filter(function (s) {
              return utils.stringStartsWith(s.name, util.format('%s(', name));
            });
          }
  ...
  site.command('swap [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (!options.quiet && !cli.interaction.confirm(util.format($('Swap slot %s from site %s into production ? [y/n] '), options.slot, context.site.name), _)) {
            return;
          }
