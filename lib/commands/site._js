...
exports.init = function (cli) {
          ...
          cli.interaction.formatOutput(sites, function (data) {
            if (data.length > 0) {
              log.table(data, function (row, item) {
                ...
                row.cell($('Location'), getSiteLocation(locations, item));
  ...
  site.command('show [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (log.format().json) {
            ...
            log.data('Username:       ', settings.filter(function (s) { return s.Key === 'SCM_GIT_USERNAME'; })[0].Value);
  ...
  site.doSitesGet = function (context, _) {
    ...
    if (!context.spaces) {
      ...
      try {
        context.spaces = service.webSpaces.list(_).webSpaces;
      } finally {
        progress.end();
      }
    }
