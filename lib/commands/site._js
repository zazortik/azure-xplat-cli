...
exports.init = function (cli) {
  ...
  site.command('set [name]')
        ...
        .option('-v, --remote-debugging-version', $('the version of remote debugging. It\'s either VS2012 or VS2013. This parameter is only valid when remote debugging is on.'))
        .option('-m, --managed-pipeline-mode', $('the mode for managed pipeline. valid values are classic and integrated.'))
        .option('-s, --subscription <id>', $('the subscription id'))
      ...
      if (options.webSocket || options.disableWebSocket) {
        siteConfigurations.webSocketsEnabled = options.webSocket === true;
      }

      if (options.remoteDebuggingVersion && !options.remoteDebugging) {
        throw new Error($('Remote debugging version can only be specified when using remote debugging'));
      }

      if (options.remoteDebugging || options.disableRemoteDebugging) {
        siteConfigurations.remoteDebuggingEnabled = options.remoteDebugging === true;
      }

      if (options.remoteDebuggingVersion) {
        siteConfigurations.remoteDebuggingVersion = options.remoteDebuggingVersion;
      }

      if (options.managedPipelineMode) {
        siteConfigurations.managedPipelineMode = options.managedPipelineMode;
      }
  ...
  function logEachData(title, data) {
    for (var property in data) {
      log.data(title + ' ' + property, data[property]);
    }
  }
