...
exports.init = function (cli) {
          ...
          if (name) {
            sites = sites.filter(function (s) {
              return utils.ignoreCaseEquals(s.name, name) ||
                     utils.stringStartsWith(s.name, util.format('%s(', name), true);
            });
          }
  ...
  site.command('delete [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (!options.quiet && !cli.interaction.confirm(util.format($('Delete site %s? [y/n] '), context.site.name), _)) {
            return;
          }
  ...
  site.command('swap [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (!options.quiet && !cli.interaction.confirm(util.format($('Swap slot %s from site %s into %s ? [y/n] '), context.site.slot, context.site.name, options.slot), _)) {
            return;
          }
