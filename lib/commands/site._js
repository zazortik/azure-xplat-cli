...
exports.init = function (cli) {
  ...
  site.command('create [name]')
        ...
        .option('--slot <slot>', $('boolean value indicating if the staging slot should be created'))
        .option('--disable-clone', $('boolean value indicating if the site configuration should not be cloned from the production slot'))
        ...
        .execute(function (nameArg, options, _) {
          ...
          if (!WebsitesClient.isProductionSlot(options.slot)) {
            ...
            context.site.disableClone = options.disableClone;
          }
          ...
          function createSiteSlot(_) {
            ...
            if (context.site.slot && !context.flags.siteSlotExists) {
              ...
              if (context.site.disableClone) {
                site.siteConfig = { };
              }

              console.log(context.site.webspace);
              websiteClient.createSite(context.subscription, context.site.webspace, site, _);
            }
          }
  ...
  site.command('delete [name]')
        ...
        .option('--slot <slot>', $('The slot to be deleted'))
        .option('-s, --subscription <id>', $('the subscription id'))
        .execute(function (name, options, _) {
          var context = {
            subscription: cli.category('account').getCurrentSubscription(options.subscription).Id,
  ...
  site.command('swap [name]')
        .description($('Delete a web site'))
        .option('--slot <slot>', $('The slot to be swaped with production'))
        .option('-s, --subscription <id>', $('the subscription id'))
        .execute(function (name, options, _) {
          var context = {
            subscription: cli.category('account').getCurrentSubscription(options.subscription).Id,
          ...
          if (options.slot) {
            if (WebsitesClient.isProductionSlot(options.slot)) {
              throw new Error('Production slot can\'t be swaped');
            }
          } else {
