...
exports.init = function (cli) {
  ...
  siteCertificates.command('add <certificate-path> <key> [name]')
    .usage('<certificate-path> <key> [name] [options]')
    .description('Adds a site certificate')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (certificatePath, key, name, options, _) {
      var context = {
        subscription: cli.category('account').lookupSubscriptionId(options.subscription),
        site: { name: name }
      };
