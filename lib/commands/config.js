...
exports.init = function (cli) {
  ...
  config.command('list')
    ...
    .action(function () {
      log.info($('Enumerating config settings'));
  ...
  config.readConfig = function () {
    var cfg = {};
    log.silly(util.format($('Reading config %s'), azureConfigPath));
  ...
  config.writeConfig = function (cfg) {
    if (!utils.pathExistsSync(azurePath)) {
      log.silly(util.format($('Creating folder %s'), azurePath));
      fs.mkdirSync(azurePath, 502); //0766
    }

    log.silly(util.format($('Writing config %s'), azureConfigPath));
