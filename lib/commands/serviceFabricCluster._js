...
exports.init = function (cli) {
  ...
  cluster.command('connect [connectionEndpoint] [clientKeyPath] [clientCertPath] [caCertPath] [strictSsl] [rejectUnauthorized]')
    ...
    .option('-t --strict-ssl <strictSsl>', $('the boolean of the strict ssl, false to disable CN verification'))
    .option('-u --reject-unauthorized <rejectUnauthorized>', $('the boolean of the reject unauthorized, false to disable CA verification'))
  ...
  cluster.command('show')
    ...
    .option('-s --select <fields>', $('select fields to show, call without this parameter to see all fields'))
  ...
  clusterHealth.command('send [sourceId] [property] [healthState] [description] [timeToLiveInMilliSeconds] [sequenceNumber] [removeWhenExpired]')
    ...
    .option('-e --remove-when-expired <removeWhenExpired>', $('the boolean of the remove when expired'))
