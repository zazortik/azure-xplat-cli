...
exports.init = function (cli) {
  ...
  var site = cli.category('site')
    .mode(Constants.API_VERSIONS.RDFE)
  ...
  deploymentScriptCommand.execute(function (name, options, _) {
    kuduscript.deploymentScriptExecute(name, options, log, function () { cli.interaction.confirm.apply(cli.interaction, arguments); } , _);
  });
  ...
  site.command('swap [name]')
        ...
        .execute(function (name, options, _) {
          ...
          if (!options.targetSlot) {
            if (context.site.slot && WebsitesClient.isStagingSlot(context.site.slot)) {
              options.targetSlot = WebsitesClient.getProductionSlotName();
            } else {
              options.targetSlot = WebsitesClient.getStagingSlotName();
            }
          }

          if (!context.site.slot) {
            context.site.slot = WebsitesClient.getProductionSlotName();
          }

          if (context.site.slot &&
              utils.ignoreCaseEquals(options.slot, context.site.slot)) {
            throw new Error($('Target and source slots must be different.'));
          }

          site.lookupSiteNameAndWebSpace(context, _);

          if (!options.quiet && !cli.interaction.confirm(util.format($('Swap slot %s from site %s into %s ? [y/n] '), context.site.slot, context.site.name, options.targetSlot), _)) {
            return;
          }

          var progress = cli.interaction.progress(util.format($('Swapping slot %s from site %s into production'), context.site.slot, context.site.name));
          try {
            var service = createWebsiteManagementService(context.subscription);

            var siteName = WebsitesClient.getSiteName(context.site.name, context.site.slot);
            service.webSites.swapSlots(context.site.webspace, siteName, options.targetSlot, _);
          } finally {
            progress.end();
          }
