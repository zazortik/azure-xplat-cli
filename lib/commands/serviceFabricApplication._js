...
exports.init = function (cli) {
  ...
  application.command('delete [applicationName]')
    ...
    .execute(function (applicationName, options, _) {
      ...
      try {
        ...
        var client = new serviceFabricClient('3.0-preview', serviceFabricUtils.createConnectionUrl(config, _), serviceFabricUtils.getClientOptions(config, _));
        applicationName = serviceFabricUtils.parseUrl(applicationName, _);
        client.applications.remove(applicationName, options, _);
  ...
  applicationHealth.command('show [applicationName] [eventsHealthStateFilter] [deployedServicePackagesHealthStateFilter]')
    ...
    .option('-n --application-name <applicationName>', $('the name of the application'))
    .option('-h --events-health-state-filter <eventsHealthStateFilter>', $('the filter of the event health state, values are [Default|None|Ok|Warning|Error|All]'))
  ...
  applicationDeployedHealth.command('show [nodeName] [applicationName] [eventsHealthStateFilter] [deployedServicePackagesHealthStateFilter]')
    ...
    .option('-n --node-name <nodeName>', $('the name of the node'))
    .option('-a --application-name <applicationName>', $('the name of the application'))
    .option('-f --events-health-state-filter <eventsHealthStateFilter>', $('the filter of the event health state'))
    .option('-h --deployed-service-packages-health-state-filter <deployedServicePackagesHealthStateFilter>', $('the filter of the deployed service packages health state'))
    .option('--select <fields>', $('select fields to show, call without this parameter to see all fields'))
  ...
  applicationDeployedHealth.command('send [nodeName] [applicationName] [sourceId] [property] [healthState] [description] [timeToLiveInMilliSeconds] [sequenceNumber] [removeWhenExpired]')
    ...
    .option('-e --sequence-number <sequenceNumber>', $('the number of the sequence'))
    .option('-r --remove-when-expired <removeWhenExpired>', $('the boolean of the remove when expired'))
  ...
  applicationUpgrade.command('start [applicationName] [targetApplicationTypeVersion] [parameters] [rollingUpgradeMode] [upgradeReplicaSetCheckTimeoutInSeconds] [forceRestart] [monitoringPolicy] [applicationHealthPolicy]')
    ...
    .option('-p --parameters <parameters>', $('the parameters'))
    ...
    .option('-s --upgrade-replica-set-check-timeout-in-seconds <upgradeReplicaSetCheckTimeoutInSeconds>', $('the name of the upgrade domain'))
    .option('-r --force-restart <forceRestart>', $('the force restart'))
    .option('-o --monitoring-policy <monitoringPolicy>', $('the policy of the monitoring'))
    .option('-h --application-health-policy <applicationHealthPolicy>', $('the policy of the health application'))
  ...
  applicationUpgrade.command('update [applicationName] [rollingUpgradeMode] [forceRestart] [failureAction] [upgradeReplicaSetCheckTimeoutInSeconds] [healthCheckWaitDurationInMilliseconds] [healthCheckStableDurationInMilliseconds] [healthCheckRetryTimeoutInMilliseconds] [upgradeTimeoutInMilliseconds] [upgradeDomainTimeoutInMilliseconds] [applicationHealthPolicy]')
    ...
    .option('-n --application-name <applicationName>', $('the name of the application'))
    .option('-m --rolling-upgrade-mode <rollingUpgradeMode>', $('the mode of the rolling upgrade, values are [UnmonitoredAuto|UnmonitoredManual|Monitored]'))
    .option('-r --force-restart <forceRestart>', $('the boolean of the force restart'))
    ...
    .option('-s --health-check-stable-duration-in-milliseconds <healthCheckStableDurationInMilliseconds>', $('the mode of the rolling upgrade'))
    .option('-t --health-check-retry-timeout-in-milliseconds <healthCheckRetryTimeoutInMilliseconds>', $('the mode of the rolling upgrade'))
