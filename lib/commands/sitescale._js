...
exports.init = function (cli) {
  ...
  siteScale.command('mode [name] <siteMode>')
    ...
    .usage('[name] <mode>')
    .option('--mode <siteMode>', 'The mode of the site')
    ...
    .execute(function (name, siteMode, options, _) {
      ...
      if (utils.ignoreCaseEquals(siteMode, 'Free')) {
        siteConfigurations.ComputeMode = 'Shared';
        siteConfigurations.SiteMode = 'Limited';
      } else if (utils.ignoreCaseEquals(siteMode, 'Shared')) {
        siteConfigurations.ComputeMode = 'Shared';
        siteConfigurations.SiteMode = 'Basic';
      } else if (utils.ignoreCaseEquals(siteMode, 'Reserved')) {
  ...
  siteScale.command('instances [name] <instances> [size]')
    ...
    .execute(function (name, instances, size, options, _) {
      ...
      if (size !== undefined) {
        if (utils.ignoreCaseEquals(size, 'small')) {
          size = 'Small';
        } else if (utils.ignoreCaseEquals(size, 'Medium')) {
          size = 'Medium';
        } else if (utils.ignoreCaseEquals(size, 'Large')) {
          size = 'Large';
        } else {
