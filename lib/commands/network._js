...
exports.init = function(cli) {
  ...
  vnet.command('list')
    .usage('[options]')
    .description('Lists your Azure virtual network')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(listVNet);
  ...
  vnet.command('show [vnet]')
    ...
    .option('--vnet <vnet>', 'name of the virtual network to show')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(showVNet);
  ...
  vnet.command('delete [vnet]')
    ...
    .option('--vnet <vnet>', 'name of the virtual network to delete')
  ...
  vnet.command('create [vnet]')
    ...
    .option('--vnet <vnet>', 'name of the virtual network to create')
  ...
  function registerDNSServer(dnsIp, options, _) {
    ...
    for (var i = 0; i < vnetConfiguration.Dns.DnsServers.length; i++) {
      if (utils.equalsIgnoreCase(vnetConfiguration.Dns.DnsServers[i].Name, dnsId)) {
  ...
  function unregisterDNSServer(dnsIp, options, _) {
    ...
    if (options.dnsId && dnsIp) {
      throw new Error('Either --dns-id or --dns-ip must present not both');
    }
    ...
    if (!options.quiet && !interaction.confirm(cli, util.format('Delete the DNS server entry %s %s ? (y/n) ', dnsNameIdentifier, dnsIdAndIp), _)) {
      return;
    }
