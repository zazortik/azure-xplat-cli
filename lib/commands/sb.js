...
exports.init = function (cli) {
  ...
  sbnamespace.command('create [name] [region]')
    .description('Create a new Service Bus namespace')
    .usage('[options] <name> <region>')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (name, region, options, callback) {
      var service = createService(options.subscription);
      promptIfNotGiven("Namespace name: ", name, function (err, name) {
        chooseIfNotGiven("Region: ", "Getting regions", region, 
          ...
          function (err, region) { 
            ...
            var progress = cli.progress('creating namespace ' + name + ' in region ' + region);
            service.createNamespace(name, region, function (err, createdNamespace) {
              progress.end();
