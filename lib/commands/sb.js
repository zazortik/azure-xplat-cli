...
exports.init = function (cli) {
  ...
  sbnamespace.command('create [namespace] [region]')
    ...
    .option('-n, --name <namespace>', 'name of namespace to create')
    .option('-r, --region <region>', 'Service Bus region to create namespace in')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (namespaceName, region, options, callback) {
      var service = createService(options.subscription);
      normalizeParameters("namespace", namespaceName, options.namespaceName, function (err, namespaceName) {
        if (err) { return callback(err); }
        normalizeParameters("region", region, options.region, function (err, region) {
          if (err) { return callback(err); }
            ...
            chooseIfNotGiven("Region: ", "Getting regions", region, 
              function (cb) {
                service.getRegions(function (err, regions) {
                  ...
                  cb(null, regions.map(function (r) { return r.Code; }));
                });
              }, 
              function (err, region) { 
                if (err) { return callback(err); }
  ...
  function normalizeParameters(paramName, positionalValue, optionValue, callback) {
    if(!_.isUndefined(positionalValue) && !_.isUndefined(optionValue)) {
      callback(new Error("You must specify " + paramName + " either positionally or by name, but not both"));
    } else {
      callback(null, positionalValue || optionValue);
    }
  }
