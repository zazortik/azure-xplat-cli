...
var ServiceBusManagementService = require('../serviceBusManagement');

exports.init = function (cli) {
  var log = cli.output;

  var sb = cli.category('sb')
    .description('Commands to manage your ServiceBus configuration');

  var sbnamespace = sb.category('namespace')
    .description('Commands to manage your ServiceBus namespaces');
  ...
  sbnamespace.command('list')
    ...
    .execute(function (name, options, callback) {
      ...
      var subscriptionId = account.lookupSubscriptionId(context.subscription);

      var service = createService(subscriptionId);
      var progress = cli.progress('Enumerating namespaces');
      service.listNamespaces(function (err, result) {
        progress.end();
        if (err) {
          return callback(err);
        }
        ...
        if(result.length === 0) {
          ...
          log.table(result, function (row, ns) {
