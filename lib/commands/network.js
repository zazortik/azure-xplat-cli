...
var VNetUtil = require('../util/vnetUtil');
...
exports.init = function(cli) {
  ...
  network.command('export <file-path>')
    ...
    .execute(function(filePath, options, callback) {
      exportNetworkConfig(filePath, options, callback);
    });
  ...
  network.command('import <file-path>')
    ...
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(filePath, options, callback) {
      importNetworkConfig(filePath, options, callback);
    });
  ...
  dnsserver.command('list')
    ...
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(filePath, options, callback) {
      listDNSServers(options, callback);
    });

  dnsserver.command('register <dnsip>')
    .usage('<dnsip>')
    .description('Register a DNS server with current Azure network')
    .option('-i, --dns-id <name>', 'Name identifier for the DNS server entry')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(dnsip, options, callback) {
      registerDNSServer(dnsip, options, callback);
    });

  dnsserver.command('unregister')
    .description('Unregister a DNS server registerd in the current Azure network')
    .option('-i, --dns-id <name>', 'Name identifier of the DNS server entry')
    .option('-p, --dns-ip <name>', 'IP address of the DNS server entry')
    .option('-q, --quiet', 'quiet mode, do not ask for unregister confirmation')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(options, callback) {
      unregisterDNSServer(options, callback);
    });

  var vnet = network.category('vnet')
  .description('Commands to manage your Virtual network');

  vnet.command('show <name>')
    .usage('<name>')
    .description('Show details about an Azure virtual network')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(name, options, callback) {
      showVNet(name, options, callback);
    });

  vnet.command('delete <name>')
    .usage('<name>')
    .description('Delete an Azure virtual network')
    .option('-q, --quiet', 'quiet mode, do not ask for delete confirmation')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(name, options, callback) {
      deleteVNet(name, options, callback);
    });

  vnet.command('create <name>')
    .usage('<name>')
    .description('Create an Azure virtual network')
    .option('-e, --address-space <ipv4>', 'Address space for the vnet')
    ...
    .option('-n, --subnet-name <name>', 'name for the subnet')
    ...
    .option('-l, --location <name>', 'location of data center')
    .option('-a, --affinity-group <name>', 'affinity group')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function(name, options, callback) {
      createVNet(name, options, callback);
    });
      ...
      if (!error) {
        ...
        fs.writeFile(filePath, networkConfigAsString, function (err) {
          if (err) {
            ...
            logger.info('Network Configuration exported to ' + filePath);
            return cmdCallback();
          }
        });
      } else {
  ...
  function importNetworkConfig(filePath, options, cmdCallback) {
    ...
    logger.verbose('Loading configuration file: ' + filePath);
      ...
      if (!error) {
        ...
        if (vnetConfiguration && vnetConfiguration.Dns && vnetConfiguration.Dns.DnsServers instanceof Array) {
          if (vnetConfiguration.Dns.DnsServers.length > 0) {
            logger.table(vnetConfiguration.Dns.DnsServers, function(row, item) {
        ...
        if (logger.format().json) {
          ...
          logger.info('No DNS servers found');
        }
      } else {
        return cmdCallback(error);
      }
  ...
  function registerDNSServer(dnsip, options, cmdCallback) {
    var dnsId = null;
    if (options.dnsId) {
      var dnsIdPattern = /^[a-z][a-z0-9\-]{0,19}$/i;
      if (dnsIdPattern.test(options.dnsId) === false) {
        return cmdCallback('--dns-id can contain only letters, numbers and hyphens with no more than 20 characters. It must start with a letter.');
      }
      dnsId = options.dnsId;
    } else {
      dnsId = 'DNS-' + crypto.randomBytes(8).toString('hex');
    }

    var vnetUtil = new VNetUtil();
    var parsedDnsIp = vnetUtil.parseIPv4(dnsip);
    if (parsedDnsIp.error) {
      return cmdCallback(parsedDnsIp.error);
    }
    ...
    getNetworkConfig(options, function (error, networkConfiguration) {
      ...
      if (!error) {
        ...
        if (!vnetConfiguration.Dns.DnsServers instanceof Array) {
          vnetConfiguration.Dns.DnsServers = [];
        }

        for (var i = 0; i < vnetConfiguration.Dns.DnsServers.length; i++) {
          if (vnetConfiguration.Dns.DnsServers[i].Name.toLowerCase() == dnsId.toLowerCase()) {
            return cmdCallback('A DNS server entry with name identifier ' + dnsId + ' already exists');
          }

          if (vnetConfiguration.Dns.DnsServers[i].IPAddress == dnsIP) {
            return cmdCallback('A DNS server entry with ip address ' + dnsIP + ' already exists');
          }
        }

        vnetConfiguration.Dns.DnsServers.push({Name:dnsId, IPAddress:dnsIp });
        if (!options.dnsId) {
          logger.info('Name identifer for the DNS server is ' + dnsId);
        }
  ...
  function unregisterDNSServer(options, cmdCallback) {
    if ((options.dnsId !== undefined) == (options.dnsIp !== undefined)) {
      return cmdCallback('Either --dns-id or --dns-ip must present not both');
    }
    ...
    if (options.dnsId) {
      ...
      var vnetUtil = new VNetUtil();
      var parsedDnsIP = vnetUtil.parseIPv4(options.dnsIp, '--dns-ip');
      if (parsedDnsIP.error) {
        return cmdCallback(parsedDnsIP.error);
      }
    ...
    getNetworkConfig(options, function (error, networkConfiguration) {
      ...
      if (!error) {
        ...
        if (!vnetConfiguration || !vnetConfiguration.Dns ||
          ...
          return cmdCallback('No DNS server registered with the network');
        }
        ...
        if (dnsEntryIndex == -1) {
          return cmdCallback('A DNS server entry with ' + (options.dnsId ? 'name identifer ' : 'ip address ') + filterValue + ' not found');
        }
            ...
            if (virtualNetworkSite.DnsServersRef instanceof Array) {
              for (var k = 0; k < virtualNetworkSite.DnsServersRef.length; k++) {
                if(virtualNetworkSite.DnsServersRef[k].Name.toLowerCase() == dnsNameIdentifier) {
                  return cmdCallback('You cannot unregister this DNS entry, it is being referrenced by the virtual network ' + virtualNetworkSite.Name);
                }
              }
            }
          }
        ...
        var _deleteDNSEntry = function(confirm) {
          if (!confirm) {
            return cmdCallback();
          }
        ...
        if (!options.quiet) {
          network.confirm('Delete the DNS server entry ' + dnsNameIdentifier + dnsIdAndIp +' ?  (y/n) ', _deleteDNSEntry);
        } else {
          _deleteDNSEntry(true);
        }
      } else {
        return cmdCallback(error);
      }
  ...
  function showVNet(name, options, cmdCallback) {
    var progress = cli.progress('Fetching Virtual Networks');
    getVirtualNetworkSites(options, function (error, virtualNetworkSites) {
      progress.end();
      if (!error) {
        if (virtualNetworkSites) {
          var virtualNetworkSite = null;
          for (var i = 0; i < virtualNetworkSites.length; i++) {
            if (virtualNetworkSites[i].Name.toLowerCase() == name.toLowerCase()) {
          ...
          if (virtualNetworkSite) {
            if (logger.format().json) {
              logger.json(virtualNetworkSite);
            } else {
              utils.logLineFormat(virtualNetworkSite, logger.data);
            }
            return cmdCallback();
          } else {
            return cmdCallback('Virtual network with name ' + name + ' not found');
          }
        } else {
          return cmdCallback('No virtual networks found');
        }
      } else {
        return cmdCallback(error);
      }
  ...
  function deleteVNet(name, options, cmdCallback) {
    var progress = cli.progress('Fetching Network Configuration');
    getNetworkConfig(options, function (error, networkConfiguration) {
      progress.end();
      if (!error) {
        ...
        if (vnetConfiguration.VirtualNetworkSites instanceof Array &&
          ...
          for (var i = 0; i < vnetConfiguration.VirtualNetworkSites.length; i++) {
            if (vnetConfiguration.VirtualNetworkSites[i].Name == name) {
              index = i;
              break;
            }
          }
          ...
          if (index != -1) {
            var _deleteVNet = function(confirm) {
              if (!confirm) {
                return cmdCallback();
              }

              progress = cli.progress('Deleting the virtual network ' + name);
            ...
            if (!options.quiet) {
              network.confirm('Delete the virtual network ' + name + ' ?  (y/n) ', _deleteVNet);
            } else {
              _deleteVNet(true);
            }
          } else {
          ...
          return cmdCallback('No virtual network found');
        }
      } else {
        return cmdCallback(error);
      }
  ...
  function createVNet(name, options, cmdCallback) {
    if ((options.location !== undefined) == (options.affinityGroup !== undefined)) {
      return cmdCallback('Either --location or --affinity-group must present not both');
    }
    ...
    if (requiredOptCheckResult.error) {
      return cmdCallback(requiredOptCheckResult.error);
    }
    ...
    if (options.subnetName) {
      if (namePattern.test(options.subnetName) === false) {
        return cmdCallback('The --subnet-name can contain only letters, numbers and hyphens with no more than 63 characters. It must start with a letter or number.');
      }
    ...
    if (namePattern.test(options.subnetName) === false) {
      return cmdCallback('The name can contain only letters, numbers and hyphens with no more than 63 characters. It must start with a letter or number.');
    }
    ...
    if (!options.addressSpace) {
      ...
      logger.info('Using default address space start IP: ' + addressSpaceStartIP);
    } else {
    ...
    if (parsedAddressSpaceStartIP.error) {
      return cmdCallback(parsedAddressSpaceStartIP.error);
    }
    ...
    if (options.maxVmCount) {
      ...
      if (isNaN(maxVmCount)) {
        return cmdCallback('--vm-count should be an integer value');
      }

      cidr = vnetUtil.getCIDRFromHostsCount(maxVmCount);
      logger.info('The cidr calculated for the given --vm-count ' + maxVmCount + ' is ' + cidr);
    } else if (options.cidr) {
      cidr = parseInt(options.cidr, 10);
    } else {
      cidr = vnetInput.addressSpaceInfo.startCidr;
      logger.info('Using default address space cidr: ' + cidr);
    }
    ...
    if (verifyCidrResult.error) {
      return cmdCallback(verifyCidrResult.error);
    }
    ...
    if (!options.subnetStartIp) {
      ...
      logger.info('Using default subnet start IP: ' + vnetInput.subnetStartIP);
    } else {
      var parsedSubnetStartIP =
        vnetUtil.parseIPv4(options.subnetStartIp, '--subnet-start-ip');
      if (parsedSubnetStartIP.error) {
        return cmdCallback(parsedSubnetStartIP.error);
      }
    ...
    if (!isSubNetInRange) {
      ...
      logger.help('The given subnet (--subnet-start-ip) ' +
        ' should belongs to the address space ' + addressSpaceRange);
      return cmdCallback('The subnet is not in the address space.');
    }
    ...
    if (options.subnetVmCount) {
      ...
      if (isNaN(subnetVmCount)) {
        return cmdCallback('--subnet-vm-count should be an integer value');
      }

      subnetCidr = vnetUtil.getCIDRFromHostsCount(subnetVmCount);
      logger.info('The cidr calculated for the given --subnet-vm-count ' +
      ...
      logger.info('Using default subnet cidr: ' + subnetCidr);
    }
    ...
    logger.verbose('Address Space [Starting IP/CIDR (Max VM Count)]: ' +
      vnetInput.addressSpaceStartIP + '/' +
      vnetInput.cidr + ' (' +
      vnetUtil.getHostsCountForCIDR(vnetInput.cidr).hostsCount +
      ')');

    logger.verbose('Subnet [Starting IP/CIDR (Max VM Count)]: ' +
      vnetInput.subnetStartIP + '/' +
    ...
    getNetworkConfig(options, function (error, networkConfiguration) {
      ...
      if (!error) {
        ...
        for (var i = 0;
          i < vnetConfiguration.VirtualNetworkSites.length;
          i++) {
          var virtualNeworkSite = vnetConfiguration.VirtualNetworkSites[i];

          if (virtualNeworkSite.Name.toLowerCase() == vnetInput.name.toLowerCase()) {
            return cmdCallback('A virtual network with name ' + vnetInput.name + ' already exists.');
          }
        }

        getOrCreateAffinityGroup(options, function (error, affinityGroup) {
          if (error) {
            return cmdCallback(error);
          }

          logger.info('Using affinity group ' + affinityGroup.Name);
          ...
          cli.progress('Updating Network Configuration');
          setNetworkConfig(options, networkConfiguration, function (error) {
            progress.end();
            return cmdCallback(error);
          });
    ...
    _getAffinityGroups(function(error, agResponse) {
      ...
      if (options.affinityGroup) {
        ...
        if (!_affinityGroupSupportsPersistentVMRole(affinityGroup)) {
          ...
          logger.help('You should create virtual network in an affinity group that support PersistentVMRole service.');
          ...
          if (vmroleSupportedAffinityGroupNames.length > 0) {
            logger.help('Following affinity groups in your subscription supports PersistentVMRole service.');
            for (var k = 0; k < vmroleSupportedAffinityGroupNames.length; k++) {
              logger.help(vmroleSupportedAffinityGroupNames[k]);
            }
          } else {
            logger.help('There is no affinity groups in your subscription that supports PersistentVMRole service.');
            logger.help('To create a new affinity group use --location option.');
          }
        ...
        _getLocations(function(error, locResponse) {
          ...
          if (!_locationSupportsPersistentVMRole(location)) {
            ...
            logger.help('You should create virtual network in a location that supports PersistentVMRole service.');
            ...
            if (vmroleSupportedLocationNames.length > 0) {
              logger.help('Following locations supports PersistentVMRole service.');
              for (var k = 0; k < vmroleSupportedLocationNames.length; k++) {
                logger.help(vmroleSupportedLocationNames[k]);
              }
            }
  ...
  function confirm(message, callback) {
    cli.confirm(message, function (y) {
      if (!y) {
        logger.warn('The operation was cancelled by the user');
      }
      callback(y);
    });
  }

  network.confirm = confirm;
