...
exports.init = function (cli) {
  ...
  siteDomain.command('add [extension] [processor] [name]')
    ...
    .option('-p, --processor <processor>', 'The script processor')
  ...
  siteDomain.command('delete [extension] [name]')
    ...
    .execute(function (extension, name, options, _) {
      ...
      if (siteConfigurations.HandlerMappings && siteConfigurations.HandlerMappings.HandlerMapping) {
        ...
        if (found) {
          if (!options.quiet && !interaction.confirm(cli, 'Delete handler mapping with ' + extension + ' extension?  (y/n) ', _)) {
            return;
          }
      ...
      if (!found) {
        throw new Error('Handler mapping for extension "' + extension + '" does not exist.');
      }
    });
