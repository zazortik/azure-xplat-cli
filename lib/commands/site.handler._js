...
exports.init = function (cli) {
  ...
  siteDomain.command('list [name]')
    ...
    .execute(function (name, options, _) {
      ...
      cli.interaction.formatOutput(siteConfigurations.HandlerMappings.HandlerMapping, function (data) {
        if (data.length > 0) {
          log.table(data, function (row, item) {
            row.cell($('Extension'), item.Extension);
            row.cell($('Script Processor Path'), item.ScriptProcessor);
            row.cell($('Additional Arguments'), __.isObject(item.Arguments) ? '' : item.Arguments);
          });
        } else {
