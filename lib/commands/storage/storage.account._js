  ...
  storageAccount.createCommand = function (name, options, _) {
    ...
    if (!availability.nameAvailable)
    {
      if (availability.reason === 'AlreadyExists') {
        ...
        throw new Error(util.format($('The storage account name "%s" is not in valid format'), name));
      } else {
        throw new Error(util.format($('The storage account named "%s" is already taken or not valid'), name));
      }
    }
