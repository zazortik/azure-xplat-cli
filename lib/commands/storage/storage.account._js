...
exports.init = function (cli) {
  ...
  function validateAccountConfiguration(options, storageOptions, isUpdate, _) {
    ...
    if (!isUpdate) {
      if (value) {
        validation.isValidEnumValue(value, Object.keys(isUpdate ? storageUtil.AccountTypeForChanging : storageUtil.AccountTypeForCreating));
      } else {
        value = cli.interaction.chooseIfNotGiven(title, info, value,
          function (cb) {
            cb(null, Object.keys(isUpdate ? storageUtil.AccountTypeForChanging : storageUtil.AccountTypeForCreating));
          }, _);
      }
  ...
  if (isResourceMode) {
    accountCreateCommand.option('--sku-name <skuName>', $('the SKU name (LRS/ZRS/GRS/RAGRS/PLRS)'))
      .option('--kind <kind>', $('the account kind'))
      .option('--access-tier <accessTier>', $('the access tier (Hot/Cool)'))
      .option('--enable-encryption-service <enableEncryptionService>', $('specifies the service which needs to enable encryption settings'))
  ...
  if (isResourceMode) {
    accountSetCommand = storageAccount.command('set <name>').description($('Update a storage account'))
      ...
      .option('--access-tier <accessTier>', $('the access tier (Hot/Cool)'))
      .option('--enable-encryption-service <enableEncryptionService>', $('specifies the service which needs to enable encryption settings'))
      .option('--disable-encryption-service <disableEncryptionService>', $('specifies the service which needs to disable encryption settings'))
      .option('--custom-domain <customDomain>', $('the custom domain'))
      .option('--subdomain', $('whether uses the \'asverify\' subdomain to preregister the custom domain'))
      .option('--tags <tags>', $('the account tags. Tags are key=value pairs and separated with semicolon(;)'));
  } else {
