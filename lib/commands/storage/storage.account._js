...
exports.init = function (cli) {
  ...
  function showSubscriptionUsage(serviceClient, subscriptionId, _) {
    ...
    try {
      progress = showProgress(message);
      var usageList = serviceClient.usage.list(_).usages;
  ...
  if (isResourceMode) {
    accountSetCommand = storageAccount.command('set <name>').description($('Update a storage account (Only one property can be updated at a time)'))
