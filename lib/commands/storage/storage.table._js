...
exports.init = function(cli) {
  ...
  */
  function showTable(table, options, _) {
    ...
    startProgress(tips);
    try {
      var exist = performStorageOperation(operation, _, table, storageOptions).exists;
      if (!exist) {
    ...
    cli.interaction.formatOutput(output[0], function(outputData) {
      ...
      if (names.length === 0) {
        ...
        logger.table(names, function(row, item) {
          ...
          row.cell($('Expiry'), item.Expiry ? item.Expiry.toUTCFormat(UTCFormat) : '');
        });
      }
    });
  }
  ...
  */
  function createTableSAS(table, permissions, expiry, options, _) {
    ...
    var output = { sas: '' };
    var sharedAccessPolicy = StorageUtil.getSharedAccessPolicy(StorageUtil.AccessType.Table, permissions, options.protocol, options.ipRange, start, expiry, tableField, options.policy);
