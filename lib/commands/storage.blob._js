...
exports.init = function(cli) {
  ...
  */
  function logBlobProperties(properties) {
    interaction.formatOutput(cli, properties, function(data) {
      if(!data) {
        logger.info('No blobs found');
      } else {
        var outputProperties = ['container', 'blob', 'blobType', 'contentLength', 'contentType'];
        var output = [];

        for(var i = 0; i < outputProperties.length; i++) {
          var obj = {property : outputProperties[i], value : data[outputProperties[i]]};
          output.push(obj);
        }
  ...
  */
  function downloadAzureBlob(containerName, blobName, destination, options, _) {
    ...
    interaction.formatOutput(cli, downloadedBlob, function (data) {
