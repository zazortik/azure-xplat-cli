...
exports.init = function(cli) {
  ...
  */
  ...
  blob.command('list [containerName] [blobName]')
    .description('List Azure Storage blob in the sepcified storage container')
  ...
  blob.command('upload [file] [containerName] [blobName]')
    .description('Upload the specified file to azure storage blob')
    .option('-f, --file <file>', 'Local file path')
    .option('-c, --container <containerName>', 'Storage container name')
    .option('-b, --blob <blobName>', 'Storage blob name')
    .option('-t, --blobtype <blobtype>', 'Storage blob type(Page, Block)')
    .option('-p, --properties <properties>', 'Storage blob properties for uploaded file')
    .option('-m, --metadata <metadata>', 'Storage blob metadata for uploaded file')
  ...
  blob.command('download [containerName] [blobName] [Destination]')
    .description('Download the specified blob i specifiednto local')
    .option('-c, --container <containerName>', 'Storage container name')
    .option('-b, --blob <blobName>', 'Storage blob name')
    .option('-d, --destination [destinatioin]', 'Download destination file or directory path')
  ...
  */
  ...
  /**
  * Ask for inputing if the sepcified value is not given
  */
  ...
  */
  ...
  /**
  * Show the details for the specified azure storage container
  * @param {string} containerName container name
  */
  ...
  */
  function createAzureContainer(containerName, options, _) {
    ...
    var tips = util.format('Creating azure container %s', containerName);
    ...
    try {
      ...
      if(created === false) {
        throw new Error(util.format('Contaienr %s already exists', containerName));
      } else if(permission) {
    ...
    logger.info(util.format('Container %s created successfully', containerName));
  }

  /**
  * Delete the specified azure storage container
  */
  function deleteAzureContainer(containerName, options, _) {
    ...
    if(force !== true) {
      force = confirm(util.format('Are you sure to remove the Storge container %s?', containerName), _);
      if(force !== true) {
        return;
      }
    }
  ...
  */
  ...
  /**
  * List azure storage blob in the specified container
  */
  function listAzureBlob(containerName, blobName, options, _) {
    ...
    var storageOptions = getStorageBlobOperationDefaultOption();
    storageOptions.prefix = params.values.blobName;
    var blobs = [];
    ...
    logger.table(blobs, function(row, item) {
      row.cell('Name', item.name);
      row.cell('BlobType', item.properties.blobtype);
      row.cell('URL', item.url);
      row.cell('Last-Modified', item.properties['last-modified']);
    });
  }
  ...
  */
  function deleteAzureBlob(containerName, blobName, options, _) {
    ...
    //FIXME it seems there is no need to prompt
    var specifiedContainerName = promptIfNotGiven('Container name:', params.values.containerName, _);
    var specifiedBlobName = promptIfNotGiven('Blob name:', params.values.blobName, _);
    var storageOptions = getStorageBlobOperationDefaultOption();
  ...
  */
  function uploadAzureBlob(file, containerName, blobName, options, _) {
    ...
    storageOptions.setBlobContentMd5 = true; //Page blob don't support in sdk
    storageOptions.blobType = BlobConstants.BlobTypes[blobTypeName];
    ...
    if (force !== true) {
      ...
      if(blobProperties !== null) {
        if (blobProperties.blobType !== storageOptions.blobType) {
          ...
          if(!confirm(util.format('Are you sure to remove the blob %s in container %s',
  ...
  */
  function downloadAzureBlob(containerName, blobName, destination, options, _) {
    ...
    var specifiedContainerName = promptIfNotGiven('Container name:', params.values.containerName, _);
    var specifiedFileName = promptIfNotGiven('Destination file path:', params.values.destination, _);
    var specifiedBlobName = promptIfNotGiven('Blob name:', params.values.blobName, _);
    var dirName = '';
    var fileName = '';
    ...
    if (force !== true && utils.fileExists(fullName, _)) {
      if(!confirm(util.format('Are you sure to overwrite %s',
        fullName), _)) {
        return;
      }
    }
    ...
    var storageOptions = getStorageBlobOperationDefaultOption();
    var operation = getStorageBlobOperation('getBlobToStream');

    startProgress(tips);
    try {
      performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, writeStream, storageOptions);
    } finally {
      endProgress();
    }
    writeStream.end();
    ...
    if(options.checkmd5) {
      ...
      if(!blobProperties.contentMd5) {
        logger.warn(util.format('Blob contentMd5 is missed, and the local file md5 is %s', fileMd5));
      } else {
