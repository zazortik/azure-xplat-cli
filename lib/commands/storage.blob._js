...
exports.init = function(cli) {
  ...
  */
  function internalShowAzureBlob(containerName, blobName, options, _) {
    ...
    try {
      ...
      if(StorageUtil.isNotFoundException(e)) {
        e = new Error(util.format('Blob %s in Container %s doesn\'t exist', specifiedBlobName, specifiedContainerName));
      }
      throw e;
    } finally {
      endProgress();
  ...
  */
  function downloadAzureBlob(containerName, blobName, destination, options, _) {
    ...
    var downloadedBlob = internalShowAzureBlob(specifiedContainerName, specifiedBlobName, getStorageAccountOptions(options), _);
    if(downloadedBlob) {
