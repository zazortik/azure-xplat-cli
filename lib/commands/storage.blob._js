...
exports.init = function(cli) {
  ...
  */
  function listAzureBlob(containerName, blobName, options, _) {
    ...
    var operation = getStorageBlobOperation(blobService, 'listBlobs');
    var storageOptions = getStorageBlobOperationDefaultOption();
    var useWildcard = false;
  ...
  function uploadAzureBlob(file, containerName, blobName, options, _) {
    ...
    try {
      if (blobTypeName.toLowerCase() == 'page') {
        //Upload page blob
        //createPageBlobIfNotExists(blobService, specifiedContainerName, specifiedBlobName, fsStatus.size, _);
  ...
  */
  function createPageBlobIfNotExists(blobService, container, blob, length, _) {
    var operation = getStorageBlobOperation(blobService, 'createPageBlob');
    var storageOptions = getStorageBlobOperationDefaultOption();
