  ...
  */
  function getSpeedPrinter(summary) {
    ...
    return function(newline) {
      if(logger.format().json) return;
      var tips = util.format('Percentage: %s%% (%s/%s) Speed: %s Elapsed Time: %s ', summary.getCompletePercent(), summary.getCompleteSize(), summary.getTotalSize(), summary.getAverageSpeed(), summary.getElapsedSeconds());
