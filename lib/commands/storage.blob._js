...
exports.init = function(cli) {
  ...
  */
  ...
  blob.command('list [containerName] [blobName]')
    .description('List storage blob in the sepcified storage container use wildcard and blob name prefix')
    .option('--container <containerName>', 'Storage container name')
  ...
  */
  function downloadAzureBlob(containerName, blobName, destination, options, _) {
    ...
    var md5sum = '';
    try {
      md5sum = performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, fullName, storageOptions);
    } catch(e) {
      if(StorageUtil.isNotFoundException(e)) {
        throw new Error(util.format('Can not find blob \'%s\' in container \'%s\'.', specifiedBlobName, specifiedContainerName));
      } else {
    ...
    if(options.checkmd5) {
      ...
      if(!blobProperties.contentMD5) {
        logger.warn(util.format('Blob contentMd5 is missing, and the local file md5 is %s', md5sum));
      } else {
        if (blobProperties.contentMD5 == md5sum) {
          logger.info(util.format('Md5checksum validation passed, and md5checksum is %s', md5sum));
        } else {
          throw new Error(util.format('Md5checksum validation failed. Blob md5 is %s, but local file md5 is %s', blobProperties.contentMD5, md5sum));
        }
      }
    }
    showAzureBlob(specifiedContainerName, specifiedBlobName, getStorageAccountOptions(options), _);
  ...
  */
  function getSpeedPrinter(summary) {
    ...
    return function(newline) {
      var tips = util.format('Percent: %s%% (%s/%s) Speed: %s Elapsed Time: %s ', summary.getCompletePercent(), summary.getCompleteSize(), summary.getTotalSize(), summary.getAverageSpeed(), summary.getElapsedSeconds());
  ...
  */
  function applyBlobServicePatch(blobService) {
    ...
    * List all blobs
    */
    //All the caller should use the options parameter
    ...
    */
    function listWithContinuation(listFunc) {
      var allItems = [];
      function listCallback(error, items, resultContinuation, response) {
        if(error) throw error;
        allItems = allItems.concat(items);
        if(resultContinuation.hasNextPage()) {
