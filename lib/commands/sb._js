...
exports.init = function (cli) {
  ...
  sbnamespace.command('list')
    ...
    .execute(function (options, _) {
      ...
      var progress = cli.progress($('Getting namespaces'));
  ...
  sbnamespace.command('create [namespace] [region]')
    ...
    .execute(function (namespaceName, region, options, _) {
      ...
      var progress = cli.progress(util.format($('Creating namespace %s in region %s'), namespaceName, region));
  ...
  sbnamespace.command('delete [namespace]')
    ...
    .execute(function (namespace, options, _) {
      ...
      if (!options.quiet && !interaction.confirm(cli, util.format($('Delete namespace %s? [y/n] '), namespace), _)) {
        return;
      }

      var service = createService(options.subscription);

      var progress = cli.progress(util.format($('Deleting namespace %s'), namespace));
      try {
