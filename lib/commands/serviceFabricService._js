...
exports.init = function (cli) {
  ...
  service.command('resolve [serviceName] [partitionKeyType] [partitionKeyValue] [previousRspVersion]')
    ...
    .option('--service-name <serviceName>', $('the name of the service'))
    .option('--partition-key-type <partitionKeyType>', $('the type of the partition key, values are [Singleton|Int64Range|Named]'))
    .option('--partition-key-value <partitionKeyValue>', $('the value of the partition key'))
    .option('--previous-rsp-version <previousRspVersion>', $('the version of the previous rsp'))
    .option('-s --select <fields>', $('select fields to show'))
