...
exports.init = function (cli) {
  ...
  siteConnectionStrings.command('delete <connectionname> [name]')
    ...
    .execute(function (connectionname, name, options, _) {
      ...
      if (!options.quiet && !interaction.confirm(cli, util.format('Delete %s connection string? (y/n) ', connectionname), _)) {
        return;
      }
    ...
    .execute(function (connectionname, name, options, _) {
      ...
      if (match) {
        interaction.formatOutput(cli, match, function (data) {
          interaction.logEachData(cli, 'Connection String', data);
        });
      } else {
        throw new Error(util.format('Connection string with name "%s" does not exist.', connectionname));
      }
