...
exports.init = function (cli) {
  ...
  siteConnectionStrings.command('list [name]')
    ...
    .execute(function (name, options, _) {
      ...
      if (siteConfigurations.ConnectionStrings && siteConfigurations.ConnectionStrings.ConnStringInfo) {
        siteConfigurations = {
          '$': siteConfigurations['$'],
          ConnectionStrings: getConnectionStrings(siteConfigurations.ConnectionStrings)
        };

        log.table(siteConfigurations.ConnectionStrings.ConnStringInfo, function (row, item) {
  ...
  siteConnectionStrings.command('add [connectionname] [value] [type] [name]')
    ...
    .description('Adds a connection string to your site')
    ...
    .execute(function (connectionname, value, type, name, options, _) {
      ...
      connectionname = interaction.promptIfNotGiven(cli, 'Connection String Name: ', params.values.connectionname, _);
      value = interaction.promptIfNotGiven(cli, 'Connection String Value: ', params.values.value, _);
      type = interaction.chooseIfNotGiven(cli, 'Connection String Type: ', 'Getting types', params.values.type,
          function (cb) {
  ...
  siteConnectionStrings.command('delete <connectionname> [name]')
    ...
    .description('Deletes a connection string for your site')
    .option('-c, --connectionname <connectionname>', 'the connection string name')
    ...
    .execute(function (connectionname, name, options, _) {
      ...
      if (!options.quiet && !confirm('Delete ' + connectionname + ' connection string?  (y/n) ', _)) {
        return;
      }
      ...
      if (siteConfigurations.ConnectionStrings && siteConfigurations.ConnectionStrings.ConnStringInfo) {
        ...
        for (var i = 0; i < siteConfigurations.ConnectionStrings.ConnStringInfo.length; i++) {
          if (siteConfigurations.ConnectionStrings.ConnStringInfo[i].Name === connectionname) {
      ...
      if (!found) {
        throw new Error('Connection string with name "' + connectionname + '" does not exist.');
      }
    });

  siteConnectionStrings.command('show <connectionname> [name]')
    .usage('[options] <connectionname> [name]')
    .description('Shows a connection string for your site')
    .option('-c, --connectionname <connectionname>', 'the connection string name')
    .option('-s, --subscription <id>', 'use the subscription id')
    .execute(function (connectionname, name, options, _) {
      var context = {
      ...
      if (siteConfigurations.ConnectionStrings && siteConfigurations.ConnectionStrings.ConnStringInfo) {
        ...
        for (var i = 0; i < siteConfigurations.ConnectionStrings.ConnStringInfo.length; i++) {
          if (siteConfigurations.ConnectionStrings.ConnStringInfo[i].Name === connectionname) {
            log.data('Value: ', siteConfigurations.ConnectionStrings.ConnStringInfo[i].ConnectionString);
            found = true;
            return;
          }
        }
