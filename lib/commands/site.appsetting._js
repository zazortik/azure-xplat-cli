      ...
      for (var setting in settings) {
        if (settings.hasOwnProperty(setting)) {
          if (siteConfigurations.AppSettings.NameValuePair.some(function (kvp) {
            return utils.ignoreCaseEquals(kvp.Name, setting);
          })) {
  ...
  siteAppsettings.deleteCommand = function (key, name, options, _) {
    ...
    if (siteConfigurations.AppSettings.NameValuePair) {
      ...
      for (var i = 0; i < siteConfigurations.AppSettings.NameValuePair.length; i++) {
        if (utils.ignoreCaseEquals(siteConfigurations.AppSettings.NameValuePair[i].Name, key)) {
      ...
      if (found) {
        if (!options.quiet && !interaction.confirm(cli, util.format($('Delete application setting %s? [y/n] '), key), _)) {
          return;
        }
    ...
    if (!found) {
      throw new Error(util.format($('Application setting with key "%s" does not exist'), setting));
    }
  };
