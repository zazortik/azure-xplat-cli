...
__.extend(WebsitesClient, {
  ...
  isProductionSlot: function (slot) {
    return utils.ignoreCaseEquals(slot, 'production');
  }
});
...
__.extend(WebsitesClient.prototype, {
  ...
  getDiagnosticsSettings: function (context, _) {
    ...
    try {
      service.getDiagnosticsSettings(_);
    } finally {
      progress.end();
    }
  },
  ...
  updateDiagnosticsSettings: function (context, settings, _) {
    ...
    try {
      service.updateDiagnosticsSettings(settings, _);
    } finally {
      progress.end();
    }
  },
  ...
  createSite: function (subscription, site, _) {
    var self = this;
    ...
    self.cli.output.verbose('Webspace', site.webSpaceName);
    ...
    try {
      var result = service.webSites.create(site.webSpaceName, site, _).webSite;
