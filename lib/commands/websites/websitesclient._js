...
__.extend(WebsitesClient.prototype, {
    ...
    try {
      ...
      if (typeof message === 'string' && message.indexOf('Access is denied.') >= 0) {
        ...
        if (confirm($('Launch browser to portal now? [y/n] '), _)) {
          ...
          var href = self.cli.environmentManager.getPortalUrl();
  ...
  getSiteConfiguration: function (options, callback) {
    ...
    var service = self.createWebsiteManagementService(options.subscription);
    service.webSites.getConfiguration(options.site.webspace, options.site.name, function (err, result) {
      progress.end();
      if (err) {
  ...
  getHostNameSuffix: function (subscription, _) {
    ...
    var subscriptionId = self.cli.category('account').getCurrentSubscription(subscription).Id;
    if (subscriptionId) {
      ...
      return hostNameSuffix.dnsSuffix || self.cli.environmentManager.getHostNameSuffix();
    } else {
      return self.cli.environmentManager.getHostNameSuffix();
    }
  },
  ...
  lookupSiteWebSpace: function (context, _) {
    ...
    self.cli.output.verbose(util.format($('Attempting to locate site '), context.site.name));
