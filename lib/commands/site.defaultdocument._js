...
exports.init = function (cli) {
  ...
  siteDefaultDocument.command('list [name]')
    ...
    .execute(function (name, options, _) {
      ...
      if (siteConfigurations.DefaultDocuments && siteConfigurations.DefaultDocuments['a:string']) {
        siteConfigurations = {
          '$': siteConfigurations['$'],
          DefaultDocuments: getDefaultDocuments(siteConfigurations.DefaultDocuments)
        };

        log.table(siteConfigurations.DefaultDocuments['a:string'], function (row, item) {
          row.cell('Name', item);
        });
      } else {
  ...
  siteDefaultDocument.command('add [document] [name]')
    .usage('[options] [name]')
    .description('Add a site default document')
  ...
  siteDefaultDocument.command('delete [document] [name]')
    ...
    .execute(function (document, name, options, _) {
      ...
      if (!found) {
        throw new Error('Default document "' + document + '" does not exist.');
      }
    });
