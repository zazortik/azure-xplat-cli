...
var cagg = require('../util/callbackAggregator');
...
exports.init = function (cli) {
      ...
      if (!options.hostedService) {
        ...
        utils.doServiceManagementOperation(channel, 'listHostedServices', function(error, response) {
          ...
          if (!error) {
            ...
            progress = cli.interaction.progress($('Getting certificates'));
  ...
  certificates.command('create <dns-name> <file> [password]')
    ...
    .execute(function(dnsName, file, password, options, callback) {
      ...
      if (!password && password !== '') {
        cli.interaction.password($('Cert password: '), '*', function(pwd){
          password = pwd;
          process.stdin.pause();
          createCert();
        });
  ...
  certificates.command('delete <thumbprint>')
    ...
    var progress = cli.interaction.progress('Deleting certificate');
    if (options.dnsName) {
    ...
    utils.doServiceManagementOperation(channel, 'listHostedServices', function(error) {
      if (error) {
        callback(error);
        return;
      }
      ...
      for (var i = 0; i < hostedServices.length; i++) {
        ...
        logger.silly(util.format($('Trying DNS prefix: %s'), dnsPrefix));
