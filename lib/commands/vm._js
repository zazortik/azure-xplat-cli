  ...
  function createVM(options, cmdCallback) {
    ...
    if (options.role) {
      ...
      if (options.community) {
        var progress = cli.interaction.progress($('Looking up community image'));
        ...
        if (found) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', found.svc, found.deploy.Name, options.name, function(error, response) {
            if (!error) {
              ...
              var progress = cli.interaction.progress($('Updating network configuration'));
