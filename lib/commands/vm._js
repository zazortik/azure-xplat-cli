...
exports.init = function(cli) {
  ...
  var acl = endpoint.category('acl')
    .description($('Commands to manage your Virtual Machine endpoint access control list'));

  acl.command('list <vm-name> <endpoint-name>')
    .description($('List all acl rules defined for the given endpoint'))
    .option('-d, --dns-name <name>', $('only consider VM for this DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
  ...
  acl.command('create <vm-name> <endpoint-name> <order-id> <action> <remote-subnet>')
    .description($('Create a new acl rule for the given endpoint'))
    .option('-d, --dns-name <name>', $('only consider VM for this DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function(name, endpointName, orderId, action, remoteSubnet, options, callback) {
      var aclInfo = {};
      var endpointUtil = new EndPointUtil();

      var orderRes = endpointUtil.validateACLOrderID(orderId, 'order-id');
      if (orderRes.error) {
        return callback(orderRes.error);
      }

      aclInfo.Order = orderRes.orderID;
      var actionRes = endpointUtil.validateACLAction(action, 'action');
      if (actionRes.error) {
        return callback(actionRes.error);
      }

      aclInfo.Action = orderRes.action;
      var subnetRes = endpointUtil.validateACLRemoteSubnet(remoteSubnet, 'remote-subnet');
      if (subnetRes.error) {
        return callback(subnetRes.error);
      }

      aclInfo.RemoteSubnet = subnetRes.remoteSubnet;
  ...
  acl.command('delete <vm-name> <endpoint-name> <order-id>')
    .description($('Delete an existing acl rule for the given endpoint'))
    .option('-d, --dns-name <name>', $('only consider VM for this DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function(name, endpointName, orderId, options, callback) {
      var aclInfo = {};
      var endpointUtil = new EndPointUtil();
  ...
  acl.command('clear <vm-name> <endpoint-name>')
    .description($('Remove all acl rules defined for the given endpoint'))
    .option('-d, --dns-name <name>', $('only consider VM for this DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function(name, endpointName, options, callback) {
      var aclInfo = {};
  ...
  acl.command('update <vm-name> <endpoint-name> <order-id>')
    .description($('Update an existing acl rule for the given endpoint'))
    .option('-i, --order-id <orderid>', $('new order for the acl rule'))
    .option('-a, --action <action>', $('new action for the acl rule, valid values are [permit, deny]'))
    .option('-r, --remote-subnet <remotesubnet>', $('new subnet for the acl rule, it should be ipv4/cidr format'))
    .option('-d, --dns-name <name>', $('only consider VM for this DNS name'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function(name, endpointName, orderId, options, callback) {
      var aclInfo = {};
      ...
      if (options.orderId) {
        var newOrderRes = endpointUtil.validateACLOrderID(options.orderId, '--order-id');
        if (newOrderRes.error) {
          return callback(newOrderRes.error);
        }

        aclInfo.Order = newOrderRes.orderID;
      }

      if (options.action) {
        var actionRes = endpointUtil.validateACLAction(options.action, '--action');
        if (actionRes.error) {
          return callback(actionRes.error);
        }

        aclInfo.Action = actionRes.action;
      }

      if (options.remoteSubnet) {
        var subnetRes = endpointUtil.validateACLRemoteSubnet(options.remoteSubnet, '--remote-subnet');
        if (subnetRes.error) {
          return callback(subnetRes.error);
        }
      ...
      if (underscore.isEmpty(aclInfo)) {
        return callback($('One of the optional paramater --order-id, --action, --remote-subnet or --description is required'));
      }

      aclInfo.UpdateOrder = oderRes.orderID;
  ...
  function createVM(options, cmdCallback) {
    ...
    function lookupImage(imageName, callback) {
      ...
      progress = cli.interaction.progress($('Looking up image'));
    ...
    function verifyUserNameAndPwd(osName, callback) {
      ...
      if (utils.ignoreCaseEquals(osName, 'windows')) {
        ...
        if (typeof options.password === 'undefined') {
          interaction.password(cli, promptMsg, '*', function(password) {
            process.stdin.pause();
            ...
            if (error) {
              ...
              for (var n = 0; n < endpointCount; n++) {
                ...
                if (key in newEndPointsResult.protocolPorts) {
                  return cmdCallback(new Error(
                    util.format($('this VM already has an %s load balancer port %s, lb port and protocol together should be unique'),
                      endpoints[n].Protocol, endpoints[n].Port)));
                }

                var key = endpoints[n].Name.toLowerCase();
  ...
  function endpointAclCUD(options, cmdCallback) {
    var vmName = options.name.toLowerCase();
    var endPointName = options.endPointName.toLowerCase();
    var aclInfo = options.aclInfo;
    var operation = options.operation; // 1: create, 2: delete, 3: update, 4: clear
        ...
        if (result.error) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', result.deployment.svc,
            ...
            if (error) {
              ...
              var lbSet = configurationSets[m].InputEndpoints[i].LoadBalancedEndpointSetName;
              if (lbSet) {
                logger.info(util.format($('This is a load balanced endpoint belongs to lb set %s, use \'lbsettings acl\' command for manage acl for this lb set'), lbSet));
                return cmdCallback($('managing acl of load balanced endpoint requires using \'lbsettings acl\''));
              }

              if (operation == 4) {
                // clear
                configurationSets[m].InputEndpoints[i].EndpointAcl = null;
              } else {
                // create | delete | update
                var endpointAcl = configurationSets[m].InputEndpoints[i].EndpointAcl;
                if (!endpointAcl || !underscore.isArray(endpointAcl.Rules)) {
                  configurationSets[m].InputEndpoints[i].EndpointAcl = { Rules: [] };
                  endpointAcl = configurationSets[m].InputEndpoints[i].EndpointAcl;
                }

                var aclEntry = {};
                for (var k = 0; ((k < endpointAcl.Rules.length) && (endpointAcl.Rules[k].Order != aclInfo.Order)); k++);
                if (k == endpointAcl.Rules.length) {
                  // delete | update
                  if (operation == 2 || operation == 3) {
                    return cmdCallback(util.format($('acl entry with order id %s not found in the endpoint %s'),
                      aclInfo.Order, endPointName));
                  }

                  // create
                  aclEntry.Order = aclInfo.Order;
                  aclEntry.Action = aclInfo.Action;
                  aclEntry.RemoteSubnet = aclInfo.RemoteSubnet;
                  if (aclInfo.Description) {
                    aclEntry.Description = aclInfo.Description;
                  }
                  endpointAcl.Rules.push(aclEntry);
                } else {
                  if (operation == 1) {
                    // create
                    return cmdCallback(util.format($('acl entry with order id %s already exists in the endpoint %s'),
                      aclInfo.Order, endPointName));
                  }

                  if (operation == 2) {
                    // Delete
                    endpointAcl.Rules.splice(k, 1);
                  } else {
                    // Update
                    if (aclInfo.UpdateOrder) {
                      var orderUsed = underscore.find(endpointAcl.Rules, function(r) { return r.Order == aclInfo.UpdateOrder });
                      if (orderUsed && orderUsed.Order != aclInfo.Order) {
                        return cmdCallback(util.format($('acl entry with order id %s already exists in the endpoint %s'),
                          aclInfo.UpdateOrder, endPointName));
                      }
                    }

                    aclEntry = endpointAcl.Rules[k];
                    if (aclInfo.UpdateOrder) {
                      aclEntry.Order = aclInfo.UpdateOrder;
                    }
                    ...
                    endpointAcl.Rules[k] = aclEntry;
                  }
                }

                configurationSets[m].InputEndpoints[i].EndpointAcl = endpointAcl;
              }
  ...
  function endpointAclShowList(options, list, cmdCallback) {
    var vmName = options.name.toLowerCase();
    var endPointName = options.endPointName.toLowerCase();
        ...
        if (result.error) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', result.deployment.svc,
            ...
            if (error) {
              ...
              var endpointAcl = configurationSets[m].InputEndpoints[i].EndpointAcl;
              if (!endpointAcl || underscore.isEmpty(endpointAcl.Rules)) {
                if (logger.format().json) {
                  logger.json([]);
                } else {
                ...
                for (var n = 0; n < endpointAcl.Rules.length; n++) {
                  if (endpointAcl.Rules[n].Description) {
                    if(endpointAcl.Rules[n].Description > 24 && !logger.format().json) {
                      endpointAcl.Rules[n].Description = endpointAcl.Rules[n].Description.substring(0, 20) + '...';
                    }
                  } else {
                    endpointAcl.Rules[n].Description = '';
                  }
                }

                if (list) {
                  logger.table(endpointAcl.Rules, function(row, item) {
                    row.cell('Order', item.Order);
                    row.cell('Action', item.Action);
                    row.cell('RemoteSubnet', item.RemoteSubnet);
                    row.cell('Description', item.Description);
                  });
                } else {
                  if (logger.format().json) {
                    logger.json(endpointAcl);
                  } else {
                    utils.logLineFormat(endpointAcl, logger.data);
                  }
                }
                return cmdCallback();
              }
