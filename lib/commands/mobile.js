...
exports.init = function (cli) {
  ...
  function getSqlChannel(options) {
    ...
    var managementEndpoint = url.parse('https://management.database.windows.net:8443/');
  ...
  mobile.deleteDatabase = function (options, serverName, resource, callback) {
...
<Application xmlns="http://schemas.microsoft.com/windowsazure">
  ...
  mobile.command('create [servicename] [username] [password]')
        ...
        .option('-q, --sqlServer <sqlServer>', 'use existing SQL server')
        .option('-q, --sqlDb <sqlDb>', 'use existing SQL database')
        .option('-l, --location <location>', 'create the service in a particular location; default West US')
        .execute(function (servicename, username, password, options, callback) {     
          ...
          options.location = options.location || 'West US';
          ...
          function ensuredServiceName(servicename) {
            ...
            function promptUsername() {
              cli.prompt('Administrator user name: ', function (username) {
            ...
            function ensuredUsername(username) {
              ...
              function promptPassword() {
                cli.password('Administrator password: ', '*', function (password) {
  ...
  mobile.command('delete [servicename]')
        ...
        // .option('-b, --deleteSqlDb', 'delete all data and SQL database')
        ...
        .execute(function (servicename, username, password, options, callback) {
          ...
          function ensuredServiceName(servicename) {
            ...
            mobile.getMobileServiceApplication(options, function (error, result) {
              ...
              flat.Resources.forEach(function (resource) {
                if (!log.format().json) {
                  log.data(resource.TypeView, resource.NameView.green || 'N/A'.green);
                }
              ...
              function doProceed(decision) {
                ...
                //   plan.push({
                //     progress: 'Deleting SQL database',
                //     success: 'Deleted SQL database',
                //     failure: 'Failed to delete SQL database',
                //     handler: function (callback) {
                //       mobile.deleteDatabase(options, 
                //         resources['Microsoft.WindowsAzure.SQLAzure.Server'].Name,
                //         resources['Microsoft.WindowsAzure.SQLAzure.DataBase'], 
                //         callback);
                //     }
                //   });                  
                // }
            ...
            function tryFinish() {
              ...
              if (log.format().json) {
                ...
                if (results.application) {
                  ...
                  flat.Resources.forEach(function (resource) {
                    log.data(resource.TypeView + ' name', resource.NameView.green || 'N/A'.green);
                    if (resource.Error) {
