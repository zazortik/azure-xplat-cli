...
exports.init = function (cli) {
  ...
  function createCluster(creationObject, options, callback) {
    var errors = 0;
    ...
    hdInsight.createCluster(creationObject, function(err, result) {
      if (result.statusCode === 200 || result.statusCode === 202) {
        ...
        doPollRequest(function(callback) {
          ...
          if (!response || ! response.body || !response.body.CloudServices || !response.body.CloudServices.CloudService) {
            errors++;
            return true;
          }

          var results = transformResponse(response.body.CloudServices.CloudService);
  ...
  // NOTE: This code will move to the SDK project (as a function of HDInsight service) 
  // with our next PR.
  function transformResponse(cloudService) {
    if (!__.isArray(cloudService)) {
        ...
        if (cloudService[i].Resources && cloudService[i].Resources.Resource) {
          ...
          for (var j = 0; j < resource.length; j++) {
            if (resource[j].ResourceProviderNamespace == 'hdinsight') {
              ...
              if (resource[j].OutputItems) {
                if (!__.isArray(resource[j].OutputItems.OutputItem)) {
                  resource[j].OutputItems.OutputItem = [resource[j].OutputItems.OutputItem];
                }
              ...
              if (resource[j].OperationStatus.Error) {
                cluster.Error = resource[j].OperationStatus.Error.Message;
              }
  ...
  function listClusters(subscriptionId, _) {
    ...
    return transformResponse(cloudService);
  }
  ...
  cluster.command('create [clusterName] [storageAccountName] [storageAccountKey] [storageContainer] [nodes] [location] [username] [clusterPassword]')
    ...
    .execute(function (clusterName, storageAccountName, storageAccountKey, storageContainer, nodes, location, username, clusterPassword, options, _) {
      ...
      var requestSubmited = createCluster(creationObject, options, _);
