...
exports.init = function(cli) {
  ...
  */
  ...
  task.command('create [jobId] [json-file]')
    ...
    .option('-f, --json-file <json-file>', $('the file containing the single task or multiple tasks object to create in JSON format'))
  ...
  task.command('show [jobId] [taskId]')
    ...
    .option('--subtasks', $('to show the subtasks information'))
  ...
  task.command('set [jobId] [taskId] [json-file]')
    ...
    .option('-f, --json-file <json-file>', $('the file containing the updated task properties to apply in JSON format'))
    .appendCommonHeaderFilterOption(true, true)
    .appendBatchAccountOption()
    .execute(updateTask);
    ...
    if (parsedResponse !== null && parsedResponse !== undefined) {
      ...
      if (!addTask || Object.keys(addTask).length === 0 ) {
        if (parsedResponse.length > 100) {
          throw new Error($('Too much tasks to add. Max task number in single request is 100.'))
        }
    ...
    if (multiTasks) {
      ...
      cli.interaction.formatOutput(result.value, function (outputData) {
        if (outputData.length === 0) {
          logger.info($('No creating task result found'));
        } else {
  ...
  */
  function showTask(jobId, taskId, options, _) {
    ...
    var batchOptions1 = {};
    batchOptions1.taskListSubtasksOptions = batchUtil.getBatchOperationDefaultOption();

    if (options.selectClause) {
      batchOptions1.taskGetOptions.select = options.selectClause;
    }
    ...
    try {
      ...
      if (options.subtasks) {
        subTasks = client.task.listSubtasks(jobId, taskId, batchOptions1, _);
      }
    } catch (e) {
