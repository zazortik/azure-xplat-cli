...
exports.init = function(cli) {
  ...
  */
  ...
  task.command('create [jobId] [json-file]')
    ...
    .option('-es, --environment-settings <environment-settings>', $('the semicolon separated list of environment variable settings for the task, ex: name1=value1;name2=value2'))
    .option('-rf, --resources-files <resources-files>', $('the semicolon separated list of files that Batch will download to the compute node before running the command line, ex: blob1=file1;blob2=file2'))
  ...
  */
  ...
  function updateTask(jobId, taskId, jsonFile, options, _) {
    ...
    if (!options.maxWallClockTime && !options.maxTaskRetryCount && !options.retentionTime) {
      throw new Error($('You must specify at least one of the folowing: --max-wall-clock-time, --max-task-retry-count and --retention-time.'));
    }
    var constraint = {};
    if (options.maxWallClockTime) {
      __.extend(constraint, { 'maxWallClockTime' : options.maxWallClockTime });
