...
exports.init = function(cli) {
  ...
  */
  ...
  jobSchedule.command('create [json-file]')
    .description($('Create a batch job schedule'))
  ...
  jobSchedule.command('delete [jobScheduleId]')
    .description($('Delete the specified batch job schedule'))
    .option('--id <jobScheduleId>', $('the batch job schedule id'))
    .option('-q, --quiet', $('remove the specified batch job schedule without confirmation'))
    .appendCommonHeaderFilterOption(true, true)
    .appendBatchAccountOption()
    .execute(deleteJobSchedule);

  jobSchedule.command('set [jobScheduleId] [json-file]')
    .description($('Update the batch job schedule'))
  ...
  */
  function deleteJobSchedule(jobScheduleId, options, _) {
    ...
    try {
      ...
      if (batchUtil.isNotFoundException(err)) {
        throw new Error(util.format($('Job schedule %s doesn\'t exist'), jobScheduleId));
      } else {
        if (err.message) {
          if (typeof err.message === 'object') {
            err.message = err.message.value;
  ...
  function updateJobSchedule(jobScheduleId, jsonFile, options, _) {
    ...
    if (options.patch) {
      if (parsedResponse !== null && parsedResponse !== undefined) {
        var resultMapper = new client.models['JobSchedulePatchParameter']().mapper();
        updateJobScheduleParam = client.deserialize(resultMapper, parsedResponse, 'result');
      }

      var tips = util.format($('Patching job schedule %s'), jobScheduleId);
      ...
      if (parsedResponse !== null && parsedResponse !== undefined) {
        var resultMapper = new client.models['JobScheduleUpdateParameter']().mapper();
        updateJobScheduleParam = client.deserialize(resultMapper, parsedResponse, 'result');
      }

      var tips = util.format($('Updating job schedule %s'), jobScheduleId);
