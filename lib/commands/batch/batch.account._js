...
var batchUtil = require('../../util/storage.util');
...
exports.init = function (cli) {
  ...
  function createBatchManagementClient(subscriptionOrName) {
    var client;
  ...
  function showAccount(serviceClient, accountName, options, _) {
    ...
    try {
      ...
      batchAccount = serviceClient.batchAccounts.getProperties(options.resourceGroup, accountName, _);
    } finally {
      endProgress(progress);
    }
  ...
  function createAccount(serviceClient, parameters, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      batchAccount = serviceClient.batchAccounts.create(options.resourceGroup, parameters.name, parameters, _);
    } finally {
      endProgress(progress);
    }
  ...
  function getAccountKeys(serviceClient, accountName, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      keys = serviceClient.batchAccounts.listKeys(options.resourceGroup, accountName, _);
    } finally {
      endProgress(progress);
    }
  ...
  function regenerateAccountKeys(serviceClient, accountName, options, _) {
    ...
    try {
      ...
      keys = serviceClient.batchAccounts.regenerateKey(options.resourceGroup, accountName, keyType, _);
    } finally {
      endProgress(progress);
    }
  ...
  batchAccount.listCommand = function (options, _) {
    var service = createBatchManagementClient(options.subscription);
  ...
  usage.usageCommand = function (subscription, options, _) {
    var service = createBatchManagementClient(subscription);
