...
exports.init = function (cli) {
  ...
  function listAccounts(serviceClient, options, _) {
    ...
    try {
      if(options.resourceGroup){
        var response = serviceClient.account.listByResourceGroup(options.resourceGroup, _);
        batchAccounts = response;

        while(response.nextLink) {
          response = serviceClient.account.listByResourceGroup(options.resourceGroup, _);
          batchAccounts.concat(response);
        }
      }
      else {
        var response = serviceClient.account.list(_);
        batchAccounts = response;

        while(response.nextLink) {
          response = serviceClient.account.list(_);
          batchAccounts.concat(response);
        }
      }
    } finally {
  ...
  function createAccount(serviceClient, accountName, parameters, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      batchAccount = serviceClient.account.create(options.resourceGroup, accountName, parameters, _);
    } finally {
      endProgress(progress);
    }
  ...
  function setAccount(serviceClient, accountName, parameters, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      batchAccount = serviceClient.account.update(options.resourceGroup, accountName, parameters, _);
    } finally {
      endProgress(progress);
    }
  ...
  function deleteAccount(serviceClient, accountName, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      serviceClient.account.deleteMethod(options.resourceGroup, accountName, _);
    }
    catch (err) {
  ...
  function syncAutoStorageKeys(serviceClient, accountName, options, _) {
    ...
    try {
      validateResourceGroupName(options, _);
      progress = showProgress(message);
      serviceClient.account.syncAutoStorageKeys(options.resourceGroup, accountName, _);
    } finally {
      endProgress(progress);
    }
  }
  ...
  batchAccount.showCommand = function (name, options, _) {
    ...
    if (batchAccount) {
      ...
      cli.interaction.formatOutput(batchAccount, function(outputData) {
        logger.data($('Name:'), outputData.name);
        logger.data($('Url:'), outputData.id);
        logger.data($('Resource Group:'), outputData.resourceGroup);
        logger.data($('Location:'), outputData.location);
        logger.data($('Endpoint:'), outputData.properties.accountEndpoint);
        logger.data($('Provisioning State:'), outputData.properties.provisioningState);
        logger.data($('Core Quota:'), outputData.properties.coreQuota);
        logger.data($('Pool Quota:'), outputData.properties.poolQuota);
        logger.data($('Active Job and Job Schedule Quota:'), outputData.properties.activeJobAndJobScheduleQuota);
        if (outputData.tags) {
          cli.interaction.logEachData($('Tags:'), outputData.tags);
        }
        if (outputData.properties.autoStorage) {
          logger.data($('Auto Storage:'));
          logger.data($('  Account Id:'), outputData.properties.autoStorage.storageAccountId);
          logger.data($('  Last Key Sync:'), outputData.properties.autoStorage.lastKeySync.toString());
        }
      });
    } else {
  ...
  batchAccount.createCommand = function (name, options, _) {
    ...
    if (options.autostorageAccountId) {
      batchCreateParameters.properties = {
        autoStorage: { 
  ...
  batchAccount.setCommand = function (name, options, _) {
    var service = batchUtil.createBatchManagementClient(options.subscription);
    var managementService = utils.createManagementClient(profile.current.getSubscription(options.subscription), logger);
    ...
    if (options.autostorageAccountId) {
      batchUpdateParameters.properties = {
        autoStorage: { 
          storageAccountId: options.autostorageAccountId, 
        },
      };
  ...
  batchAccount.command('create <name>').description($('Create a batch account'))
  ...
  batchAccount.command('set <name>').description($('Update a batch account'))
    .option('--tags <tags>', $('the account tags. Tags are key=value pairs and separated with semicolon(;)'))
    .option('--autostorage-account-id <autostorageAccountId>', $('the resource id of the storage account to be used for auto storage'))
    .appendSubscriptionAndResourceGroupOption()
  ...
  batchAccount.command('sync-autostorage-keys <name>')
    .description($('Synchronizes access keys for the auto storage account configured for the batch account'))
    .appendSubscriptionAndResourceGroupOption()
