...
exports.init = function(cli) {
  ...
  */
  ...
  job.command('show [jobId]')
    .description($('Show details of the batch job'))
    .option('--id <jobId>', $('the batch job id'))
    .appendODataFilterOption(true, false, true)
    .appendBatchAccountOption()
    .execute(showJob);

  job.command('delete [jobId]')
    .description($('Delete the specified batch job'))
    .option('--id <jobId>', $('the batch job id'))
    .option('-q, --quiet', $('remove the specified batch job without confirmation'))
    .appendCommonHeaderFilterOption(true, true)
    .appendBatchAccountOption()
    .execute(deleteJob);

  job.command('set [jobId] [json-file]')
    .description($('Update the batch job'))
  ...
  */
  function createJob(jsonFile, options, _) {
    ...
    logger.verbose(util.format($('Job schedule %s has been created successfully'), addJob.id));
    showJob(addJob.id, options, _);
  }

  /**
  * Show the details of the specified Batch job schedule
  * @param {string} [jobId] job schedule id
  * @param {object} options command line options
  * @param {callback} _ callback function
  */
  function showJob(jobId, options, _) {
    ...
    jobId = interaction.promptIfNotGiven($('Job schedule id: '), jobId, _);
  ...
  */
  ...
  function updateJob(jobId, jsonFile, options, _) {
    ...
    if (options.patch) {
      if (parsedResponse !== null && parsedResponse !== undefined) {
        var resultMapper = new client.models['JobPatchParameter']().mapper();
        updateJobParam = client.deserialize(resultMapper, parsedResponse, 'result');
      }

      var tips = util.format($('Patching job %s'), jobId);
      ...
      if (parsedResponse !== null && parsedResponse !== undefined) {
        var resultMapper = new client.models['JobUpdateParameter']().mapper();
        updateJobParam = client.deserialize(resultMapper, parsedResponse, 'result');
      }

      var tips = util.format($('Updating job %s'), jobId);
