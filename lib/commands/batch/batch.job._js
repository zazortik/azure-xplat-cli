...
exports.init = function(cli) {
  ...
  */
  ...
  var jobPrepAndReleaseStatus = job.category('prep-and-release-task-status').description($('Commands to manage the status of your job preparation and release tasks'));
  ...
  job.command('disable [jobId]')
    ...
    .option('-i, --id <jobId>', $('the id of the job'))
    .option('--disable-option <disableOption>', $('specifies what to do with active tasks during a disable job operation, available options include "requeue", "terminate", and "wait"'))
    .appendCommonHeaderFilterOption(true, true)
    .appendBatchAccountOption()
    .execute(disableJob);

  job.command('terminate [jobId]')
    .description($('Terminates the specified job, marking it as completed'))
    .option('-i, --id <jobId>', $('the id of the job'))
    .option('--terminate-reason [terminateReason]', $('the text you want to appear as the job\'s terminate reason'))
    .appendCommonHeaderFilterOption(true, true)
    .appendBatchAccountOption()
