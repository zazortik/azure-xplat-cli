...
batchUtil.init = function (azureCli) {
  ...
  Object.getPrototypeOf(batch).appendCommonHeaderFilterOption = function (eTag, lastModified) {
    if (eTag) {
      this.option('--if-match <if-match>', $("only perfome if resource's ETag is an exact match as specified"));
      this.option('--if-none-match <if-none-match>', $("only perfome if resource's ETag does not match as specified"));
    }

    if (lastModified) {
      this.option('--if-modified-since <if-modified-since>', $('only perfome if the resource has been modified since the specified time'));
      this.option('--if-unmodified-since <if-unmodified-since>', $('only perfome if the resource has not been modified since the specified time'));
    }
...
batchUtil.isNotFoundException = function (e) {
  var notFoundErrors = ['NotFound', 'ResourceNotFound', 'PoolNotFound', 'NodeNotFound', 'JobNotFound', 'TaskNotFound'];
