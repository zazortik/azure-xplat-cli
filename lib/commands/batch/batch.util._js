...
*/
batchUtil.ENV_SDK_ACCOUNT_ENDPOINT = 'AZURE_BATCH_URI';
...
batchUtil.isNotFoundException = function (e) {
  var notFoundErrors = ['NotFound', 'ResourceNotFound'];
...
batchUtil.createBatchServiceClient = function(options) {
  ...
  if (!accountName || !accountKey || !accountEndpoint) {
    throw new Error($('Please specify a Batch account name, access key, and endpoint URL in one of the two following ways:' +
      ...
      '\n 2. Set the AZURE_BATCH_ACCOUNT, AZURE_BATCH_ACCESS_KEY, and AZURE_BATCH_URI environment variables.'));
  }

  client = utils.createBatchClient(options.accountName, options.accountKey, options.accountEndpoint);
  return client;
};
