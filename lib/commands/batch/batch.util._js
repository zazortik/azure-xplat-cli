...
batchUtil.init = function (azureCli) {
  ...
  Object.getPrototypeOf(batch).appendBatchAccountOption = function () {
    ...
    this.option('-u, --account-endpoint <endpointUrlString>', $('the Batch account URL'));
    return this;
  };

  Object.getPrototypeOf(batch).appendODataFilterOption = function (select, filter, expand) {
    if (select) {
      this.option('--select-clause <selectClause>', $('list of a subset of properties to select'));
    }
    
    if (filter) {
      this.option('--filter-clause <filterClause>', $('an OData expression by which the results will be filtered'));
    }
    
    if (expand) {
      this.option('--expand-clause <expandClause>', $('list of related entites by which the results can be expanded'));
    }
    return this;
  };
