...
exports.init = function (cli) {
  ...
  .execute(function (name, resourceGroup, location, options, _) {
    ...
    if (options.maxMemoryPolicy) {
      ...
      parameters.redisConfiguration = {'maxmemory-policy' : options.maxMemoryPolicy};
    }
    ...
    cli.interaction.formatOutput(result, function (data) {
      if (!data) {
        ...
        log.data($('Max Memory Policy  :'), result.redisConfiguration['maxmemory-policy']);
  ...
  .execute(function (options, _) {
    ...
    var operation = client.redis.list(options.resourceGroup, _);
    ...
    if (result.length === 0) {
      ...
      log.table(result, function (row, item) {
        ...
        row.cell($('Max Memory Policy'), item.redisConfiguration['maxmemory-policy']);
  ...
  .execute(function (name, resourceGroup, maxMemoryPolicy, options, _) {
    ...
    cli.interaction.formatOutput(updateResult, function (data) {
      if (!data) {
        ...
        log.data($('Max Memory Policy  :'), updateResult.redisConfiguration['maxmemory-policy']);
