...
_showFrontendIpDetails = function (ipConfig, output, indent) {
  ...
  if (ipConfig.publicIpAddress) {
    ...
    output.nameValue('Public IP allocation method', ipConfig.publicIpAddress.publicIpAllocationMethod, indent);
    if (ipConfig.publicIpAddress.actualPublicIpAddress) {
      output.nameValue($('Public IP address'), ipConfig.publicIpAddress.actualPublicIpAddress, indent);
    }
    if (ipConfig.publicIpAddress.fqdn) {
      output.nameValue($('FQDN'), ipConfig.publicIpAddress.fqdn, indent);
    }
  }

  if (ipConfig.privateIpAddress) {
    output.nameValue($('Private IP address'), ipConfig.privateIpAddress, indent);
  }
  if (ipConfig.subnet) {
