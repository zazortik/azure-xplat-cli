    ...
    interaction.formatOutput(dnsRecord.recordSet, function (record) {
      ...
      if (!__.isEmpty(record.properties.aRecords)) {
        ...
        for (var aRecordNum in record.properties.aRecords) {
          ...
          output.header($('Record ' + aRecordNumToShow), 2);
      ...
      if (!__.isEmpty(record.properties.aaaaRecords)) {
        ...
        for (var aaaaRecordNum in record.properties.aaaaRecords) {
          ...
          output.header($('Record ' + aaaaRecordNumToShow), 2);
      ...
      if (!__.isEmpty(record.properties.mxRecords)) {
        ...
        for (var mxRecordNum in record.properties.mxRecords) {
          ...
          output.header($('Record' + mxRecordNumToShow), 2);
          output.nameValue($('Preference'), mxRecord.preference, 4);
          output.nameValue($('Exchange'), mxRecord.exchange, 4);
        }
        output.data($(''), '');
      }
      ...
      if (!__.isEmpty(record.properties.nsRecords)) {
        ...
        for (var nsRecordNum in record.properties.nsRecords) {
          ...
          output.header($('Record ' + nsRecordNumToShow), 2);
          output.nameValue($('Domain name'), nsRecord.nsdname, 4);
        }
        output.data($(''), '');
      }
      ...
      if (!__.isEmpty(record.properties.srvRecords)) {
        ...
        for (var srvRecordNum in record.properties.srvRecords) {
          ...
          output.header($('Record ' + srvRecordNumToShow), 2);
      ...
      if (!__.isEmpty(record.properties.txtRecords)) {
        ...
        for (var txtRecordNum in record.properties.txtRecords) {
          ...
          output.header($('Record ' + txtRecordNumToShow), 2);
      ...
      if (!__.isEmpty(record.properties.soaRecord)) {
        ...
        output.nameValue($('Email'), record.properties.soaRecord.email, 2);
        output.nameValue($('Expire time'), record.properties.soaRecord.expireTime, 2);
        output.nameValue($('Host'), record.properties.soaRecord.host, 2);
        output.nameValue($('Serial number'), record.properties.soaRecord.serialNumber, 2);
        output.nameValue($('Minimum TTL'), record.properties.soaRecord.minimumTtl, 2);
        output.nameValue($('Refresh time'), record.properties.soaRecord.refreshTime, 2);
        output.nameValue($('Retry time'), record.properties.soaRecord.retryTime, 2);
        output.nameValue($(''), '');
      }
      ...
      if (!__.isEmpty(record.properties.ptrRecords)) {
        ...
        for (var ptrRecordNum in record.properties.ptrRecords) {
          ...
          var ptrRecordNumToShow = parseInt(ptrRecordNum) + 1;
          output.header($('Record ' + ptrRecordNumToShow), 2);
          output.nameValue($('Ptrd name'), ptrRecord.ptrdname, 4);
        }
        output.data($(''), '');
      }
    });
