...
__.extend(LoadBalancerCRUD.prototype, {
  ...
  addLBRule: function (resourceGroupName, lbName, ruleName, options, _) {
    ...
    var lbRule = utils.findFirstCaseIgnore(lb.properties.loadBalancingRules, {name: ruleName});
    if (lbRule) {
    ...
    endPointUtil.validatePort(options.frontendport, 'front end port');
    if (endPointUtil.error) {
      throw new Error(endPointUtil.error);
    }

    endPointUtil.validatePort(options.backendport, 'back end port');
    if (endPointUtil.error) {
      throw new Error(endPointUtil.error);
    }
    ...
    // probes must exist
    if (!lb.properties.probes || lb.properties.probes.length === 0) {
      throw new Error(util.format($('No probes found for the load balancer "%s"'), lbName));
    }

    // probe with provided name must exist
    var probe = utils.findFirstCaseIgnore(lb.properties.probes, {name: options.probName});
    if (!probe) {
      throw new Error(util.format($('Probe "%s" not found in the load balancer "%s"'), options.probName, lbName));
    }

    var backEndPool = lb.properties.backendAddressPools[0];
    var frontendIPConfiguration = lb.properties.frontendIpConfigurations[0];
    ...
    lb.properties.loadBalancingRules.push(rule);
  ...
  updateLBRule: function (resourceGroupName, lbName, ruleName, options, _) {
    ...
    var endPointUtil = new EndPointUtil();
    if (options.protocol) {
      endPointUtil.validateProtocol(options.protocol, 'load balancing rule protocol');
      if (endPointUtil.error) {
        throw new Error(endPointUtil.error);
      }

      lbRule.properties.protocol = options.protocol;
    }

    if (options.frontendport) {
      endPointUtil.validatePort(options.frontendport, 'front end protocol');
      if (endPointUtil.error) {
        throw new Error(endPointUtil.error);
      }

      lbRule.properties.frontendPort = options.frontendport;
    }
    ...
    if (options.backendport) {
      ...
      lbRule.properties.backendPort = options.backendport;
    }
    ...
    if (options.idletimeout) {
      ...
      lbRule.properties.idleTimeoutInMinutes = options.idletimeout;
    }
    ...
    if (options.enableFloatingip) {
      ...
      lbRule.properties.enableFloatingIP = options.enableFloatingip;
    }
    ...
    if (options.probName) {
      ...
      lbRule.properties.probe.id = probe.id;
    }
  ...
  deleteLBRule: function (resourceGroupName, lbName, ruleName, options, _) {
    ...
    // check if rule exists
    var ruleIndex = utils.indexOfCaseIgnore(lb.properties.loadBalancingRules, {name: ruleName});
    if (!ruleIndex) {
    ...
    lb.properties.loadBalancingRules.splice(ruleIndex, 1);
    this.update(resourceGroupName, lbName, lb, _);
  },
  ...
  addFrontEndIPConfig: function (resourceGroupName, lbName, ipConfigName, options, _) {
    ...
    if (options.publicipName) {
      var publicipCRUD = new PublicIPCRUD(this.cli, this.networkResourceProviderClient);
      var publicip = publicipCRUD.get(resourceGroupName, options.publicipName, _);
      if (!publicip) {
  ...
  addBackendAddressPool: function (resourceGroupName, lbName, poolName, options, _) {
    ...
    if (options.nicName) {
      var nicCRUD = new NicCRUD(this.cli, this.networkResourceProviderClient);
      var nic = nicCRUD.get(resourceGroupName, options.nicName, _);
      if (!nic) {
  ...
  _tryValidateAndAddPrivateIp: function (resourceGroupName, frontendIPConfig, virtualNetwork, subnet, privateipAddress, _) {
    if (subnet || privateipAddress || virtualNetwork) {
      ...
      // subnet
      var subnetCRUD = new SubnetCRUD(this.cli, this.networkResourceProviderClient);
      var vnetCRUD = new VNetCRUD(this.cli, this.networkResourceProviderClient);
      var vnetObj = vnetCRUD.get(resourceGroupName, virtualNetwork, _);

      if (!vnetObj) {
        throw new Error(util.format($('Virtual network "%s" not found'), virtualNetwork));
      }

      var subnetObj = subnetCRUD.get(resourceGroupName, virtualNetwork, subnet, _);
