...
__.extend(DnsZone.prototype, {
  ...
  setRecordSet: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    if (!existingSet) {
      throw new Error(util.format($('A record set with name "%s" of type "%s" not found in the zone "%s"'),
        setName, options.type, zoneName));
    }
  ...
  deleteRecordSet: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    if (result.code === 204) {
      throw new Error(util.format($('DNS Record set "%s" of type "%s" not found in the zone "%s"'), setName, options.type, zoneName));
    }
  },
  ...
  addRecord: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    var existingSet = self.getRecordSet(resourceGroupName, zoneName, setName, options.type, _);
    if (!existingSet) {
      throw new Error(util.format($('A record set with name "%s" of type "%s" not found in the DNS zone with name "%s"'),
        setName, options.type, zoneName));
    }
  ...
  deleteRecord: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    var existingSet = self.getRecordSet(resourceGroupName, zoneName, setName, options.type, _);
    if (!existingSet) {
      throw new Error(util.format($('A record set with name "%s" of type "%s" not found in the zone "%s"'), setName,
        options.type, zoneName));
    }
  ...
  updateRecordSet: function (resourceGroupName, zoneName, setName, setType, parameters, _) {
    ...
    // TODO: remove this workaround after azure-arm-dns package update.
    if(!parameters.recordSet.location) {
      parameters.recordSet.location = constants.dnsZone.defLocation;
    }
    try {
  ...
  _handleRecordSetOptions: function (recordSet, options, useDefaults) {
    ...
    // TODO: remove this workaround after azure-arm-dns version update.
