...
__.extend(DnsZone.prototype, {
    ...
    recordSets.recordSets.forEach(function (item) {
      ...
      if (!__.isEmpty(item.properties.aRecords)) {
        for (var aRecordNum in item.properties.aRecords) {
          ...
          fileData += util.format($('%s IN %s %s \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], aRecord.ipv4Address);
          indent = name.length;
        }
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.aaaaRecords)) {
        for (var aaaaRecordNum in item.properties.aaaaRecords) {
          ...
          fileData += util.format($('%s IN %s %s \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], aaaaRecord.ipv6Address);
          indent = name.length;
        }
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.cnameRecord)) {
        fileData += util.format($('%s IN %s %s \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], item.properties.cnameRecord.cname);
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.mxRecords)) {
        for (var mxRecordNum in item.properties.mxRecords) {
          ...
          fileData += util.format($('%s IN %s %s %s\n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], mxRecord.preference, mxRecord.exchange);
          indent = name.length;
        }
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.nsRecords)) {
        for (var nsRecordNum in item.properties.nsRecords) {
          ...
          fileData += util.format($('%s IN %s %s \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], nsRecord.nsdname);
          indent = name.length;
        }
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.srvRecords)) {
        for (var srvRecordNum in item.properties.srvRecords) {
          ...
          fileData += util.format($('%s IN %s %s %s %s %s\n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex],
      ...
      if (!__.isEmpty(item.properties.txtRecords)) {
        for (var txtRecordNum in item.properties.txtRecords) {
          ...
          fileData += util.format($('%s IN %s "%s" \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], txtRecord.value);
          indent = name.length;
        }
      }
      typeIndex++;
      ...
      if (!__.isEmpty(item.properties.soaRecord)) {
        ...
        fileData += util.format($('%s IN %s '), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex]);
      ...
      if (!__.isEmpty(item.properties.ptrRecords)) {
        for (var ptrRecordNum in item.properties.ptrRecords) {
          ...
          fileData += util.format($('%s IN %s %s \n'), self._setIndent(indent), constants.dnsZone.recordTypes[typeIndex], ptrRecord.ptrdname);
          indent = name.length;
        }
      }
      fileData += '\n';
  ...
  _getDefaultSerialNumber: function() {
    ...
    return util.format($('%s%s%s%s'), now.getFullYear(), now.getMonth(), now.getMinutes(), defaultNumber);
  },

  _setIndent: function(num) {
    if (num > 0) {
