...
__.extend(DnsZone.prototype, {
      ...
      if (outputData.length === 0) {
        ...
        formattedRecordSetList.forEach(function (recordSet) {
          ...
          if (recordSet.records && recordSet.records.length > 0) {
            ...
            self.output.list(recordSet.records, 2);
          }
          if(recordSet.record) {
  ...
  addRecord: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    var recordSet = self.getRecordSet(resourceGroupName, zoneName, setName, options.type, _);
    if (recordSet) {
      if (options.type === constants.dnsZone.CNAME && !self.interaction.confirm(
          util.format($('The DNS record set with type "%s" permits only one record per record set.' +
            'Do you want to replace the existing "%s" record value to new "%s"? [y/n]'), options.type, recordSet.cnameRecord.cname, options.cname), _)) {
        return;
      }
    } else {
