...
var VNetUtil = require('../../asm/network/vnetUtil');
...
__.extend(VNetCRUD.prototype, {
    create: function(resourceGroupName, vnet, location, options, _) {
        try {
            var vNetExists = this.networkResourceProviderClient.virtualNetworks.get(resourceGroupName, vnet, _);
        } catch (e) {
        ...
        if (!location) {
            throw new Error($('Either --location must be present'));
        }
    ...
    delete: function(resourceGroupName, virtualNetworkName, _) {
        var progressGet = this.cli.interaction.progress(util.format($('Checking if virtual network "%s" exists'), virtualNetworkName));
        try {
            var vNet = this.networkResourceProviderClient.virtualNetworks.get(resourceGroupName, virtualNetworkName, _);
        } catch (e) {
            progressGet.end();
            if (e.code === 'ResourceNotFound'){
    ...
    show: function (resourceGroupName, virtualNetworkName, _) {
        var progress = this.cli.interaction.progress(util.format($('Getting virtual network "%s"'), virtualNetworkName));
        try {
            var vNet = this.networkResourceProviderClient.virtualNetworks.get(resourceGroupName, virtualNetworkName, _);
        } catch (e) {
            progress.end();
    ...
    list: function(resourceGroupName, _){
        ...
        try {
            var vNets = this.networkResourceProviderClient.virtualNetworks.list(resourceGroupName, _);
        } finally {
            progress.end();
        }
