...
__.extend(Nsg.prototype, {
  ...
  set: function (resourceGroupName, nsgName, options, _) {
    ...
    try {
      self.networkResourceProviderClient.networkSecurityGroups.createOrUpdate(resourceGroupName, nsgName, nsg.networkSecurityGroup, _);
    } finally {
      progress.end();
    }
    self.show(resourceGroupName, nsgName, options, _);
  ...
  show: function (resourceGroupName, nsgName, options, _) {
    ...
    if (nsg) {
      ...
      self.interaction.formatOutput(nsg.networkSecurityGroup, function (nsg) {
