...
__.extend(Nsg.prototype, {
  ...
  _parseAndValidateSecurityRule: function (params, useDefaults) {
    ...
    if (params.protocol) {
      ...
      nsgProfile.protocol = this.DEFAULT_PROTOCOL;
    }

    if (params.sourcePortRange) {
      nsgProfile.sourcePortRange = this._parsePortRange(params.sourcePortRange);
    } else if (useDefaults) {
      nsgProfile.sourcePortRange = this.DEFAULT_SOURCE_PORT;
    }

    if (params.destinationPortRange) {
      nsgProfile.destinationPortRange = this._parsePortRange(params.destinationPortRange);
    } else if (useDefaults) {
      nsgProfile.destinationPortRange = this.DEFAULT_DESTINATION_PORT;
    }

    if (params.sourceAddressPrefix) {
      nsgProfile.sourceAddressPrefix = this._parseAddressPrefixes(params.sourceAddressPrefix, vNetUtil);
    }
    else if (useDefaults) {
      nsgProfile.sourceAddressPrefix = this.DEFAULT_SOURCE_ADDRESS_PREFIX;
    }

    if (params.destinationAddressPrefix) {
      nsgProfile.destinationAddressPrefix = this._parseAddressPrefixes(params.destinationAddressPrefix, vNetUtil);
    } else if (useDefaults) {
      nsgProfile.destinationAddressPrefix = this.DEFAULT_DESTINATION_ADDRESS_PREFIX;
    }
    ...
    if (params.access) {
      ...
      nsgProfile.access = this.DEFAULT_ACCESS;
    }
    ...
    if (params.priority) {
      ...
      nsgProfile.priority = this.DEFAULT_PRIORITY;
    }
    ...
    if (params.direction) {
      ...
      nsgProfile.direction = this.DEFAULT_DIRECTION;
    }

    return nsgProfile;
  },

  _parsePortRange: function (port) {
    if (port === '*') {
      return port;
    }
    ...
    if (isNaN(port) || port < 0 || port > 65535) {
      throw new Error($('port parameter must be an integer between 0 and 65535'));
    }

    return port;
  },

  _parseAddressPrefixes: function (ipInCidrFormat, vNetUtil) {
    if (utils.stringIsNullOrEmpty(ipInCidrFormat)) {
      throw new Error($('IPv4 address must not be null or empty string'));
    }
    ...
    if (ipValidationResult.error || ipValidationResult.cidr === null) {
      throw new Error($('IPv4 address must be in CIDR format. Asterix can also be used'));
    }
