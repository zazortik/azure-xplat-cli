  ...
  show: function (resourceGroupName, lbName, params, _) {
    ...
    if (lb) {
      ...
      if (!__.isEmpty(lb.loadBalancer.frontendIpConfigurations)) {
        ...
        for (var i in lb.loadBalancer.frontendIpConfigurations) {
          var frontendIpConfg = lb.loadBalancer.frontendIpConfigurations[i];
          if (frontendIpConfg.publicIpAddress) {
            var uriParts = utils.parseResourceReferenceUri(frontendIpConfg.publicIpAddress.id);
            var publicip = publicIPcrud.get(uriParts.resourceGroupName, uriParts.resourceName, _);
            frontendIpConfg.publicIpAddress.publicIpAllocationMethod = publicip.publicIpAddress.publicIpAllocationMethod;
            if (publicip.publicIpAddress.dnsSettings && publicip.publicIpAddress.dnsSettings.fqdn) {
              frontendIpConfg.publicIpAddress.fqdn = publicip.publicIpAddress.dnsSettings.fqdn;
            }
            if (publicip.publicIpAddress.ipAddress) {
              frontendIpConfg.publicIpAddress.actualPublicIpAddress = publicip.publicIpAddress.ipAddress;
            }
          }
        }
      }
      ...
      interaction.formatOutput(lb.loadBalancer, function (lb) {
        ...
        if (!__.isEmpty(lb.frontendIpConfigurations)) {
          ...
          for (var i in lb.frontendIpConfigurations) {
        ...
        if (!__.isEmpty(lb.backendAddressPools)) {
          ...
          for (var poolNum in lb.backendAddressPools) {
            ...
            if (!__.isEmpty(pool.backendIpConfigurations)) {
              for (var i in pool.backendIpConfigurations) {
