...
__.extend(LoadBalancer.prototype, {
      ...
      if (!__.isEmpty(lb.loadBalancer.frontendIpConfigurations)) {
        ...
        for (var vipNum in lb.loadBalancer.frontendIpConfigurations) {
          ...
          if (vip.publicIpAddress) {
            var resourceInfo = utils.parseResourceReferenceUri(vip.publicIpAddress.id);
            var publicip = publicIPcrud.get(resourceInfo.resourceGroupName, resourceInfo.resourceName, _);
  ...
  _parseInboundRuleParams: function (resourceGroup, lb, inboundRule, options, useDefaults, _) {
    ...
    var firstIpConfigById;
    if (options.vmId) {
      if (options.vmId !== true) {
        firstIpConfigById = this._getFirstIpConfigFromVMById(resourceGroup, options, _);
        inboundRule.backendIPConfiguration = {id: firstIpConfigById.id};
      }
    } else if (options.vmName) {
      firstIpConfigById = this._getFirstIpConfigFromVMByName(resourceGroup, options, _);
      inboundRule.backendIPConfiguration = {id: firstIpConfigById.id};
    } else if (useDefaults) {
  ...
  _handleAddressPool: function (resourceGroupName, lbName, backendAddressPool, options, _) {
    ...
    var optionalVmIds = utils.getOptionalArg(options.vmIds);
    if (optionalVmIds.hasValue) {
      if (optionalVmIds.value !== null) {
    ...
    var optionalVmNames = utils.getOptionalArg(options.vmNames);
    if (optionalVmNames.hasValue) {
      if (optionalVmNames.value !== null) {
