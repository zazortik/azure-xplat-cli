...
__.extend(LoadBalancer.prototype, {
    ...
    if (lb) {
      ...
      interaction.formatOutput(lb.loadBalancer, function (lb) {
        ...
        if (!__.isEmpty(lb.frontendIpConfigurations)) {
          ...
          for (i = 0; i < lb.frontendIpConfigurations.length; i++) {
            var frontendIpConf = lb.frontendIpConfigurations[i];
        ...
        if (!__.isEmpty(lb.backendAddressPools)) {
          ...
          for (i = 0; i < lb.backendAddressPools.length; i++) {
            var pool = lb.backendAddressPools[i];
            ...
            if (!__.isEmpty(pool.backendIpConfigurations)) {
              for (i = 0; i < pool.backendIpConfigurations; i++) {
                var backendIpConfig = pool.backendIpConfigurations[i];
        ...
        if (!__.isEmpty(lb.loadBalancingRules)) {
          ...
          for (i = 0; i < lb.loadBalancingRules.length; i++) {
            var rule = lb.loadBalancingRules[i];
            ...
            if (!__.isEmpty(rule.frontendIPConfigurations)) {
              for (i = 0; i < rule.frontendIPConfigurations.length; i++) {
                var loadBalancingRuleIpConfig = rule.frontendIPConfigurations[i];
        ...
        if (!__.isEmpty(lb.inboundNatRules)) {
          ...
          for (i = 0; i < lb.inboundNatRules.length; i++) {
            var inboundNatRule = lb.inboundNatRules[i];
            ...
            if (!__.isEmpty(inboundNatRule.frontendIPConfigurations)) {
              for (i = 0; i < inboundNatRule.frontendIPConfigurations.length; i++) {
                var inboundRuleIpConfig = inboundNatRule.frontendIPConfigurations[i];
        ...
        if (!__.isEmpty(lb.probes)) {
          ...
          for (i = 0; i < lb.probes; i++) {
            ...
            if (!__.isEmpty(probe.loadBalancingRules)) {
              for (i = 0; i < probe.loadBalancingRules.length; i++) {
                var probeRule = probe.loadBalancingRules[i];
  ...
  _parseInboundNatRule: function (resourceGroup, lb, inboundRule, options, useDefaults, _) {
    ...
    if (options.vip) {
      ...
      var ipConfigurations = options.vip.split(',');
      for (var num in ipConfigurations) {
        ...
        inboundRule.frontendIPConfigurations.push({id: frontendIpConfFound.id});
      }
    } else if (useDefaults) {
      if (!inboundRule.frontendIPConfigurations) {
        ...
        inboundRule.frontendIPConfigurations.push({id: frontendIpConfFoundList[0].id});
