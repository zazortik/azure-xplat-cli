...
__.extend(LoadBalancer.prototype, {
      ...
      interaction.formatOutput(lb.loadBalancer, function (lb) {
        ...
        if (!__.isEmpty(lb.frontendIpConfigurations)) {
          ...
          for (var vipNum in lb.frontendIpConfigurations) {
            ...
            output.data($('   Public IP address:           '), vip.publicIpAddress ? vip.publicIpAddress.id : '');
            output.data($('   Subnet:                      '), vip.subnet ? vip.subnet.id : '');
            output.data($('   Load balancing rules:        '), '');
            if (vip.loadBalancingRules && vip.loadBalancingRules.length > 0) {
              for (var vipRuleNum in vip.loadBalancingRules) {
                var vipRule = vip.loadBalancingRules[vipRuleNum];
                output.data($('      '), vipRule.id);
              }
            }
            output.data($('   Inbound NAT rules:  '), '');
            if (vip.inboundNatRules && vip.inboundNatRules.length > 0) {
              for (var natRuleNum in vip.inboundNatRules) {
                var natRule = vip.inboundNatRules[natRuleNum];
                output.data($('      '), natRule.id);
              }
            }
            output.data($(''), '');
          }
        ...
        if (!__.isEmpty(lb.backendAddressPools)) {
          ...
          for (var poolNum in lb.backendAddressPools) {
            ...
            output.data($('   Load balancing rules:      '), '');
            if (!__.isEmpty(pool.loadBalancingRules)) {
              for (var backendRuleNum in pool.loadBalancingRules) {
                var poolBalancingRule = pool.loadBalancingRules[backendRuleNum];
                output.data($('      '), poolBalancingRule.id);
              }
            }
  ...
  createInboundRule: function (resourceGroupName, lbName, name, options, _) {
    ...
    inboundRule = this._parseInboundRuleParams(resourceGroupName, lb.loadBalancer, inboundRule, options, true, _);
  ...
  updateInboundRule: function (resourceGroupName, lbName, name, options, _) {
    ...
    this._parseInboundRuleParams(resourceGroupName, lb.loadBalancer, currentInboundRule, options, false, _);
    this.update(resourceGroupName, lbName, lb.loadBalancer, _);
  ...
  listInboundRules: function (resourceGroupName, lbName, options, _) {
    var lb = this.get(resourceGroupName, lbName, _);
  ...
  _parseInboundRuleParams: function (resourceGroup, lb, inboundRule, options, useDefaults, _) {
    var endPointUtil = new EndPointUtil();
    ...
    if (options.vmId) {
      if (options.vmId !== true) {
        var firstIpConfigById = this._getFirstIpConfigFromVMById(resourceGroup, options, _);
        inboundRule.backendIPConfiguration = {id: firstIpConfigById.id};
      }
    } else if (options.vmName) {
      var vmId = this._getVmIdByName(resourceGroup, options.vmName);
      if (!vmId) {
        throw new Error(util.format($('Error parsing %s virtual machine ID'), options.vmName));
      }
      inboundRule.backendIPConfiguration = {id: vmId};
    } else if (useDefaults) {
  ...
  _handleAddressPool: function (resourceGroupName, lbName, backendAddressPool, options, _) {
    ...
    if (options.vmIds) {
      if (options.vmIds === true) {
        this.log.verbose($('Clearing backend IP configurations'));
        backendAddressPool.backendIpConfigurations = [];
      } else {
  ...
  _parseLBRuleParameters: function (lb, rule, options, useDefaults) {
    ...
    if (options.vipNames) {
      ...
      var vips = options.vipNames.split(',');
      for (var num in vips) {
        var vip = vips[num];
      ...
      if (!lb.frontendIpConfigurations || lb.frontendIpConfigurations.length === 0) {
        throw new Error($('Load balancer must have at least one frontend IP configuration if --vip-names parameter is not specified.'));
      }
