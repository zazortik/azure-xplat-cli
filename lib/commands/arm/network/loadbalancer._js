...
function LoadBalancer(cli, serviceClients) {
  ...
  this.DEFAULT_PROBE_PROTOCOL = 'Http';
  this.DEFAULT_PROBE_PORT = 80;
  this.DEFAULT_PROBE_PATH = '/';
  this.DEFAULT_PROBE_COUNT = 10;
  this.DEFAULT_PROBE_INTERVAL = 15;
}
...
__.extend(LoadBalancer.prototype, {
  ...
  createInboundRule: function (options, _) {
    ...
    inboundRule = this._parseInboundRuleParams(options.resourceGroupName, lb.loadBalancer, inboundRule, options, true, _);
    var output = this.cli.output;
  ...
  updateInboundRule: function (options, _) {
    ...
    var inboundRule = this._parseInboundRuleParams(options.resourceGroupName, lb, currentInboundRule, options, false, _);
    ...
    var newLb = this.get(options.resourceGroupName, options.lbName, _);
    var updatedRule = utils.findFirstCaseIgnore(newLb.loadBalancer.inboundNatRules, {name: options.newName || options.name});
  ...
  _parseAndValidateProbe: function (probeName, params, useDefaults) {
    ...
    if (params.path) {
      ...
      output.warn(util.format($('Using default probe path: %s'), self.DEFAULT_PROBE_PATH));
      probeProfile.requestPath = self.DEFAULT_PROBE_PATH;
    }
    ...
    if (params.interval) {
      ...
      output.warn(util.format($('Using default probe interval: %s'), self.DEFAULT_PROBE_INTERVAL));
      probeProfile.intervalInSeconds = self.DEFAULT_PROBE_INTERVAL;
    }
    ...
    if (params.count) {
      ...
      output.warn(util.format($('Using default number of probes: %s'), self.DEFAULT_PROBE_COUNT));
      probeProfile.numberOfProbes = self.DEFAULT_PROBE_COUNT;
    }
  ...
  _parseInboundRuleParams: function (resourceGroup, lb, inboundRule, options, useDefaults, _) {
    ...
    if (options.vmId) {
      ...
      var firstIpConfigByName = this._getFirstIpConfigFromVMByName(resourceGroup, options, _);
      inboundRule.backendIPConfiguration = {id: firstIpConfigByName.id};
    } else if (useDefaults) {
