...
__.extend(VNet.prototype, {
  create: function (resourceGroup, name, location, options, _) {
    ...
    if (options.addressPrefixes) {
      ...
      requestBody.properties.addressSpace.addressPrefixes.push(defaultAddressPrefix);
    }
    ...
    this.cli.interaction.formatOutput(vNets.virtualNetworks, function (outputData) {
      if (outputData.length === 0) {
        ...
        output.table(outputData, function (row, item) {
          ...
          row.cell($('Address prefixes'), item.properties.addressSpace.addressPrefixes);
          var dnsServers = '';
          if (item.properties.dhcpOptions) {
            dnsServers = item.properties.dhcpOptions.dnsServers;
          }
  ...
  _showVnet: function (resource) {
    ...
    this.log.data($('Provisioning state:  '), resource.properties.provisioningState);
    this.log.data($('Address prefixes:    '), resource.properties.addressSpace.addressPrefixes);
    this.log.data($('DNS servers:         '), resource.properties.dhcpOptions.dnsServers);
    this.log.data('');
  },
  ...
  _handleDnsServers: function(options, vNet, vNetUtil) {
    ...
    for (var dnsNum in dnsServers) {
      ...
      vNet.properties.dhcpOptions.dnsServers.push(dnsServer);
    }
  },
  ...
  _handleAddressPrefixes: function(options, vNet, vNetUtil) {
    ...
    for (var addNum in addressPrefixes) {
      ...
      vNet.properties.addressSpace.addressPrefixes.push(addressPrefix);
    }
  }
});
