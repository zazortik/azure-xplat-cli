...
exports.init = function (cli) {
  ...
  probe.command('set [resource-group] [lb-name] [name]')
    ...
    .option('-t, --path <path>', $('the new value for probe path'))
  ...
  var loadBalancerVip = lb.category('vip')
    .description('Commands to manage vips of a load balancer');

  loadBalancerVip.command('create [resource-group] [lb-name] [name]')
  ...
  loadBalancerVip.command('set [resource-group] [lb-name] [name]')
  ...
  loadBalancerVip.command('list [resource-group] [lb-name]')
    .description($('Lists VIP\'s in the load balancer within a resource group'))
    .usage('[options] <resource-group> <lb-name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
  ...
  loadBalancerVip.command('delete [resource-group] [lb-name] [name]')
  ...
  loadBalancerAddressPool.command('set [resource-group] [lb-name] [name]')
  ...
  loadBalancerLBRule.command('list [resource-group] [lb-name]')
  ...
  var inboundrule = lb.category('inbound-nat-rule')
  ...
  inboundrule.command('create [resource-group] [lb-name] [name]')
    ...
    .option('-b, --backend-port <backend-port>', $('the backend port'))
    .option('-e, --enable-floating-ip <enable-floating-ip>', $('enable floating point ip'))
    .option('-i, --vip <vip>', $('the name of the vip'))
    .option('-m, --vm-id <vm-id>', $('the VM id.' +
    '\n     e.g. /subscriptions/<subscription-id>/resourceGroups/<resource-group-name>/providers/Microsoft.Compute/virtualMachines/<vm-name>'))
    .option('-a, --vm-name <vm-name>', $('the VM name.This VM must exists in the same resource group as the lb.' +
  ...
  inboundrule.command('list [resource-group] [lb-name]')
    .usage('[options] <resource-group> <lb-name>')
    .description($('Lists load balancing inbound NAT rule in the load balancer within a resource group'))
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, lbName, options, _) {
      ...
      var networkClient = new NetworkClient(cli, options.subscription);
      networkClient.listInboundRules(resourceGroup, lbName, options, _);
    });
  ...
  nsgRules.command('create [resource-group] [nsg-name] [name]')
    ...
    .option('-o, --source-port-range <source-port-range>', util.format($('the source port range %s'), Constants.toRange(Constants.portBoundsList)))
    .option('-e, --destination-address-prefix <destination-address-prefix>', $('the destination address prefix'))
    .option('-u, --destination-port-range <destination-port-range>', util.format($('the destination port range %s'), Constants.toRange(Constants.portBoundsList)))
    .option('-c, --access <access>', util.format($('the access mode [%s]'), Constants.accessModesList))
    .option('-y, --priority <priority>', util.format($('the priority'), Constants.toRange(Constants.priorityBoundsList)))
    .option('-r, --direction <direction>', util.format($('the direction [%s]'), Constants.directionModesList))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
  ...
  dnsZone.command('show [resource-group] [name]')
    .description($('Shows a dns zone within a resource group'))
    .usage('[options] <resource-group> <name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-n, --name <name>', $('the name of the dns zone.' +
    '\n     Must not end with \'.\'' +
  ...
  var profile = trafficManager.category('profile')
    .description($('Commands to manage traffic manager'));

  profile.command('create [resource-group] [name]')
    .description($('Creates a Traffic Manager Profile'))
    .usage('[options] <resource-group> <name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-n, --name <name>', $('the name of the profdsghdfhile'))
    .option('-r, --relative-dns-name <relative-dns-name>', $('relative DNS name of the profile + e.g. trafficmanager.net'))
    .option('-t  --ttl <ttl>', $('time to live in seconds'))
    .option('-l, --load-balancing-method <load-balancing-method>', $('the load balancing method for the profile, valid values are [performance, weighted, priority]'))
    ...
    .option('-u, --status <status> ', $('the profile status, valid values are [enable, disable], default is enable'))
    .option('-t, --tags <tags>', $('the tags set on this profile. Can be multiple, in the format of \'name=value\'.\' ' +
    '\n       Name is required and value is optional. For example, -t tag1=value1;tag2'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, name, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      ...
      var networkClient = new NetworkClient(cli, options.subscription);
      networkClient.createTrafficManager(resourceGroup, name, options, _);
    });
};