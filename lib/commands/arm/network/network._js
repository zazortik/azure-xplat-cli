...
exports.init = function (cli) {
  ...
  trafficManagerProfile.command('create [resource-group] [name]')
    ...
    .option('-e, --ttl <ttl>', $('time to live in specified in seconds'))
  ...
  trafficManagerProfile.command('set [resource-group] [name]')
    ...
    .option('-m, --traffic-routing-method <traffic-routing-method>', util.format($('the traffic routing method for the profile,' +
    '\n     valid values are [%s], default is %s'), constants.trafficManager.routingMethod, constants.trafficManager.routingMethod[0]))
    .option('-e, --ttl <ttl>', $('time to live specified in seconds'))
    .option('-p, --monitor-protocol <monitor-protocol>', util.format($('the monitor protocol, valid values are' +
    '\n     [%s], default is %s'), constants.trafficManager.protocols, constants.trafficManager.protocols[0]))
    .option('-o, --monitor-port <monitor-port>', $('the monitoring port'))
    .option('-a, --monitor-path <monitor-path>', $('the monitoring path'))
  ...
  trafficManagerEndpoint.command('create [resource-group] [profile-name] [name] [type] [location]')
    .description($('Create an endpoint in Traffic Manager profile'))
    .usage('[options] <resource-group> <profile-name> <name> <type> <location>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    ...
    .option('-l, --location <location>', $('the endpoint location'))
    ...
    .option('-i, --target-resource-id <target-resource-id>', $('the Azure Resource URI of the of the endpoint. Not applicable to endpoints of type \'ExternalEndpoints\''))
    .option('-w, --weight <weight>', util.format($('the endpoint weight used in the load balancing algorithm,' +
    ...
    .option('-p, --priority <priority>', util.format($('the endpoint priority used in the load balancing algorithm,' +
    '\n       valid range is [%s, %s] Lower values represent higher priority'), constants.trafficManager.priorityMin, constants.trafficManager.priorityMax))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, profileName, name, type, location, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      ...
      options.location = cli.interaction.promptIfNotGiven($('Endpoint location: '), location, _);
