...
__.extend(LoadBalancer.prototype, {
  ...
  showLBRule: function(rule) {
    this.cli.interaction.formatOutput(rule, function (rule) {
      lbShowUtil.showLBRule(rule, this.cli.output);
    });
  },
  ...
  updateInboundNatRule: function (resourceGroupName, lbName, name, options, _) {
    ...
    this.showInboundRule(updatedRule);
  },
  ...
  showInboundRule: function(rule) {
    var cli = this.cli;
    cli.interaction.formatOutput(rule, function (rule) {
      lbShowUtil.showInboundRule(rule, cli.output);
    });
  },
  ...
  addFrontEndIPConfig: function (resourceGroupName, lbName, ipConfigName, options, _) {
    ...
    this.showFrontendIpConfig(newFrontendIpConf);
  },
  ...
  showFrontendIpConfig: function(ipConfig) {
    var cli = this.cli;
    cli.interaction.formatOutput(ipConfig, function (ipConfig) {
      lbShowUtil.showFrontendIpConfig(ipConfig, cli.output);
    });
  },
  ...
  addBackendAddressPool: function (resourceGroupName, lbName, poolName, options, _) {
    ...
    this.showBackendAddressPool(newBackendAddressPool);
  },
  ...
  showBackendAddressPool: function(backendAddressPool) {
    var cli = this.cli;
    cli.interaction.formatOutput(backendAddressPool, function (backendAddressPool) {
      lbShowUtil.showBackendAddressPool(backendAddressPool, cli.output);
    });
  },
