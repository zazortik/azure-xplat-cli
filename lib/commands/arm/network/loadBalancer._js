...
__.extend(LoadBalancer.prototype, {
  ...
  _parseRule: function (lb, rule, options, useDefaults) {
    ...
    if (options.loadDistribution) {
      var loadDistributionValidation = self.endpointUtil.validateLBLoadDistribution(options.loadDistribution, '--load-distribution');
      if (loadDistributionValidation.error) {
        throw new Error(loadDistributionValidation.error);
      }

      rule.loadDistribution = options.loadDistribution;
    } else if (useDefaults) {
      options.loadDistribution  = constants.lb.defLoadDistribution;
      self.output.warn(util.format($('Using default load distribution: %s'), options.loadDistribution));
      rule.loadDistribution = options.loadDistribution;
    }
