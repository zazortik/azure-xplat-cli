...
__.extend(LoadBalancer.prototype, {
      ...
      interaction.formatOutput(lb.loadBalancer, function (lb) {
        ...
        if (!__.isEmpty(lb.frontendIpConfigurations)) {
          ...
          lb.frontendIpConfigurations.forEach(function (frontendIpConf) {
            ...
            if (frontendIpConf.subnet) {
              output.data($('   Subnet:                      '), frontendIpConf.subnet.id);
            }
            output.data($(''), '');
          });
        }
        ...
        if (!__.isEmpty(lb.loadBalancingRules)) {
          ...
          lb.loadBalancingRules.forEach(function (rule) {
            ...
            if (rule.frontendIPConfiguration) {
              output.data($('   Frontend IP configuration: '), rule.frontendIPConfiguration.id);
            }
            if (rule.backendAddressPool) {
              output.data($('   Backend address pool:      '), rule.backendAddressPool.id);
            }
            if (rule.probe) {
              output.data($('   Probe:                     '), rule.probe.id);
            }
            output.data($(''), '');
          });
        }
        if (!__.isEmpty(lb.inboundNatRules)) {
          ...
          lb.inboundNatRules.forEach(function (inboundNatRule) {
            ...
            if (inboundNatRule.frontendIPConfiguration) {
              output.data($('   Frontend IP configuration: '), inboundNatRule.frontendIPConfiguration.id);
            }
            if (inboundNatRule.backendIPConfiguration) {
              output.data($('   Backend IP Configuration:  '), inboundNatRule.backendIPConfiguration.id);
            }
            output.data($(''), '');
          });
        }
  ...
  _showLBRule: function (resource) {
    ...
    this.cli.interaction.formatOutput(resource, function (resource) {
      ...
      log.data($('Frontend IP configuration Id:'), resource.frontendIPConfiguration ? resource.frontendIPConfiguration.id : '');
      log.data($('Backend address pool Id:     '), resource.backendAddressPool);
      ...
      log.data($('Probe Id:                    '), resource.probe ? resource.probe.id : '');
      log.data('');
    });
  },
  ...
  _showFrontendIpConfig: function (resource) {
    ...
    this.cli.interaction.formatOutput(resource, function (resource) {
      ...
      log.data($('Subnet Id:                   '), resource.subnet ? resource.subnet.id : '');
      log.data($('Public IP address Id:        '), resource.publicIpAddress ? resource.publicIpAddress.id : '');
