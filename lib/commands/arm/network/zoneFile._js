...
__.extend(ZoneFile.prototype, {
  ...
  parseRR: function (rr, $origin, zoneName, $ttl, prevRrName) {
    ...
    var validTypes = ['A', 'AAAA', 'CNAME', 'MX', 'NS', 'PTR', 'SOA', 'SRV', 'TXT'];
    var validClasses = ['IN', 'CS', 'CH', 'HS'];
    var rrTokens = rr.trim().split(/\s+/g);
    var regexp = /^\s/;
    if(rr[0].match(regexp)) {
      self.output.warn(util.format($('Empty space at the beginning of the line "%s" means that record should use previous record name "%s"'), rrTokens, prevRrName));
     ...
     */
    if (!res.name) {
      console.log('setting previous name ' + prevRrName);
