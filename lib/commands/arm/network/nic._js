...
__.extend(Nic.prototype, {
  ...
  set: function (resourceGroupName, nicName, params, _) {
    ...
    if (params.networkSecurityGroupId || params.networkSecurityGroupName) nic.networkInterface.networkSecurityGroup = nicProfile.networkSecurityGroup;
    if (params.publicIpId || params.publicIpName) {
      nic.networkInterface.ipConfigurations[0].publicIpAddress = nicProfile.ipConfigurations[0].publicIpAddress;
    }
    if (params.tags) nic.networkInterface.tags = nicProfile.tags;
  ...
  _parseAndValidateNIC: function (resourceGroupName, nicName, params, _) {
    ...
    if (params.subnetId) {
      if (params.subnetName || params.subnetVnetName) {
        output.warn($('--subnet-name, --subnet-vnet-name parameters will be omitted because --subnet-name, --subnet-vnet-name and --subnet-id are mutually exclusive'));
      }
    ...
    if (params.networkSecurityGroupId) {
      if (params.networkSecurityGroupName) output.warn($('--network-security-group-name parameter will be omitted because --network-security-group-id and --network-security-group-name are mutually exclusive'));
    ...
    if (params.publicIpId) {
      if (params.publicIpName) output.warn($('--public-ip-name parameter will be omitted because --public-ip-id and --public-ip-name are mutually exclusive'));
