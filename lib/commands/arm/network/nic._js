...
__.extend(Nic.prototype, {
        ...
        if (!__.isEmpty(nic.ipConfigurations)) {
          ...
          for (var configNum in nic.ipConfigurations) {
            ...
            if (config.loadBalancerBackendAddressPools && config.loadBalancerBackendAddressPools.length > 0) {
              output.data($('   Load balancer backend address pools:     '), '');
              for (var i in config.loadBalancerBackendAddressPools) {
                var loadBalancerBackendAddressPool = config.loadBalancerBackendAddressPools[i];
                output.data($('      Id:'), loadBalancerBackendAddressPool.id);
              }
            }

            if (config.loadBalancerInboundNatRules && config.loadBalancerInboundNatRules.length > 0) {
              output.data($('   Load balancer inbound NAT rules:     '), '');
              for (var j in config.loadBalancerInboundNatRules) {
                var loadBalancerInboundNatRule = config.loadBalancerInboundNatRules[j];
                output.data($('      Id:'), loadBalancerInboundNatRule.id);
              }
            }
  ...
  _handleNicsAddressPool: function(resourceGroupName, nicName, params, isAdding, _) {
    ...
    if (!nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools) {
      nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools = [];
    }
    ...
    if (isAdding) {
      if (!utils.findFirstCaseIgnore(nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools, {id: poolId})) {
        nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools.push({id: poolId});
      } else {
      ...
      var index = utils.indexOfCaseIgnore(nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools, {id: poolId});
      if (index !== null) {
        nic.networkInterface.ipConfigurations[0].loadBalancerBackendAddressPools.splice(index, 1);
      } else {
  ...
  _handleNicsInboundRule: function(resourceGroupName, nicName, params, isAdding, _) {
    ...
    if (!nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules) {
      nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules = [];
    }
    ...
    if (isAdding) {
      if (!utils.findFirstCaseIgnore(nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules, {id: ruleId})) {
        nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules.push({id: ruleId});
      } else {
      ...
      var index = utils.indexOfCaseIgnore(nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules, {id: ruleId});
      if (index !== null) {
        nic.networkInterface.ipConfigurations[0].loadBalancerInboundNatRules.splice(index, 1);
      } else {
