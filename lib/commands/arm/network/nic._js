...
__.extend(Nic.prototype, {
  create: function (resourceGroupName, nicName, params, _) {
    ...
    if (params.subnetId || (params.subnetName && params.subnetVnetName)) {
      ...
      var nicProfile = self._parseNic(resourceGroupName, nicName, params, _);
      ...
      self.show(resourceGroupName, nicName, params, _);
    } else {
  ...
  set: function (resourceGroupName, nicName, params, _) {
    ...
    self.update(resourceGroupName, nicName, nic.networkInterface, _);
    self.show(resourceGroupName, nicName, params, _);
  },
  ...
  list: function (resourceGroupName, params, _) {
    ...
    self.interaction.formatOutput(nics.networkInterfaces, function (outputData) {
      if (outputData.length === 0) {
        self.output.info($('No network interfaces found'));
      } else {
        self.output.table(outputData, function (row, nic) {
  ...
  show: function (resourceGroupName, nicName, params, _) {
    ...
    if (nic) {
      ...
      self.interaction.formatOutput(nic.networkInterface, function (nic) {
  ...
  delete: function (resourceGroupName, nicName, params, _) {
    ...
    if (!params.quiet && !self.interaction.confirm(util.format($('Delete network interface "%s"? [y/n] '), nicName), _)) {
      return;
    }
  ...
  addAddressPool: function (resourceGroupName, nicName, params, _) {
    this._handleNicsAddressPool(resourceGroupName, nicName, params, true, _);
  },

  removeAddressPool: function (resourceGroupName, nicName, params, _) {
  ...
  removeInboundRule: function (resourceGroupName, nicName, params, _) {
  ...
  _handleNicsAddressPool: function (resourceGroupName, nicName, params, isAdding, _) {
    ...
    if (params.lbAddressPoolId) {
      ...
      var lb = self.LoadBalancerCrud.get(resourceGroupName, params.lbName, _);
      if (!lb) {
  ...
  _parseNic: function (resourceGroupName, nicName, params, _) {
    var self = this;
    ...
    if (params.privateIpAddress) {
      var ipValidationResult = self.vnetUtil.parseIPv4(params.privateIpAddress);
    ...
    if (params.networkSecurityGroupId) {
      if (params.networkSecurityGroupName) self.output.warn($('--network-security-group-name parameter will be ignored because --network-security-group-id and --network-security-group-name are mutually exclusive'));
    ...
    if (params.publicIpId) {
      if (params.publicIpName) self.output.warn($('--public-ip-name parameter will be ignored because --public-ip-id and --public-ip-name are mutually exclusive'));
