...
__.extend(Nic.prototype, {
 ...
 //   console.log('NIC:  %j', nic);
  ...
  _parseIpConfig: function(resourceGroupName, ipConfig, options, _) {
    var self = this;

    if (options.privateIpVersion) {
      ipConfig.privateIPAddressVersion = utils.verifyParamExistsInCollection(constants.publicIp.version,
        options.privateIpVersion, '--private-ip-version');
    }

    if (options.privateIpAddress) {
     /*  TODO add support for IPv6 validation after 'true' multiple ipConfig feature supported on Azure end
      ...
      ipConfig.privateIPAddress = options.privateIpAddress;
    }
