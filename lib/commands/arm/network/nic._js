    ...
    if (nic) {
      ...
      interaction.formatOutput(nic.networkInterface, function (nic) {
        ...
        nic.ipConfigurations.forEach(function (config) {
          output.nameValue($('Name'), config.name, false, 2);
          output.nameValue($('Provisioning state'), config.provisioningState, false, 2);
          if (config.publicIpAddress) {
            output.nameValue($('Public IP address'), config.publicIpAddress.id, false, 2);
          }
          output.nameValue($('Private IP address'), config.privateIpAddress, false, 2);
          output.nameValue($('Private IP Allocation Method'), config.privateIpAllocationMethod, false, 2);
          output.nameValue($('Subnet'), config.subnet.id, false, 2);

          if (config.loadBalancerBackendAddressPools.length > 0) {
            output.header($('Load balancer backend address pools'), false, 2);
            config.loadBalancerBackendAddressPools.forEach(function (pool) {
              output.nameValue($('Id'), pool.id, false, 4);
            });
          }

          if (config.loadBalancerInboundNatRules.length > 0) {
            output.header($('Load balancer inbound NAT rules'), false, 2);
            config.loadBalancerInboundNatRules.forEach(function (rule) {
              output.nameValue($('Id'), rule.id, false, 4);
            });
          }
