...
__.extend(AppGateways.prototype, {
  ...
  start: function (resourceGroup, appGatewayName, options, _) {
    ...
    try {
      if(options.forceStop) {
        self.output.warn(util.format($('Application gateway "%s" modification command would be stopped. ' +
          '/n       Item is in "Updating" state and unavailable to use up to the end of process.'), appGatewayName));
        setTimeout(function() {
          process.exit();
        }, constants.appGateway.forceStopTimeout);
      }
  ...
  _setAppGateway: function (resourceGroup, appGatewayName, appGateway, options, _) {
    ...
    var message = options.forceStop ? util.format($('Application gateway %s modification command would be forcibly stopped' +
      '\n         Specified item is in Updating state and unavailable to use up to the end of process'), appGatewayName)
      : util.format($('Setting long-running configuration for an application gateway %s'), appGatewayName);

    self.output.warn(message);

    var progress = self.interaction.progress();
