...
__.extend(AppGateways.prototype, {
    ...
    self.interaction.formatOutput(items, function (data) {
      if (data.length === 0) {
        ...
        data.forEach(function(pool) {
          formattedPools.push({name: pool.name});
          if(pool.backendAddresses.length !== 0) {
            formattedPools[0].ip = pool.backendAddresses[0].ipAddress;
          }
  ...
  _showAppGatewayHttpListener: function(listener, indent) {
    ...
    self.interaction.formatOutput(listener, function (listener) {
      ...
      var sslCertificate = resourceUtils.getResourceInformation(listener.sslCertificate.id);
