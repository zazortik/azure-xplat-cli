...
__.extend(AppGateways.prototype, {
  ...
  listUrlPathMaps: function (resourceGroup, appGatewayName, options, _) {
    ...
    self._listAppGatewayUrlPathMaps(appGateway.urlPathMaps);
  },
  ...
  _listAppGatewayUrlPathMaps: function(urlPathMaps, indent) {
    var self = this;

    self.interaction.formatOutput(urlPathMaps, function (urlPathMaps) {
  ...
  _showAppGateway: function (appGateway) {
    ...
    self.interaction.formatOutput(appGateway, function (appGateway) {
      ...
      self._listAppGatewayUrlPathMaps(appGateway.urlPathMaps, indent);
  ...
  _showAppGatewayUrlPathMap: function(urlPathMap, indent) {
    ...
    self.interaction.formatOutput(urlPathMap, function (urlPathMap) {
      ...
      self._showAppGatewayUrlPathMapRule(urlPathMap.pathRules, indent);
    });
  },

  _showAppGatewayUrlPathMapRule: function(pathRules, indent) {
    var self = this;

    self.interaction.formatOutput(pathRules, function (pathRules) {
      if (pathRules.length > 0) {
        self.output.header($('Rules'), indent);
        indent += 2;
        pathRules.forEach(function (item) {
          var addressPool = resourceUtils.getResourceInformation(item.backendAddressPool.id);
          var httpSetting = resourceUtils.getResourceInformation(item.backendHttpSettings.id);
          ...
          if (item.paths.length > 0) {
            ...
            self.output.list(item.paths, indent + 2);
          }
          self.output.data($(''), '');
      });
      }
