...
__.extend(Traffic.prototype, {
  ...
  checkDnsAvailability: function (resourceGroupName, relativeDnsName, options, _) {
    ...
    for (var i = 0; i < profiles.profiles.length; i++) {
      ...
      if (item.properties.dnsConfig && item.properties.dnsConfig.relativeName === relativeDnsName) {
        tmProfile = item;
        break;
      }
    }
    ...
    if (tmProfile) {
      this.log.info(util.format($('The DNS name "%s" is already reserved by "%s" traffic manager profile'), relativeDnsName, tmProfile.name));
    } else {
  ...
  _showProfile: function (tmProfile) {
    ...
    this.cli.interaction.formatOutput(tmProfile, function (tmProfile) {
      ...
      log.data($('Name:                 '), tmProfile.name);
      log.data($('Type:                 '), tmProfile.type);
      log.data($('Location:             '), tmProfile.location);
      log.data($('Status:               '), tmProfile.properties.profileStatus);
      log.data($('Routing method:       '), tmProfile.properties.trafficRoutingMethod);
      log.data($('DNS name:             '), tmProfile.properties.dnsConfig.relativeName);
      log.data($('Time to live:         '), tmProfile.properties.dnsConfig.ttl);
      log.data($('Monitoring protocol:  '), tmProfile.properties.monitorConfig.protocol);
      ...
      if (endpoints.length !== 0) {
        ...
        log.table(endpoints, function (row, item) {
          row.cell($('Name'), item.name);
          if (item.properties.endpointLocation) {
      ...
      if (!__.isEmpty(tmProfile.tags)) {
        log.data($('Tags:                 '), tagUtils.getTagsInfo(tmProfile.tags));
      }
    });
  }
});
