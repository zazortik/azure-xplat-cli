...
function Traffic(cli, trafficManagerProviderClient) {
  ...
  this.DEFAULT_PROFILE_STATUS = 'Enabled';
  this.DEFAULT_TRAFFIC_ROUTING_METHOD = 'Performance';
  this.DEFAULT_TIME_TO_LIVE = 300;
  this.DEFAULT_MONITOR_PROTOCOL = 'http';
  this.DEFAULT_MONITOR_PORT = {'http': '80', 'https': 443};
  this.VALID_ENDPOINT_STATUSES = ['Enabled', 'Disabled'];
  this.VALID_ENDPOINT_TYPES = ['externalEndpoint'];
}
...
__.extend(Traffic.prototype, {
  ...
  _parseEndpoint: function (endpointName, params, useDefaults) {
    ...
    if (params.type) {
      endpoint.type = utils.verifyParamExistsInCollection(this.VALID_ENDPOINT_TYPES,
        params.type, 'endpoint type');

      if (endpoint.type == this.VALID_ENDPOINT_TYPES[0]) {
    ...
    if (params.endpointStatus) {
      endpoint.properties.endpointStatus = utils.verifyParamExistsInCollection(this.VALID_ENDPOINT_STATUSES,
        params.endpointStatus, 'endpoint status');
    } else if (useDefaults) {
      output.warn(util.format($('Using default endpoint status: %s'), this.VALID_ENDPOINT_STATUSES[0]));
      endpoint.properties.endpointStatus = this.VALID_ENDPOINT_STATUSES[0];
    }
  ...
  _parseProfile: function (options, useDefaults) {
    ...
    if (options.location) {
      ...
      if (useDefaults) {
        parameters.location = this.DEFAULT_LOCATION;
      }
    }
    ...
    if (options.profileStatus) {
      ...
      if (useDefaults) {
        parameters.properties.profileStatus = this.DEFAULT_PROFILE_STATUS;
      }
    }
    ...
    if (options.trafficRoutingMethod) {
      ...
      if (useDefaults) {
        parameters.properties.trafficRoutingMethod = this.DEFAULT_TRAFFIC_ROUTING_METHOD;
      }
    }
    ...
    if (options.ttl) {
      ...
      if (useDefaults) {
        parameters.properties.dnsConfig.ttl = this.DEFAULT_TIME_TO_LIVE;
      }
    }
    ...
    if (options.monitorProtocol) {
      ...
      if (useDefaults) {
        parameters.properties.monitorConfig.protocol = this.DEFAULT_MONITOR_PROTOCOL;
      }
    }
    ...
    if (options.monitorPort) {
      ...
      if (useDefaults) {
        if (parameters.properties.monitorConfig.protocol === 'http') {
          parameters.properties.monitorConfig.port = this.DEFAULT_MONITOR_PORT.http;
        }
        if (parameters.properties.monitorConfig.protocol === 'https') {
          parameters.properties.monitorConfig.port = this.DEFAULT_MONITOR_PORT.https;
        }
      }
    }
  ...
  _showProfile: function (tmProfile) {
    ...
    this.cli.interaction.formatOutput(tmProfile, function (tmProfile) {
      ...
      if (endpoints.length !== 0) {
        ...
        log.table(endpoints, function (row, item) {
          row.cell($('Name'), item.name);
