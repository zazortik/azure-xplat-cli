...
__.extend(Subnet.prototype, {
  create: function (resourceGroup, vnetName, name, options, _) {
    ...
    var parameters = this._getSubnetParams(resourceGroup, vnetName, options, this.log, true, _);
  ...
  set: function (resourceGroup, vnetName, name, options, _) {
    ...
    var subnetProfile = this._getSubnetParams(resourceGroup, vnetName, options, this.log, false, _);
  ...
  _getSubnetParams: function (resourceGroup, vnetName, options, log, useDefaultSubnetCidr, _) {
    ...
    if (options.networkSecurityGroupId) {
      if (options.networkSecurityGroupName) {
        log.warn($('Options --network-security-group-id and --network-security-group-name are mutually exclusive, --network-security-group-name will be ommited'));
      }
      parameters.networkSecurityGroup = {
        id: options.networkSecurityGroupId
      };
    } else if (options.networkSecurityGroupName) {
      var nsgId = this._getNsgIdByName(resourceGroup, this.NsgCrud, options.networkSecurityGroupName, _);
      parameters.networkSecurityGroup = {
        id: nsgId
      };
    }
