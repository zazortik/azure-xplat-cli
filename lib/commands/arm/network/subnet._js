  ...
  _getSubnetParams: function (resourceGroup, vnetName, options, useDefaultSubnetCidr, _) {
    ...
    if (options.networkSecurityGroupId) {
      if (options.networkSecurityGroupName) {
        output.warn($('--network-security-group-name parameter will be ommited because --network-security-group-id and --network-security-group-name parameters are mutually exclusive'));
      }
