...
__.extend(Subnet.prototype, {
  ...
  _getSubnetParams: function (resourceGroup, vnetName, options, useDefaultSubnetCidr, _) {
    ...
    if (options.networkSecurityGroupId) {
      ...
      if (options.networkSecurityGroupId !== true && options.networkSecurityGroupId !== '\'\'') {
        self._validateNetworkSecurityGroupId(resourceGroup, options.networkSecurityGroupId, _);
  ...
  _showSubnet: function (subnet) {
    ...
    this.cli.interaction.formatOutput(subnet, function (subnet) {
      ...
      if (subnet.networkSecurityGroup) {
        log.data($('Network security group id:'), subnet.networkSecurityGroup.id);
      }
    });
  },
  ...
  _validateNetworkSecurityGroupId: function (resourceGroup, nsgId, _) {
    var lowerCaseNsgId = nsgId.toLowerCase();
    var nsg = this.networkResourceProviderClient.networkSecurityGroups.list(resourceGroup, _);
    var networkSecurityGroups = nsg.networkSecurityGroups;
    for (var i = 0; i < networkSecurityGroups.length; i++) {
      var itemId = networkSecurityGroups[i].id.toLowerCase();
      if (itemId === lowerCaseNsgId || itemId === lowerCaseNsgId.substr(1)) {
        return;
      }
    }
    throw new Error(util.format($('No network security group found with id "%s" in the resource group "%s"'), nsgId, resourceGroup));
  }
});
