...
__.extend(VirtualNetwork.prototype, {
  ...
  show: function (resourceGroupName, vnetName, options, _) {
    ...
    self.interaction.formatOutput(vnet, function (vnet) {
      if (vnet === null) {
        self.output.warn(util.format($('Virtual network "%s" not found'), vnetName));
      } else {
        self._showVNet(vnet);
      }
    });
  ...
  get: function (resourceGroupName, vnetName, _) {
    var self = this;
    var progress = self.interaction.progress(util.format($('Looking up the virtual netwrok "%s"'), vnetName));
    try {
