...
__.extend(RouteTable.prototype, {
  ...
  createRoute: function (resourceGroupName, routeTableName, routeName, addressPrefix, nextHopType, options, _) {
    var self = this;
    options.addressPrefix = addressPrefix;
    options.nextHopType = nextHopType;
  ...
  _parseRoute: function (routeName, options, _) {
    ...
    if (options.addressPrefix) {
      ...
      if (prefixValidationResult.error && options.addressPrefix !== '*') {
        throw new Error($('address prefix must be in CIDR format. Asterix can also be used'));
      }
    ...
    if (options.nextHopType) {
      ...
      if (options.nextHopType === 'VirtualAppliance' && !options.nextHopIpAddress) {
    ...
    if (options.nextHopIpAddress) {
      ...
      if (ipValidationResult.error) {
        throw new Error(util.format($('next hop ip address "%s" is no valid IPv4 address'), options.nextHopIpAddress));
      }
