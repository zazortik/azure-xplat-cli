...
__.extend(PublicIp.prototype, {
  ...
  create: function (resourceGroupName, publicIpName, options, _) {
    ...
    self._showPublicIP(publicIp, resourceGroupName, publicIpName);
  },
  ...
  _parsePublicIP: function (publicIp, options, useDefaults) {
    ...
    if (options.ipVersion) {
      publicIp.publicIPAddressVersion = utils.verifyParamExistsInCollection(constants.publicIp.version,
        options.ipVersion, '--ip-version');
    } else if (useDefaults) {
      var defIpVersion = constants.publicIp.version[0];
      self.output.warn(util.format($('Using default ip version: %s'), defIpVersion));
      publicIp.publicIPAddressVersion = defIpVersion;
    }
