  ...
  _showVirtualNetworkGatewayConnection: function (connection) {
    ...
    this.cli.interaction.formatOutput(connection, function (connection) {
      ...
      if (!__.isEmpty(connection.virtualNetworkGateway1)) {
        ...
        log.nameValue($('Id'), connection.virtualNetworkGateway1.id, false, 2);
        log.nameValue($('Name'), connection.virtualNetworkGateway1.name, false, 2);
        log.nameValue($('Type'), connection.virtualNetworkGateway1.type, false, 2);
        log.nameValue($('Location'), connection.virtualNetworkGateway1.location, false, 2);
        log.nameValue($('Gateway Type'), connection.virtualNetworkGateway1.gatewayType, false, 2);
        log.nameValue($('Gateway Size'), connection.virtualNetworkGateway1.gatewaySize, false, 2);
      }
      ...
      if (!__.isEmpty(connection.localNetworkGateway2)) {
        log.nameValue($('Local network gateway 2'), false, 2);
        log.nameValue($('Id'), connection.localNetworkGateway2.id, false, 2);
        log.nameValue($('Name'), connection.localNetworkGateway2.name, false, 2);
        log.nameValue($('Type'), connection.localNetworkGateway2.type, false, 2);
        log.nameValue($('Location'), connection.localNetworkGateway2.location, false, 2);
        log.nameValue($('Gateway IP Address'), connection.localNetworkGateway2.gatewayIpAddress, false, 2);
      }
