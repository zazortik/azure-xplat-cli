...
__.extend(VpnGateway.prototype, {
  createVirtualNetworkGateway: function (resourceGroup, name, location, options, _) {
    var gatewayProfile = this._parseAndValidateVnetGateway(name, options, true);
    var gateway = this.getVirtualNetworkGateway(resourceGroup, name, _);
    if (gateway) {
      throw new Error(util.format($('A virtual network gateway with name "%s" already exists in the resource group "%s"'), name, resourceGroup));
    }

    var progress = this.cli.interaction.progress(util.format($('Creating virtual network gateway "%s"'), name));
    try {
      this.networkResourceProviderClient.gateways.createOrUpdateVirtualNetworkGateway(resourceGroup, name, gatewayProfile, _);
    } finally {
      progress.end();
    }
    this.showVirtualNetworkGateway(resourceGroup, name, options, _);
  },

  setVirtualNetworkGateway: function (resourceGroup, name, location, options, _) {
    var gatewayProfile = this._parseAndValidateVnetGateway(name, options);
    var gateway = this.getVirtualNetworkGateway(resourceGroup, name, _);
    if (gateway) {
      gateway = gateway.virtualNetworkGateway;

      if (options.type) gateway.gatewayType = gatewayProfile.gatewayType;
      ...
      this.updateVirtualNetworkGateway(resourceGroup, name, gateway, _);
      this.showVirtualNetworkGateway(resourceGroup, name, options, _);
    } else {
      throw new Error(util.format($('A virtual network gateway with name "%s" not found in the resource group "%s"'), name, resourceGroup));
    }
  },
  ...
  addIpConfigToVirtualNetworkGateway: function (resourceGroup, vnetGatewayName, options, _) {
    var gateway = this._addAndValidateIpConfig(resourceGroup, vnetGatewayName, options, _);
    this.updateVirtualNetworkGateway(resourceGroup, vnetGatewayName, gateway, _);
    this.showVirtualNetworkGateway(resourceGroup, vnetGatewayName, options, _);
  },

  removeIpConfigToVirtualNetworkGateway: function (resourceGroup, vnetGatewayName, options, _) {
    var gateway = this._removeAndValidateIpConfig(resourceGroup, vnetGatewayName, options, _);
    this.updateVirtualNetworkGateway(resourceGroup, vnetGatewayName, gateway, _);
    this.showVirtualNetworkGateway(resourceGroup, vnetGatewayName, options, _);
  },
  ...
  listVirtualNetworkGatewayConnections: function (resourceGroup, options, _) {
    var output = this.cli.output;
    var connections = this.getAllVirtualNetworkGatewayConnections(resourceGroup, options, _);
  ...
  getAllVirtualNetworkGatewayConnections: function (resourceGroup, options, _) {
  ...
  _parseAndValidateVnetGateway: function (name, options, useDefaults) {
    var parameters = {};
    ...
    if (options.type) {
      ...
      parameters.type = constants.vnetGatewayTypes[0];
    }

    if (options.size) {
      var size = parseInt(options.size);
      if (!size || size < 0) {
        throw new Error('size parameter must be a positive integer value');
      }
      parameters.gatewaySize = options.size;
    }
    ...
    if (options.vpnclientAddressPool) {
      ...
      for (var addressPrefix in addressPrefixes) {
        var vNetUtil = new VNetUtil();
  ...
  _addAndValidateIpConfig: function (resourceGroup, vnetGatewayName, options, _) {
    var self = this;
    var output = self.cli.output;
    var gateway = this.getVirtualNetworkGateway(resourceGroup, vnetGatewayName, _);
    if (!gateway) {
      throw new Error(util.format($('A virtual network gateway with name "%s" not found in the resource group "%s"'), vnetGatewayName, resourceGroup));
    }
    gateway = gateway.virtualNetworkGateway;
    ...
    if (options.publicIpId) {
      ...
      item = {publicIpAddress: {id: options.publicIpId}};
      gateway.ipConfigurations.push(item);
    } else {
      if (options.publicIpName) {
        var publicip = self.PublicipCrud.get(resourceGroup, options.publicIpName, _);
        if (!publicip) {
          throw new Error(util.format($('A public ip address  with name "%s" not found in the resource group "%s"'), options.publicIpName, resourceGroup));
        }
        item = {publicIpAddress: {id: publicip.publicIpAddress.id}};
        gateway.ipConfigurations.push(item);
      }
    }
    ...
    if (options.subnetId) {
      ...
      item = {subnet: {id: options.subnetId}};
      gateway.ipConfigurations.push(item);
    } else {
      if (options.subnetName && options.vnetName) {
        var subnet = self.SubnetCrud.get(resourceGroup, options.vnetName, options.subnetName, _);
        if (!subnet) {
          throw new Error(util.format($('A subnet with name "%s" not found in the resource group "%s"'), options.subnetName, resourceGroup));
        }
  ...
  _removeAndValidateIpConfig: function (resourceGroup, vnetGatewayName, options, _) {
    var gateway = this.getVirtualNetworkGateway(resourceGroup, vnetGatewayName, _);
    if (!gateway) {
      throw new Error(util.format($('A virtual network gateway with name "%s" not found in the resource group "%s"'), vnetGatewayName, resourceGroup));
    }
    ...
    for (var i = 0; i < gateway.ipConfigurations.length; i++) {
      var item = gateway.ipConfigurations[i];
      if (options.publicIpId && item.publicIpAddress.id === options.publicIpId) {
        gateway.ipConfigurations.splice(i, 1);
        break;
      }
