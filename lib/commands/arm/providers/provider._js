      ...
      cli.interaction.formatOutput(providers, function (data) {
        ...
        if (data.length === 0) {
          ...
          data.forEach(function(provider){
            ...
            if(RsrcLocObj.registeredProvider.namespace && RsrcLocObj.registeredProvider.registrationState){
              log.data('');
...
function extractResourceTypesWithLocation(providers, namespace){
  ...
  if(registeredProvider === undefined || registeredProvider.length === 0) {
    return { ResourceTypes: null, Locations: null, registeredProvider: null };
  } else {
      // Get all resources of provider
      var resourceTypes = registeredProvider
                          .resourceTypes
                            .map(function(resourceType){ return resourceType.resourceType; });
      // Get all unique locations of resources 
      var locations = registeredProvider
                      .resourceTypes
                        .map(function(resourceType){return resourceType.locations;})
