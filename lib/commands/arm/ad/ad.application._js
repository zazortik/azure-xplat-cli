...
exports.init = function (cli) {
  ...
  adApp.command('set')
    ...
    .option('--objectId <objectId>', $('the object id of the application to update. Either provide objectId or appId. objectId is more preferable.'))
    .option('--appId <appId>', $('the applicationId of the application to update. If you have created a service principal using this application then this will be the spn of the service principal.'))
    .option('-n --name <name>', $('the new display name for the application.'))
    .option('--home-page <home-page>', $('the new URL to the application homepage'))
    .option('-a --available', $('indicates if the application will be available to other tenants (true/false)'))
    ...
    .execute(function (options, _) {
      ...
      try {
        if (appId && !objectId) {
          var parameters = { filter: 'appId eq \'' + appId + '\'' };
          applications = client.applicationOperations.list(parameters, _);
          if (!applications || (applications && applications.length === 0)) {
            throw new Error($(util.format('The provided appId: \'%s\' was not found. Hence the application cannot be updated. ' + 
              'If you are not sure, then please execute \'azure ad app list\' and find out the correct appId or objectId.', appId)));
          }
          objectId = applications[0].objectId;
        }
        application = withProgress(util.format($('Updating application with objectId: \'%s\'.'), objectId),
        function (log, _) {
          return client.applicationOperations.patch(objectId, appParams, _);
        }, _);
      } catch (ex) {
        if (ex.statusCode && ex.statusCode === 403) {
          // Check if the User is a Guest user
