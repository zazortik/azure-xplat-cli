...
exports.init = function (cli) {
  ...
  adGroup.command('create [display-name] [mail-nickname]')
    ...
    .execute(function (displayName, mailNickname, options, _) {
      if (!displayName) {
        return cli.missingArgument('displayName');
      }

      if (!mailNickname) {
        return cli.missingArgument('mailNickname');
      }
      
      var mailEnabled = false;
      if (options && options.mailEnabled && options.mailEnabled.toLowerCase() === 'true') {
        mailEnabled = true;
      }
      ...
      var group;
      try {
        group = client.groupOperations.create(parameters, _);
      } finally {
        progress.end();
      }
  ...
  adGroup.command('delete [objectId]')
    ...
    .option('--objectId <objectId>', $('the object Id of the group to delete'))
    ...
    .execute(function (objectId, options, _) {
      ...
      try {
        client.groupOperations.deleteMethod(objectId, _);
      } finally {
        progress.end();
      }
    });
