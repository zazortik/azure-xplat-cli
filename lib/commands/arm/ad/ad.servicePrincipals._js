...
exports.init = function (cli) {
  ...
  adUser.command('show [servicePrincipalName] [objectID] [search]')
    .description($('Get an active directory user'))
    .usage('[options] <name>')
    .option('--spn <servicePrincipalName>', $('the name of the service principal to return'))
    ...
    try {
        if (servicePrincipalName) {
          servicePrincipals = client.servicePrincipal.getByServicePrincipalName(servicePrincipalName, _).servicePrincipals;
        } else if (objectId) {
          ...
          servicePrincipals = client.servicePrincipal.list(search, _)
        }
    } finally {
      progress.end();
    }
