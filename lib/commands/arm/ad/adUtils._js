...
exports.displayAUser = function (user, log) {
...
function displayGroupMembers(members, log) {
  for (var i = 0; i < members.length; i++) {
    if (members[i].objectType === 'User') {
      exports.displayAUser(members[i], log);
    } else if (members[i].objectType === 'Group') {
      displayAGroup(members[i], log);
    } else {
      console.log('an unexpected object type:' + members[i].objectType);
    }
    log.data('');
  }
}
...
function displayAGroup(group, log) {
