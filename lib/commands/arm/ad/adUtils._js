...
exports.createApplicationCredential = function(client, objectId, password, certValue, startDate, endDate, _) {
  ...
  var startDate = startDate ? new Date(Date.parse(startDate)) : new Date(Date.now());
  var endDate = (function () {
    if (endDate) {
  ...
  if (password) {
    ...
    console.log("Password credential list before: ", passwordCredentials);
    passwordCredentials.push(passwordCredential);

    console.log("Password credential list after: ", passwordCredentials);
...
exports.getObjectsByObjectIds = function (graphClient, objectIds, throwOnError, _) {
  ...
  try {
    objects = graphClient.objects.getObjectsByObjectIds({ objectIds: objectIds, includeDirectoryObjectReferences: true }, _);
  } catch (ex) {
