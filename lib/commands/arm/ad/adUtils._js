...
exports.getObjectId = function (principal, graphClient, throwIfNoOption, objectType, _) {
  ...
  if (principal.mail) {
    graphQueryResult = graphClient.user.list(principal.mail, null, _);
    if (graphQueryResult.users.length > 0) {
      objectType.value = 'user';
      return graphQueryResult.users[0].objectId;
    } else {
      graphQueryResult = graphClient.group.list(principal.mail, null, _);
