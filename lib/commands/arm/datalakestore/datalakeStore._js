...
exports.init = function (cli) {
      ...
      function (log, _) {
        ...
        try {
          ...
          while(offset < fileSizeInBytes ) {
            ...
            if(offset === 0) {
              response = client.fileSystem.internalBeginCreate(destination, accountName, parameters, _);
              client.fileSystem.create(response.location, buffer, _);
            }
            else {
              response = client.fileSystem.internalBeginAppend(destination, accountName, null, _);
  ...
  dataLakeStoreFileSystem.command('concat [accountName] [paths] [destination] [force]')
    ...
    .usage('[options] <accountName> <paths> <destination> <force>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    ...
    .execute(function (accountName, paths, destination, force, options, _) {
      ...
      withProgress(util.format($('Concatenating specified files into target location: %s'), destination),
        function (log, _) {
          client.fileSystem.msConcat(destination, accountName, paths, _);
      }, _);
  ...
  dataLakeStoreFileSystem.command('move [accountName] [path] [destination] [force]')
    .description($('Concatenates the specified list of files into the specified destination file.'))
    .usage('[options] <accountName> <path> <destination> <force>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
  ...
  dataLakeStoreFileSystem.command('addcontent [accountName] [path] [value]')
    ...
    .execute(function (accountName, path, value, options, _) {
      ...
      function (log, _) {
        var response = client.fileSystem.internalBeginAppend(path, accountName, null, _);
      ...
      function (log, _) {
        ...
        try {
          ...
          while(offset < fileSizeInBytes ) {
            ...
            var response = client.fileSystem.internalBeginOpen(path, accountName, parameters, _);
  ...
  dataLakeStoreFileSystem.command('read [accountName] [path] [length] [offset]')
    ...
    .usage('[options] <accountName> <path> <length> <offset>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    ...
    .execute(function (accountName, path, length, offset, options, _) {
      ...
      function (log, _) {
        var client = utils.createDataLakeStoreFileSystemManagementClient(subscription);
        response = client.fileSystem.internalBeginOpen(path, accountName, parameters, _);
      }, _);
 ...
 dataLakeStoreFileSystemPermissions.command('delete [accountName] [path] [defaultAcl]')
    ...
    .usage('[options] <accountName> <path> <default>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    ...
    .execute(function (accountName, path, defaultAcl, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete Data Lake ACLs for account %s at path %s? [y/n] '), accountName, path), _)) {
        return;
      }
    ...
    .execute(function (accountName, path, aclEntries, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete Data Lake ACL entries: %s for account %s at path %s? [y/n] '), aclEntries, accountName, path), _)) {
        return;
      }
    ...
    .description($('sets the specified Data lake folder ACE entry or entries'))
    .usage('[options] <accountName> <path> <aclEntries>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    ...
    .execute(function (accountName, path, aclEntries, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Potentially overwrite existing Data Lake ACL entries: %s for account %s at path %s? [y/n] '), aclEntries, accountName, path), _)) {
        return;
      }
    ...
    .execute(function (accountName, path, aclSpec, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Overwrite existing Data Lake ACL with the following ACL: %s for account %s at path %s? [y/n] '), aclSpec, accountName, path), _)) {
        return;
      }
  ...
  dataLakeStoreAccount.command('show [accountName] [resource-group]')
    ...
    .execute(function (accountName, resourceGroup, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete Data Lake Account %s? [y/n] '), accountName), _)) {
        return;
      }
