...
exports.init = function (cli) {
  ...
  dataLakeStoreFileSystem.command('list [accountName] [path] [listSize]')
    ...
    .execute(function (accountName, path, value, folder, force, options, _) {
      ...
      if(folder) {
        var result = client.fileSystem.mkdirs(path, accountName, _).operationResult;
  ...
  dataLakeStoreAccount.command('set [accountName] [resource-group] [defaultGroup]')
    ...
    .execute(function (accountName, resourceGroup, defaultGroup, options, _) {
      ...
      var dataLakeStoreAccount = client.dataLakeStoreAccount.get(resourceGroup, accountName, _);
  ...
  function createOrUpdateDataLakeStoreAccount(subscription, accountName, resourceGroup, location, defaultGroup, tags, _) {
      ...
      return client.dataLakeStoreAccount.get(resourceGroup, accountName, _);
  }
  ...
  function listAllDataLakeStoreAccounts(subscription, resourceGroup, _) {
    ...
    if (!resourceGroup) {
      ...
      while (response.nextPageLink) {
        response = client.dataLakeStoreAccount.listNext(response.nextPageLink);
        accounts.push.apply(accounts, response.value);
      }
    }
    else {
      var response = client.dataLakeStoreAccount.listByResourceGroup(resourceGroup, _);
      var accounts = response.value;
      while (response.nextPageLink) {
        response = client.dataLakeStoreAccount.listByResourceGroupNext(response.nextPageLink);
        accounts.push.apply(accounts, response.value);
      }
    }
