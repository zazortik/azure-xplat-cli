...
var KEY_DEST_TYPE_MAP = { 'Software': 'RSA', 'HSM': 'RSA-HSM' };
var KEY_DESTS = Object.keys(KEY_DEST_TYPE_MAP);

var KEY_OPS = ['encrypt', 'decrypt', 'wrap', 'unwrap', 'sign', 'verify', 'backup', 'restore'];

exports.init = function(cli) {
    var log = cli.output;
    var withProgress = cli.interaction.withProgress.bind(cli.interaction);

    var key = cli.category('vault').category('key')
        .description($('Commands to manage vault keys'));
    ...
    key.command('list [vault-name]')
        ...
        .execute(function(vaultName, options, _) {
            ...
            log.table(keys, function(row, item) {
                row.cell($('Id'), item.kid);
    ...
    key.command('list-versions [vault-name] [key-name]')
        ...
        .execute(function(vaultName, keyName, options, _) {
            ...
            log.verbose('arguments: ' + JSON.stringify({vaultName:vaultName,keyName:keyName,options:options}));
                  ...
                  for (;;) {
                      ...
                      if (items && items.length) {
                          for (var i = 0; i < items.length; ++i) {
                              var keyIdentifier = parseKeyIdentifier(items[i].kid);
    ...
    key.command('create [vault-name] [key-name]')
        ...
        .execute(function(vaultName, keyName, options, _) {
            ...
            parseKeyPropertiesArguments(vaultName, keyName, keyVersion, options, true, _);
    ...
    key.command('import [vault-name] [key-name]')
        ...
        .option('-d, --destination <destination>', util.format($('tells if the key is software-protected or HSM-protected; valid values: [%s]'), KEY_DESTS.join(', ')))
        .option('--enabled <enabled>', $('tells if the key should be enabled; valid values: [false, true]; default is true'))
        .option('-e, --expires <datetime>', $('key expiration time, in UTC format'))
        .option('-n, --not-before <datetime>', $('time before which key cannot be used, in UTC format'))
        .option('-o, --key-ops <key-ops>', util.format($('JSON-encoded array of strings representing key operations; each string can be one of [%s]'), KEY_OPS.join(', ')))
        .option('-t, --tags <tags>', $('Tags to set on the key. Can be multiple in the format \'name=value\'. Name is required and value is optional. For example, -t tag1=value1;tag2'))
        .execute(function(vaultName, keyName, options, _) {
            ...
            var keyVersion; // must be undefined on this command.
            parseKeyPropertiesArguments(vaultName, keyName, keyVersion, options, options.pemFile, _);
            ...
            if (v != 1) {
                ...
                throw new Error($('Could not establish file to import from command arguments'));
            }
            ...
            if (options.pemFile) {
              ...
              var data = fs.readFileSync(options.pemFile);
              log.verbose('setting RSA parameters from PEM data');
              setRsaParameters(request.key, forge.pki.privateKeyFromPem(data));
              ...
              var data = fs.readFileSync(options.byokFile);
    ...
    key.command('set-attributes [vault-name] [key-name] [key-version]')
        ...
        .execute(function(vaultName, keyName, keyVersion, options, _) {
            ...
            parseKeyPropertiesArguments(vaultName, keyName, keyVersion, options, false, _);
    ...
    key.command('show [vault-name] [key-name] [key-version]')
        ...
        .execute(function(vaultName, keyName, keyVersion, options, _) {
            ...
            log.verbose('arguments: ' + JSON.stringify({vaultName:vaultName,keyName:keyName,keyVersion:keyVersion,options:options}));
    ...
    key.command('backup [vault-name] [key-name] [output-file]')
        ...
        .execute(function(vaultName, keyName, outputFile, options, _) {
            ...
            log.verbose('arguments: ' + JSON.stringify({vaultName:vaultName,keyName:keyName,outputFile:outputFile,options:options}));
            ...
            var data = base64UrlToBuffer(blob.value);
    ...
    key.command('restore [vault-name] [input-file]')
        ...
        .usage('[--vault-name] <vault-name> [--key-name] <key-name> [--input-file] <input-file> [options]')
        .option('-u, --vault-name <vault-name>', $('the vault name'))
        ...
        .execute(function(vaultName, inputFile, options, _) {
            ...
            log.verbose('arguments: ' + JSON.stringify({vaultName:vaultName,inputFile:inputFile,options:options}));
            ...
            var request = {
                value: bufferToBase64Url(buffer)
            };
    ...
    function parseKeyIdentifier(keyIdentifier) {
        var parsed = url.parse(keyIdentifier);
        ...
        var keyPath = parsed.pathname.split('/');
        if (keyPath.length < 3 || !utils.ignoreCaseEquals(keyPath[1], 'keys')) {
            throw unsupported();          
        }
        
        var keyName = keyPath[2];
        var keyVersion;
        if (keyPath.length > 3) {
            keyVersion = keyPath[3];
        }
        ...
        function unsupported() {
            throw new Error(util.format($('Unsupported key identifier: %s'), keyIdentifier));
        }        
    }

    function parseKeyPropertiesArguments(vaultName, keyName, keyVersion, options, requireDestination, _) {
        ...
        options.keyOps = parseArrayArgument('key-ops', options.keyOps, KEY_OPS, []);
    ...
    function showKey(key) {
        cli.interaction.formatOutput(key, function(key) {
            cli.interaction.logEachData($('key'), key.key);
            cli.interaction.logEachData($('attributes'), getAttributesWithPrettyDates(key.attributes));
            cli.interaction.logEachData($('tags'), key.tags);
            log.data('');
        });
    }
    ...
    function setHsmParameters(dest, byokBlob) {
        dest.key_hsm = bufferToBase64Url(byokBlob);
    }
