...
exports.init = function(cli) {
    ...
    vault.command('create [vault-name]')
        ...
        .option('--enabled-for-deployment', $('reserved for future use'))
        .option('-s, --subscription <subscription>', $('the subscription identifier'))
        .execute(function(vaultName, options, _) {
            ...
            options.sku = parseEnumArgument('sku', options.sku, SKU_TYPE, SKU_TYPE[0]);
            options.tags = parseTagsArgument('tags', options.tags);
            options.enabledForDeployment = options.enabledForDeployment ? true : false; // Normalize boolean value.
            ...
            var properties = {
                enabledForDeployment: options.enabledForDeployment,
                sku: {
                    family: 'A',
                    name: options.sku
                },
    ...
    vault.command('set-attributes [vault-name]')
        ...
        .execute(function(vaultName, options, _) {
            ...
            options.sku = parseEnumArgument('sku', options.sku, SKU_TYPE, properties.sku.name);

            options.tags = parseTagsArgument('tags', options.tags);
            if (options.tags) {
              if (!options.resetTags) {
                options.tags = mergeTags(vaultResource.tags, options.tags);
              }
            ...
            if (!__.isUndefined(options.enabledForDeployment)) {
                ...
                options.enabledForDeployment = properties.enableForDeployment;
            }
            ...
            properties.enableForDeployment = options.enableForDeployment;
    ...
    vault.command('set-policy [vault-name]')
        ...
        .execute(function(vaultName, options, _) {
            ...
            options.permsToKeys = parseArrayArgument('perms-to-keys', options.permsToKeys, KEY_PERMS, []);
            options.permsToSecrets = parseArrayArgument('perms-to-secrets', options.permsToSecrets, SECRET_PERMS, []);
            
            if (!options.permsToKeys.length && !options.permsToSecrets.length) {
                log.error($('You must inform permissions in at least one of the following:'));
                log.error($('    --perms-to-keys <perms-to-keys>'));
                log.error($('    --perms-to-secrets <perms-to-secrets>'));
                log.error($('Or use the delete-policy command to remove all permissions'));
                throw new Error($('Policy cannot have empty permissions'));
            }
            ...
            policies.push({
                ...
                permissions: {
                    keys: options.permsToKeys,
                    secrets: options.permsToSecrets
                }
            });
    ...
    function parseEnumArgument(argName, argValue, validValues, _default) {
