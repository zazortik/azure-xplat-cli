...
underscore.extend(RoleAssignments.prototype, {
  ...
  queryAssignmentsForList: function (principal, scope, roleName, shouldExpandPrincipalGroups, shouldIncludeClassicAdmins, cli, subscription, _) {
    var assignments;
    var objectType = {};
  ...
  queryAssignmentsForDelete: function (principal, scope, roleName, roleId, cli, subscription, _) {
    ...
    if (roleName) {
      ...
      roleDefinitions = this.authzClient.roleDefinitions.listWithFilters(filterParameters, _).roleDefinitions;
    }
    else {
      // Get role definition by Id Guid
      var roleDefinition = this.authzClient.roleDefinitions.get(roleId, _).roleDefinition;
