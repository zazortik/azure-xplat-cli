...
underscore.extend(RoleAssignments.prototype, {
  ...
  fillInPrincipalName: function (assignments, _) {
    ...
    assignments.forEach(function (assignment) {
      //fill in object id as the default, and we will then try to find the logical name.
      principalNames[assignment.properties.principalId] = assignment.properties.principalId;
    });
    ...
    if (allIds.length > 0) {
      ...
      for (var i = 0; i < objects.length; i++) {
        principalNames[objects[i].objectId] = (objects[i].upn || objects[i].displayName);
      }
      assignments.forEach(function (assignment) {
