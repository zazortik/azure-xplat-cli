...
function displayARoleDefinition(row, role) {
  ...
  var actions = [];
  var notActions = [];
  for (var i = 0; i < role.properties.permissions.length; i++) {
    actions = actions.concat(role.properties.permissions[i].actions);
    notActions = notActions.concat(role.properties.permissions[i].notActions);
  }

  row.cell($('actions'), actions.join());
  row.cell($('notActions'), notActions.join());
}