...
exports.init = function (cli) {
  ...
  role.command('show [name]')
    ...
    console.log('ROLE_SHOW:%s', util.inspect(roles[0]));
  ...
  role.command('create [inputfile] [roledefinition]')
    ...
    .execute(function(file, roledefinition, options, _) {

      if (options.inputfile && options.roledefinition) {
      ...
      var roleToCreate = roleUtils.getRoleToCreateFromOptions(options, _);
      var parameters = roleUtils.validateAndConstructRoleDefinitionParameters(cli, roleToCreate, _);

      var subscription = profile.current.getSubscription(options.subscription);
      var authzClient = rbacClients.getAuthzClient(subscription);
      
      var progress = cli.interaction.progress($('Creating role definition'));
      var roleDef = null;
      try {
        roleDef = authzClient.roleDefinitions.createOrUpdate(parameters.roleDefinition.name, parameters, _);
      } finally {
        progress.end();
      } 
      ...
      cli.interaction.formatOutput(roleDef, function (role) {
        if (role) {
          roleUtils.showRoleDefinition(role);
        }
      });
    });
};
