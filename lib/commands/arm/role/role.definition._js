...
exports.init = function (cli) {
  ...
  role.command('delete [id] [name]')
    ...
    if (!id && !name) {
      throw new Error($('Either id or name need to be specified.'));
    }
    ...
    if (id) {
      ...
      if (roles.length === 0) {
        throw new Error(util.format($('No role definition found with name "%s"'), name));
      }
    ...
    if (options.passthru && deleteResult.roleDefinition) {
      cli.interaction.formatOutput(deleteResult, function (data) {
