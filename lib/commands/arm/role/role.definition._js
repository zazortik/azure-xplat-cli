...
exports.init = function (cli) {
  ...
  role.command('create [inputfile] [roledefinition]')
    ...
    .execute(function(inputfile, roledefinition, options, _) {
      ...
      var roleToCreate = roleUtils.getRoleToCreateFromOptions(inputfile, roledefinition, _);
      ...
      cli.interaction.formatOutput(roleDef, function (log, role) {
        if (role) {
          roleUtils.showRoleDefinition(log, role);
        }
      });
    });
};
