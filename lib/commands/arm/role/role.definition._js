...
exports.init = function (cli) {
  ...
  role.command('set [inputfile] [roledefinition]')
    ...
    '\n     The role definition for the updated custom role MUST contain the Id and all other required properties of the role even if they are not updated: DisplayName, Description, Actions, AssignableScopes. NotActions is optional.' +
    '\n' +
    ...
    .option('-f --inputfile <inputfile>', $('File name containing a single json role definition to be updated. Id, DisplayName, Description, Actions and AssignableScopes are required.'))
    ...
    if (!inputRole.id) {
      throw new Error($('Id of the role to be updated must be specified as a valid guid.'))
    }
