...
exports.init = function (cli) {
  ...
  role.command('create [inputfile] [roledefinition]')
    ...
    .option('--subscription <subscription>', $('Subscription id or name of where the role definition will be created.'))
    .execute(function(inputfile, roledefinition, options, _) {
      ...
      var roleToCreate = roleUtils.getRoleToCreateOrUpdate(inputfile, roledefinition, _);
      var parameters = roleUtils.validateAndConstructCreateParameters(cli, roleToCreate, _);
  ...
  role.command('set [inputfile] [roledefinition]')
    ...
    .option('-f --inputfile <inputfile>', $('File name containing a single role definition.'))
    .option('-r --roledefinition <roledefinition>', $('A JSON-formatted string containing the role definition'))
    .option('--subscription <subscription>', $('Subscription id or name of where the role definition will be updated.'))
    .execute(function(inputfile, roledefinition, options, _) {
      if (inputfile && roledefinition) {
        throw new Error($('Either inputfile or roledefinition need to be specified. Not both.'));
      }
      ...
      var inputRole = roleUtils.getRoleToCreateOrUpdate(inputfile, roledefinition, _);
      ...
      var parameters = roleUtils.constructRoleDefinitionUpdateParameters(cli, inputRole, getResult, _);
      var roleDefinitionIdGuid = parameters.roleDefinition.name;
