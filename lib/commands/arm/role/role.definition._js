...
exports.init = function (cli) {
  ...
  role.command('create [inputfile] [roledefinition]')
    .description($('Creates a custom role in Azure RBAC. Provide either a JSON role definition file or a JSON-formatted string containing the role definition as input. Use the Get-AzureRmProviderOperation command to list all operations that can be secured using RBAC.'+
    ...
    '\n     3) Actions: the set of operations to which the custom role grants access. Use Get-AzureRmProviderOperations to get the operation for Azure resource providers that can be secured using Azure RBAC. Following are some valid operation strings' +
