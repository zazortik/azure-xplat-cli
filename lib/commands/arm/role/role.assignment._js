...
function getObjectId(objectId, upn, mail, spn, subscription, _) {
  if (objectId) {
    return objectId;
  }
  ...
  if (upn) {
    graphQueryResult = graphClient.user.getBySignInName(objectName, _);
  ...
  if (spn) {
    graphQueryResult = graphClient.servicePrincipal.getBySPN(spn, _); //@@@ library support
