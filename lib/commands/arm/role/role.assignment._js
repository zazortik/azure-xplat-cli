...
exports.init = function (cli) {
  ...
  roleAssignment.command('create [objectId] [signInName] [spn] [roleName] [roleId] [scope] [resource-group] [resource-type] [resource-name]')
    ...
    .option('-d --roleId <roleId>', $('The Id of the RBAC role that needs to be assigned to the principal.'))
    ...
    .execute(function (objectId, signInName, spn, roleName, roleId, scope, resourceGroup, resourceType, resourceName, options, _) {
    ...
    var progress = cli.interaction.progress($('Finding role with specified name'));
      try {
        if (roleName) {
          ...
          matchedRoles = authzClient.roleDefinitions.list(scope, filterParameters, _).roleDefinitions;
        }
        else {
          matchedRoles.push(authzClient.roleDefinitions.get(roleId, scope, _).roleDefinition);
        }
    } finally {
      progress.end();
    }
    ...
    if (matchedRoles) {
      if (matchedRoles.length === 1) {
        ...
        throw new Error(util.format($('Multiple role definitions found with name \'%s\'. Specify role definition using it\'s Id instead.'), roleName));
      }
      else {
        if (roleName) {
          ...
          throw new Error(util.format($('Role with Id \'%s\' was not found'), roleId));
        }
      }
    }
    ...
    try {
      ...
      if (createdAssignment) {
        var assignmentToDisplay = helper.fillRoleAndPrincipalDetailsForAssignment(createdAssignment.roleAssignment, role, _);
  ...
  roleAssignment.command('list [objectId] [signInName] [spn] [roleName] [roleId] [scope] [resource-group] [resource-type] [resource-name]')
    .usage('[objectId] [signInName] [spn] [roleName] [roleId] [scope] [resource-group] [resource-type] [resource-name]' +
      '\n' +
      '\n           -----    Example 1   -------' +
    ...
    .option('-o --roleName <roleName>', $('Name of the Role that is assigned to the principal i.e. Reader, Contributor, Virtual Network Administrator, etc.'))
    .option('-d --roleId <roleId>', $('Id of the Role that is assigned to the principal'))
    ...
    .execute(function (objectId, signInName, spn, roleName, roleId, scope, resourceGroup, resourceType, resourceName, options, _) {
      ...
      try {
        assignments = assignmentCollection.queryAssignmentsForList(
          {
            objectId: objectId,
            signInName: signInName,
            spn: spn
          },
          scopeString, roleName, roleId, expandGroups, includeAdmins, cli, subscription, _);
  ...
  roleAssignment.command('delete [objectId] [signInName] [spn] [roleName] [roleId] [scope] [resource-group] [resource-type] [resource-name]')
   .usage('[objectId] [signInName] [spn] [roleName] [roleId] [scope] [resource-group] [resource-type] [resource-name]' +
      '\n' +
      '\n           -----    Example 1   -------' +
    ...
    .option('-d --roleId <roleId>', $('The Id of the RBAC role for which the assignment needs to be deleted.'))
    ...
    .execute(function (objectId, signInName, spn, roleName, roleId, scope, resourceGroup, resourceType, resourceName, options, _) {
      ...
      var role = roleName ? roleName : roleId;
      ...
      var assignments = assignmentCollection.queryAssignmentsForDelete(principal, scopeString, roleName, roleId, cli, subscription, _);

      if (roleId){
        // filter assignments on roleId
        assignments = assignments.filter(function (a) {
          return utils.ignoreCaseEquals(a.properties.roleDefinitionId, roleId);
        });
      }
      ...
      if (assignments.length === 1) {
        ...
        // delete all, if all assignments are to the same roleId
        ...
        }))
        {
          ...
          try {
            for (var i = 0; i < assignments.length; i++) {
              authzClient.roleAssignments.deleteById(assignments[i].id, _);
            }
          } finally {
            progress.end();
          }
