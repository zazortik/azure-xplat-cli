    ...
    var progress = cli.interaction.progress($('Finding role with specified name'));
      try {
        if (roleName) {
          ...
          matchedRoles = authzClient.roleDefinitions.listWithFilters(filterParameters, _).roleDefinitions;
        }
        else {
          matchedRoles.push(authzClient.roleDefinitions.get(roleId, _).roleDefinition);
        }
    } finally {
      progress.end();
    }
