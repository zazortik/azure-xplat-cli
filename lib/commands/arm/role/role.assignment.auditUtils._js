...
exports.processInsightsResults = function (cli, log, insightsResults, subscription, graphClient, authzClient, _) {
  ...
  var scopeForRoleDefinitions = '/subscriptions/' + subscription.id;
  var filterParameters = { atScopeAndBelow: true };

  var result = authzClient.roleDefinitions.list(scopeForRoleDefinitions, filterParameters, _);
...
function getPrincipalDetails(principalId, principalDetailsCache, graphClient, _) {
  ...
  try {
    user = graphClient.userOperations.get(principalId, _);
  } catch (ex) {
  ...
  if (user) {
    ...
    var group;
    try {
      group = graphClient.groupOperations.get(principalId, _);
    } catch (ex) {
      if (ex.statusCode !== 404) {
        throw ex;
      }
    ...
    if (group) {
      ...
      var sp;
      try {
        sp = graphClient.servicePrincipalOperations.get(principalId, _);
      } catch (ex) {
        if (ex.statusCode !== 404) {
          throw ex;
        }
