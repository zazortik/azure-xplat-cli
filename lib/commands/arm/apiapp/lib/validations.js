...
//
function populateDeploymentTemplateSchemas(initialDownloadUris, failedSchemaUris, done) {
  ...
  async.each(urlsToDownload, function (uri, next) {
    ...
    populateDeploymentTemplateSchemas([], failedSchemaUris, done);
  });
}
...
function hasApiDefinition(packageDir, callback) {
  readManifest(packageDir, function (err, manifest) {
    ...
    fs.exists(swaggerPath, function (exists) {
      if (!exists) {
        return callback(null, [fmt($('Package %s has neither dynamic or static api definition'), packageDir)]);
      }
      callback(null, []);
    });
  });
...
function validatePackageSource(packageDir, callback) {
  dirExists(packageDir, function (err, existsErrors) {
    ...
    populateDeploymentTemplateSchemas([constants.templateSchemaUri], function (err) {
      if (err) { return callback(err); }
