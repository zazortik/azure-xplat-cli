...
exports.init = function initApiAppCommands(cli) {
  ...
  apiapp.command('list [resource-group]')
    ...
    .execute(function (resourceGroup, options, _) {
      ...
      var response = cli.interaction.withProgress($('Listing ApiApps'),
        function (log, _) {
          if (!resourceGroup) {
            return client.apiApps.listAll(_);
          } else {
            return client.apiApps.list(resourceGroup, _);
          }
        },
      _);

      cli.interaction.formatOutput(response.apiApps, function (data) {
        if (data.length > 0) {
          log.table(data, function (row, item) {
            ...
            row.cell($('Status'), statusCode(item.status));
  ...
  apiapp.command('create [resource-group] [name] [plan]')
    ...
    .option('-p --plan <plan>', $('Resource Id of the App Service Hosting Plan to use. Location of the plan must be the same as the location of the resource group'))
    .option('-u --nuget-package <package>', $('Package name and version to deploy (optional)'))
    .option('--parameters <parameters>', $('A JSON-formatted string containing parmaeters'))
    .option('--parameters-file <parametersFile>', $('A file containing parameters in JSON format'))
    .option('-s --subscription <subscription>', $('The subscription identifier'))
    .option('--nowait', $('Do not wait for the deployment to complete. Return as soon as the deployment is created'))
    .execute(function (resourceGroup, name, plan, options, _) {
      ...
      cli.interaction.withProgress($('Checking resource group and hosting plan'),
        function (log, _) {
          ...
          if (!hostingPlanResource) {
            throw new Error(util.format($('The hosting plan \'%s\' does not exist'),
              options.plan));
          }
...
function statusCode(statusCheckEntries) {
  if (!statusCheckEntries || statusCheckEntries.length === 0) {
    return $('Ok');
  }

  return statusCheckEntries[0].StatusLevel;
}
...
function filePrompter(options, _) {
  if (options.parametersFile) {
    var contents = fs.readFile(options.parametersFile, { encoding: 'utf8'}, _);
    var parameterValues = JSON.parse(contents.trim());
    if (options.name) {
      parameterValues.$apiAppName = options.name;
    }
    return propertyPrompts.object(parameterValues);
