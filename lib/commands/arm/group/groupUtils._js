  ...
  cli.interaction.withProgress($('Registering providers'),
    function (log, _) {
      for (var i = 0; i < result.requiredProviders.length; i++) {
        ...
        subscription.registerArmProvider(namespace, _);
      }
    },
    _);
...
exports.getGalleryTemplateFile = function (subscription, galleryTemplateName, _) {
  var galleryClient = utils.createGalleryClient(subscription);
...
function getTemplateParams(cli, subscription, options, _)
{
  var templateContent = options.templateFile ?
    getTemplateContent(subscription, options.templateFile, options.galleryTemplate, _) :
    getTemplateContent(subscription, options.templateUri, options.galleryTemplate, _);
