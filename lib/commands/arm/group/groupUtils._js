...
function missingParameters(userParameters, options, subscription, _) {
  var templateContent = options.templateFile ?
    getTemplateContent(subscription, options.templateFile, _) :
    getTemplateContent(subscription, options.templateUri, _);
  var templateParameters = Object.keys(templateContent.parameters);
  var deploymentParameters = Object.keys(userParameters);

  if(templateParameters.length !== deploymentParameters.length){
    var smallerList = templateParameters.length < deploymentParameters.length ? 'Template' : 'Deployment';
    var missing = getMissingParameterDefintions(templateParameters, deploymentParameters); 
  ...
  var missingValues = templateParameters.filter(function(parameter) {
    return userParameters[parameter].value === undefined || userParameters[parameter].value === null || userParameters[parameter].value === '';
  });

  var missingObj = {};
  missingObj.parameters = {};
  missingValues.forEach(function(key) {
      if(userParameters.hasOwnProperty(key)){
        missingObj.parameters[key] = userParameters[key];
        missingObj.parameters[key].type = templateContent.parameters[key].type;
        missingObj.parameters[key].metadata = templateContent.parameters[key].metadata;
      }
  });
  return missingObj;
}
