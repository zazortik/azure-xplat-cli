...
exports.createDeployment = function (cli, resourceGroup, name, options, _) {
  ...
  var deploymentObject = cli.interaction.withProgress($('Creating a deployment'),
    function (log, _) {
      ...
      createResponse.requiredProviders = getTemplateProvidres(validationResponse);
      return createResponse;
    }, _);

  cli.output.info(util.format($('Created template deployment "%s"'), name));
 
  if(deploymentObject.requiredProviders.length > 0) {
    // register providres required for new template
    for(var i=0; i<deploymentObject.requiredProviders.length; i++) {
      var namespace = String(deploymentObject.requiredProviders[i]).toLowerCase();
      addKnownProviders(subscription);
      if(!__.contains(profile.knownResourceNamespaces(), namespace)){
        cli.interaction.withProgress(
          util.format($('Registering provider %s'), namespace),
          callRegisterResourceNamespace(subscription, namespace, _),
          _);
      }
    }
  }
...
exports.validateTemplate = function (cli, resourceGroup, options, _) {
  ...
  response.requiredProviders = getTemplateProvidres(response);
...
function getTemplateProvidres(validationResponse)
{
...
function addKnownProviders(subscription) {
  __.each(subscription.registeredResourceNamespaces, function(ns) {
    profile.addKnownResourceNamespace(String(ns));
  });

  __.each(subscription.registeredProviders, function(ns) {
    profile.addKnownProvider(String(ns));
  });
}

function callRegisterResourceNamespace(subscription, namespace, callback) {
  subscription.registerResourceNamespace(namespace, callback);
}