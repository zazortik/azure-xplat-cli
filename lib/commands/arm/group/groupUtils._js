...
exports.createDeployment = function (cli, resourceGroup, name, options, _) {
  ...
  var result = cli.interaction.withProgress($('Creating a deployment'),
    function (log, _) {
      var createResponse;
      if (options.nowait) {
        createResponse = client.deployments.beginCreateOrUpdate(resourceGroup, name, templateParameters, _);
      } else {
        createResponse = client.deployments.createOrUpdate(resourceGroup, name, templateParameters, _);
      }
