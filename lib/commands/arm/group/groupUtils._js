...
function createGalleryClient(environment) {
  return utils.createClient('createGalleryClient', new azure.AnonymousCloudCredentials(),
    profile.current.getEnvironment(environment).galleryEndpointUrl);
}
...
function createDeploymentParameters(cli, subscription, resourceGroup, options, _) {
  if (options.galleryTemplate && options.templateFile && options.templateUri) {
    throw new Error($('Either --gallery-template or --template-file or --template-uri need to be specified. Not combined.'));
  } else if (!options.galleryTemplate && !options.templateFile && !options.templateUri) {
    throw new Error($('Either --gallery-template or --template--file or --template-uri need to be specified.'));
  }
