...
exports.init = function (cli) {
  ...
  group.createResourceGroup = function (name, location, options, _) {
    ...
    if ('tags' in options) {
      tags = options.tags ? tagUtils.buildTagsObject(options.tags) : {};
    }
  ...
  group.command('list')
    ...
    .execute(function (options, _) {
      ...
      if (options.tags) {
        tagUtils.populateWithTagInfo(options.tags, parameters);
      }
  ...
  function showResourceGroup(group) {
    ...
    log.data($('Tags: %s'), tagUtils.getTagsInfo(group.tags));
  }
