...
exports.init = function (cli) {
  ...
  group.command('list')
    ...
    .execute(function (options, _) {
      ...
      try {
        resourceGroups = client.resourceGroups.list(parameters, _).resourceGroups;
      } finally {
        progress.end();
      }
      ...
      if (options.details) {
        ...
        try {
          for (var i in resourceGroups) {
            ...
            resourceGroup.permissions = client.resourceGroups.getPermissions(resourceGroup.name, _).permissions;
          }
        } finally {
          progress.end();
        }
  ...
  group.command('show [name]')
    ...
    .execute(function (name, options, _) {
      ...
      try {
        ...
        resourceGroup.permissions = client.resourceGroups.getPermissions(name, _).permissions;
      } finally {
        progress.end();
      }
  ...
  function showResourceGroup(group) {
    log.data($('Name:'), group.name);
    log.data($('Location:'), group.location);
    log.data($('Provisioning State:'), group.provisioningState);
    log.data($('Tags:'), tagUtils.getTagsInfo(group.tags));
  }
