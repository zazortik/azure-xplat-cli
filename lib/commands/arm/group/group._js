...
exports.init = function (cli) {
    ...
    .execute(function (options, _) {
      ...
      try {
        ...
        while(response.nextLink) {
          ...
          resourceGroups.concat(response);
        }
      } finally {
        progress.end();
      }
      ...
      if (parameters.tagName) {
        resourceGroups = resourceGroups.filter(function (element) {
          return element.tags && (element.tags.hasOwnProperty(parameters.tagName)) &&
      ...
      if (options.details) {
        ...
        try {
          ...
          for (var i in resourceGroups) {
            ...
            resourceGroup.resources = client.resources.list({ resourceGroupName: resourceGroup.name }, _);
  ...
  group.command('show [name]')
    ...
    .execute(function (name, options, _) {
      ...
      try {
        resourceGroup.resources = client.resourceGroups.listResources(name, _);
  ...
  group.command('export [name] [directory]')
  ...
  .option('-c --includeComments', $('export template with comments.'))
  .option('-p --includeParameterDefaultValue', $('export template parameter with default value'))
  ...
  .execute(function (name, directory, options, _) {
    ...
    var client = utils.createResourceClient(subscription);
    try {
      client.resourceGroups.checkExistence(name, _);
    }
    catch (err) {
      throw new Error(util.format($('Resource group %s does not exist.'), name));
    }

    var confirm = cli.interaction.confirm.bind(cli.interaction);
    var result = cli.interaction.withProgress(
      util.format($('Exporting resource group as template %s'), name),
      function (log, _) {
        var exportOptions;
        ...
        return client.resourceGroups.exportTemplate(name, parameters, _);
      }, _);

    var template = result.template;

    //If there were errors in the response, display warnings instead. When ARM tries to export resources, even if one succeeds, we send 200 and return the unexported resources in error object
    ...
    if (fileName) {
      ...
      log.info(util.format($('Template downloaded to %s'), fileName));
    }
