...
exports.init = function(cli) {
  ...
  groupTemplate.command('validate [resource-group]')
    ...
    //TODO: comment out till CSM supports validation mode
...
function createGalleryClient(environment) {
  ...
  if (!environment && profile.current.currentSubscription) {
    galleryEndpointUrl = profile.current.currentSubscription.publicGalleryEndpointUrl;
  } else {
    galleryEndpointUrl = profile.current.getEnvironment(environment).publicGalleryEndpointUrl;
  }
