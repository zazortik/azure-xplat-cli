...
exports.init = function (cli) {
  ...
  webapp.command('create [resourcegroup] [name] [location] [plan]')
        .description($('create a web app'))
        .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
        .option('--name <name>', $('the name of the webapp to create'))
        .option('--slot <slot>', $('the name of the slot'))
        .option('--location <location>', $('the geographic region to create the webapp'))
        .option('--plan <plan>', $('the name of the app service plan eg: Default1.'))
  ...
  webapp.command('stop [resourcegroup] [name]')
        .description($('Stop a web app'))
        .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
        .option('--name <name>', $('the name of the webapp to stop'))
        .option('--slot <slot>', $('the name of the slot'))
      ...
      log.info('Site '  + webSiteSlotName + ' has been stopped ');
    });

  webapp.command('start [resourcegroup] [name]')
        .description($('Start a web app'))
        .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
        .option('--name <name>', $('the name of the webapp to start'))
        .option('--slot <slot>', $('the name of the slot'))
        .option('-s, --subscription <id>', $('the subscription id'))
        .execute(function (resourcegroup,name, options, _) {
      var subscription = profile.current.getSubscription(options.subscription);
      ...
      log.info('Site '  + webSiteSlotName + ' has been started ');
    });
  webapp.command('restart [resourcegroup] [name]')
        .description($('Stop and then start a web app'))
        .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
        .option('--name <name>', $('the name of the webapp to restart'))
        .option('--slot <slot>', $('the name of the slot'))
        .option('-s, --subscription <id>', $('the subscription id'))
        .execute(function (resourcegroup,name,options, _) {
      var subscription = profile.current.getSubscription(options.subscription);
      ...
      log.info('Site '  + webSiteSlotName + ' has been restarted ');
    });
  webapp.command('delete [resourcegroup] [name]')
    .description($('Delete a webapp'))
    .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
    .option('--name <name>', $('the name of the webapp to delete'))
    ...
    .execute(function (resourcegroup,name,options, _) {
      ...
      log.info(util.format($('Web app %s has been deleted'), name));
    });

  webapp.command('list [resourcegroup]')
    ...
    .execute(function (resourcegroup,options, _) {
      ...
      cli.interaction.formatOutput(result.webSites, function (data) {
        if (data.length > 0) {
          ...
          log.info($('No web apps found.'));
        }
      });
    });

  webapp.command('show [resourcegroup] [name]')
    .description($('Get an available webapp'))
    .option('--resourcegroup <resourcegroup>', $('the name of the resourceGroup'))
    .option('--name <name>', $('the name of the webapp to show'))
