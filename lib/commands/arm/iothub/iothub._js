...
exports.init = function (cli) {
  ...
  iotHubConnectionString.command('show [resource-group] [name]')
    ...
    .execute(function (resourceGroup, name, options, _) {
      ...
      var subscription = profile.current.getSubscription(options.subscription);
      var client = utils.getiotHubClient(subscription);
      var progress = cli.interaction.progress($('Getting IoT hub connection stirng '));
      var result;
      var key;
      ...
      log.info($('Primary ConnectionString    : HostName=' + result.properties.hostName + ';SharedAccessKeyName=' + authorizationPolicies[index].keyName + ';SharedAccessKey=' + authorizationPolicies[index].primaryKey));
      log.info($('Secondary ConnectionString  : HostName=' + result.properties.hostName + ';SharedAccessKeyName=' + authorizationPolicies[index].keyName + ';SharedAccessKey=' + authorizationPolicies[index].secondaryKey));
