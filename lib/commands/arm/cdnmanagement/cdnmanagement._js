...
exports.init = function(cli) {
  ...
  //================================================================================================================================
  //Profiles opertaion
  var profiles = cli.category('profile')
  ...
  // Get profile
  profiles.command('show <profile-name> <resource-group>')
    .description($('Get and show the infomation of a specific cdn profile'))
    .usage('[options] <profile-name> <resource-group>')
    .option('-p, --profile-name [profile-name]', $('Name of the Cdn Profile'))
    .option('-g, --resource-group [resource-group]', $('Name of the Resource Group'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Get cdn profile %s ...'), options.profileName));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.profiles.get(options.profileName, options.resourceGroup, [_]);
      ...
      cli.interaction.formatOutput(result, function(data) {
        if (!data) {
          log.info($('No profile named %s found.'), options.profileName);
        } else {
          log.data('');
  ...
  // Create Profile
  profiles.command('create <profile-name> <resource-group> <location> <sku-name>')
    .description($('Create a profile under given resource group and subscription'))
    .usage('[options] <profile-name> <resource-group> <location> <sku-name> [tags]')
    ...
    .option('-s, --sku-name <sku-name>', $('The pricing sku name of the Azure Cdn Profile'))
    ...
    .option('--subscription <subscription>', $('the subscription identifier'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(profileName, resourceGroup, location, skuName, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Attempting to create cdn profile %s ...'), options.profileName));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.profiles.create(options.profileName, creationParameter, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 200) {
        log.info('Cdn profile ' + options.profileName + ' is getting created...');
      } else {
        log.info('Failed in creating profile ' + options.profileName);
      }
    });

  // Delete Profile
  profiles.command('delete [profile-name] [resource-group]')
    .description($('Create a profile under given resource group and subscription'))
    .usage('[options] <profile-name> <resource-group>')
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    ...
    .execute(function(profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Deleting Cdn Profile %s'), options.profileName));
      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.profiles.deleteIfExists(options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } finally {
        progress.end();
      ...
      if (response.statusCode == 200) {
        log.info('Delete command successfully invoked for Cdn Profile ' + options.profileName);
      } else if (response.statusCode == 204) {
        log.info('Delete sucess, but no profile named ' + options.profileName + ' was found');
      } else {
        log.info('Error in deleting profile ' + options.profileName);
      }
    });

  // Update profile
  profiles.command('set <profile-name> <resource-group>')
    .description($('Update a profile\'s tags'))
    .usage('[options] <profile-name> <resource-group> [tags]')
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile will be created in'))
    .option('-t, --tags [tags]', $('Tags to set to the profile. Can be multiple. ' +
      'In the format of \'name=value\'. Name is required and value is optional. ' +
    ...
    .execute(function(profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Attempting to update tags for cdn profile %s ...'), options.profileName));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.profiles.update(options.profileName, options.resourceGroup, tags, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 202) {
        log.info('Successfully updated tags of profile ' + options.profileName);
      } else {
        log.info('Failed in updating tags of profile ' + options.profileName);
      }
    });

  //================================================================================================================================
  //SSO Uri operation
  var ssoUri = cli.category('ssouri')
  ...
  //Get endpoint
  endpoint.command('show <endpoint-name> <profile-name> <resource-group>')
    ...
    .execute(function(endpointName, profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Getting endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.get(options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  //Create endpoint
  endpoint.command('create <endpoint-name> <profile-name> <resource-group> <location> <origin-name> <origin-host-name>')
    .description($('Create endpoint with given name and properties.'))
    .usage('[options] <endpoint-name> <profile-name> <resource-group> <location> <origin-name> <origin-host-name> [origin-host-header] [origin-path] [content-type-to-compress] [is-compression-enabled] [is-http-allowed] [is-https-allowed] [query-string-caching-behavior]  [http-port] [https-port] [tags]')
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile'))
    ...
    .option('-n, --origin-host-name <origin-host-name>', $('The host name of the origin'))
    .option('-r, --origin-host-header [origin-host-header]', $('The origin host header of the Azure Cdn Endpoint'))
    ...
    .option('-d, --is-compression-enabled [is-compression-enabled]', $('Is the compression enabled for the Cdn'))
    .option('-w, --is-http-allowed [is-http-allowed]', $('Is the http traffic allowed for the Cdn'))
    .option('-a, --is-https-allowed [is-https-allowed]', $('Is the https traffic allowed for the Cdn'))
    ...
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(endpointName, profileName, resourceGroup, location, originName, originHostName, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ///////////////////////
      ...
      var progress = cli.interaction.progress(util.format($('Creating endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.create(options.endpointName, endpointCreateParameters, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  //Update Endpoint
  endpoint.command('set <endpoint-name> <profile-name> <resource-group>')
    .description($('Update endpoint with given properties.'))
    .usage('[options] <endpoint-name> <profile-name> <resource-group> [origin-host-header] [origin-path] [content-type-to-compress] [is-compression-enabled] [is-http-allowed] [is-https-allowed] [query-string-caching-behavior] [tags]')
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile'))
    .option('-r, --origin-host-header [origin-host-header]', $('The origin host header of the Azure Cdn Endpoint'))
    ...
    .option('-c, --content-types-to-compress [content-types-to-compress]', $('The list of mime types that need to be compressed by Cdn edge nodes'))
    .option('-d, --is-compression-enabled [is-compression-enabled]', $('Is the compression enabled for the Cdn'))
    .option('-u, --is-http-allowed [is-http-allowed]', $('Is the http traffic allowed for the Cdn'))
    .option('-s, --is-https-allowed [is-https-allowed]', $('Is the https traffic allowed for the Cdn'))
    .option('-q, --query-string-caching-behavior [query-string-caching-behavior]', $('The way Cdn handles requests with query string'))
    .option('-t, --tags [tags]', $('The tags to associate with the Azure Cdn Endpoint'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(endpointName, profileName, resourceGroup, options, _) {
      ...
      if (options.isCompressionEnabled) {
        endpointUpdateParameters.isCompressionEnabled = cdnManagementUtil.getBooleanFromString(isCompressionEnabled);
      }
      ...
      var progress = cli.interaction.progress(util.format($('Updating endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.update(options.endpointName, endpointUpdateParameters, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  //Delete Endpoint
  endpoint.command('delete <endpoint-name> <profile-name> <resource-group>')
    ...
    .execute(function(endpointName, profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Deleting endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.deleteIfExists(options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 200) {
        log.info('Delete command successfully invoked for endpoint ' + options.endpointName);
      } else if (response.statusCode == 204) {
        log.info('Delete sucess, but no endpoint named ' + options.endpointName + ' was found');
      } else {
        log.info('Error in deleting endpoint ' + options.endpointName);
      }
    });

  //Start endpoint
  endpoint.command('start <endpoint-name> <profile-name> <resource-group>')
    ...
    .execute(function(endpointName, profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Starting endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.start(options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 200) {
        log.info('Command successfully invoked for endpoint ' + options.endpointName + ' and it is now running');
      } else {
        log.info('Error in starting endpoint ' + options.endpointName);
      }
    });

  //Stop endpoint
  endpoint.command('stop <endpoint-name> <profile-name> <resource-group>')
    ...
    .execute(function(endpointName, profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Stopping endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.stop(options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 200) {
        log.info('Command successfully invoked for endpoint ' + options.endpointName + ' and it is now stopped');
      } else {
        log.info('Error in stopping endpoint ' + options.endpointName);
      }
    });

  //Endpoint Purge Content
  endpoint.command('purge <endpoint-name> <profile-name> <resource-group> <content-paths>')
    .description($('Stop an endpoint by endpoint name, profile name, and resource group'))
    .usage('[options] <endpoint-name> <profile-name> <resource-group>')
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    ...
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(endpointName, profileName, resourceGroup, contentPaths, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ///////////////////////
      ...
      var contentPaths = options.contentPaths ? options.contentPaths.split(",") : [];

      var progress = cli.interaction.progress(util.format($('Purging content for endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.purgeContent(options.endpointName, options.profileName, options.resourceGroup, contentPaths, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  endpoint.command('load <endpoint-name> <profile-name> <resource-group> <content-paths>')
    .description($('Stop an endpoint by endpoint name, profile name, and resource group'))
    .usage('[options] <endpoint-name> <profile-name> <resource-group>')
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    ...
    .execute(function(endpointName, profileName, resourceGroup, contentPaths, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Loading content for endpoint named ' + options.endpointName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.endpoints.loadContent(options.endpointName, options.profileName, options.resourceGroup, contentPaths, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  endpoint.command('check <endpoint-name>')
    ...
    .option('-e, --endpoint-name <endpoint-name>', $('Endpoint name'))
    .option('--subscription <subscription>', $('the subscription identifier'))
  ...
  //==================================================================================================================================
  //Origin operation
  var origin = cli.category('origin')
    .description($('Commands to manage Azure cdn profile endpoints'));

  //Get Origin
  origin.command('show <origin-name> <endpoint-name> <profile-name> <resource-group>')
    .description($('Get origin by origin name, endpoint name, profile name, and resource group'))
    .usage('[options] <origin-name> <endpoint-name> <profile-name> <resource-group>')
    .option('-o, --origin-name <origin-name>', $('Name of the origin'))
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(originName, endpointName, profileName, resourceGroup, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ///////////////////////
      ...
      var progress = cli.interaction.progress(util.format($('Getting origin named ' + options.originName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.origins.get(options.originName, options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  //Update Origin
  origin.command('set <origin-name> <endpoint-name> <profile-name> <resource-group>')
    .description($('Update origin of the given origin name, endpoint name, profile name, and resource group'))
    .usage('[options] <origin-name> <endpoint-name> <profile-name> <resource-group> [host-name] [http-port] [https-port]')
    .option('-o, --origin-name <origin-name>', $('Name of the origin'))
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile'))
    .option('-n, --host-name [host-name]', $('Host name'))
    .option('-r, --http-port [http-port]', $('Http port'))
    .option('-s, --https-port [https-port]', $('Https port'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(originName, endpointName, profileName, resourceGroup, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ...
      var progress = cli.interaction.progress(util.format($('Updating origin named ' + options.originName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.origins.update(options.originName, originUpdateParameter, options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  var customDomain = cli.category('customDomain')
  ...
  //Get Custom Domain
  customDomain.command('show <custom-domain-name> <endpoint-name> <profile-name> <resource-group>')
    .description($('Get custom domains by custom domain name, endpoint name, profile name, and resource group'))
    .usage('[options] <endpoint-name> <profile-name> <resource-group>')
    .option('-c, --custom-domain-name <custom-domain-name>', $('Name of the custom domain'))
    .option('-e, --endpoint-name <endpoint-name>', $('Name of the endpoint'))
    .option('-p, --profile-name <profile-name>', $('Name of the profile'))
    .option('-g, --resource-group <resource-group>', $('The resource group of the Azure Cdn Profile'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(customDomainName, endpointName, profileName, resourceGroup, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ///////////////////////
      ...
      options.customDomainName = options.customDomainName || customDomainName;
      options.endpointName = options.endpointName || endpointName;
      options.profileName = options.profileName || profileName;
      options.resourceGroup = options.resourceGroup || resourceGroup;
      ...
      var progress = cli.interaction.progress(util.format($('Getting custom domain named ' + options.customDomainName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.customDomains.get(options.customDomainName, options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      cli.interaction.formatOutput(result, function(data) {
        if (!data) {
          log.info($('No custom domain named %s found.'), options.customDomainName);
        } else {
          log.data('');
  ...
  customDomain.command('create <custom-domain-name> <endpoint-name> <profile-name> <resource-group> <custom-domain-host-name>')
    ...
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function(customDomainName, endpointName, profileName, resourceGroup, customDomainHostName, options, _) {
      ///////////////////////
      // Parse arguments.  //
      ///////////////////////
      ...
      var progress = cli.interaction.progress(util.format($('Creating custom domain named ' + options.customDomainName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.customDomains.create(options.customDomainName, options.endpointName, options.profileName, options.resourceGroup, options.customDomainHostName, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
  ...
  customDomain.command('delete <custom-domain-name> <endpoint-name> <profile-name> <resource-group>')
    ...
    .execute(function(customDomainName, endpointName, profileName, resourceGroup, options, _) {
      ...
      var progress = cli.interaction.progress(util.format($('Updating custom domain named ' + options.customDomainName)));

      var callbackArgs = [];
      var result, response;
      try {
        callbackArgs = client.customDomains.deleteIfExists(options.customDomainName, options.endpointName, options.profileName, options.resourceGroup, [_]);
        result = callbackArgs[0];
        response = callbackArgs[2];
      } catch (e) {
        throw e;
      ...
      if (response.statusCode == 200) {
        log.info('Delete command successfully invoked for custom domain ' + options.customDomainName);
      } else if (response.statusCode == 204) {
        log.info('Delete success, but no custom domain named ' + options.customDomainName + ' was found');
      } else {
