...
exports.init = function (cli) {
  ...
  usage.command('list [reportedStartTime] [reportedEndTime]')
    ...
    if (!reportedStartTime) {
	  reportedStartTime = cli.interaction.promptIfNotGiven($('reportedStartTime mm/dd/yyyy: '), reportedStartTime, _);
    }
    if (!reportedEndTime) {
	  reportedEndTime = cli.interaction.promptIfNotGiven($('reportedEndTime mm/dd/yyyy: '), reportedEndTime, _);
    }
