	...
	usage.command('list [reportedStartTime] [reportedEndTime]')
	  ...
	  .execute(function (reportedStartTime, reportedEndTime, options, _) {
	  	if (!reportedStartTime) {
	  		return cli.missingArgument('reportedStartTime');
	  	}
	  	if (!reportedEndTime) {
	  		return cli.missingArgument('reportedEndTime');
	  	}

	  	var subscription = profile.current.getSubscription(options.subscription);
	  	var client = usageUtils.createUsageManagementClient(subscription);
	  	var progress = cli.interaction.progress($('Listing usage aggregates'));

	  	var result;
	  	try {
	  		result = client.usageAggregates.get(new Date(reportedStartTime), new Date(reportedEndTime), options.granlarity, options.showDetails, options.continuationToken, _);
	  	} finally {
	  		progress.end();
	  	}
	  	console.log("made it past thefinally block");
	  	cli.interaction.formatOutput(result, function (data) {
	  		if (!data) {
	  			log.info($('No usage aggregates for that time period'));
	  		} else {
	  			console.log(data);
