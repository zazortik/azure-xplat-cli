...
__.extend(VirtualMachine.prototype, {
  ...
  getVMList: function (resourceGroupName, _) {
    ...
    try {
      if (resourceGroupName) {
        ...
        vms = this.serviceClients.computeManagementClient.virtualMachines.listAll(_);
        for (i = 0; i < vms.length; i++) {
          vm = vms[i];
          var resourceInfo = utils.parseResourceReferenceUri(vm.id);
          vm.resourceGroupName = resourceInfo.resourceGroupName;
          vm.powerState = this._getVMPowerState(resourceInfo.resourceGroupName, vm.name, _);
        }
      }
    } finally {
      progress.end();
  ...
  getInstanceView: function (resourceGroupName, vmName, _) {
    ...
    try {
      ...
      var result = this.serviceClients.computeManagementClient.virtualMachines.get(resourceGroupName, vmName, getOptions, _);
      return result;
    } catch (e) {
  ...
  createOrUpdateVM: function (resourceGroupName, virtualMachine, isNewVM, _) {
    ...
    try {
      ...
      var vmResult = this.serviceClients.computeManagementClient.virtualMachines.createOrUpdate(resourceGroupName, virtualMachine.name, virtualMachine, _);
      if (vmResult.error) {
  ...
  getVMExtension: function (resourceGroupName, vmName, extensionName, _) {
    ...
    try {
      var getOptions = { expand : 'instanceView' };
      var result = this.serviceClients.computeManagementClient.virtualMachineExtensions.get(resourceGroupName, vmName, extensionName, getOptions, _);
      return result;
    } catch (e) {
      if (e.code === 'ResourceNotFound') {
        return null;
  ...
  createOrUpdateVMExtension: function (resourceGroupName, vmName, vmExtension, isNewExtension, _) {
    ...
    var progress = this.cli.interaction.progress(progressMessage);
    try {
      var result = this.serviceClients.computeManagementClient.virtualMachineExtensions.createOrUpdate(resourceGroupName, vmName, vmExtension.name, vmExtension, _);
      return result;
    } finally {
      progress.end();
    }
  ...
  _expandVM: function (vm, depth, memoize, dependencies, _) {
    ...
    if (utils.hasValidProperty(virtualMachine, 'networkProfile')) {
      ...
      if (networkProfile.networkInterfaces instanceof  Array) {
        for (var i = 0; i < networkProfile.networkInterfaces.length; i++) {
          ...
          var netReferenceUri = networkInterface.id.toLocaleLowerCase();
    ...
    if (utils.hasValidProperty(virtualMachine, 'availabilitySetReference')) {
      var availabilitySetRef = virtualMachine.availabilitySet;
      if (availabilitySetRef.id !== null && availabilitySetRef.id !== undefined) {
        var availReferenceUri = availabilitySetRef.id.toLocaleLowerCase();
  ...
  _getVMPowerState: function (resourceGroupName, vmName, _) {
    try {
      var getOptions = { expand : 'instanceView' };
      var intanceViewResult = this.serviceClients.computeManagementClient.virtualMachines.get(resourceGroupName, vmName, getOptions, _);
      var statuses = intanceViewResult.instanceView.statuses;
