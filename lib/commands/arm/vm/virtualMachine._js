...
__.extend(VirtualMachine.prototype, {
  ...
  getAllVMs: function (resourceGroupName, _) {
    var vms;
  ...
  createOrUpdateVM: function (resourceGroupName, virtualMachine, isNewVM, _) {
    ...
    try {
      ...
      var vmResult = this.serviceClients.computeManagementClient.virtualMachines.createOrUpdate(resourceGroupName, parameters, _);
      return vmResult;
    } finally {
      progress.end();
    }
      ...
      if (networkProfile.networkInterfaces instanceof  Array) {
        for (var i = 0; i < networkProfile.networkInterfaces.length; i++) {
          ...
          if (!memoize[netReferenceUri]) {
            ...
            networkInterface.expanded = dependencies.networkNic.getNICExpanded(netReferenceUri, depth, memoize, dependencies, _);
          }
        }
      }
    }
