    ...
    if (this.params.disableBootDiagnostics !== null || this.params.enableBootDiagnostics !== null || this.params.bootDiagnosticsStorageUri !== null) {
      if (this.params.enableBootDiagnostics && this.params.bootDiagnosticsStorageUri === null) {
        ...
        if (stoProfile !== null && stoProfile.primaryEndpoints && stoProfile.primaryEndpoints.blob) {
          var foundElem = false;
          for (var indexElem = 0; indexElem < storageUtil.AccountTypeForChanging.length; indexElem++) {
            if (storageUtil.AccountTypeForChanging[indexElem] === stoProfile.accountType) {
              foundElem = true;
              break;
            }
          }
          ...
          if (!foundElem) {
            ...
            var bootStorageName = vmStorageProfile._generateNewStorageAccountNameWithPrefix('', _);
            ...
            this.params.bootDiagnosticsStorageUri = bootStoProfile.storageAccount.primaryEndpoints.blob;
          }
          else {
