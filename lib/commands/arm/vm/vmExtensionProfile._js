...
__.extend(VMExtensionProfile.prototype, {
  ...
  _checkAndGenerateDockerCertificatesIfNeeded: function(dockerCertPaths, _) {
    ...
    if (!dockerDirExists) {
      ...
      try {
        this._generateDockerCertificates(dockerCertPaths, password, _);
      } finally {
        progress.end();
      }
    } else {
      ...
      if (missingClientCertificates.length === 0) {
        this.cli.output.verbose($('Found docker client certificates.'));
        ...
        if (missingServerCertificates.length === 0) {
          this.cli.output.verbose($('Found docker server certificates.'));
        } else {
          this._generateDockerServerCertificates(dockerCertPaths, password, _);
        }
      } else {
        for (i = 0; i < missingClientCertificates.length; i++) {
          this.cli.output.verbose(missingClientCertificates[i]);
        }
  ...
  _generateDockerCertificates: function(dockerCertPaths, password, _) {
  ...
  _generateDockerServerCertificates: function(dockerCertPaths, password, _) {
    this._executeOpensslCommand('genrsa', {
      des3: true,
      passout: 'pass:' + password,
