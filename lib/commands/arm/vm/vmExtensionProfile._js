...
__.extend(VMExtensionProfile.prototype, {
  ...
  _parseExtensionProfileParams: function(params) {
    ...
    if (params.publicConfigPath) {
      try {
        ...
        extensionProfile.settings = JSON.parse(publicConfig.toString());
      } catch (err) {
        throw new Error(util.format($('Error parsing public config from file %s. %s'), params.publicConfigPath, err));
      }
    }
    ...
    if (params.privateConfigPath) {
      try {
        ...
        extensionProfile.protectedSettings = JSON.parse(privateConfig.toString());
      } catch (err) {
        throw new Error(util.format($('Error parsing private config from file %s. %s'), params.privateConfigPath, err));
      }
    }
  ...
  _createVMAccessLinuxPrivateConfig: function() {
    ...
    if (!utils.stringIsNullOrEmpty(this.params.sshKeyFile)) {
      var publicKeyResult = this._parseSSHPublicKeyPemFile(this.params.sshKeyFile);
      privateConfig['ssh_key'] = publicKeyResult.sshPublickeyPemDataBase64;
    }
