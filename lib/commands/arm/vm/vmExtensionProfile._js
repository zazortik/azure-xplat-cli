...
__.extend(VMExtensionProfile.prototype, {
  ...
  _generateAzureDiskEncryptionWindowsProfile: function() {
    ...
    if ((utils.stringIsNullOrEmpty(this.params.AadClientID)) || 
      ...
      throw new Error($('AadClientID, DiskEncryptionKeyVaultUrl, DiskEncryptionKeyVaultId are mandatory inputs'));
    }

    if ((utils.stringIsNullOrEmpty(AadClientSecret)) && 
	    (utils.stringIsNullOrEmpty(this.params.AadClientCertThumbprint))){
      throw new Error($('Please provide AadClientSecret or AadClientCertThumbprint correspondong to AadClientID'));
    }
	...
	if (!utils.stringIsNullOrEmpty(KeyEncryptionKeyUrl) &&
	    (utils.stringIsNullOrEmpty(KeyEncryptionKeyVaultId))){
	  throw new Error($('Please provide KeyEncryptionKeyVaultId correspondong to KeyEncryptionKeyUrl'));
	}

    if (!options.quiet && !this.cli.interaction.confirm($('This cmdlet prepares the VM and enables encryption which may reboot the machine and takes 10-15 minutes to finish. Please save your work on the VM before confirming. Do you want to continue? [y/n] '), _)) {
      return;
    }
