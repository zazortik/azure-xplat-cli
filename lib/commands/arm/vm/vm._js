...
exports.init = function (cli) {
  ...
  vm.command('enable-disk-encryption [resource-group] [name]')
    ...
    .option('-g, --resource-group <resource-group>', $('the resource group name'))
    .option('-n, --name <name>', $('the virtual machine name'))
    .option('-a, --aad-client-id <aadclientid>', $('Client ID of AAD app with permissions to write secrets to KeyVault'))
    .option('-p, --aad-client-secret <aadClientSecret>', $('Client Secret of AAD app with permissions to write secrets to KeyVault'))
    .option('-c, --aad-client-cert-thumbprint <aadClientCertThumbprint>', $('Thumbprint of AAD app certificate with permissions to write secrets to KeyVault'))
    .option('-k, --disk-encryption-keyvault-url <diskEncryptionKeyVaultUrl>', $('URL of the KeyVault where generated encryption key will be placed to'))
	.option('-r, --disk-encryption-keyvault-id <diskEncryptionKeyVaultId>', $('ResourceID of the KeyVault where generated encryption key will be placed to'))
	.option('-u, --key-encryption-key-url <keyEncryptionKeyUrl>', $('Versioned KeyVault URL of the KeyEncryptionKey used to encrypt the disk encryption key'))
	.option('-i, --key-encryption-keyvault-id <keyEncryptionKeyVaultId>', $('ResourceID of the KeyVault containing the KeyEncryptionKey used to encrypt the disk encryption key'))
	.option('-l, --key-encryption-algorithm <keyEncryptionAlgorithm>', $('KeyEncryption Algorithm used to encrypt the volume encryption key'))
	.option('-v, --volume-type <volumeType>', $('Type of the volume OS or Data or All to perform encryption operation'))
    .option('-q, --sequence-version <sequenceVersion>', $('Sequence version of encryption operation. This must be incremented to perform repeated encryption operations on the same VM'))
    .option('-e, --extension-version <version>', util.format($('Version of Diagnostics extension. Default values are [%s] for Windows'), vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_VERSION))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, name, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      name = cli.interaction.promptIfNotGiven($('Virtual machine name: '), name, _);
