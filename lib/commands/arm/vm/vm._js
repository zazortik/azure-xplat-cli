...
exports.init = function (cli) {
  ...
  vm.command('create <resourceGroupName> <vmName> <nicName> <location> <osType>')
    .description($('Create a VM'))
    .usage('[options] <resourceGroupName> <vmName> <nicName> <location> <osType>')
    ...
    .option('-n, --publicip-domain-name <publicipDomainName>', $('the public ip domain name, this set DNS to <publicip-domain-name>.<location>.cloudapp.azure.com'))
    .option('-m, --publicip-allocationmethod <publicipAllocationmethod>', $('the public ip allocaton method [Dynamic]'))
    .option('-t, --publicip-idletimeout <publicipIdletimeout>', $('the public ip idle timeout'))
    ...
    .option('-c, --os-disk-caching <osDiskCaching>', $('os disk caching, supported values are None, ReadOnly, ReadWrite'))
    .option('-d, --os-disk-vhd <osDiskVhd>', $('name or url of the OS disk Vhd'))
    .option('-a, --data-disk-caching <dataDiskCaching>', $('data disk caching, supported values are None, ReadOnly, ReadWrite'))
    ...
    .execute(function (resourceGroupName, vmName, nicName, location, osType, options, _) {
     var vmClient = new VMClient(cli, options.subscription);
     vmClient.createVM(resourceGroupName, vmName, nicName, location, osType, options, _);
    });

  vm.command('list [resourceGroupName]')
    .description($('Lists the virtual machines within a resource group'))
    .usage('[options] <resourceGroupName>')
    .option('--resourceGroup-name <resourceGroupName>', $('the resource group name'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroupName, options, _) {
      ...
      var vmClient = new VMClient(cli, options.subscription);
      vmClient.listVM(resourceGroupName, options, _);
    });

  vm.command('show [resourceGroupName] [name]')
    .description($('Gets one virtual machine within a resource group'))
    .usage('[options] <resourceGroupName> <name>')
    .option('--resourceGroup-name <resourceGroupName>', $('the resource group name'))
    .option('--name <name>', $('the virtual machine name'))
    .option('--subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroupName, name, options, _) {
      resourceGroupName = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroupName, _);
      name = cli.interaction.promptIfNotGiven($('Virtual machine name: '), name, _);
      var vmClient = new VMClient(cli, options.subscription);
      vmClient.showVM(resourceGroupName, name, options, _);
    });
};