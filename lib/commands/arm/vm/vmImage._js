...
__.extend(VMImage.prototype, {
  ...
  getVMImageDetails: function (imageFilter, _) {
    ...
    var queryParams = {
      ...
      offer: imageFilter.offer,
      skus: imageFilter.skus,
      version: imageFilter.version
    };
    
    var imageQueries = [];
    imageQueries.push(function(callBack) {
      that.serviceClients.computeManagementClient.virtualMachineImages.get(queryParams, function(error, imgResult) {
        if (error) {
          callBack(error);
          return;
        }
  ...
  _getExtImageListForLocation: function (location, _) {
    var that = this;
    var publishers = this.serviceClients.computeManagementClient.virtualMachineImages.listPublishers({location: location}, _);
    var typeQueries = [];
    publishers.resources.forEach(function(publisherItem) {
      typeQueries.push(function(callBack) {
        that.serviceClients.computeManagementClient.virtualMachineExtensionImages.listTypes({
          location: location,
          publisherName: publisherItem.name
        }, function (error, types) {
          types = error ? { resources: [] } : types;
    ...
    typeCollections.forEach(function(typeCollection) {
      typeCollection.resources.forEach(function(typeItem) {
        imageQueries.push(function(callBack) {
          that.serviceClients.computeManagementClient.virtualMachineExtensionImages.listVersions({
            location: typeCollection.location,
            publisherName: typeCollection.publisher,
            type: typeItem.name
          }, function(error, verItems) {
            verItems = error ? { resources: [] } : verItems;
    ...
    verCollections.forEach(function(verCol) {
      if (verCol.resources) {
        verCol.resources.forEach(function(verItem) {
          imgResult.push({
  ...
  _getExtImageListForPublisher: function (location, publisher, _) {
    ...
    var verQueries = [];
    typeNames.resources.forEach(function(typeItem) {
    ...
    verCollections.forEach(function(verCol) {
      if (verCol.resources) {
        verCol.resources.forEach(function(resourceItem) {
          imgResult.push({
            publisher: verCol.publisher,
            location: verCol.location,
            typeName: verCol.typeName,
