...
__.extend(VMImage.prototype, {
  ...
  getVMExtensionImageVersionList: function (location, publisherName, typeName, _) {
    var that = this;
    try {
      var progress = that.cli.interaction.progress(util.format($('Getting virtual machine extension image verions (Publisher: "%s" Type:"%s" Location:"%s")'), publisherName, typeName, location));
      var extVersions = { resources : [] };
      var resultList = that.serviceClients.computeManagementClient.virtualMachineExtensionImages.listVersions({
        location: location,
        publisherName: publisherName,
        type: typeName
      }, _);
