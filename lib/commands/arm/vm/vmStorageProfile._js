...
__.extend(VMStorageProfile.prototype, {
        generateStorageProfile: function(_) {
          ...
          if (storageProfile.osDiskDataDiskImgInfo.imageInfo) {
            if (storageProfile.osDiskDataDiskImgInfo.imageInfo.useImageName) {
              // using imageName
        ...
        _parseOSDataDiskImageParams: function (params, _) {
            ...
            if (params.imageName) {
              this.cli.output.warn(util.format($('The image "%s" will be used for VM'), params.imageName));
              if (params.imageUrn) {
                this.cli.output.warn('image-urn parameter will be ignored');
              }
            ...
            if (osDiskDataDiskImageInfo.imageInfo === null && osDiskDataDiskImageInfo.osDiskInfo === null) {
              throw new Error($('Either imageName, imageUrn or os-disk-vhd parameter is required to create VM'));
            }
        ...
        _parseOSDiskParams: function(params, createOption, _) {
          ...
          if (attachMode) {
            ...
            if (utils.stringIsNullOrEmpty(params.osDiskType)) {
              throw new Error($('osDiskType is required when OSDisk configuration defines an OS Disk VHD'));
            }
            ...
            if (utils.stringIsNullOrEmpty(params.osDiskVhd)) {
              throw new Error($('osDiskVhd is required when OSDisk configuration defines an OS Disk VHD'));
            }
          }
        ...
        _getFirstStorageAccount: function (location, _) {
          ...
          try {
            var storageAccounts = this.storageManagementClient.storageAccounts.list(_);
            if (storageAccounts && (storageAccounts instanceof Array)) {
        ...
        _generateNewStorageAccountName: function (_) {
          return this._normalizeString('cli' + this._getPrefix(_) + (new Date()).getTime().toString());
        },
