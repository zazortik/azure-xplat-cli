...
__.extend(NetworkNic.prototype, {
    ...
    getNICExpanded: function (referenceUri, depth, memoize, _) {
      ...
      if (networkInterface.properties.ipConfigurations instanceof Array) {
        for (var i = 0; i < networkInterface.properties.ipConfigurations.length; i++) {
          ...
          if (utils.hasValidProperty(ipConfiguration.properties, 'subnet')) {
            ipConfiguration.properties.subnet.expanded = networkVNetSubnet.getSubnetExpanded(ipConfiguration.properties.subnet.id, depth, memoize, _);
          }

          if (utils.hasValidProperty(ipConfiguration.properties, 'publicIpAddress')) {
            ipConfiguration.properties.publicIpAddress.expanded = networkPublicIP.getPublicIPExpanded(ipConfiguration.properties.publicIpAddress.id, depth, memoize, _);
          }
        }
      }
