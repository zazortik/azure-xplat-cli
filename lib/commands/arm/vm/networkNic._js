...
__.extend(NetworkNic.prototype, {
    ...
    getNICExpanded: function (referenceUri, depth, memoize, dependencies, _) {
      ...
      var nic = this._getNIC(resourceInfo.resourceGroupName, resourceInfo.resourceName, _);
      if (depth === 0 || nic === null) {
        return nic;
      }
      ...
      if (networkInterface.properties.ipConfigurations instanceof Array) {
        for (var i = 0; i < networkInterface.properties.ipConfigurations.length; i++) {
          var ipConfiguration = networkInterface.properties.ipConfigurations[i];
          if (utils.hasValidProperty(ipConfiguration.properties, 'subnet')) {
            var subnetReferenceId = ipConfiguration.properties.subnet.id.toLowerCase();
            if (!memoize[subnetReferenceId]) {
              ipConfiguration.properties.subnet.expanded = networkVNetSubnet.getSubnetExpanded(subnetReferenceId, depth, memoize, dependencies, _);
            }
          }

          if (utils.hasValidProperty(ipConfiguration.properties, 'publicIpAddress')) {
            var publicIpReferenceId = ipConfiguration.properties.publicIpAddress.id.toLowerCase();
            if (!memoize[publicIpReferenceId]) {
              ipConfiguration.properties.publicIpAddress.expanded = networkPublicIP.getPublicIPExpanded(publicIpReferenceId, depth, memoize, dependencies, _);
            }
          }
        }
      }
    ...
    _updateNICIfRequired: function (nic, params, allowNewIpConfig,  _) {
      ...
      if (allowNewIpConfig) {
        ...
        if (existingIpConfiguration) {
          ...
          var ipConfigInfo = this._createNewIPConfigurationTrySetPublicIP(subnetId, this.params.nicName, _);
          if (!ipConfigInfo.ipConfiguration.properties.publicIpAddress) {
          ...
          networkInterface.properties.ipConfigurations.push(ipConfigInfo.ipConfiguration);
        ...
        this.cli.output.info(util.format($('Found an IP configuration with virtual network subnet id "%s" in the NIC "%s"'), existingIpConfiguration.properties.subnet.id, this.params.nicName));
      }
      ...
      if (utils.hasValidProperty(existingIpConfiguration.properties.publicIpAddress, 'id')) {
        ...
        if (utils.stringIsNullOrEmpty(this.params.publicipName)) {
          ...
          if (utils.stringEndsWith(publicipId, partialPublicIdFromParams)) {
            this.cli.output.info(util.format($('The identified NIC ip configuration is already configured with the provided public ip "%s"'), this.params.publicipName));
          } else {
      ...
      var c = this._createNewIPConfigurationTrySetPublicIP(existingIpConfiguration.properties.subnet.id, this.params.nicName, _);
    ...
    _createNewIPConfigurationTrySetSubnetPublicIP: function (nicName, _) {
      ...
      var r = this._createNewIPConfigurationTrySetPublicIP(result.vnetInfo.subnetInfo.profile.id, nicName, _);
    ...
    _createNewNIC: function (resourceGroupName, params, _) {
      ...
      createRequestProfile.properties.ipConfigurations.push(ipConfigInfo.ipConfiguration);
