...
__.extend(NetworkNic.prototype, {
    createNICIfRequired: function(_) {
      ...
      if (nic) {
        ...
        if ((nic.networkInterface.virtualMachine !== undefined) && (nic.networkInterface.virtualMachine.id !== undefined)) {
          var attachedVMName = nic.networkInterface.virtualMachine.id.split(['/']).splice(-1)[0];
    ...
    getNICExpanded: function (referenceUri, depth, memoize, dependencies, _) {
      ...
      if (networkInterface.ipConfigurations instanceof Array) {
        for (var i = 0; i < networkInterface.ipConfigurations.length; i++) {
          var ipConfiguration = networkInterface.ipConfigurations[i];
          if (utils.hasValidProperty(ipConfiguration, 'subnet')) {
            ...
            if (!memoize[subnetReferenceId]) {
              ipConfiguration.subnet.expanded = networkVNetSubnet.getSubnetExpanded(subnetReferenceId, depth, memoize, dependencies, _);
            }
          }
          ...
          if (utils.hasValidProperty(ipConfiguration, 'publicIpAddress')) {
            ...
            if (!memoize[publicIpReferenceId]) {
              ipConfiguration.publicIpAddress.expanded = networkPublicIP.getPublicIPExpanded(publicIpReferenceId, depth, memoize, dependencies, _);
            }
          }
        }
      }
    ...
    _updateNICIfRequired: function (nic, params, allowNewIpConfig,  _) {
      ...
      if (allowNewIpConfig) {
        ...
        this.cli.output.info(util.format($('Found an IP configuration with virtual network subnet id "%s" in the NIC "%s"'), existingIpConfiguration.subnet.id, this.params.nicName));
      }
      ...
      if (utils.hasValidProperty(existingIpConfiguration.publicIpAddress, 'id')) {
        var publicipId = existingIpConfiguration.publicIpAddress.id.toLocaleLowerCase();
      ...
      var c = this._createNewIPConfigurationTrySetPublicIP(existingIpConfiguration.subnet.id, this.params.nicName, _);
      if (!c.ipConfiguration.publicIpAddress) {
      ...
      existingIpConfiguration.publicIpAddress = c.ipConfiguration.publicIpAddress;
      this._updateNIC(nic, _);
    ...
    _createNewIPConfigurationTrySetSubnetPublicIP: function (nicName, _) {
      ...
      var ipConfigInfo = this._createNewIPConfigurationTrySetPublicIP(result.vnetInfo.subnetInfo.profile.id, nicName, _);
      result.ipConfiguration = ipConfigInfo.ipConfiguration;
      result.publicipInfo = ipConfigInfo.publicipInfo;
      return result;
    },
    ...
    _createNewIPConfigurationTrySetPublicIP: function (subnetId, nicName, _) {
      ...
      if (networkPublicIP.hasAnyPubIPParameters(this.params)) {
        ...
        if (utils.hasValidProperty(publicipInfo.profile.ipConfiguration, 'id')) {
          ...
          var connectedNicIPConfigId = publicipInfo.profile.ipConfiguration.id.split(['/']);
