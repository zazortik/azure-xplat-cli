...
__.extend(NetworkNic.prototype, {
    ...
    _updateNICIfRequired: function (resourceGroupName, networkInterface, params, allowNewIpConfig,  _) {
      ...
      var ipConfigInfo2 = this._createNewIPConfigurationTrySetPublicIP(existingIpConfiguration.subnet.id, networkInterface.name, _);
      if (!ipConfigInfo2.ipConfiguration.publicIpAddress) {
        this.cli.output.info($('public ip parameters is ignored or absent, using this NIC with subnet'));
        return nicUpdateInfo;
      } else {
        this.cli.output.info(util.format($('Configuring identified NIC IP configuration with PublicIP "%s"'), ipConfigInfo2.publicipInfo.profile.name));
      }

      existingIpConfiguration.publicIpAddress = ipConfigInfo2.ipConfiguration.publicIpAddress;
      this._updateNIC(resourceGroupName, nic, _);
      nicUpdateInfo.publicipInfo = ipConfigInfo2.publicipInfo;
      nicUpdateInfo.updated = true;
      return nicUpdateInfo;
    },
