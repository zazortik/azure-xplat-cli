...
__.extend(VMClient.prototype, {
  ...
  setChefExtension: function(resourceGroupName, vmName, options, _) {
    ...
    virtualMachine = vmResult.virtualMachine;
    if (virtualMachine.storageProfile.oSDisk.operatingSystemType === 'Windows') {
      var extensionName = 'ChefClient'
     if (!virtualMachine.oSProfile.windowsConfiguration || !virtualMachine.oSProfile.windowsConfiguration.provisionVMAgent) {
        throw new Error($('Provision Guest Agent must be enabled on the VM before setting VM Extension.'));
      }
    } else {
      var extensionName = 'LinuxChefClient'
    }
    ...
    if(!version) {
      ...
      options.version = versions[versions.length-1].name;
    }
    ...
    pub_config['client_rb'] = fs.readFileSync(params.clientConfig).toString();
    pub_config['runlist'] = options.runList;
    pub_config['autoUpdateClient'] = options.autoUpdateClient ? 'true' : 'false';
    pub_config['deleteChefConfig'] = options.deleteChefConfig ? 'true' : 'false';
    if (options.bootstrapOptions) {
      try {
        pub_config['bootstrap_options'] = JSON.parse(options.bootstrapOptions);
      } catch (er) {
    ...
    pri_config['validation_key'] = fs.readFileSync(params.validationPem ).toString();
    options.publicConfig = pub_config;
    options.privateConfig = pri_config;
