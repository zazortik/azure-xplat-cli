...
__.extend(VMClient.prototype, {
  createVM: function (resourceGroupName, vmName, location, osType, options, _) {
    ...
    params.diskEncryptionKeyVaultId = options.encryptionKeyVaultId;
    params.diskEncryptionKeySecretUrl = options.encryptionKeySecretUrl;
    params.keyEncryptionKeyVaultId = options.encryptionMasterKeyVaultId;
    params.keyEncryptionKeyUrl = options.encryptionMasterKeyUrl;
  ...
  setAzureDiskEncryption: function(resourceGroupName, name, options, _) {
    ...
    console.log("vmClient.js: options.aadClientID : %s", options.aadClientID);
    ...
    virtualMachine.createOrUpdateVMExtension(resourceGroupName, name, azureDiskEncryptionExtension.profile, true, _);
	...
	if(!extensionResult ||
	   (extensionResult.Publisher !== vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_PUBLISHER) ||
	   (extensionResult.ExtensionType !== vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_NAME))
    {
      throw new Error(util.format($('Virtual machine "%s" extension not found after installing extension  '), vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_NAME));
	}
	var secretUrl = extensionResult.Statuses[0].Message;

	// Get VM model 
    vmResult = virtualMachine.getVM(resourceGroupName, name, _);
    if (!vmResult ||
	    !vmResult.virtualMachine ||
		!vmResult.virtualMachine.StorageProfile) 
	{
    ...
    var encryptionSettings = {
      diskEncryptionKey: {
        ...
        secretUrl: options.diskEncryptionKeyVaultUrl
      },
	...
	vmResult.virtualMachine.StorageProfile.OSDisk.EncryptionSettings = encryptionSettings;
	return virtualMachine.createOrUpdateVM(resourceGroupName, vmResult, false, _);
  }
