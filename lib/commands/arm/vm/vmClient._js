    ...
    if (!instanceViewResult) {
      ...
      if (vmResult) {
        ...
        if (instanceView && instanceView.bootDiagnostics && instanceView.bootDiagnostics.serialConsoleLogBlobUri)
        {
          ...
          if (!stream.Writable) {
            this.cli.output.warn('Stream output is not supported in the current Node version.');
            this.cli.output.warn('Please use "azure storage blob" command to download the log file for viewing.');
            return;
          }
