...
__.extend(VMClient.prototype, {
  ...
  listVMSizesOrLocationVMSizes: function(options, _) {
    ...
    if (options.vmName) {
      var vmResult = this._getVM(options.resourceGroup, options.vmName, computeManagementClient, _);
      if (!vmResult) {
        throw new Error(util.format($('Virtual machine "%s" not found under the resource group "%s"'), options.vmName, options.resourceGroup));
      }

      progress = this.cli.interaction.progress(util.format($('Listing virtual machine sizes available for the VM "%s"'), options.vmName));
      try {
        sizeResult = computeManagementClient.virtualMachines.listAvailableSizes(options.resourceGroup, options.vmName, _);
      } finally {
        progress.end();
      }
    } else {
      progress = this.cli.interaction.progress(util.format($('Listing virtual machine sizes available in the location "%s"'), options.location));
      try {
        sizeResult = computeManagementClient.virtualMachineSizes.list(options.location, _);
      } finally {
        progress.end();
      }
    }
