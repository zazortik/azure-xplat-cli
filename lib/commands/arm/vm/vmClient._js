...
__.extend(VMClient.prototype, {
  ...
  enableAemVM : function(resourceGroupName, name, options, _){
    ...
    var osDiskUri = blobUtil.splitDestinationUri(vmResult.virtualMachine.storageProfile.oSDisk.virtualHardDisk.uri);
    saProperties = this._getStorageAccountProperties(serviceClients, storageAccountsResult.storageAccounts, osDiskUri.accountName, _);
    vmStorageAccounts[saProperties.name] = saProperties;

    var i;
    for(i = 0; i < vmResult.virtualMachine.storageProfile.dataDisks.length; i++){
        var dataDisk = vmResult.virtualMachine.storageProfile.dataDisks[i];
        var dataDiskUri = blobUtil.splitDestinationUri(dataDisk.virtualHardDisk.uri);
        if(!vmStorageAccounts[dataDiskUri.accountName]){
            saProperties = this._getStorageAccountProperties(serviceClients, storageAccountsResult.storageAccounts, dataDiskUri.accountName, _);
            vmStorageAccounts[saProperties.name] = saProperties;
        }
    }
    ...
    for(i = 0; i < vmStorageAccountNames.length; i++){
        ...
        if(vmStorageAccount.type === 'Standard'){ //logging is only appliable for standard storage
            this._enableStorageAccountAnalytics(vmStorageAccount, _);
        }
    }
    
    //Check and config diagnostic extension
    var diagExtension = __.find(vmResult.virtualMachine.extensions, function(extension){ 
        return extension.extensionType === vmConstants.EXTENSIONS.IAAS_DIAG_NAME && extension.publisher === vmConstants.EXTENSIONS.IAAS_DIAG_PUBLISHER ||
               extension.extensionType === vmConstants.EXTENSIONS.LINUX_DIAG_NAME && extension.publisher === vmConstants.EXTENSIONS.LINUX_DIAG_PUBLISHER;
    });
    ...
    if(!wadStorageAccountName){
        wadStorageAccountName = option.diagnosticStorageAccountName;
    ...
    if(!vmStorageAccounts[wadStorageAccountName]){
        saProperties = this._getStorageAccountProperties(serviceClients, storageAccountsResult.storageAccounts, wadStorageAccountName, _);
        vmStorageAccounts[saProperties.name] = saProperties;
    }
    ...
    var aemParams = {
        ...
        location: vmResult.virtualMachine.location,
        osType: vmResult.virtualMachine.storageProfile.oSDisk.operatingSystemType,
        vmSize: vmResult.virtualMachine.hardwareProfile.virtualMachineSize,
        wadStorageAccount: wadStorageAccountName,
        osDisk: vmResult.virtualMachine.storageProfile.oSDisk,
  ...
  _getStorageAccountProperties: function(serviceClients, storageAccounts, storageAccountName, _){
    var saProperties = {};
    ...
    saProperties.key = keys.storageAccountKeys.key1;
   
    return saProperties;
  },

  _enableStorageAccountAnalytics: function(saProperties, _){
    var options = {
    ...
    if(!loggingProperties ||
        ...
        try{
            ...
            var defaultLoggingPerperties = this._getDefaultLoggingPerperties();
  ...
  _getDefaultLoggingPerperties: function(){
    return {
        Logging:{ 
            Version: '1.0',
            Delete: true,
