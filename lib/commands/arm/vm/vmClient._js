...
__.extend(VMClient.prototype, {
  createVM: function(resourceGroupName, vmName, nicName, location, osType, options, _) {
    ...
    var dependencies = {
      vmOsProfile: new VMOsProfile(this.cli, params),
      vmHardwareProfile: new VMHardwareProfile(this.cli, params),
      vmAvailSetProfile: new VMAvailabilitySetProfile(this.cli, resourceGroupName, params, serviceClients),
      vmNetworkProfile: new VMNetworkProfile(this.cli, resourceGroupName, params, serviceClients),
      vmStorageProfile: new VMStorageProfile(this.cli, resourceGroupName, params, serviceClients)
    };

    var vmProfile = new VMProfile(this.cli, resourceGroupName, params, dependencies);
  ...
  listVM: function(resourceGroupName, options, _) {
    ...
    var vmsResult = virtualMachine.getAllVMs(resourceGroupName, _);
    var output = this.cli.output;
  ...
  setExtension: function(resourceGroup, vmName, extensionName, publisherName, version, options, _) {
    ...
    if (options.uninstall) {
      this._uninstallExtension(resourceGroup, vmName, extensionName, serviceClients.computeManagementClient, _);
      return;
    }

    this._createOrUpdateExtension(resourceGroup, vmName, extensionName, publisherName, version, options, serviceClients.computeManagementClient, _);
  },
