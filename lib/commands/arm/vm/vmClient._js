...
__.extend(VMClient.prototype, {
  ...
  getVM: function(resourceGroupName, vmName, options, _) {
    var sub = this.subscription || options.subscription;
    var subscription = profile.current.getSubscription(sub);
    ...
    try {
      var result = computeManagementClient.virtualMachines.get(resourceGroupName, vmName, _);
      return result;
    } finally {
      progress.end();
    }
  ...
  getVMs: function(resourceGroupName, options, _) {
    var sub = this.subscription || options.subscription;
    var subscription = profile.current.getSubscription(sub);
    var computeManagementClient = this._getSubscriptionComputeClient(subscription);
