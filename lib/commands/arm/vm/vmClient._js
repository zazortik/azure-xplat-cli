  ...
  enableAemVM : function(resourceGroupName, name, options, _){
    ...
    if (vmResult.resources)
      for(i = 0; i < vmResult.resources.length; i++){
        ...
        if (extension.name === vmConstants.EXTENSIONS.IAAS_DIAG_NAME && extension.publisher === vmConstants.EXTENSIONS.IAAS_DIAG_PUBLISHER ||
            extension.name === vmConstants.EXTENSIONS.LINUX_DIAG_NAME && extension.publisher === vmConstants.EXTENSIONS.LINUX_DIAG_PUBLISHER)
