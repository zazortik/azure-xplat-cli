...
__.extend(VMClient.prototype, {
      ...
      if (vmResult) {
        ...
        if (instanceView && instanceView.bootDiagnostics && instanceView.bootDiagnostics.serialConsoleLogBlobUri)
        {
          ...
          if (hostSegments.length > 0 && pathSegments.length == 2) {
            ...
            //this.cli.output.info(util.format($('accountName = "%s"'), result.accountName));
            //this.cli.output.info(util.format($('containerName = "%s"'), result.containerName));
            //this.cli.output.info(util.format($('blobName = "%s"'), result.blobName));
            ...
            var op = performStorageOperation(operation, _, result.containerName, result.blobName, ws, storageOptions);
            
            var max_len = options.maxLength == null ? 1000000 : parseInt(options.maxLength, 10);
            var start_pos = 0;
            while (start_pos < content.length) {
              if (start_pos > 0 && !this.cli.interaction.confirm('Do you want to view more log?', _)) {
                break;
              }

              var strlen = start_pos + max_len < content.length ? max_len : content.length - start_pos;
              this.cli.output.info(content.substr(start_pos, strlen));
              start_pos += parseInt(strlen, 10);
            }
          }
        }
      }
  ...
  _getStorageAccountContainerAndBlobFromUri : function(storageUri) {
    var result = {
        accountName : null,
        containerName : null,
        blobName : null
