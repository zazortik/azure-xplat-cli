...
__.extend(VMClient.prototype, {
  ...
  _createOrUpdateExtension: function(resourceGroupName, vmName, extensionName, publisherName, version, options, serviceClients, _) {
	...
	if (options.autoUpgradeMinorVersion && options.version) {
	  var majorVersion = version.split('.')[0];
	  var vmImage = new VMImage(this.cli, serviceClients);
      var versionsResult = vmImage.getVMExtensionImageVersionList(options.location, publisherName, extensionName, _);
	  var filterVersionResult = versionsResult.filter( function(obj){return obj.name.split('.')[0] === majorVersion});
	  var latestMinerVersion = __.max(filterVersionResult, function(obj){return parseInt(obj.name.split('.')[1]);}).name.split('.')[1];
	  options.version = majorVersion + '.' + latestMinerVersion;
	}
