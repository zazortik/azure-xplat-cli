...
__.extend(VMClient.prototype, {
  ...
  setAzureDiskEncryption: function(resourceGroupName, name, options, _) {
    ...
    if(!extensionResult ||
       ...
       !utils.ignoreCaseEquals(extensionResult.virtualMachineExtension.publisher , vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_PUBLISHER) ||
    ...
    {
      throw new Error(util.format($('Virtual machine "%s" extension with successful status not found after installing extension  '), vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_NAME));
    }
  ...
  showAzureDiskEncryptionStatus: function(resourceGroupName, name, options, _) {
    ...
    if(vmModel.virtualMachine.storageProfile.oSDisk.encryptionSettings)
    {
      ...
      if((extensionStatus) &&
         ...
         utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.publisher , vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_PUBLISHER) &&
