...
__.extend(VMClient.prototype, {
  ...
  listVMSizesOrLocationVMSizes: function(options, _) {
    ...
    if (options.vmName) {
      ...
      progress = this.cli.interaction.progress(util.format($('Listing virtual machine sizes available in the location "%s"'), location));
      try {
        sizeResult = computeManagementClient.virtualMachineSizes.list(location, _);
      } finally {
        progress.end();
      }
    }
