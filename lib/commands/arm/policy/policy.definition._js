...
imports('./lib/util/jsonLint.js', { exec: true, export: false });
...
exports.init = function (cli) {
  ...
  definition.command('create [name] [policy]')
    ...
    .execute(function (name, policy, options, _) {
      ...
      var policyContent = policy ? jsonlint.parse(utils.stripBOM(fs.readFileSync(policy))) : jsonlint.parse(options.policyString); // jshint ignore:line
  ...
  definition.command('set [name]')
    ...
    .execute(function (name, options, _) {
      ...
      var policyContent;
      if (options.policy) {
        policyContent = jsonlint.parse(utils.stripBOM(fs.readFileSync(options.policy))); // jshint ignore:line
      } else if (options.policyString) {
        policyContent = jsonlint.parse(options.policyString); // jshint ignore:line
      }
