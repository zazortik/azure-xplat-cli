...
exports.init = function (cli) {
  ...
  resource.command('show [resource-group] [name] [resource-type] [api-version]')
    ...
    .execute(function (resourceGroup, name, resourceType, apiVersion, options, _) {
      if(!options.resourceId) {
        ...
        var values = extractAllValues(options.resourceId);
  ...
  function parsed(obj, type){
    try{
      ...
      throw new Error('Error with ' + type + ' object. ' + e);
    }
  } 
  ...
  function extractAllValues(resourceId){
    ...
    var firstValue = segments.indexOf('subscriptions') === -1 ? 'resourceGroups' : 'subscriptions';
    ...
    var providerList = segments.slice(segments.indexOf('providers') + 1, segments.length-1);
      ...
      values['resourceGroup'] = segments[segments.indexOf('resourceGroups') + 1];
      if(segments.length > 4) {
        ...
        values['name'] = segments[segments.length - 1]; 
      }
    return values;
  }
