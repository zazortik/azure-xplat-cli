    ...
    .execute(function (resourceGroup, options, _) {
      ...
      if (options.details) {
        for (var i = 0; i < resources.length; i++) {
          ...
          resources[i].permissions = client.resources.getPermissions(resourceInformation.resourceGroup, {
  ...
  resource.command('show [resource-group] [name] [resource-type] [api-version]')
    ...
    .execute(function (resourceGroup, name, resourceType, apiVersion, options, _) {
      ...
      try {
        ...
        resource.permissions = client.resources.getPermissions(resourceGroup, identity, _).permissions;
      } finally {
        progress.end();
      }

      cli.interaction.formatOutput(resource, function (outputData) {
        var resourceInformation = resourceUtils.getResourceInformation(outputData.id);
