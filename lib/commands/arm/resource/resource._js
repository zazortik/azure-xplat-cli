  ...
  resource.command('move [ids] [destination-group] [api-version]')
    ...
    .execute(function (ids, destinationGroup, apiVersion, options, _) {
      ...
      try {
		...
		var version = validateApiVersion(apiVersion);
        var parameters = {
	...
	function validateApiVersion(dateString) {
	  ...
	  if (currentApiVersions[dateString]) {
		return dateString;
	  } else {
	    console.log("");
	    log.error("Incorrect Format or not a valid Api Version (YYYY-MM-DD). Current Api Versions: ", Object.keys(currentApiVersions), ". Defaulting to 2015-11-01");
		console.log("");
        return "2015-11-01";
	  }
	}
