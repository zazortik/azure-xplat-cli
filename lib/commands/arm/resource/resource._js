...
exports.init = function (cli) {
  ...
  resource.command('create [resource-group] [name] [resource-type] [api-version]')
    ...
    .execute(function (resourceGroup, name, resourceType, apiVersion, options, _) {
      ...
      resource.createResource(values.resourceGroup, values.name, values.resourceType, values.apiVersion, values.properties, values.options, _);
    });
  ...
  resource.command('show [resource-group] [name] [resource-type] [api-version]')
    ...
    .execute(function (resourceGroup, name, resourceType, apiVersion, options, _) {
      ...
      var client = utils.createResourceClient(subscription);
      var authzClient = rbacClients.getAuthzClient(subscription);
      var progress = cli.interaction.progress(util.format($('Getting resource %s'), values.name));
      var resource;
      try {
        var identity = {
  ...
  resource.command('delete [resource-group] [name] [resource-type] [api-version]')
    ...
    .execute(function (resourceGroup, name, resourceType, apiVersion, options, _) {
      var values = getValuesForCommand(resourceGroup, name, resourceType, apiVersion, options);
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete resource %s? [y/n] '), values.name), _)) {
        return;
      }
      var subscription = profile.current.getSubscription(options.subscription);
      var client = utils.createResourceClient(subscription);
      var progress = cli.interaction.progress(util.format($('Deleting resource %s'), values.name));
      deleteResource(values.resourceType, values.resourceGroup, values.apiVersion, values.name, values.options, client, progress, _);
    });
  ...
  function getValuesForCommand(resourceGroup, name, resourceType, apiVersion, options){
    ...
    if(!options.resourceId) {
      ...
      values.name = valuesFromId['name'];
    }
      values.apiVersion = apiVersion;
      values.options = options;
      checkMissingParameters(values.resourceGroup, values.name, values.resourceType, values.apiVersion);
      return values;
  }
  ...
  function extractAllValues(resourceId){
    ...
    var providerList = segments.slice(segments.indexOf('providers') + 1, segments.length-1);
      values['subscription'] = firstValue === 'subscriptions' ? segments[segments.indexOf('subscriptions') + 1] : 'No subscription provided.';
