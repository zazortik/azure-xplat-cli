...
exports.getSourceResourceGroupForMove = function (resources) {
  var rgRegEx = new RegExp('resourceGroups/[A-Za-z0-9]+/');
  var sourceGroup;
  for (var i = 0; i < resources.length; ++i) {
    var match = rgRegEx.exec(resources[i]);
    if (!match) {
      throw new Error(util.format($('%s is not a valid resource'), resources[i]));
    }
    if (!sourceGroup) {
      sourceGroup = match[0].split('/')[1];
    }
    else if (sourceGroup && match[0].split('/')[1] != sourceGroup) {
      throw new Error('Resources to move must all reside in the same resource group.');
    }
  }
  return sourceGroup;
};
