...
exports.init = function (cli) {
  ...
  dataLakeAnalyticsJob.command('create [accountName] [jobName] [script] [runtime] [compileMode] [compileOnly] [degreeOfParallelism] [priority]' )
    ...
    .execute(function (accountName, jobName, script, runtime, compileMode, compileOnly, degreeOfParallelism, priority, options, _) {
      ...
      var jobResponse = {};
      if(compileOnly) {
        jobResponse = client.jobs.build(accountName, parameters, _);
      }
      else {
        jobResponse = client.jobs.create(accountName, job.jobId, parameters, _);
      }
  ...
  dataLakeAnalyticsJob.command('show [accountName] [jobId] [includeDebugInfo] [includeStatistics]' )
    ...
    .execute(function (accountName, jobId, includeDebugInfo, includeStatistics, options, _) {
      ...
      var jobResponse = client.jobs.get(accountName, jobId, _);
      
      if(includeStatistics) {
        try {
          var statistics = client.jobs.getStatistics(accountName, jobId, _);
      ...
      if(includeDebugInfo) {
        try {
          var debugData = client.jobs.getDebugDataPath(accountName, jobId, _).paths;
  ...
  dataLakeAnalyticsJob.command('cancel [accountName] [jobId]' )
    ...
    .execute(function (accountName, jobId, includeDebugInfo, includeStatistics, options, _) {
      ...
      client.jobs.cancel(accountName, jobId, _);
  ...
  dataLakeAnalyticsJob.command('list [accountName] [jobName] [submitter] [submittedAfter] [submittedBefore] [state] [result]' )
    ...
    .execute(function (accountName, jobName, submitter, submittedAfter, submittedBefore, state, result, options, _) {
      ...
      withProgress(util.format($('Retrieving job list for account: %s'), accountName),
        function (log, _) {
          var response = client.jobs.list(accountName, parameters, _);
          jobList = response.value;
          var pushJobs = function(eachValue) {jobList.push(eachValue);};
          while(response.nextPageLink && response.nextPageLink.length > 0) {
            response = client.jobs.listNext(response.nextPageLink, _);
  ...
  dataLakeAnalyticsAccount.command('show [accountName] [resource-group]')
    ...
    .execute(function (accountName, resourceGroup, options, _) {
      ...
      var dataLakeAnalyticsAccount = client.dataLakeAnalyticsAccount.get(resourceGroup, accountName, _);
    ...
    .execute(function (accountName, resourceGroup, options, _) {
      ...
      client.dataLakeAnalyticsAccount.deleteMethod(resourceGroup, accountName, _);
  ...
  function createOrUpdateDataLakeAnalyticsAccount(subscription, accountName, resourceGroup, location, defaultDataLakeStore, tags, _) {
      ...
      if(create) {
        ...
        client.dataLakeAnalyticsAccount.create(resourceGroup, accountParams.name, accountParams, _);
      }
      else {
        if (defaultDataLakeStore) {
          accountParams.dataLakeAnalyticsAccount.properties.defaultDataLakeStoreAccount = defaultDataLakeStore;
        ...
        client.dataLakeAnalyticsAccount.update(resourceGroup, accountParams.name, accountParams, _);
      }
      
      return client.dataLakeAnalyticsAccount.get(resourceGroup, accountName, _);
  }
  ...
  function getCatalogItem(subscription, accountName, itemPath, itemType, _) {
      ...
      switch (itemType.toLowerCase()) {
        case 'database':
          if(isList) {
            toReturn = client.catalog.listDatabases(accountName, _).value;
          }
          else {
         ...
         case 'schema':
          if(isList) {
            toReturn = client.catalog.listSchemas(accountName, catalogItem.databaseName, _).value;
          }
          else {
        ...
        case 'assembly':
          if(isList) {
            toReturn = client.catalog.listAssemblies(accountName, catalogItem.databaseName, _).value;
          }
          else {
        ...
        case 'externaldatasource':
          if(isList) {
            toReturn = client.catalog.listExternalDataSources(accountName, catalogItem.databaseName, _).value;
          }
          else {
        ...
        case 'credential':
          if(isList) {
            toReturn = client.catalog.listCredentials(accountName, catalogItem.databaseName, _).value;
          }
          else {
        ...
        case 'table':
          if(isList) {
            toReturn = client.catalog.listTables(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, _).value;
          }
          else {
        ...
        case 'tablevaluedfunction':
          if(isList) {
            toReturn = client.catalog.listTableValuedFunctions(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, _).value;
          }
          else {
        ...
        case 'tablestatistics':
          if(isList) {
            toReturn = client.catalog.listTableStatistics(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, catalogItem.tableOrTableValuedFunctionName, _).value;
          }
          else {
        ...
        case 'view':
          if(isList) {
            toReturn = client.catalog.listViews(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, _).value;
          }
          else {
        ...
        case 'procedure':
          if(isList) {
            toReturn = client.catalog.listProcedures(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, _).value;
          }
          else {
        ...
        case 'types':
          if(isList) {
            toReturn = client.catalog.listTypes(accountName, catalogItem.databaseName, catalogItem.schemaAssemblyOrExternalDataSourceName, _).value;
          }
          else {
  ...
  function listAllDataLakeAnalyticsAccounts(subscription, resourceGroup, _) {
    ...
    var accounts;
    if (!resourceGroup) {
      var response = client.dataLakeAnalyticsAccount.list(_);
      accounts = response.value;
      while (response.nextPageLink) {
        response = client.dataLakeAnalyticsAccount.listNext(response.nextPageLink);
        accounts.push.apply(accounts, response.value);
      }
    }
    else {
      var response = client.dataLakeAnalyticsAccount.listByResourceGroup(resourceGroup, _);
      accounts = response.value;
      while (response.nextPageLink) {
        response = client.dataLakeAnalyticsAccount.listByResourceGroupNext(response.nextPageLink);
        accounts.push.apply(accounts, response.value);
      }
    }
