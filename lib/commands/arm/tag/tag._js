...
function getTags(client, _) {
  ...
  //filter away hiddne tag used by portal, whose name starts with "hidden-related:/"
  //TODO: Remove this filter once service side filtering is implemented (RDTask 1911507)
  for (var i = tags.length-1; i >= 0; i--) {
    if (tags[i].name.indexOf('hidden-related:/') === 0) {
      tags.splice(i, 1);
    }
  }
  return tags;
