...
exports.init = function (cli) {
  ...
  powerbiembeddedcli.command('list [resourceGroup]')
    ...
    .execute(function (resourceGroup, options, _) {
      ...
      if(options.resourceGroup) {
        var progress = cli.interaction.progress($('Getting workspace collections in subscription ' + subscription.id + ' and within resource group: ' + options.resourceGroup));
        try {
        ...
        var progress = cli.interaction.progress($('Getting workspace collections in subscription: ' + subscription.id));
        try {
  ...
  powerbiembeddedcli.command('regenerate-key <resourceGroup> <name> <keyName>')
    ...
    .execute(function (resourceGroup, name, keyName, options, _) {
      ...
      if (!/^key(1|2)$/.test(keyName)) {
        return log.error('keyName must be either: "key1" or "key2". You passed: ' + keyName);
      }
