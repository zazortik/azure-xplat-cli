...
exports.init = function (cli) {
  ...
  */
  ...
  virtualMachineScaleSetsScale.command('scale [resource-group] [name] [out]')
  .description($('The operation to scale out Virtual machine scale set'))
  .usage('[options] <resource-group> <name> <out>')
  .option('-g, --resource-group <resource-group>', $('resource-group'))
  .option('-n, --name <name>', $('name'))
  .option('-i, --out <out>',$('the new capacity value that is higher than the current value'))
  .option('-s, --subscription <subscription>', $('The subscription identifier'))
  .execute(function(resourceGroup, name, out, options, _) {
    if (!resourceGroup) {
    ...
    if (!out) {
      out = cli.interaction.promptIfNotGiven($('New capacity value : '), out , _);
    }

    cli.output.verbose('New capacity value = ' + out);
    var subscription = profile.current.getSubscription(options.subscription);
    var computeManagementClient = utils.createComputeManagementClient(subscription);
    var vmss = computeManagementClient.virtualMachineScaleSets.get(resourceGroup, name, _);
    if(out < vmss.sku.capacity) {
     throw new Error('VMSS does not support scale down.Existing Capacity of ' + name + ' is ' +vmss.sku.capacity + '.We recommend to explicitly delete the specific instance as we cannot determine the right one. To delete the specific instance use the command azure vmss delete-instances <resource-group> <name> <instance-ids>');
    }
    if (out == vmss.sku.capacity) {
      throw new Error('New capacity value should be greater than existing capacity.');
    }
    vmss.sku.capacity = parseInt(out);
