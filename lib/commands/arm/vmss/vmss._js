  ...
  */
  ...
  .execute(function(resourceGroup, name, out, options, _) {
    ...
    if(out < vmss.sku.capacity) {
     throw new Error(
       'VMSS does not support scale down. Existing capacity of ' +
       name + ' is ' +vmss.sku.capacity + '. ' +
       'We recommend to explicitly delete the specific instance as we cannot determine the right one. ' +
       'To delete the specific instance use the command azure vmss delete-instances <resource-group> <name> <instance-ids>');
    }
    if (out == vmss.sku.capacity) {
