...
function createStorageResources(cli, vmClient, storageManagementClient, resourceGroupName, namePrefix, storageType, hash, location, capacity, _) {
  ...
  for (var i = 0; i < capacity; i++) {
    var stoName = generateNewStorageAccountName(namePrefix + 'sto' + i.toString());
    if (i === 0) {
      ...
      cli.output.verbose('stoName (reconciled) = \'' + stoName + '\'');
    }
    else {
      createStorageAccount(storageManagementClient, resourceGroupName, stoName, storageType, location, _);
    }
