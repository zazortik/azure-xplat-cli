...
__.extend(AvailsetClient.prototype, {
  listAvailSet: function(resourceGroupName, options, _) {
    ...
    var availsetsResult = availSetHelper.getAllAvailSets(resourceGroupName, _);
    ...
    this.cli.interaction.formatOutput(availsetsResult.availabilitySets, function (outputData) {
      if (outputData.length !== 0) {
        output.table(outputData, function (row, item) {
          ...
          if (item.properties) {
            row.cell($('Update domain count'), item.properties.platformUpdateDomainCount);
            row.cell($('Fault domain count'), item.properties.platformFaultDomainCount);
          }
        });
      } else {
