...
__.extend(AvailabilitySet.prototype, {
    ...
    createAvailSetIfRequired: function(_) {
      ...
      if (availSet) {
        if (!utils.ignoreCaseAndSpaceEquals(availSet.location, this.params.location)) {
          throw new Error(util.format($('An Availability set with name "%s" already exists in another region "%s"'), this.params.availsetName, availSet.location));
        }

        this.cli.output.info(util.format($('Found an Availability set "%s"'), this.params.availsetName));
        var connectedVMRefs = availSet.virtualMachinesReferences;
        if (connectedVMRefs instanceof Array) {
          ...
          for (var i = 0 ; i < connectedVMRefs.length; i++) {
            if (utils.stringEndsWith(connectedVMRefs[i].id, expectedVMId, true)) {
              throw new Error(util.format($('A VM with name "%s" (reference "%s") is already in the availability set "%s"'), this.params.vmName, connectedVMRefs[i].id, this.params.availsetName));
            }
          }
        }
    ...
    createNewAvailSet: function (resourceGroupName, params, _) {
      ...
      try {
        this.computeManagementClient.availabilitySets.createOrUpdate(resourceGroupName, createRequestProfile.name, createRequestProfile,  _);
        return createRequestProfile;
      } finally {
        progress.end();
      }
    ...
    _expandAvailSet: function (availSet, depth, memoize, dependencies, _) {
      ...
      if (availabilitySet.virtualMachinesReferences instanceof Array) {
        for (var i = 0; i < availabilitySet.virtualMachinesReferences.length; i++) {
          ...
          var vmReferenceId = vmReference.id.toLowerCase();
