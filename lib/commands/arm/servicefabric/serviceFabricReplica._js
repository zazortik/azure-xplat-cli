...
exports.init = function (cli) {
        ...
        if (data.length === 0) {
          ...
          log.table(data, function (row, item) {
            ...
            if (item.serviceKind === 1) {
              ...
              row.cell($('ReplicaRole'), serviceFabricUtils.getEnumValue('replicaRole', item.replicaRole));
            }
            row.cell($('ReplicaStatus'), serviceFabricUtils.getEnumValue('replicaStatus', item.replicaStatus));
  ...
  replicaDeployed.command('detail [nodeName] [partitionName] [replicaId]')
    ...
    .execute(function (nodeName, partitionName, replicaId, options, _) {
      ...
      if (res.serviceKind === 1) {
        res.currentServiceOperation = serviceFabricUtils.getEnumValue('currentServiceOperation', res.currentServiceOperation);
      }
      else if (res.serviceKind === 2) {
        res.currentReplicatorOperation = serviceFabricUtils.getEnumValue('currentReplicatorOperation', res.currentReplicatorOperation);
      }
      res.serviceKind = serviceFabricUtils.getEnumValue('serviceKind', res.serviceKind);
      cli.interaction.formatOutput(res, function (data) {
        log.json(data);
      });
    });
  ...
  replicaHealth.command('send [partitionId] [replicaId] [sourceId] [property] [healthState] [description] [timeToLiveInMilliSeconds] [sequenceNumber] [removeWhenExpired]')
    ...
    .execute(function (partitionId, replicaId, sourceId, property, healthState, description, timeToLiveInMilliseconds, sequenceNumber, removeWhenExpired, options, _) {
      ...
      if (healthState) replicaHealthReport.healthState = serviceFabricUtils.getEnumValue('health', healthState);
