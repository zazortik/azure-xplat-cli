...
exports.init = function (cli) {
  ...
  location.command('list')
    ...
    .execute(function (client, options, _) {
      ...
      var subscriptionClientOptions = {noSubscription: true};
      var subscriptionClient = utils.createSubscriptionClient(subscription, subscriptionClientOptions);
      ...
      withProgress($('Getting ARM registered providers'),
        function (log, _) {
          ...
          listOfLocations = locationUtils.takeAwayLatLonAddProviders(listOfLocations, providers);
        }, _);
      if (!options.json) {
      ...
      cli.interaction.formatOutput(listOfLocations, function (data) {
        if (data.length === 0) {
          ...
          data.forEach(function(location){
            ...
            log.data($('Providers   : '), locationUtils.showList(location.providers, 4));
          });
        }
      });
  });
