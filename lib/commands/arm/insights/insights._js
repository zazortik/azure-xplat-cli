...
exports.init = function (cli) {
  ...
  var insightsEventsResCommand = insightsEventsCommand.category('provider')
    .description($('Retrieve operation logs for a resource provider'))
    .command('list [resourceProvider]')
  ...
  insightsCommand._executeEventsCmd = function (client, queryFilter, selectedFields, keepTheRecord, options, _) {
    ...
    try {
      ...
      log.silly(response != undefined && response != null ? util.inspect(response) : "nothing in response");
      log.silly(response != undefined && response != null && response.eventDataCollection ? util.inspect(response.eventDataCollection) : "nothing in eventDataCollection");

      __.each(response.eventDataCollection.value, function (element) { if (keepTheRecord(element)) { result.push(element) } });
