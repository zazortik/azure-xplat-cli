...
var insightsClientLib = require('azure-insights');
...
exports.defaultWindowSize = 'PT1H';
...
exports.createInsightsClient = function (log, options) {
  ...
  // Creating the client just like the other clients are created in utils
  return insightsClientLib.createInsightsClient(credentials, utils.stringTrimEnd(subscription.resourceManagerEndpointUrl, '/'))
      .withFilter(utils.certAuthFilter(credentials))
...
exports.createInsightsManagementClient = function (log, options) {
  ...
  // Creating the client just like the other clients are created in utils
  return insightsClientLib.createInsightsManagementClient(credentials, utils.stringTrimEnd(subscription.resourceManagerEndpointUrl, '/'))
      .withFilter(utils.certAuthFilter(credentials))
      .withFilter(log.createLogFilter())
      .withFilter(azure.UserAgentFilter.create(utils.getUserAgent()))
      .withFilter(utils.createPostBodyFilter())
...
exports.addConditionIfPresent = function (currentQueryFilter, name, value) {
  if (value != undefined && !__.isNull(value) && value != '') {
...
function _validateDateRange(startDate, endDate) {
  ...
  if (difference > _maximumDateDifferenceAllowed) {
    throw new Error(util.format($('Time range exceeds maximum allowed. Start date: %s, end date: %s'), startDate.toISOString(), endDate.toISOString()));
  }
}

// Validation of input TimeSpans
exports.validateTimeSpan = function(timeSpan) {
  if (__.isString(timeSpan)) {
    var parts = timeSpan.split(":");
    if (parts.length != 3) {
      throw new Error(util.format($('Incorrect format form TimeSpan input: %s. Expected: hh:mm:ss'), timeSpan));
    }
    ...
    var totalSecs = seconds + minutes * 60 + hours * 3600;
    ...
    //return util.format("%s:%s:%s", hours < 10 ? '0' + hours : hours, minutes < 10 ? '0' + minutes : minutes, seconds < 10 ? '0' + seconds : seconds);
    ...
    return {
      ...
      toIsoString: function() {
        if (seconds > 0) {
          return util.format("PT%sS", hours * 3600 + minutes * 60 + seconds);
        } else if (minutes > 0) {
          return util.format("PT%sM", hours * 60 + minutes);
        } else {
          return util.format("PT%sH", hours);
        }
      }
    }
  } else {
...
exports.showObject = function (log, object, indentationTabs) {
  ...
  for (var propertyName in object) {
    if (__.isNull(object[propertyName]) || __.isUndefined(object[propertyName])) {
      ...
      log.data(spaces + propertyName + ':');
      __.each(object[propertyName], function (element) { exports.showObject(log, element, tabs + 1); });
    }
...
exports.formatOutputList = function (cli, log, options, values) {
  ...
  if (options.json) {
    ...
    __.each(values, function (element) {
