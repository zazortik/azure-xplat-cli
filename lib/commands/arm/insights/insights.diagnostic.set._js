...
exports.init = function (cli) {
  ...
  insightsDiagnosticCommand._prepareAndExecute = function (options, _) {
    ...
    if (this._isEmptyOrSpaces(options.resourceId))
    {
      throw new Error($('The resourceId parameter is required.'));
    }

    if (this._isEmptyOrSpaces(options.enabled))
    {
  ...
  insightsDiagnosticCommand._executeCmd = function (client, options, _) {
    ...
    if (options.enabled && 
        ...
        this._isEmptyOrSpaces(options.serviceBusRuleId)) {
    ...
    if (!this._isEmptyOrSpaces(options.serviceBusRuleId))
    {
