...
exports.init = function (cli) {
  ...
  var insightsDiagnosticCommand = cli.category('insights').category('diagnostic')
    ...
    .command('set')
      ...
      .option('-s, --storageId <storageId>', $('storage account Id.'))
  ...
  insightsDiagnosticCommand._executeCmd = function (client, options, _) {
    ...
    if (!options.categories && !options.timegrains) {
      for (var i = 0; i < properties.logs.length; i++) {
        properties.logs[i].enabled = options.enabled;
      }

      for (var i = 0; i < properties.metrics.length; i++) {
      ...
      if (options.categories) {
        for (var i = 0; i < options.categories.length; i++) {
          ...
          for (var j = 0; j < properties.logs.length; j++) {
      ...
      if (options.timegrains) {
        for (var i = 0; i < options.timegrains.length; i++) {
          var metricSettings = null;

          for (var j = 0; j < properties.metrics.length; j++) {
          ...
          if (!metricSettings) {
              throw new Error(util.format("Metric timegrain '%s' is not available for '%s'", timegrain, options.storageId));
          }
