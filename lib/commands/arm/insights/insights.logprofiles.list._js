...
exports.init = function (cli) {
  ...
  insightsDiagnosticCommand._executeCmd = function (client, cli, log, options, _) {
    var getResponse = client.logProfilesOperations.list(_);
    for (var i = 0; i < getResponse.logProfileCollection.value.length; i++) {
      var logProfile = getResponse.logProfileCollection.value[i];
      logProfile.properties.id = logProfile.id;
      logProfile.properties.name = logProfile.name;
      getResponse.logProfileCollection.value[i] = logProfile;
    }

    insightsUtils.formatOutput(cli, log, options, getResponse.logProfileCollection.value);
  };
};
