...
_.extend(ServiceBusManagementService.prototype, {
  ...
  */
  createNamespace: function (namespaceName, region, callback) {
    ...
    namespaceNameIsValid(namespaceName, function (err) {
      ...
      that._getResult(path, 'put', body, 'NamespaceDescription', callback);
    });
  },
  ...
  */
  deleteNamespace: function (namespaceName, callback) {
    ...
    namespaceNameIsValid(namespaceName, function (err) {
      if (err) { callback(err); }
       var path = that._makePath('Namespaces') + namespaceName;
       that._getResult(path, 'del', 'NamespaceDescription', callback);
    });
  },
  ...
  */
  verifyNamespace: function (name, callback) {
    var that = this;
    namespaceNameIsValid(name, function (err, isValid) {
      if (err) {
        return callback(err);
      }
      var path = that._makePath('CheckNamespaceAvailability') + "?namespace=" + name;
      that._getResult(path, 'get', 'NamespaceAvailability', function (err, result) {
        if (err) {
          return callback(err);
        }
