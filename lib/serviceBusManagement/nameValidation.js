...
function namespaceNameIsValid(name) {
  if (name.length < 6 || name.length > 50) {
    throw new Error('Service Bus namespace names must be 6 to 50 characters long.');
  }
  if (!validNameRegex.test(name)) {
    throw new Error('Service Bus namespace names must start with a letter and include only letters, digits, and hyphens');
  }
  if (illegalEndings.test(name)) {
    throw new Error('Service Bus namespace names may not end with "-", "-sb", "-mgmt", "-cache", or "-appfabric"');
  }
}
