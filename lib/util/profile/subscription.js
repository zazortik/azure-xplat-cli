...
_.extend(Subscription.prototype, {
  ...
  */
  ...
  isArmProviderRegistered: function (providerName) {
    var result = false;
    if (this.registeredResourceNamespaces.some(function (item) {
      return utils.ignoreCaseEquals(item, providerName);
    })) {
      result = true;
    }
  ...
  registerArmProvider: function (namespace, callback) {
    ...
    //return if ARM provider is already registered
    if (self.isArmProviderRegistered(namespace)) { 
      return callback();
    }
  ...
  unRegisterArmProvider: function (namespace, callback) {
    ...
    client.providers.unregister(namespace, function (err) {
      ...
      //remove the namespace from the list of registered resource namespaces
      if (self.registeredResourceNamespaces) {
        self.registeredResourceNamespaces = 
          self.registeredResourceNamespaces.filter(function (item) {
            return (!utils.ignoreCaseEquals(item, namespace));
        });
      }
      self.emit('updated');
      callback();
