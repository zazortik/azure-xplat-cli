  ...
  _createCredentials: function (resourceId) {
    ...
    if (this.user) {
      switch (this.user.type) {
        case 'user':
          cred = new adalAuth.AzureADUserCredential(authConfig, this.user.name);
          break;

        case 'servicePrincipal':
          authConfig.tenantId = this.user.tenant;
          cred = new adalAuth.ServicePrincipalCredential(authConfig, this.user.name);
          break;

        default:
          throw new Error($('Unknown user type, cannot create credentials'));
      }
      return new AccessTokenCloudCredentials(cred, this.id);
