...
_.extend(Subscription.prototype, {
  ...
  */
  createClient: utils.overload(
    ...
    function (factory, credentials, endpoint) {
      return utils.createClient(factory, credentials, endpoint);
    }
  ),
  ...
  */
  ...
  // RDFE and CSM provider registration, called from registrationFilter
  registerProvider: function (providerName, callback) {
    ...
    client.subscriptions.registerResource(providerName, function (err) {
      if (err) {
        ...
        // registered. Update when we have a reliable way to detect already registered vs
        // other errors.
        log.verbose(util.format($('Registration of resource %s failed'), providerName));
      }
  ...
  registerResourceNamespace: function (namespace, callback) {
    var self = this;
    var client = utils.createClient('createResourceManagementClient', self._createCredentials(), self.resourceManagementEndpointUrl);
    var pollIntervalInMS = 5 * 1000;

    function waitForRegistrationComplete(retriesLeft, cb) {
      if (retriesLeft === 0) {
        return cb(new Error(util.format($('Namespace %s registration took too long to complete'), namespace)));
      }

      client.providers.get(namespace, function (err, result) {
        if (!err) {
          if (utils.equalsIgnoreCase(result.provider.registrationState,'Registered')) {
            return cb();
          }
        }
    ...
    client.providers.register(namespace, function (err) {
      ...
      waitForRegistrationComplete(5, function (err) {
        if (!err) {
