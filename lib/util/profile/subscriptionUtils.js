...
function getSubscriptions(environment, username, password, tenant, callback) {
  environment.acquireToken(username, password, tenant, function (err, authContext) {
    ...
    //No ARM support, such as the China Cloud
    if (!environment.isResourceManagerEndpointDefined()) {
      return _getRdfeSubscriptions(environment, authContext, username, callback);
    }
...
function _getRdfeSubscriptions(environment, authContext, username, callback) {
  var asmClient = environment.getAsmClient(_createCredential(authContext));
  asmClient.subscriptions.list(function (err, result) {
