...
function getSubscriptions(environment, username, tenant, callback) {
  ...
  async.waterfall([
    ...
    function (authContext, userId, callback) {
      subscriptionUtils.getAllTenants(environment, authContext, function (err, result) {
        if (err) return callback(err);
    ...
    function (authContext, userId, callback) {
      ...
      subscriptionUtils.getSubscriptionsFromTenants(environment, userId, defaultTenantInfo, callback);
    }
