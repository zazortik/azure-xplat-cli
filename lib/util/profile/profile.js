...
_.extend(Profile.prototype, {
  ...
  clearActiveDirectoryToken: function (username) {
    ...
    // user doesn't appear logging in
    if (!currentSubscription || !currentSubscription.accessToken){
      return false;
    }

    //if there is certificate, we will keep it and only remove the token; 
    //otherwise, we just clean up the whole subscription
    if (currentSubscription.managementCertificate) {
      delete currentSubscription.accessToken;
    } else {
...
function addKnownResourceNamespace(ns) {
  knownResourceNamespaces.add(ns);
}

function addKnownProvider(p) {
