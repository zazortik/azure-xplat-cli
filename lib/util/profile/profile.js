...
_.extend(Profile.prototype, {
  ...
  addSubscription: function (subscription) {
    ...
    if (existingSubscription) {
      ...
      _.extend(existingSubscription, subscription);
  ...
  logoutUser: function (username) {
    ...
    var subscriptions = _.values(this.subscriptions);
    var subscriptionsToRemove = [];
    var defaultSubscription = this.currentSubscription;

    //construct impacted subscription list, we will make sure to remove the default subscription at last
    //so that the logic of reselecting the new default one will run only once. 
    subscriptions.forEach(function (subData) {
      if (subData.username === username){
        if (subData !== defaultSubscription){
          subscriptionsToRemove.push(subData);
        } else {
          subscriptionsToRemove.unshift(subData);
        }
      }
    });

    var logoutOccurred = false;
    for (var i = subscriptionsToRemove.length-1; i>=0; i--){
      var subscription = subscriptionsToRemove[i];
      if (subscription.accessToken){
        if (subscription.managementCertificate) {
          delete subscription.accessToken;
        } else {
          this.deleteSubscription(subscription.name);
        }
        logoutOccurred = true;
      }
    }

    return logoutOccurred;
  },
  ...
  getSubscription: function (idOrName) {
    ...
    if (!idOrName) {
      ...
      if (!subscription) {
        throw new Error($('There is no current subscription. Please use the azure account login command to set your current subscription.'));
      }
    } else {
      ...
      if (!subscription) {
        throw new Error(util.format(
          $('The subscription \'%s\' was not found. Please check your spelling, or use the azure account login command to set your subscription.'),
          idOrName));
      }
    }
    return subscription;
