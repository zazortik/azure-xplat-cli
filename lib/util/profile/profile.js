...
_.extend(Profile.prototype, {
  ...
  deleteSubscription: function (subscriptionOrId) {
    var subscription = subscriptionOrId;
    if (_.isString(subscriptionOrId)) {
      subscription = this.subscriptions[subscriptionOrId];
    }
  ...
  logoutUser: function (username, done) {
    ...
    adalAuth.logoutUser(username, function (err) {
  ...
  getSubscription: function (idOrName) {
    ...
    if (!idOrName) {
      ...
      _.values(this.subscriptions)
      .filter(function (s) { return utils.ignoreCaseEquals(s.name, idOrName); })[0];
      if (!subscription) {
  ...
  getSubscriptionByName: function (name) {
    ...
    if (!name) {
      ...
      matchingSubscriptions.push(this.currentSubscription);
    } else {
      //Try to find a subscription match by name.
      matchingSubscriptions = _.values(this.subscriptions).filter(function (s) { return utils.ignoreCaseEquals(s.name, name); });
