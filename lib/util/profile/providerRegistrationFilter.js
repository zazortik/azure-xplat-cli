...
function subscriptionRegistrationFilter(subscription) {
  return function (request, next, callback) {
    var stream = azureCommon.requestPipeline.interimStream(function (input, output) {
      ...
      registerUnregisteredRDFEProviders(subscription, function (err) {
        if (err) {
...
function register(subscription, registerMethod, alreadyRegistered, allToRegister, callback) {
  ...
  var stuffToRegister = new StringSet(profile.providerKeyTransform)
    .add(allToRegister)
    .delete(alreadyRegistered);

  log.verbose(util.format($('All known providers: %s'), allToRegister.join(', ')));
  log.verbose(util.format($('Already registered providers: %s'), alreadyRegistered.join(', ')));
  log.verbose(util.format($('Providers to register via %s: %s'), registerMethod, stuffToRegister.keys().join(',')));

  if (stuffToRegister.size() === 0) {
    return callback();
  }

  var registerDone = _.after(stuffToRegister.size(), callback);

  stuffToRegister.forEach(function (p) {
    log.version(util.format($('Registering %s with function %s'), p, registerMethod));
...
function registerUnregisteredRDFEProviders(subscription, callback) {
  var profile = require('./profile');
