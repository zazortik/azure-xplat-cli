...
Account.prototype.load = function (username, password, tenant, loginType, callback) {
  ...
  if (loginType.servicePrincipal) {
    ...
    self._getMFASubscriptions(tenant, processAndReturnSubscriptions);
  } else {
    self._getNonMFASubscriptions(username, password, tenant, processAndReturnSubscriptions);
  }
};

Account.prototype._getMFASubscriptions = function (tenant, callback) {
  var self = this;
  ...
  async.waterfall([
    ...
    function (userCodeResponse, callback) {
      ...
      self._adalAuth.authenticateWithDeviceCode(authConfig, userCodeResponse, function (err, credential) {
        ...
        async.waterfall([
          function (callback) {
            self._buildTenantList(self._username, null, tenant, credential, callback);
          },
          function (tenantList, callback) {
            self._getSubscriptionsFromTenants(self._username, tenantList, callback);
          },
...
Account.prototype._getSubscriptionsFromTenants = function (username, tenantList, callback) {
  ...
  async.eachSeries(tenantList, function (tenant, cb) {
    ...
    var credential = new self._adalAuth.UserTokenCredentials(self._env.getAuthConfig(tenantId), username);
