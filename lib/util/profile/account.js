...
function Account(activeDirectoryResourceId, activeDirectoryEndpointUrl, resourceManagerEndpointUrl) {
...
Account.prototype.load = function (username, password, tenant, loginType, callback) {
  ...
  if (loginType.servicePrincipal) {
    adalAuth.acquireServicePrincipalToken(self._getAuthConfig(tenant), username, password, function (err, token) {
      if (err) { return callback(err); }
      var tenantInfo = {
        tenantId: tenant,
        authContext: token
...
Account.prototype._getNonMtaSubscriptions = function (username, password, tenant, callback) {
  var self = this;
  adalAuth.acquireToken(self._getAuthConfig(tenant), username, password, function (err, authContext) {
...
Account.prototype._buildTenantList = function (username, password, tenant, authContext, callback) {
  ...
  self._getAllTenants(authContext, function (err, result) {
    ...
    async.eachSeries(result.tenantIds/*'tenantInfos' could be a better name*/, function (tenantInfo, cb) {
      adalAuth.acquireToken(self._getAuthConfig(tenantInfo.tenantId), username, password, function (err, authContext) {
...
Account.prototype._getArmClient = function (credentials) {
  return resourceClient.createResourceSubscriptionClient(credentials, this._resourceManagerEndpointUrl);
};
