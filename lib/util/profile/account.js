...
function Account(env, adalAuth, resourceClient) {
  this._env = env;
...
Account.prototype.load = function (username, password, tenant, loginType, callback) {
  ...
  if (loginType.servicePrincipal) {
    this._adalAuth.createServicePrincipalCredential(self._env.getAuthConfig(tenant), username, password, function (err, credential) {
      if (err) { return callback(err); }
      var tenantInfo = {
        tenantId: tenant,
...
Account.prototype._getSubscriptionsFromTenants = function (username, tenantList, callback) {
  ...
  async.eachSeries(tenantList, function (tenant, cb) {
    ...
    var armClient = self._getArmClient(_createCredential(tenant.credential));
...
Account.prototype._getAllTenants = function (credential, callback) {
  var armClient = this._getArmClient(_createCredential(credential));
...
function _createCredential(credential) {
  return new AccessTokenCloudCredentials(credential, 'notUsed');
}
