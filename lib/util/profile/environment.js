...
var ResourceClient = require('azure-mgmt-resource');//TODO get rid of this and go through factory method
...
_.extend(Environment.prototype, {
  ...
  getAccountSubscriptions: function (token, callback) {
    ...
    subClient.subscriptions.list(function (err, subscriptions) {
      ...
      var client = ResourceClient.createResourceSubscriptionClient(credentials, self.resourceManagerEndpointUrl);
      client.subscriptions.list(function(err, subscriptions){
        if (err) { return callback(err); }
        //merge with csm subscriptions
        var allSubscriptions = rdfeSubscriptions;
        var csmSubscriptions = subscriptions.subscriptions;
        for (var i = 0; i<csmSubscriptions.length; i++){
          for (var j = 0; j<allSubscriptions.length; j++) {
            if (csmSubscriptions[i].subscriptionId === allSubscriptions[j].subscriptionId) {
              break;
            }
          }
          if (j === allSubscriptions.length){
            ...
            //populate 2 extra proeprties needed for commands
            csmSubscriptions[i].subscriptionName = csmSubscriptions[i].displayName;
            csmSubscriptions[i].activeDirectoryTenantId = tenantId;
