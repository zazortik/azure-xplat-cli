...
_.extend(Environment.prototype, {
  ...
  addAccount: function (username, password, callback) {
    ...
    self.acquireToken(self.authConfig, username, password, function (err, accessTokenObj) {
      ...
      self.getAccountSubscriptions(accessTokenObj, username, password, function (err, subscriptions) {
        if (err) { return callback(err); }
  ...
  getAccountSubscriptions: function (token, username, password, callback) {
    var credentials = new AccessTokenCloudCredentials(token, 'notUsed');
    var self = this;
    var asmClient = azure.createSubscriptionClient(credentials, this.managementEndpointUrl);
    asmClient.subscriptions.list(function (err, asmSubscriptions) {

      if (err) { return callback(err); }
      
      var armClient = ResourceClient.createResourceSubscriptionClient(credentials, self.resourceManagerEndpointUrl);

      armClient.tenants.list(function (error, armTenants) {
        for (var i = 0; i < armTenants.tenantIds.length; i++) {
          var currentTenantId = armTenants.tenantIds[0].tenantId;
          var currentAuthCfg = {
            authorityUrl: self.activeDirectoryEndpointUrl,
          ...
          self.acquireToken(currentAuthCfg, username, password, function (err, accessTokenObj) {
            if (err) { return callback(err); }
            var currentCredentials = new AccessTokenCloudCredentials(accessTokenObj, 'notUsed');
            var armClient2 = ResourceClient.createResourceSubscriptionClient(currentCredentials, self.resourceManagerEndpointUrl);
            armClient2.subscriptions.list(function (err, armSubscriptions) {
              if (err) { return callback(err); }
              callback(null, mergeSubscriptions(asmSubscriptions.subscriptions, armSubscriptions.subscriptions, currentTenantId));
            });
          });
        }
      });
