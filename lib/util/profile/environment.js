...
function Environment(envData) {
  ...
  _.defaults(values, {
    ...
    resourceManagementEndpointUrl: constants.DEFAULT_RESOURCE_ENDPOINT_URL,
  ...
  Object.defineProperties(this, {
    ...
    resourceManagementEndpointUrl: propDescriptor('resourceManagementEndpointUrl', 'AZURE_RESOURCEENDPOINT_URL', getField),
    ...
    sqlServerHostnameSuffix: propDescriptor('sqlServerHostNameSuffix', 'AZURE_SQL_SERVER_HOSTNAME_SUFFIX', getField),
    activeDirectoryEndpointUrl: propDescriptor('activeDirectoryEndpointUrl', 'AZURE_ACTIVEDIRECTORY_ENDPOINT_URL', getField),
    commonTenantName: propDescriptor('commonTenantName', 'AZURE_ACTIVEDIRECTORY_COMMON_TENANT_NAME', getField),
    storageEndpoint: propDescriptor('storageEndpoint', 'AZURE_STORAGE_ENDPOINT', getField),
    isPublicEnvironment: {
      enumerable: true,
      configurable: false,
      get: function () {
...
_.extend(Environment.prototype, {
  toJSON: function () {
    return _.extend({ name: this.name, realm: this.realm }, this.values);
  },

  addAccount: function (username, password, tenantId, callback) {
    var self = this;

    if (_.isFunction(tenantId)) {
      callback = tenantId;
      tenantId = null;
    }
    tenantId = tenantId || adalAuth.tenantIdForUser(username);
    var authConfig = {
    ...
    self.acquireToken(authConfig, username, password, function (err, accessToken) {
      ...
      // TODO: Call back to azure to get subscription information.
      // Currently doesn't work, fill in dummy values for now.

      var subscription = new Subscription({
        id: 'to be determined',
        name: 'newSubscription',
        managementEndpointUrl: self.managementEndpointUrl,
        resourceManagementEndpointUrl: self.resourceManagementEndpointUrl,
