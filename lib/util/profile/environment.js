...
_.extend(Environment.prototype, {
  ...
  getAccountSubscriptions: function (token, callback) {
    ...
    asmClient.subscriptions.list(function (err, asmSubscriptions) {
      ...
      //No 2-hops here, which is supposed to log into each tenant and get an access token back.
      //The reason is with org-id, the initial token is good enough to acces each tenant.
      armClient.subscriptions.list(function (err, armSubscriptions) {
        if (err) { return callback(err); }
        callback(null, mergeSubscriptions(asmSubscriptions.subscriptions, armSubscriptions.subscriptions, tenantId));
      });
