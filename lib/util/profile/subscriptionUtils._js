...
function getSubscriptions(environment, username, password, tenant, _) {
  var accessToken = environment.acquireToken(username, password, tenant, _);
  ...
  if (!tenant) {
    ...
    tenants = armClient.tenants.list(_);
  } else {
    tenants.tenantIds = [{
      tenantId: tenant,
      token: accessToken
  ...
  for (var i = 0; i < tenants.tenantIds.length; i++) {
    ...
    tenantAccessToken = tenants.tenantIds[i].token;
