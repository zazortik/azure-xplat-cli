...
function getSubscriptions(environment, username, password, _) {
  ...
  // If resource manager endpoint is not defined, the environment getter will throw
  // catch the exception and use service management to list subscriptions instead in 
  // this case.
  try {
    var resourceManagerEndpoint = environment.resourceManagerEndpointUrl;
  } catch (exception) {
...
function getSubscriptionsInTenant(environment, username, tenantId, accessToken, _) {
  // If resource manager endpoint is not defined, the environment getter will throw
  // catch the exception and replace with a better execption in thie case
  try {
    var resourceManagerEndpoint = environment.resourceManagerEndpointUrl;
  } catch (exception) {
    throw new Error('You cannot use ServicePrincipal credentials in this environment.');
  }
