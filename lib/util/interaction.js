...
__.extend(Interactor.prototype, {
  ...
  _onlyLogError: function (log) {
    log = log || this.cli.output;
    return log && (log.format().level === 'error');
  },
  ...
  clearProgress: function () {
    var self = this;
    if (self._onlyLogError && self._onlyLogError()) {
      return;
    }
  ...
  progress: function(label, log) {
    ...
    //TODO: add comments for why _onlyLogError method might not exist
    if (self._onlyLogError && self._onlyLogError(log)) {
      return {
        write: function () { },
        end: function () { }
      };
