...
function generatePolicy(policyName, start, expiry, permissions) {
  ...
  policy.AccessPolicy.Start = start ? (start === StorageUtil.SPACE_PARAMETER ? StorageUtil.SPACE_PARAMETER : utils.parseDateTime(start)) : '';
  policy.AccessPolicy.Expiry = expiry ? (expiry === StorageUtil.SPACE_PARAMETER ? StorageUtil.SPACE_PARAMETER : utils.parseDateTime(expiry)) : '';
