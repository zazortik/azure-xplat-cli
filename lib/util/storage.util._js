...
function getStorageServiceSettingWithSAS(accountName, sasToken) {
  ...
  if (delimiter === -1) {
    ...
    protocol = uri.substring(0, uri.indexOf('/') + 2);
    endpointSuffix = uri.substring(uri.indexOf('.'), uri.length);
  }

  var getEndpointLite = function (type) {
    return getEndpoint(protocol, accountName, type, endpointSuffix);
  };
...
StorageUtil.startAsyncCopy = function (startCopyParams, sourceUri, destContainerOrShare, options, _) {
  ...
  var sourceContainerOrShare = options.isSourceBlobDefined ? options.sourceContainer : (options.isSourceFileDefined ? options.sourceShare : undefined);
  var sourceBlobOrFile = options.isSourceBlobDefined ? options.sourceBlob : (options.isSourceFileDefined ? options.sourcePath : undefined);
