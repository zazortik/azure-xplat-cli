...
function getStorageServiceSettings(connection) {
  ...
  var getFullConnectionString = function (accountName, accountKey) {
    var template = 'DefaultEndpointsProtocol=https;AccountName=%s;AccountKey=%s;BlobEndpoint=%s;TableEndpoint=%s;QueueEndpoint=%s;FileEndpoint=%s';
    var endpointSuffix = getStorageServiceEndpointSuffix();
    var blobEndpoint = accountName + '.blob.' + endpointSuffix; 
    var tableEndpoint = accountName + '.table.' + endpointSuffix; 
    var queueEndpoint = accountName + '.queue.' + endpointSuffix; 
    var fileEndpoint = accountName + '.file.' + endpointSuffix; 
    return util.format(template, accountName, accountKey, blobEndpoint, tableEndpoint, queueEndpoint, fileEndpoint);
  };
...
function getStorageServiceEndpointSuffix() {
  ...
  if (__.keys(profile.current.subscriptions).length > 0) {
    ...
    uri = profile.current.environments.AzureCloud.managementEndpointUrl;
  }
