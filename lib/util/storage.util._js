...
StorageUtil.getBlobService = function(connectionString) {
  var service = null;
  var key = 'blobServiceClient';
  service = ObjectPool.get(key);
  if(!service) {
    ...
    service.withFilter(new ExponentialRetryPolicyFilter());
    ObjectPool.set(key, service);
  }
  return service;
};
