...
StorageUtil.getBlobService = function(connection) {
  ...
  } else if (serviceSettings.sharedaccesssignature) {
    service = storage.createBlobServiceWithSas(serviceSettings._blobEndpointUri, serviceSettings.sharedaccesssignature);
  } else {
    service = storage.createBlobService(serviceSettings._name, serviceSettings._key, serviceSettings._blobEndpointUri);
  }
...
StorageUtil.getTableService = function(connection) {
  ...
  if (serviceSettings === null) {
    ...
    service = storage.createTableServiceWithSas(serviceSettings._tableEndpointUri, serviceSettings.sharedaccesssignature);
  } else {
    service = storage.createTableService(serviceSettings._name, serviceSettings._key, serviceSettings._tableEndpointUri);
  }
  return service.withFilter(new ExponentialRetryPolicyFilter());
};
...
StorageUtil.getQueueService = function(connection) {
  ...
  if (serviceSettings === null) {
    ...
    service = storage.createBlobServiceWithSas(serviceSettings._queueEndpointUri, serviceSettings.sharedaccesssignature);
  } else {
    service = storage.createQueueService(serviceSettings._name, serviceSettings._key, serviceSettings._queueEndpointUri);
  }
  return service.withFilter(new ExponentialRetryPolicyFilter());
};
...
StorageUtil.getFileService = function(connectionString) {
  ...
  if (serviceSettings === null) {
    ...
    service = storage.createFileService(serviceSettings._name, serviceSettings._key, serviceSettings._fileEndpointUri);
  }
  return service.withFilter(new ExponentialRetryPolicyFilter());
};
...
*/
StorageUtil.listWithContinuation = function(listFunc, storageServiceObject) {
  var allItems = {};
  function listCallback(error, result) {
    ...
    if (result.continuationToken) {
      StorageUtil.listWithContinuation(listFunc, storageServiceObject);
    } else {
  ...
  var callArguments = Array.prototype.slice.call(arguments).slice(2, arguments.length - 1);
...
StorageUtil.parseDateTime = function(stringDateTime) {
  var date;
  try {
...
function getStorageServiceSettings(connection) {
  ...
  if (typeof connection === 'string') {
    ...
    if (options.connectionString) {
      ...
      } else if (options.accountName && options.sas) {
        return getStorageServiceSettingWithSAS(options.accountName, options.sas);
      }
    }
  }
...
function getStorageServiceSettingWithSAS(accountName, sasToken) {
  ...
  serviceSettings.sharedaccesssignature = sasToken;
  serviceSettings._blobEndpointUri = protocol + accountName + '.' + StorageUtil.OperationType.Blob + endpoint;
  serviceSettings._tableendpoint = protocol + accountName + '.' + StorageUtil.OperationType.Table + endpoint;
  serviceSettings._queueendpoint = protocol + accountName + '.' + StorageUtil.OperationType.Queue + endpoint;
  serviceSettings._fileendpoint = protocol + accountName + '.' + StorageUtil.OperationType.File + endpoint;
