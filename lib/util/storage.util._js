...
function sendingRequestHandler(webresource) {
  ...
  // Check whether the user-agent starts with the given string to avoid appending the string multiple times in retry
  if (originalUserAgent.length < userAgent.length || originalUserAgent.substring(0, userAgent.length) != userAgent) {
