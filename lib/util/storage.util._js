...
StorageUtil.formatBlobProperties = function(properties, target) {
  var validPropertyName = ['contentType', 'contentEncoding', 'contentLanuage', 'contentMD5', 'cacheControl'];
  for(var item in properties) {
    validation.isValidEnumValue(item, validPropertyName);
    target[item] = properties[item];
  }
};
