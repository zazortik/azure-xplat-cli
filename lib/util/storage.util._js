...
StorageUtil.performStorageOperation = function (storageOperation, _) {
  ...
  service.setProxy('127.0.0.1', 8888);
...
* Escape blob name
*/
StorageUtil.escapeFileName = function(name) {
  name = encodeURIComponent(name);
  return name.replace(/%20/img, ' '); //Space is valid for file system
};

/**
* Recursive mkdir
*/
StorageUtil.recursiveMkdir = function(dirPath) {
  var dirs = dirPath.split('/'); //From URI component.
  var dirPath = '';
