...
StorageUtil.selectPolicy = function(policySettings, _) {
  ...
  return policySettings.policyName ? (index >= 0 ? policies.slice(index, index+1) : null) : policies;
}
...
function generatePolicy(policyName, start, expiry, permissions) {
  ...
  policy.AccessPolicy.Start = start ? (start === SPACE_PARAMETER ? '' : validation.parseDateTime(start)) : '';
  policy.AccessPolicy.Expiry = expiry ? (expiry === SPACE_PARAMETER ? '' : validation.parseDateTime(expiry)) : '';

  if (start && expiry && policy.AccessPolicy.Start.getTime() >= policy.AccessPolicy.Expiry.getTime()) {
    throw new Error($('The expiry time of the specified access policy should be greater than start time'));
  }
...
function normalizePolicy(policy, accessType) {
  ...
  if (!policy.AccessPolicy.Permission) {
    ...
    if (accessType === StorageUtil.AccessType.Container) {
      policy.AccessPolicy.Permissions = pickupValidPermission(policy.AccessPolicy.Permission, StorageUtil.BlobPermission);
    } else if (accessType === StorageUtil.AccessType.Table) {
      policy.AccessPolicy.Permissions = pickupValidPermission(policy.AccessPolicy.Permission, StorageUtil.TablePermission);
    } else if (accessType === StorageUtil.AccessType.Queue) {
      policy.AccessPolicy.Permissions = pickupValidPermission(policy.AccessPolicy.Permission, StorageUtil.QueuePermission);
    }
  }
}
...
function modifyPolicySet(newPolicy, existingPolicies, accessType, operation) {
  ...
  if (operation == StorageUtil.PolicyOperation.Create) {
    ...
    if (index >= 0) {
      ...
      } else if (newPolicy.AccessPolicy.Start === ' ') {
      ...
      } else if (newPolicy.AccessPolicy.Expiry === ' ') {
      ...
      throw new Error(util.format($('The policy %s doesn\'t exists.'), newPolicy.Id));
    }
  ...
  console.log('[PROD] polies: ' + util.inspect(existingPolicies));
};
...
*/
function pickupValidPermission(permissions, allowOps) {
