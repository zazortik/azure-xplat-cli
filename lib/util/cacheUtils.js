...
function expiredPlugins(plugins) {
  var daysDifference = moment(plugins.timestamp).diff(new Date(), 'minutes');
  if (plugins.timestamp && daysDifference < 5) {
    return false;
  }
...
CacheUtils.readPlugins = readPlugins;
CacheUtils.savePlugins = savePlugins;
CacheUtils.clearPlugins = clearPlugins;
