...
exports.getUserIdByPrincipalName = function getUserObjectId(user, subscription, callback) {
  ...
  credentials.accessToken.getGraphAccessInfo(function (err, graphAccess) {
    ...
    getUserGraphObject(authorization, graphAccess.tenantGuid, user, subscription.environment, function (err, response) {
      if (err) {
        callback(new Error(err));
        return;
      }
...
function getUserGraphObject(authorization, tenantId, principalName, environment, callback) {
  ...
  var graphProfileUrl = baseGraphUrl + userSubQueryUrl + '?api-version=' + environment.activeDirectoryGraphApiVersion;
