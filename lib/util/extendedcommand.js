...
_.extend(ExtendedCommand.prototype, {
  ...
  categoryHelpInformation: function(containsJson) {
    ...
    if (containsJson) {
      ...
      this.output.format({ json: true, level: 'data' });
  ...
  getJSONMetadata: function (isCategoryHelp) {
    ...
    try {
      ...
      throw new Error(util.format('Error occurred while parsing the json file \'%s\'', cmdMetadatafile));
    }
    ...
    if (!isCategoryHelp) {
      ...
      var topCmd = self.findItem(jsonInfo.commands, command);
    ...
    if (!isCategoryHelp) {
      return self.findItem(categories.commands, command);
    } else {
      return categories;
    }
  },
