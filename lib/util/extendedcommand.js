...
_.extend(ExtendedCommand.prototype, {
  ...
  helpInformation: function() {
    ...
    if (!self.parent) {
      ...
      if (raw.indexOf('-v') >= 0) {
        ...
        this.output.info('Microsoft Azure: Microsoft\'s Cloud Platform');
        this.output.info('');
  ...
  helpCommands: function() {
    ...
    this.commands.forEach(function (cmd) {
      var desc = (typeof(cmd.description) === "function") ? cmd.description() : cmd.description;
      var fullName = (typeof(cmd.fullName) === "function") ? cmd.fullName() : cmd.fullName;
      var usage = (typeof(cmd.usage) === "function") ? cmd.usage() : cmd.usage;
      self.output.help('');
      self.output.help(desc.cyan);
      self.output.help(' ', fullName + ' ' + usage);
    });
  },
