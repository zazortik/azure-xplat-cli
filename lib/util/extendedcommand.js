...
_.extend(ExtendedCommand.prototype, {
  ...
  helpInformation: function() {
    ...
    if (!self.parent) {
      ...
      if (raw.indexOf('-v') >= 0) {
        ...
        if (this.output.format().log === 'on') {
          this.output.info('         _    _____   _ ___ ___'.cyan);
          this.output.info('        /_\\  |_  / | | | _ \\ __|'.cyan);
          this.output.info('  _ ___'.grey + '/ _ \\'.cyan + '__'.grey + '/ /| |_| |   / _|'.cyan + '___ _ _'.grey);
          this.output.info('(___  '.grey + '/_/ \\_\\/___|\\___/|_|_\\___|'.cyan + ' _____)'.grey);
          this.output.info('   (_______ _ _)         _ ______ _)_ _ '.grey);
          this.output.info('          (______________ _ )   (___ _ _)'.grey);
          this.output.info('');
        }

        this.output.info('Windows Azure: Microsoft\'s Cloud Platform');
        this.output.info('');
  ...
  helpCategoriesSummary: function(showMore) {
    ...
    function scan(parent, levels, each) {
      for (var name in parent.categories) {
        ...
        if (self.hasCurrentModeCommands()) {
          each(cat);
  ...
  helpCommands: function() {
    ...
    this.commands.forEach(function (cmd) {
      if (self.isCurrentMode(cmd._mode)) {
        self.output.help('');
  ...
  isCurrentMode: function (commandMode) {
    var cliMode = config.mode;
    if (!cliMode) {
      // Default mode is RDFE
      cliMode = 'rdfe';
    }

    return !commandMode ||
      utils.ignoreCaseEquals(cliMode, commandMode);
  },

  hasCurrentModeCommands: function () {
    var self = this;
    ...
    for (var i = 0; i < self.commands.length; i++) {
      var currentCommand = self.commands[i];

      if (self.isCurrentMode(currentCommand._mode)) {
        return true;
      }
    }

    // check subcategories
    for (var categoryName in self.categories) {
      var currentCategory = self.categories[categoryName];

      if (currentCategory.hasCurrentModeCommands()) {
        return true;
      }
    }
