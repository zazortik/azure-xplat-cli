...
GithubClient.prototype.ensureCredentials = function (context, _) {
  ...
  if (!context.github.username) {
    context.github.username = interaction.prompt(this.cli, 'Username: ', _);
  }

  if (!context.github.password) {
    context.github.password = interaction.promptPasswordOnce(this.cli, 'Password: ', _);
  }
};
...
GithubClient.prototype.init = function (context, _) {
  ...
  if (!context.github.repository) {
    ...
    context.github.repository = repositories[interaction.choose(this.cli, repositories.map(function (repository) {
