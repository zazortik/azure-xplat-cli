...
function EndPointUtil() {
  ...
  this.endPointConsts = {
    ...
    lbLoadDistribution: [this.lbLoadDistributionValues.DEFAULT,
                         this.lbLoadDistributionValues.SOURCEIP,
                         this.lbLoadDistributionValues.SOURCEIPPROTOCOL],
    defaultProbePath: '/',
    probeTimeOutMin: 5,
...
_.extend(EndPointUtil.prototype, {
  ...
  validateLBLoadDistribution: function(loadDistribution, paramName) {
    loadDistribution = loadDistribution.toUpperCase();
    loadDistribution = this.lbLoadDistributionValues[loadDistribution];
    if (!_.contains(this.endPointConsts.lbLoadDistribution, loadDistribution)) {
      var loadDistributionAsStr = this.listToString(this.endPointConsts.lbLoadDistribution);
      return {
        error: util.format($('valid values for %s are %s'), paramName, loadDistributionAsStr)
      };
    }
