  ...
  */
  _loadEntries: function (callback) {
    if (this._entries !== null) {
      console.log('Loading cache, already loaded');
      return callback(null, this._entries);
    }

    var entries = [];
    console.log('Loading entries from keychain');
    keychain.list()
      .on('data', function (entry) {
        console.log('entry from keychain: ', entry);
      })
      .on('end', function () {
        console.log('*** keychain entries have finished');
      })
      .pipe(es.map(function (entry, cb) {
        if (entry.desc !== description) {
          // Not ours, drop it.
          console.log('not ours, dropping it');
          return cb();
        }
        ...
        keychain.get(entry.acct, entry.svce, function (err, password) {
          ...
          console.log('*** Got password, keychain entry =', password);
          cb(null, encoding.decodeObject(password));
        });
      }))
      .on('data', function (entry) {
        console.log('loaded keychain entry', entry);
        entries.push(entry);
      })
      .on('end', function (err) {
        if (!err) {
        ...
        console.log('All keychain entries loaded');
  ...
  remove: function remove(entries, callback) {
    ...
    self._loadEntries(function (err, _entries) {
      ...
      var entriesToRemove = _.filter(_entries, function (element) {
        return !shouldKeep(element);
      });

      var entriesToKeep = _.filter(_entries, shouldKeep);

      var index = 0;

      function removeEntry(n) {
        var entry = entriesToRemove[n];
        var key = encoding.encodeObject(_.omit(entry, ['accessToken', 'refreshToken']));

        keychain.remove(key, entry.resourceId, function (err) {
          if (err) {
            return callback(err);
          } else {
            if (n < entriesToRemove.length) {
              removeEntry(n + 1);
            } else {
  ...
  add: function add(entries, callback) {
    ...
    self._loadEntries(function (err, _entries) {
      ...
      // Add the new entries to the keychain
      function addToKeyChain(entries) {
        if (entries.length === 0) {
        ...
        keychain.set(key, entry.resourceId, description, password, function (err) {
          if (err) { return callback(err); }
          self._entries.push(entry);
          addToKeyChain(entries.slice(1));
        });
      }

      addToKeyChain(entries);
    });
  },
