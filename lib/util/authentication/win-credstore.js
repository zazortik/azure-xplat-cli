...
var credExePath = path.resolve('../../../bin/windows/creds.exe');
...
function list() {
  ...
  return credsProcess.stdout.pipe(parser());
}
 ...
 */
 function set(targetName, credential, callback) {
  ...
  childProcess.execFile(credExePath, args)
    .on('exit', function (code, stdout, stderr) {
      if (code === 0) {
        callback(null);
      } else {
        callback(new Error('Could not store credential, ' + stderr));
      }
    });
 }
  ...
  childProcess.execFile(credExePath, args)
    .on('exit', function (code, stdout, stderr) {
      if (code === 0) {
        ...
        callback(new Error('Could not delete credentials, ' + stderr));
      }
    });
}
