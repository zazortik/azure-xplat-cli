...
function authenticateWithDeviceCode(authConfig, userCodeResponse, userId, callback) {
  ...
  return context.acquireTokenWithDeviceCode(authConfig.resourceId, authConfig.clientId, userCodeResponse, function (err, tokenResponse) {
    if (err) { return callback(err); }
    //it is by-design that a token request to the 'common' tenent will get token
    //issued from the default tenant, we will use the default tenant id if under 2fa. The reason
    //is we do not acquire token again from the default tenant and this is the only moment we can
