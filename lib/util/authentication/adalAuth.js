...
_.extend(AdalAccessToken.prototype, {
  ...
  getGraphAccessInfo: function (callback) {
    var context = createAuthenticationContext(this.authConfig);
    //Because of token sharing, same access token for azure management can be used for graph querying
    context.acquireToken(this.authConfig.resourceId, this.userId, this.authConfig.clientId, function (err, result) {
      if (err) {
        return callback(new Error($('Credentials have expired, please reauthenticate')));
      } else {
        return callback(null, { tokenType: 'Bearer', token: result.accessToken, tenantGuid: result.tenantId });
      }
    });
  }
});
