...
function AzureADUserCredential(authConfig, userId) {
  this.authConfig = authConfig;
  this.userId = userId;
}

_.extend(AzureADUserCredential.prototype, {
  retrieveTokenFromCache: function (authorizer) {
...
function removeInvalidServicePrincipalKey(appId, tenantId, callback) {
  var query = {
    servicePrincipalId: appId,
    servicePrincipalTenant: tenantId
  };
  ...
  exports.tokenCache.find(query, function (err, entries) {
    if (err) { return callback(err); }
    exports.tokenCache.remove(entries, callback);
  });
}

function ServicePrincipalCredential(authConfig, appId) {
  this.authConfig = authConfig;
  this.appId = appId;
}

_.extend(ServicePrincipalCredential.prototype, {
  retrieveTokenFromCache: function (callback) {
    var self = this;
    loadServicePrincipalKey(self.appId, self.authConfig.tenantId, function (err, key) {
      if (err) {
        return callback(new Error(util.format($('No service key found for appid %s'), self.appId)));
      }
      ...
      context.acquireTokenWithClientCredentials(self.authConfig.resourceId, self.appId, key,
        function (err, result) {
          if (err) {
              ...
              //will fail
              removeInvalidServicePrincipalKey(self.appId, self.authConfig.tenantId, function(errorOnRemove){
                if (errorOnRemove) {
...
* Returns credential wrapper object via callback.
*
...
function authenticateWithUsernamePassword(authConfig, username, password, callback) {
  ...
  context.acquireTokenWithUsernamePassword(authConfig.resourceId, username, password, authConfig.clientId, function (err, response) {
    if (err) { return callback(err); }
    callback(null, new AzureADUserCredential(authConfig, response.userId));
  });
}
...
function authenticateWithDeviceCode(authConfig, userCodeResponse, userId, callback) {
  ...
  exports.tokenCache.setUserIdWhenUseDeviceFlow(userId);
  return context.acquireTokenWithDeviceCode(authConfig.resourceId, authConfig.clientId, userCodeResponse, function (err, tokenResponse) {
    ...
    return callback(null, new AzureADUserCredential(authConfig, tokenResponse.userId));
  });
}
/**
...
*
*/
function createServicePrincipalCredential(authConfig, appId, serviceKey, callback) {
  saveServicePrincipalKey(appId, authConfig.tenantId, serviceKey, function (err) {
    if (err) { return callback(err); }

    callback(null, new ServicePrincipalCredential(authConfig, appId, authConfig.tenantId));
  });
}
...
*
* @param {string}             username  username or service principal.
...
_.extend(exports, {
  ...
  AzureADUserCredential: AzureADUserCredential,
  ...
  createServicePrincipalCredential: createServicePrincipalCredential,
