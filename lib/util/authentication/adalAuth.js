...
*/
function logoutUser(username, isServicePrincipal, cache, done) {
  if (typeof cache === 'function') {
    done = cache;
    cache = exports.tokenCache;
  }
  var query = isServicePrincipal ? { _clientId: username } : { userId: username };
  cache.find(query, function (err, found) {
    if (err) return done(err);
    if (isServicePrincipal) {
      cache.find({ servicePrincipalId: username }, function (err, found2) {
        if (err) return done(err);
