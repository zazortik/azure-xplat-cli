...
function AdalAuthContextEx(authConfig, userId) {
  this.authConfig = authConfig;
  this.userId = userId;
}

_.extend(AdalAuthContextEx.prototype, {
...
_.extend(ServicePrincipalAccessToken.prototype, {
  authenticateRequest: function (callback) {
    var self = this;
...
function acquireToken(authConfig, username, password, callback) {
  ...
  context.acquireTokenWithUsernamePassword(authConfig.resourceId, username, password, authConfig.clientId, function (err, response) {
    if (err) { return callback(err); }
    callback(null, new AdalAuthContextEx(authConfig, response.userId));
  });
}
...
function acquireTokenWithDeviceCode(authConfig, userCodeResponse, userId, callback) {
  var context = createAuthenticationContext(authConfig);
  ...
  return context.acquireTokenWithDeviceCode(authConfig.resourceId, authConfig.clientId, userCodeResponse, function (err, tokenResponse) {
    ...
    return callback(null, new AdalAuthContextEx(authConfig, tokenResponse.userId));
  });
}
/**
