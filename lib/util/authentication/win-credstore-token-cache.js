...
var MAX_CREDENTIAL_BYTES = 16; // artifically low for testing
  ...
  remove: function remove(entries, callback) {
    ...
    self._loadEntries(function (err, _entries) {
      ...
      function removeEntry(entry, callback) {
        ...
        credStore.remove(targetName, function (err) {
          if (err) { return callback(err); }
          credStore.remove(targetName + "--*", callback);
        });
      }
      ...
      function addToCredStore(entry, callback) {
        ...
        async.eachSeries(entryParts,
          function (entry, entrycb) {
            console.log('Adding', entry[0], 'to credential store');
...
module.exports.JoinerStream = JoinerStream;
