...
function get(userName, service, callback) {
  ...
  var securityProcess = childProcess.spawn(securityPath, args);
  var password;
  // Passwords are delivered on stderr
  securityProcess.stderr.pipe(es.split()).on('data', function (line) {
    var match = /^password: (?:0x[0-9A-F]+  )?"(.*)"$/.exec(line);
    if (match) {
      password = match[1].replace(/\\134/g, '\\');
    }
  });

  securityProcess.on('exit', function (code) {
    if (code === 0) {
      callback(null, password);
    } else {
      callback(new Error('getting password failed, exit code ' + code));
    }
  });
}
