...
function doReport(indentation, reportFormat, data) {
  ...
  reportFormat.forEach(function (item) {
    ...
    if (formatter instanceof Array) {
      log.data(spaces(indentation) + toWidth(title, headerWidth));
      doReport(indentation + headerWidth, formatter, value);
    } else {
      log.data(spaces(indentation) + toWidth(title, headerWidth) + formatter(value));
    }
  });
}

log.report = function (reportFormat, data) {
  doReport(0, reportFormat, data);
};

log.report.allProperties = function (data) {
  //console.log('allProperties', require('util').inspect(data));
