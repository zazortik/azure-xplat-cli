...
log.nameValue = function (name, value, displayNullValue, indent) {
  var delimiter = ': ';
...
log.header = function (header, newLine, indent) {
  var delimiter = ':';
  if (newLine) {
    log.data('');
  }
...
log.list = function (header, items, newLine, indent) {
  log.header(header, false, indent);
  indent += 2;
  items.forEach(function (item) {
    log.listItem(item, newLine, indent);
  });
};

log.listItem = function (item, newLine, indent) {
  if (newLine) {
    log.data('');
  }
