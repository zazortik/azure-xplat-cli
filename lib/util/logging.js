...
log.pool = function (level, data, transform) {
    if (arguments.length == 2) {
        transform = data;
        data = level;
        level = 'data';
...
log.createLogFilter = function () {
  return function handle(resource, next, callback) {
    ...
    try {
      ...
      if (loggedResource.body && loggedResource.body.length > 2 * 1024 * 1024) { // if the body is large, log only that the body was too large to log and was more than likely file content
