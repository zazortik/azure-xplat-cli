...
exports.aliasFileRelativePath = './aliases/image.json';

exports.getImageAliasUrn = function(alias, aliasObj) {
  if (!aliasObj) {
    var content = fs.readFileSync(__dirname + '/' + exports.aliasFileRelativePath);
    aliasObj = JSON.parse(content);
  }
  var i = null;
  var winObj = aliasObj.outputs.aliases.value.Windows;
  for (i in winObj) {
    if (i.toLowerCase() === alias.toLowerCase()) {
      return [winObj[i].publisher, winObj[i].offer, winObj[i].sku, winObj[i].version].join(':');
    }
  }
  var linuxObj = aliasObj.outputs.aliases.value.Linux;
  for (i in linuxObj) {
    if (i.toLowerCase() === alias.toLowerCase()) {
      return [linuxObj[i].publisher, linuxObj[i].offer, linuxObj[i].sku, linuxObj[i].version].join(':');
    }
  }
  return null;
}
...
exports.getImageAliasList = function() {
  ...
  exports.writeFileSyncMode(__dirname + '/' + exports.aliasFileRelativePath, content, 'utf-8');
