 ...
 * 
 * @param {object} [options] checks for isTenantBased and endpoint properties
 * and creates the client accordingly.
 *
 * @returns {object} An instance of the client
 */
...
exports.createAutoRestClient = function (FactoryMethod, subscription, options) {
  ...
  if (options && options.isTenantBased && options.isTenantBased === true && options.endPoint) {
    subscriptionOrTenantId = subscription.tenantId;
    credentials = subscription._createCredentials(options.endPoint);
    baseUri = exports.stringTrimEnd(subscription.activeDirectoryGraphResourceId, '/');
  } else {
  ...
  var client = new FactoryMethod(credentials, subscriptionOrTenantId, baseUri, options);
  return client;
};
