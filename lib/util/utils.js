...
/**
 * Looks through the list and returns the element that matches specific
 * property by it's value,  case-insensitive.
 */
exports.findWhere = function (array, propName, propValue) {
  for (var i = 0; i < array.length; i++) {
    if ((array[i].hasOwnProperty(propName)) &&
      (array[i][propName].toLowerCase() === propValue.toLowerCase())) {
      return array[i];
    }
  }
};
...
/**
 * Looks through the list and returns the index of element that matches specific
 * property by it's value,  case-insensitive.
 */
exports.indexOf = function (array, propName, propValue) {
  for (var i = 0; i < array.length; i++) {
    if ((array[i].hasOwnProperty(propName)) &&
      (array[i][propName].toLowerCase() === propValue.toLowerCase())) {
      return i;
