...
exports.createWebsiteManagementService = function (subscription, logger, callback) {
  var client;
  console.log('before create web site client');
  if (!subscription.registeredProviders.some(function(item) {
    return utils.ignoreCaseEquals(item, 'website');
  })) {
    subscription.registerProvider('website', function(err) {
      if (err) {
        return callback(err);
      }
      console.log('from correct register provider');
      client = createService('createWebsiteManagementService', subscription, logger);
    });
  } else {
    console.log('no need to re-register');
    client = createService('createWebsiteManagementService', subscription, logger);
  }
  
  console.log('all good, and return');
