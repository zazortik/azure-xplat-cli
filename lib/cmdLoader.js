...
_.extend(CmdLoader.prototype, {
  ...
  getFiles: function (scanPath, recursively) {
    var results = [];
  ...
  cmdMetadataExists: function () {
    return this.pathExistsSync(this.cmdMetadataFile);
  },

  _loadCmdsFromFolder: function (scanPath, recursively) {
    var results = this.getFiles(scanPath, recursively);
  ...
  _loadCmdsFromNodeModules: function (scanPath) {
    ...
    results = results.filter(function (item) {
      ...
      item.packageStat = this.pathExistsSync(item.packagePath) ? fs.statSync(item.packagePath) : undefined;
  ...
  _serializeIndividualEntity: function (entity) {
    ...
    if (entity.filePath) {
      cmdOrCat.filePath = path.relative(this.cmdBasePath, entity.filePath);
    }
