...
function CmdLoader(topCmd) {
  this.topCmd = topCmd;
  this.cmdMetadataFile = path.join(utils.azureDir(), 'plugins.json');
}
...
_.extend(CmdLoader.prototype, {
  harvestPlugins: function () {
    ...
    // Load mode plugins
    var modePlugins = this._loadCmdsFromFolder(path.join(basePath, 'commands', self.topCmd.getMode()), true);
