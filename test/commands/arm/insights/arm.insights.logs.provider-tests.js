      ...
      describe('provider', function() {
        ...
        describe('list', function () {
          ...
          it('should work', function(done) {
            suite.execute('insights logs provider list %s -s %s -e %s -d', 'microsoft.web', '2015-04-02T11:06:00', '2015-04-02T12:06:00', function(result) {
              result.exitStatus.should.equal(0);
              done();
            });
          });
          ...
          it('should fail if the start date is later than Now', function(done) {
            suite.execute('insights logs provider list %s -s %s -d', 'microsoft.web', '2100-01-01T01:00:00', function(result) {
          ...
          it('should fail if the end date is earlier than the start date', function(done) {
            suite.execute('insights logs provider list %s -s %s -e %s -d', 'microsoft.web', '2015-03-02T13:00:00', '2015-03-01T13:00:00', function(result) {
              result.exitStatus.should.equal(1);
          ...
          it('should fail if the end date and the start date are too far apart', function(done) {
            suite.execute('insights logs provider list %s -s %s -e %s -d', 'microsoft.web', '2015-03-01T13:00:00', '2015-04-01T13:00:00', function(result) {
              result.exitStatus.should.equal(1);
