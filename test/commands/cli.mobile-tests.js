...
allTests = function (backend) {
    ...
    if (process.env.AZURE_MOBILE_NOCK_REC) {
      ...
      nockhelper.nock.recorder.play().forEach(function (line) {
        if (line.indexOf('nock') >= 0) {
          ...
          if (line.match(/{\"externalPushEntityState\":\"activating\"}/)) {
            return;
          }
  ...
  it('auth microsoftaccount set ' + servicename + ' 123 456 --packageSid 789 --json', function (done) {
    var cmd = ('node cli.js mobile auth microsoftaccount set ' + servicename + ' 123 456 --packageSid 789 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth microsoftaccount get ' + servicename + ' --json (123 456 789)', function (done) {
    var cmd = ('node cli.js mobile auth microsoftaccount get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('auth microsoftaccount set ' + servicename + ' 1234 5678 --json', function (done) {
    var cmd = ('node cli.js mobile auth microsoftaccount set ' + servicename + ' 1234 5678 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth microsoftaccount set ' + servicename + ' --packageSid 123456789 --json', function (done) {
    var cmd = ('node cli.js mobile auth microsoftaccount set ' + servicename + ' --packageSid 123456789 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth microsoftaccount delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile auth microsoftaccount delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth facebook set ' + servicename + ' 1234 5678 --json', function (done) {
    var cmd = ('node cli.js mobile auth facebook set ' + servicename + ' 1234 5678 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth facebook get ' + servicename + ' --json (1234 5678)', function (done) {
    var cmd = ('node cli.js mobile auth facebook get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('auth facebook delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile auth facebook delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth twitter set ' + servicename + ' 12345 6789 --json', function (done) {
    var cmd = ('node cli.js mobile auth twitter set ' + servicename + ' 12345 6789 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth twitter get ' + servicename + ' --json (12345 6789)', function (done) {
    var cmd = ('node cli.js mobile auth twitter get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('auth twitter delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile auth twitter delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth google set ' + servicename + ' 45678 9123 --json', function (done) {
    var cmd = ('node cli.js mobile auth google set ' + servicename + ' 45678 9123 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth google get ' + servicename + ' --json (45678 9123)', function (done) {
    var cmd = ('node cli.js mobile auth google get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('auth google delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile auth google delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth aad set ' + servicename + ' 123456789 --json', function (done) {
    var cmd = ('node cli.js mobile auth aad set ' + servicename + ' 123456789 --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('auth aad get ' + servicename + ' --json (123456789)', function (done) {
    var cmd = ('node cli.js mobile auth aad get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('auth aad delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile auth aad delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      result.text.should.equal('');
  ...
  it('push nh enable ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile push nh enable ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      checkScopes(scopes);
  ...
  it('push nh get ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile push nh get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('push mpns set ' + servicename + ' ' + __dirname + '/mobile/cert.pfx foobar --enableUnAuthenticatedPush --json', function (done) {
    var cmd = ('node cli.js mobile push mpns set ' + servicename).split(' ');
    cmd.push(__dirname + '/mobile/cert.pfx');
    cmd.push('foobar');
    cmd.push('--json');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
  ...
  it('push mpns get ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile push mpns get ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      var response = JSON.parse(result.text);
  ...
  it('push mpns delete ' + servicename + ' --json', function (done) {
    var cmd = ('node cli.js mobile push mpns delete ' + servicename + ' --json').split(' ');
    var scopes = setupNock(cmd);
    executeCmd(cmd, function (result) {
      result.exitStatus.should.equal(0);
      checkScopes(scopes);
  ...
  it('log ' + servicename + ' --source /table/table1.insert.js --json (get logs from specific source)', function (done) {
    ...
    executeCmd(cmd, function (result) {
      ...
      if (backend === 'node') {
        ...
        existingContinuationToken = 'someToken';
      }
      checkScopes(scopes);
      done();
    });
